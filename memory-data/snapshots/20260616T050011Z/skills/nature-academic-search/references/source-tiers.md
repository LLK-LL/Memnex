...
|------|-------|-------------|-------------------|
| **T1** | API-backed, structured | Official REST/XML API, stable schema | Use first. If fails 鈫?next T1 source. |
...
| CNKI / 涓囨柟 | No programmatic access | Chinese only, manual download |
...
3. If (result found AND relevance > threshold) 鈫?ACCEPT
4. If T1 exhausted or insufficient 鈫?ESCALATE to T2
5. If T1+T2 exhausted 鈫?ESCALATE to T3 + WARN USER
6. If all exhausted 鈫?return partial results + suggest query refinement
```

### Domain 鈫?Tier Mapping
...
| Citation verification | CrossRef (DOI) 鈫?PubMed (PMID) | Semantic Scholar | Google Scholar |

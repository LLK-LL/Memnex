...
**Uses:**
- [Citation Parser](../citation-parser.md) 鈥?extraction strategies per source format.
- [Dedup Engine](../dedup-engine.md) 鈥?collapse duplicate candidate matches before classification.

## Procedure

1. **Extract citations** from document using [Citation Parser](../citation-parser.md).
   Prefer T1 sources for primary verification (CrossRef DOI lookup 鈫?PubMed PMID confirmation). Use T2 (Semantic Scholar) for cross-checking ambiguous or missing results. See [Source Tiers](../source-tiers.md) for full routing.
2. **Resolve each citation:**
   - DOI 鈫?`search_crossref` or `get_paper_by_doi`
   - PMID 鈫?`pubmed_fetch_articles`
   - arXiv ID 鈫?`search_arxiv`

...
**Spec:** v3.6.7 搂7.2 鈥?audit prompt template for downstream-agent deliverable cross-model audit.

# REDACTED: sensitive-looking memory line

# REDACTED: sensitive-looking memory line

This template fixes a single audit-prompt structure so the orchestrator's audit hooks (spec 搂5) generate consistent, reproducible prompts.

---

## Template structure

A v3.7.1 multi-file audit prompt has eight sections in this order (Section 0 prepended additively per v3.7.1 D2; Sections 1鈥? stay byte-equivalent to v3.6.7):
...
The orchestrator fills the placeholders in `{curly_braces}`. Everything else rides verbatim. Section 0 is non-negotiable 鈥?its purpose is to disclose the audit's actual coverage so a "PASSED" verdict cannot mask un-retrieved sources (spec v3.7.1 搂3.2 / Pattern D2). Section 7 is non-negotiable 鈥?omitting it leaves sub-agents free to fake audit-passed metadata, which is the failure mode v3.6.7 spec 搂5.3 + Pattern C3 + `feedback_subagent_tool_hallucination.md` exist to prevent.

---

## Section 0 鈥?Scope Report (mandatory; v3.7.1 D2)
...
**Two firm rules (spec v3.7.1 搂3.2):**
...
**Fill rules:**
- `target_rounds` defaults to 3 per spec 搂4.2 (Lesson D2: convergence requires 3+ rounds, not 1 or 2). Increase to 4鈥? when prior round count exceeded 3 without convergence.
- `summary_of_prior_findings` cites finding counts by severity for each prior round, e.g. "Round 1: P1脳4 / P2脳5 / P3脳1 (10 total). Round 2: P1脳0 / P2脳3 / P3脳1 (4 total)."
- For first round, `Previous rounds: none (first round, baseline audit)`.

---

## Section 2 鈥?Bundle inventory
...
## Section 3 鈥?Audit dimensions (the 7)

Codex evaluates the bundle along seven dimensions. Each dimension surfaces a specific class of pattern from spec 搂3:
...
**Patterns surfaced:** Pattern D2 (convergence requires 3+ rounds), Pattern D3 (PARTIAL 鈮?CLOSED).
...
Note: word-count convention, 3鈥?% buffer adherence, and protected-hedge preservation against compression are **not** covered by 搂3.7. They are handled by the bundle-specific check in Section 4 (f) below 鈥?see the report_compiler_agent example for the expected `(f)` clause.

---

## Section 4 鈥?Round-specific job
...
(c) Run the 7 audit dimensions (搂3.1鈥撀?.7) plus the bundle-specific Section 4(f) check on the primary deliverables. Report each finding with the dimension or `4(f)` that surfaced it.
...
**Fill rules:**
- (a)鈥?e) ride verbatim every round (with N substitution).
- (f) is bundle-specific. The orchestrator picks the bundle-specific check based on which downstream agent produced the deliverable. For report_compiler_agent bundles, (f) is mandatory and combines three sub-checks; the others are single-check. Examples:
  ...
  - report_compiler_agent bundle (mandatory three-part check): (i) word count = `len(body.split())` 鈮?publisher cap minus 3鈥?% buffer per `word_count_conventions.md`; (ii) every entry of upstream `protected_hedges` block per `protected_hedging_phrases.md` appears verbatim in the abstract; (iii) no claim in the abstract is less hedged than its anchor in the body. Failure of any sub-check is a P1 finding.

---

## Section 5 鈥?Convergence target
...
**Fill rules:**
- The fallback condition is part of spec 搂5.2 stop conditions. Default fallback: "after 5 rounds, escalate remaining findings to user for ship-or-iterate decision."
- Do NOT loosen convergence to "0 P1+P2 findings" or "0 P1 findings" 鈥?those are weaker stop conditions that v3.6.7 replaces with the strict 0-anything bar.

---

## Section 6 鈥?Output format

```
Output:
- Cumulative numbered findings (carry forward IDs from round 1; new findings get next available ID).
- For each finding: ID, severity (P1/P2/P3), dimension (one of 搂3.1鈥撀?.7 or `4(f)` for bundle-specific failures), file:line anchor, one-line description, suggested fix.
# REDACTED: sensitive-looking memory line
- End with severity-bucket count summary: "Round {N}: P1脳{n1} / P2脳{n2} / P3脳{n3} ({total} total)".
...
## Section 7 鈥?Anti-fake-audit guard (Pattern C3, mandatory)
...
**Why:** Sub-agents have been observed (see `feedback_subagent_tool_hallucination.md`) reporting that they ran codex audit and surfacing simulated findings, when in fact no audit ran. The deliverable then carries a fake audit-passed marker that downstream consumers trust. The guard above is verbatim what the agent prompts say, plus the orchestrator-side post-verification (spec 搂5.3) that reads audit transcript metadata and rejects deliverables claiming audit completion without matching transcript.
...
```
Audit round: 2 of 3
Previous rounds: Round 1: P1脳3 / P2脳7 / P3脳2 (12 total); all P1 closed; 4 P2 closed; 3 P2 + 2 P3 carried.

Bundle scope: Phase 2 chapter deliverables 鈥?synthesis output for chapter 4
...
- deep-research/agents/synthesis_agent.md (the agent prompt 鈥?for understanding patterns, not for audit)
- chapter4_bibliography.json (bibliography_agent output, V2_CLEAN 鈥?verified upstream)
...
- `shared/references/irb_terminology_glossary.md` 鈥?referenced by dimension 搂3.5.
- `shared/references/psychometric_terminology_glossary.md` 鈥?referenced by dimension 搂3.5.
- `shared/references/protected_hedging_phrases.md` 鈥?referenced by Section 4(f) report_compiler bundle check (sub-checks ii + iii).
- `shared/references/word_count_conventions.md` 鈥?referenced by Section 4(f) report_compiler bundle check (sub-check i).
- ARS feedback memory `feedback_codex_iterative_spec_review_to_zero.md` 鈥?convergence target rationale.
- ARS feedback memory `feedback_cross_model_review_cascade_inconsistency.md` 鈥?round-N+1 cascade audit rationale.
- ARS feedback memory `feedback_subagent_tool_hallucination.md` 鈥?anti-fake-audit guard rationale.
- ARS feedback memory `feedback_codex_xhigh_for_drift_audit.md` 鈥?model + reasoning-effort selection (gpt-5.5 + xhigh for high-blast-radius bundles).

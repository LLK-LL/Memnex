...
Choose:
  (a) backfill 鈥?return to Stage 2 and add missing material
  (b) manual handling 鈥?edit manuscript directly, rerun compliance
  (c) acknowledge limitation 鈥?invoke user override (see 搂Override Ladder)
```
...
Compliance reports and fixture templates use these canonical tags in `gaps[].reason`, `principle_evidence[]`, and narrative fields. They are lexical signals 鈥?`compliance_agent` (Task 8) and downstream readers pattern-match on them.

| Tag | Where it appears | Meaning |
|---|---|---|
| `[MATERIAL GAP]` | `gaps[].reason`, `principle_evidence[]`, `evidence[]` | The item cannot be verified because the underlying material (passport field, manuscript section, supplementary file) is missing. Apply [Anti-Leakage Protocol](../academic-paper/references/anti_leakage_protocol.md) 鈥?do not hallucinate. |
...
Casing is significant 鈥?uppercase square-bracketed. Lowercase or missing brackets are treated as plain prose and will not be recognised as gap signals.
...
| 3rd | Require rationale 鈮?100 chars. Only released on rationale confirmation. |

Round count is per-stage-per-pipeline-run, stored in `compliance_history[].user_override` entries.

# REDACTED: sensitive-looking memory line

On any successful override, the agent generates `disclosure_addendum` text and the orchestrator **auto-injects** it into the manuscript's AI disclosure section. The addendum is non-removable 鈥?this is the concrete form of the `no detection evasion` iron rule in CONTRIBUTING.md.
...
- `<item_title>`: look up the short title from `shared/prisma_trAIce_protocol.md` (e.g. `M4` 鈫?"Input data").
...
After 3 unresolved rounds of retry at the same stage, orchestrator invokes the existing `pipeline_state_machine.md 搂Integrity Check FAIL Loop` procedure:
...
`material_passport.compliance_history[]` never overwrites. On retry-pass, the previous fail entry is preserved with its own timestamp. The AI Self-Reflection Report at Stage 6 cites the full history to demonstrate how the correction was made 鈥?this is RAISE transparency applied to ARS itself.

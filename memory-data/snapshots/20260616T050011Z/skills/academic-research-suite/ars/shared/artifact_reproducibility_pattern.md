...
ARS produces research artifacts 鈥?draft papers, literature reviews, review reports. A year
...
When the Anthropic automated-w2s-researcher paper (2026) claimed "2 researchers 脳 7 days
vs 9 agents 脳 5 days," readers had no way to audit the conditions. This pattern ensures
that ARS-produced artifacts carry their configuration on record 鈥?not as a replay mechanism,
...
you CONFIGURATION DOCUMENTATION sufficient to INVESTIGATE divergence 鈥?not to PREVENT it.
...
additions bump minor. Old readers fail loudly on unknown versions 鈥?`check_repro_lock.py`
...
That coupling is intentional 鈥?version boundaries mark coordinated changes.
...
- `check_repro_lock.py` can be run standalone on any passport file. It is not wired into
  the CI lint suite by default 鈥?it validates on-demand against specific passport files.

## Red flags when reading a populated repro_lock

- `model.weight_stable: true` 鈥?this value is not implemented yet; if you see it, the
# REDACTED: sensitive-looking memory line
  suspicious until a signed attestation mechanism exists.
- `prompts.hash_timing` anything other than `"skill-load"` 鈥?this is an unknown variant
...
downstream stages. The `stochasticity_declaration` must be included verbatim 鈥?do not
...
deterministic schedulers, and cached external data 鈥?none of which ARS controls today.

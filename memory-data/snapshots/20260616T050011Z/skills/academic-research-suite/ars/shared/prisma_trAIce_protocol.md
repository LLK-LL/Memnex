...
> 鈿狅笍 **Upstream sync warning.** If `cqh4046/PRISMA-trAIce` updates, a freshness CI check emits an annotation but does not block merges. Maintainers must manually re-sync this file and bump `snapshot_date` + `upstream_version_commit`. See `scripts/check_prisma_trAIce_freshness.py`.
...
> See Holst D, et al. JMIR AI. 2025. doi:[10.2196/80247](https://doi.org/10.2196/80247) 鈥?Discussion and Limitations sections.
...
**ARS check procedure:** Locate manuscript abstract. Verify it names 鈮? AI tool AND the review stages.
**Pass criterion:** Both named. Absence of either 鈫?fail.
...
**Pass criterion:** Rationale present and tied to 鈮? specific AI task.

### M1 鈥?Protocol registration
**Tier:** Mandatory
**Original:** "State if the use of AI was pre-specified in a protocol, and document deviations."
**ARS check procedure:** Check `material_passport.protocol_registration` field OR manuscript 搂Methods for explicit pre-registration statement. If AI was added mid-review, check for a "deviation from protocol" paragraph.
**Pass criterion:** Either (a) pre-registration clearly stated, or (b) deviation explicitly disclosed.

### M2 鈥?Tool identification and access
**Tier:** Mandatory
...
**Pass criterion:** Per-tool, per-stage prose description of 鈮? sentence in manuscript 搂Methods.
...
**ARS check procedure:** Verify manuscript 搂Methods documents output format per tool + any post-processing pipelines.
**Pass criterion:** Format + post-processing both described.

### M6 鈥?Prompt engineering
**Tier:** Mandatory
...
### M8 鈥?Human-AI interaction
**Tier:** Mandatory
**Original:** "Describe human interaction (number of reviewers, verification process)."
**ARS check procedure:** Verify manuscript 搂Methods states reviewer count, qualifications, and oversight mechanism (e.g., independent dual screening, adjudication).
**Pass criterion:** Reviewer count + oversight mechanism both present.

### M9 鈥?AI performance evaluation methods
**Tier:** Mandatory
**Original:** "Describe methods used to evaluate AI performance (metrics, reference standards)."
**ARS check procedure:** Verify manuscript 搂Methods describes what performance metrics were computed (e.g., sensitivity, agreement) and against what gold standard.
**Pass criterion:** Metric + reference standard both stated.

### M10 鈥?Data management
**Tier:** Recommended
**Original:** "Describe data management, storage, privacy measures, and compliance."
**ARS check procedure:** Verify manuscript 搂Methods addresses data handling (where stored, privacy controls, GDPR/IRB compliance if applicable).
**Pass criterion:** 鈮? of {storage, privacy, compliance} addressed.

### R1 鈥?Study selection (AI-assisted)
**Tier:** Mandatory
...
### R2 鈥?AI performance metrics (results)
**Tier:** Mandatory
**Original:** "Report results of any AI performance evaluations (e.g., agreement rates)."
**ARS check procedure:** Verify manuscript 搂Results reports numeric performance outcomes referenced by M9.
**Pass criterion:** 鈮? quantitative metric (% agreement, Cohen's 魏, sensitivity, etc.).

### D1 鈥?Limitations of AI use
**Tier:** Recommended
**Original:** "Discuss limitations (technical issues, biases, hallucinations) and their impact."
**ARS check procedure:** Verify manuscript 搂Discussion has 鈮? paragraph specifically on AI limitations (not generic review limitations).
**Pass criterion:** Dedicated AI-limitations paragraph of 鈮? sentences.

### D2 鈥?Implications of AI use
**Tier:** Optional
**Original:** "Briefly discuss the experience of using AI tools for future reviews."
**ARS check procedure:** Verify manuscript 搂Discussion reflects on the usability/experience of AI for this review, forward-looking.
**Pass criterion:** 鈮? forward-looking sentence about AI utility.

## Summary counts

- Mandatory: 10 鈥?M1, M2, M3, M4, M5, M6, M8, M9, R1, R2
- Highly Recommended: 1 鈥?M7
- Recommended: 3 鈥?I1, M10, D1
- Optional: 3 鈥?T1, A1, D2

#!/usr/bin/env python3
"""check_claim_audit_consistency.py 鈥?ARS v3.8 claim-faithfulness audit lint.

Enforces the 38 cross-field invariants spanning the six aggregates that
claim_ref_alignment_audit_agent populates:

    claim_audit_results[]           鈥?INV-1..INV-18      (搂3.1)
    claim_intent_manifests[]        鈥?M-INV-1..M-INV-4   (搂3.2)
    uncited_assertions[]            鈥?U-INV-1..U-INV-4   (搂3.3)
    claim_drifts[]                  鈥?D-INV-1..D-INV-4   (搂3.4)
    constraint_violations[]         鈥?CV-INV-1..CV-INV-4 (搂3.5)
    audit_sampling_summaries[]      鈥?S-INV-1..S-INV-4   (搂4 step 3)

Plus the 搂3.1 allowed (judgment, audit_status, defect_stage) matrix, and
schema-shape validation for every entry in each aggregate against the
corresponding shipped JSON Schema.

Cross-field invariants are intentionally NOT expressed in JSON Schema 鈥?
...
搂3.1-搂3.5 and 搂6 for the full invariant catalogue.
"""
...
# unittest path puts repo root on sys.path 鈥?the insert below covers the gap.
...
# Inline schema for audit_sampling_summary (spec 搂4 step 3) 鈥?not a shipped file.
...
# Allowed (judgment, audit_status, defect_stage) triples 鈥?搂3.1 table.
...
def _check_inv_11(e: dict[str, Any]) -> list[Finding]:
    """ref_retrieval_method=not_attempted 鈫?anchor_kind=none."""
...
def _check_inv_12(e: dict[str, Any]) -> list[Finding]:
    """ref_retrieval_method=not_found 鈫?RETRIEVAL_FAILED + completed + retrieval_existence."""
    method = e.get("ref_retrieval_method")
...
def _check_matrix(e: dict[str, Any]) -> list[Finding]:
    ...
    if triple not in ALLOWED_MATRIX:
        return [
            Finding(
                "matrix",
                f"triple {triple!r} not in 搂3.1 allowed (judgment, audit_status, defect_stage) matrix",
            )
        ]
    return []
...
def _iter_dicts(value: Any) -> list[dict[str, Any]]:
    ...
    than a traceback (#119 + #120 P2-2 鈥?Step 13 R6 codex P2).
    """
...
def _check_manifest_invariants(manifests: list[dict[str, Any]]) -> list[Finding]:
    ...
    # Skip non-string manifest_ids per v3.8.1 round 2 鈥?schema records the
    ...
    for i, m in enumerate(manifests):
        ...
        # Per spec 搂3.2: "claim-level can ADD via NC-C{n}-{m}, never via MNC-*" 鈥?
        ...
        # MNC ids 鈥?codex round 3 P2; removed in v3.8.1.)
        for j, claim in enumerate(_iter_dicts(m.get("claims"))):
            for nc in _iter_dicts(claim.get("negative_constraints")):
                nc_id_raw = nc.get("constraint_id", "")
                nc_id = nc_id_raw if isinstance(nc_id_raw, str) else ""
...
def _build_manifest_constraint_index(
    ...
    for m in manifests:
        mid = m.get("manifest_id")
        # #130: same guard as _build_manifest_index 鈥?non-string mid
...
def _check_constraint_violation_invariants(
    ...
    # same passport carrying colliding MNC-* / NC-* ids 鈥?each legitimately
    # violated on the same sentence text 鈥?do not false-positive as duplicates.
...
# ---------------------------------------------------------------------------
# uncited_audit_failure invariants (UAF-INV-1..UAF-INV-6) 鈥?v3.8.2 / #118.
...
# is required. See docs/design/2026-05-15-issue-103-claim-alignment-audit-spec.md 搂3.6.
# ---------------------------------------------------------------------------
...
def _check_uaf_invariants(
    ...
    for e in entries:
        ...
        if not manifest_resolved:
            ...
            # Don't `continue` here 鈥?UAF-INV-5 (rationale prefix) is
        ...
        # manifest itself failed to resolve 鈥?the pair check would
        ...
        # invariant 鈥?the prefix must match the row.
        # Guard `rationale` with isinstance(str): the row's raw rationale
        # may be list/dict on a malformed passport 鈥?schema flags it
    ...
    for uaf in entries:
        ...
        if key in cv_keys:
            findings.append(
                Finding(
                    "UAF-INV-6",
                    f"finding_id={uaf.get('finding_id')!r}: (manifest, section, claim) overlaps a constraint_violations[] row 鈥?UAF and CV are mutually exclusive verdict states",
                )
            )
...
def _check_sampling_invariants(entries: list[dict[str, Any]]) -> list[Finding]:
    ...
    for e in entries:
        ...
        # S-INV-2: count 鈮?cap AND count 鈮?total.
        ...
        # indices (#119 R6 codex P2 + Step 13 R8 P2-2) 鈥?schema records the
...
def _validate_against_schema(
    ...
    malformed passport 鈥?`entries` could be non-list, or a list with
...
def validate_passport(body: Any) -> list[Finding]:
    ...
    # Raw aggregates 鈥?preserved for schema-shape validation below (which
    ...
    def _coerce_aggregate(value: Any) -> list[dict[str, Any]]:
        ...
        invariant loops below attempt `.get()` / dictionary indexing 鈥?
    ...
    # Schema-shape 鈥?gives the lint a chance to surface malformed entries with

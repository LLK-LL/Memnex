...
def test_schema_includes_seven_v3_7_1_trust_fields(schema: dict[str, Any]) -> None:
    """Spec 搂 Step 1 requires exactly seven entry-stored trust fields."""
    expected = {
        "source_acquired",
    ...
    # (per spec 搂3.1 firm rule #3 + 搂3.6 firm rule #1).
...
# ---------- Rule #1 鈥?verified=true preconditions ----------
...
# ---------- Rule #2 鈥?source_acquired=false 鈫?description_last_audit 鈭?{null, 'none'} ----------
...
def test_rule2_acquired_false_with_audit_null_fails(validator) -> None:
    """Round-6 codex P2 closure: spec 搂3.1 firm rule #2 says REQUIRES
...
def test_rule2_acquired_false_with_missing_audit_field_fails(validator) -> None:
    """Round-1 codex P2 closure: REQUIRES is strict 鈥?the field MUST be present.
...
def test_description_source_accepts_arbitrary_bibliography_revision(validator) -> None:
    """Round-1 codex P2 closure: spec 搂 3.1 yaml uses `bibliography_v<n>` as a
...
def test_description_source_rejects_unrelated_strings(validator) -> None:
    """The pattern is anchored 鈥?typos and unrelated strings still fail."""
...
# ---------- Rule #3 鈥?no literal human_read_* on entry ----------
...
def test_rule3_literal_human_read_source_rejected_by_lint() -> None:
    ...
    errors = check_entry(entry, "smith2024")
    assert any(
        "Rule #3" in e and "human_read_source" in e and "搂3.6 peer file" in e
        for e in errors
    )

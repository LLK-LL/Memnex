...
def test_doi_lookup_with_matching_title(monkeypatch):
    """DOI hit + matching title 鈫?success."""
    from openalex_client import OpenAlexClient
...
def test_429_triggers_2s_backoff_3_retries(monkeypatch):
    """Per protocol: 429 鈫?2s backoff 脳 3 retries 鈫?raise OpenAlexUnavailable."""
...
def test_5xx_skips_immediately(monkeypatch):
    """Per protocol: 5xx 鈫?no retry (call count == 1), raise OpenAlexUnavailable."""
    from openalex_client import OpenAlexClient, OpenAlexUnavailable
...
def test_doi_404_treated_as_miss_not_unavailable(monkeypatch):
    """DOI not indexed in OpenAlex (404) 鈫?return None (miss), not raise OpenAlexUnavailable."""
    from openalex_client import OpenAlexClient, OpenAlexUnavailable
...
def test_oserror_during_read_raises_unavailable(monkeypatch):
    ...
    mid-stream). Per v3.9.0 spec 搂3.7 degradation contract, this MUST be
...
def test_invalid_utf8_body_raises_unavailable(monkeypatch):
    ...
    Per v3.9.0 spec 搂3.7, translate to OpenAlexUnavailable not bubble out as
    UnicodeDecodeError."""
    from openalex_client import OpenAlexClient, OpenAlexUnavailable
...
def test_invalid_json_body_raises_unavailable(monkeypatch):
    ...
    HTML error page slipped through). Per v3.9.0 spec 搂3.7, translate to
...
def test_truncated_read_raises_unavailable(monkeypatch):
    ...
    OSError 鈥?so an OSError-only handler still lets it escape. v3.9.1 codex
    R1 P2: catch this explicitly to honor the v3.9.0 搂3.7 per-API
...
def test_throttle_uses_monotonic_clock(monkeypatch):
    ...
    Aligns OA/CR with semantic_scholar_client.py (#128 搂6)."""
    from openalex_client import OpenAlexClient
    ...
    client = OpenAlexClient()
    # Initial state: no prior request 鈥?_throttle short-circuits.
    client._throttle()

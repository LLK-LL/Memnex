...
Per spec v3.9.0 搂3.8 rules 1-7:
  Rule 1 鈥?marker syntax: 3 new v3.9.0 markers present in finalizer suffix table
# REDACTED: sensitive-looking memory line
  Rule 3 鈥?v3.7.3 legacy compat: k=1 k_max=1 S2 row 鈫?CONTAMINATED-UNMATCHED (not COVERAGE-NOISE)
# REDACTED: sensitive-looking memory line
  Rule 5 鈥?formatter pass-through allowlist set-equality (R-L3-2-E lint)
  Rule 6 鈥?refusal-list-unchanged guard
  Rule 7 鈥?CI integration (done in spec-consistency.yml)
...
Exit codes:
    0 鈥?all checks pass
    1 鈥?one or more checks failed
    2 鈥?invocation error (e.g., file missing)
"""
...
V3_9_0_SECTION_HEADER = "## Cite-Time Provenance Finalizer 鈥?v3.9.0 extension"
...
# Canonical 9-suffix allowlist per spec v3.9.0 搂3.8 rule 5.
# REDACTED: sensitive-looking memory line
    # v3.7.3 legacy (3)
...
def check_legacy_compat(subsection_text: str) -> list[str]:
    ...
    v3.9.0 spec 搂3.6 coverage-noise invariant).
    """
    failures = []
    ...
    for line in subsection_text.splitlines():
        ...
        if is_preprint:
            ...
            if "`CONTAMINATED-PREPRINT+UNMATCHED`" not in line:
                failures.append(
                    f"rule 3 (legacy compat 鈥?preprint variant): k=1 k_max=1 "
            ...
            if "`CONTAMINATED-PREPRINT+COVERAGE-NOISE`" in line:
                failures.append(
                    f"rule 3 (legacy compat 鈥?preprint drift): preprint legacy row contains "
            ...
            if "`CONTAMINATED-UNMATCHED`" not in line:
                failures.append(
                    f"rule 3 (legacy compat 鈥?bare variant): k=1 k_max=1 "
            ...
            if "`CONTAMINATED-COVERAGE-NOISE`" in line:
                failures.append(
                    f"rule 3 (legacy compat 鈥?bare drift): bare legacy row contains "
...
# REDACTED: sensitive-looking memory line
    if in_refusal:
        failures.append(
            f"rule 6 (R-L3-2-E violation 鈥?CONTAMINATED-* in refusal rules): {sorted(in_refusal)}"
        )

...
Spec: docs/design/2026-04-30-ars-v3.6.7-step-6-orchestrator-hooks-spec.md 搂7.4

This harness exercises 搂5.6 Path B's verdict-to-ship/block decision logic
...
- 搂7.4 success criterion 4 (audit artifact lifecycle): inventory-driven
...
PATTERN_TO_DIMENSION = {
    # Anchored in shared/templates/codex_audit_multifile_template.md 搂3 "Patterns
    ...
    "B5": "3.1",  # primary-source list mismatch 鈥?audit template 搂3.1 explicitly lists B5 (line 76); 搂3.5 enumerates B1-B4 only (line 100)
    "C1": "4(f)",  # compression overclaim 鈥?4(f) sub-check (ii) protected hedge
    ...
    "D3": "3.6",  # PARTIAL 鈮?CLOSED / round framing
    "D4": "4(f)",  # word-count cap bust 鈥?4(f) sub-check (i)
    # D2 is convergence theatre 鈥?no finding, special handling.
}

# 搂7.4 criterion 4 鈥?inventory-driven passport mutation rule.
# Mirrors the 搂5.6 verification failure state inventory (24 rows: 7 P-PA-* + 17 P-PB-*)
# plus the two happy-path phases (A7, B10). When 搂5.6's inventory grows in v3.6.8+,
...
PHASE_TO_PASSPORT_MUTATION = {
    # Happy paths
    "A7": "none",          # Path A success 鈥?entry already there
    "B10": "appended",     # Path B success 鈥?fresh proposal merged
    # Path A failure phases 鈥?passport unchanged (silent fall-through to B)
    "P-PA-precond": "none",
    "P-PA-schema": "none",
    "P-PA-gate": "none",
    ...
    # Path B failure phases 鈥?proposal stays in <output-dir>, passport unchanged
    "P-PB-empty": "none",
    ...
    # Duplicate / consume / crash phases (continuation rows 鈥?final state checked,
    ...
    # double-append. For success-path completion these reach B10 鈫?"appended"; for
# REDACTED: sensitive-looking memory line
    # "none" reading the pre-existing entry.
# REDACTED: sensitive-looking memory line
    "P-PB-dup-other": "conditional",     # continues B1a/B2 with remaining candidates
# REDACTED: sensitive-looking memory line
    "P-PB-consume-fail": "appended",     # B9 atomic-rename succeeded 鈫?entry committed
    "P-PB-crash": "conditional",         # depends on whether B9 atomic-rename fired
    # Round-cap escalation phase (搂5.4 / B11) 鈥?append still happens (B10 ran
    ...
    "B11": "appended",                   # round == target_rounds MATERIAL 鈫?escalation
}

# Total enumerated phases (must equal 24 搂5.6 inventory rows + 2 happy-path
...
def _simulate_orchestrator_decision(
    ...
    """Apply 搂5.6 Path B verdict鈫抯hip/block decision to a synthesized verdict.

    Returns a dict with the orchestrator action that an actual run would
    produce. This is a thin reimplementation of 搂5.6's B10 + 搂5.3 mapping.
    """
...
def _validate_finding_counts_match(verdict: dict) -> None:
    """搂3.7 family A row A5 鈥?finding_counts must agree with severity tally."""
...
def _validate_status_count_consistency(verdict: dict) -> None:
    """搂3.2 cross-field rules 鈥?status/count agreement."""
...
def test_micro_bad_run_signal_matches_expectation(pattern_id):
    """搂7.4 criterion 1: BAD case produces the pattern-specific failure signal."""
    ...
    if pattern_id == "D2":
        # D2 convergence theatre 鈥?PASS verdict, but fixture must declare the
        # convergence-policy assertion (per 搂7.4 criterion 1 special case).
        ...
        # All other patterns 鈥?BAD must carry MATERIAL or MINOR verdict with
    ...
    if expected_phase:
        ...
        if bad_status == "PASS":
            # D2 convergence-theatre special case 鈥?verdict is PASS but pattern flagged.
            assert expected_phase in {"B10", "A7"}, (
                f"{pattern_id} BAD PASS verdict (D2 special) requires phase 鈭?{{B10, A7}}; got {expected_phase!r}"
            )
    ...
    if pattern_id != "D2" and bad_verdict["verdict_status"] in {"MATERIAL", "AUDIT_FAILED"}:
        ...
        assert "[AUDIT GATE" in msg, (
            ...
            f"substring per 搂5.6 BLOCK message format; got {msg!r}"
        )


@pytest.mark.parametrize("pattern_id", _all_micro_fixtures())
def test_micro_good_run_passes(pattern_id):
    """搂7.4 criterion 2: GOOD case produces PASS + empty findings."""
    fixture_dir = FIXTURE_ROOT / pattern_id
    manifest = _load_json(fixture_dir / "manifest.json")
    ...
    # empty block_message AND phase 鈭?{B10, A7} (B11 reserved for round-cap).
    ...
    if expected_phase:
        assert expected_phase in {"B10", "A7"}, (
            f"{pattern_id} GOOD PASS verdict requires phase 鈭?{{B10, A7}}; got {expected_phase!r} "
...
def test_micro_run_id_format(pattern_id):
    """搂3.7 family F F1: run_id matches canonical regex."""
    fixture_dir = FIXTURE_ROOT / pattern_id
    manifest = _load_json(fixture_dir / "manifest.json")
...
def test_integration_three_round_escalation(integration_manifest):
    """搂7.3: integration fixture exercises 3-round MATERIAL escalation."""
...
def test_integration_per_round_per_agent_slots_present():
    """搂7.3 directory tree: round_{1,2,3}/<agent>/<files>."""
...
def test_integration_escalation_artifacts_present():
    """搂7.3: escalation/ holds user_response + expected passport state."""
    base = _integration_dir()
...
def test_integration_patterns_triggered_subset(integration_manifest):
    """搂7.3: patterns_triggered matches the curated A3+C2+D4+C1 subset."""
...
# F-004 closure: drive the 搂7.3 round/escalation scenario end-to-end against
...
def test_integration_escalation_passport_consistent():
    """搂7.3 escalation: expected_passport_state.yaml acknowledgement entries'
...
def test_integration_acknowledged_findings_exist_in_round_3():
    """搂5.4: acknowledged finding_ids MUST appear as finding.id in the round-3
    MATERIAL verdicts. Acknowledging a non-existent finding is a hand-edit
    attack surface (搂3.7 family A row A4 + B10 finding_ids cross-reference)."""
    base = _integration_dir()
    user_response = _load_yaml(base / "escalation" / "user_response.yaml")
...
def test_integration_finding_id_lineage_carry_forward():
    ...
    - A3 (synthesis_agent) round-1 finding gets ID X; same A3 partial-fix surfaces
      at round 2 and 3 鈫?MUST carry the same ID X. (A3 is the lineage with rounds 1+2+3.)
    - D4 (report_compiler_agent) round-1 finding gets ID Y; same D4 word-cap residue
      at rounds 2 and 3 鈫?MUST carry the same ID Y.
    ...
    assert len(set(a3_seen)) == 1, (
        f"A3 (synthesis_agent) finding ID must be carried forward across rounds 1鈫?鈫?; "
    ...
    assert len(d4_unique_ids) == 1, (
        f"D4 (report_compiler_agent) finding ID must be carried forward across rounds 1鈫?鈫?; "
        f"got {d4_round_ids}"
    )


# F-201 closure: state runner driving the 搂7.3 5-step procedure.
...
def _simulate_round(
    ...
    """Drive one round through 搂5.6 Path B for each of three agents.
    ...
    each agent's persisted entry per 搂5.6 B10 (or B11 for round==target MATERIAL).
    """
    ...
    for agent in ("synthesis_agent", "research_architect_agent", "report_compiler_agent"):
        ...
        # B10/B11 always appends to passport per 搂5.6 鈥?the harness emulates this.
        ...
        # micro-fixture phase鈫抪ath/mutation rules so an integration step
        ...
        # F-802 closure: block-message shape per 搂5.6 / 搂5.4 split.
...
# REDACTED: sensitive-looking memory line
            ...
            if expected_phase == "B11":
                ...
                for choice in ("ship_with_known_residue", "another_round", "abort_stage"):
                    assert choice in block_msg, (
                        f"round {round_n} {agent} B11 escalation message must list 搂5.4 choice "
    ...
    if any_blocking:
        if final_round:
            ship_or_block = "escalation_prompt"  # 搂5.4 + B11
        else:
            ship_or_block = "block"
    else:
        ship_or_block = "ship"
    ...
    for agent_name, agent_decision in per_agent_decisions.items():
        ...
        if final_round and verdict_status in {"MATERIAL", "AUDIT_FAILED"}:
            assert phase == "B11", (
                ...
                f"requires expected_phase=B11 per 搂5.4; got {phase!r}"
            )
            ...
            assert phase in {"B10", "A7"}, (
                f"round {round_n} {agent_name} PASS verdict requires expected_phase鈭坽{B10, A7}}; "
...
def test_integration_state_runner_drives_full_pipeline():
    """搂7.3 lines 2085-2092: 5-step harness procedure drives each round's verdict
    through orchestrator 搂5.6, accumulates passport state, verifies expected
    ...
    # F-501 closure: Path A re-verification axis 鈥?spec 搂7.3 line 1990 says
    ...
    # already-persisted entries from the prior round (A1鈫扐7 happy path with
    # NO new passport append per 搂5.6 A7 invariant). The harness models this
    ...
    def _drive_path_a_reverification(prior_round: int) -> None:
        ...
        prior round. A7 success on each 鈫?no new append, no mutation. Logs
    ...
    # Step 1+2: load each round's verdicts + drive 搂5.6 procedure.
    rounds_outcome = []
    for round_n in (1, 2, 3):
        ...
        # Step 3 (full equality per 搂7.3 line 2089): compare every declared field
        ...
        # 3e (F-502 + F-601 closure): user options match per 搂5.4 round-cap escalation
    ...
    assert final_round["ship_or_block"] == "escalation_prompt", (
        "round 3 with MATERIAL must emit escalation prompt per 搂5.4"
    )
    user_response = _load_yaml(base / "escalation" / "user_response.yaml")
    assert user_response["user_choice"] == "ship_with_known_residue"
    acked_ids = set(user_response["acknowledged_finding_ids"])

    # Append acknowledgement entries per 搂5.4 mechanics. The orchestrator
    ...
    if "proceed_to" in expected_passport:
        # F-601: synthesis_agent stage 2 鈫?stage 3 is the deterministic next stage
        ...
        assert expected_passport["proceed_to"] == "stage_3", (
            f"passport.proceed_to must equal 'stage_3' (synthesis_agent stage 2 鈫?stage 3); "
            f"got {expected_passport['proceed_to']!r}"
        )

    # F-501 closure: assert the Path A re-verification axis fired at least once.
    # spec 搂7.3 line 1990 lists Path A vs Path B fall-through as a structural axis;
    # rounds 2 and 3 each re-verify all prior-round persisted entries via A7 happy
    # path (3 agents 脳 2 rounds = 6 Path A re-verify legs minimum).
    assert len(path_a_reverification_log) >= 6, (
        f"搂7.3 axis 'Path A on resume' under-exercised: "
        f"only {len(path_a_reverification_log)} re-verify legs (expected 鈮? = 3 agents 脳 rounds 2+3)"
    )
    ...
    # (residue accepted as-is) 鈥?these are exclusive lifecycle terminals.
...
# ---------------------------------------------------------------------------
# 搂7.5 coverage cross-check (defense in depth 鈥?also covered by manifest validator)
# ---------------------------------------------------------------------------
...
def test_phase_inventory_complete():
    """搂5.6 verification failure state inventory has 24 rows + 2 happy paths.
    ...
    assert len(PHASE_TO_PASSPORT_MUTATION) == EXPECTED_PHASE_COUNT, (
        ...
        f"搂5.6 inventory + happy paths require {EXPECTED_PHASE_COUNT}. "
        "Either 搂5.6 grew (extend the map) or this assertion is stale."
    )


def test_every_fixture_phase_in_inventory():
    """Every fixture's expected_phase must be a known 搂5.6 inventory row."""
    ...
    assert not bad_phases, "fixtures reference phases not in 搂5.6 inventory:\n" + "\n".join(bad_phases)


# F-901 closure: synthetic per-phase injections per spec 搂7.3 line 2093 promise.
# 搂5.6 verification failure state inventory enumerates 24 phases + 2 happy paths
...
def test_synthetic_inject_none_mutation_phase(phase):
    """For every 搂5.6 inventory row whose Passport mutation = none, a synthetic
    ...
    # (no-op 鈥?synthetic_passport stays at initial_size)
...
def test_synthetic_inject_append_mutation_phase(phase):
    """For every 搂5.6 inventory row whose Passport mutation = appended, a
    synthetic injection at that phase MUST append exactly one entry."""
    synthetic_passport: list = []
    initial_size = len(synthetic_passport)
    rule = PHASE_TO_PASSPORT_MUTATION[phase]
    assert rule == "appended"
    # Simulate the append per 搂5.6 (B10 / B11 happy-or-escalation paths +
...
def test_synthetic_inject_conditional_phases_documented():
    ...
    # Surface count for visibility: spec 搂5.6 has 5 conditional rows.
    assert len(_CONDITIONAL_MUTATION_PHASES) >= 4, (
        f"expected 鈮? conditional rows (P-PB-dup-early / dup-other / dup-late / crash); "
...
# REDACTED: sensitive-looking memory line
    """搂5.6 A1.5 superseding-proposal preflight: when an unmerged proposal in
...
# REDACTED: sensitive-looking memory line
# REDACTED: sensitive-looking memory line
    higher-round filter, BLOCK with P-PB-supersede-missing 鈥?do NOT silently

...
Spec 搂7 names the file `tests/test_uncited_assertion.py`; the repo
...
搂"Uncited-assertion detector (D4-c)" and constants ride in
...
class TestDetectUncited(unittest.TestCase):
    """Per-condition predicate tests (spec 搂7.4 T-U1..T-U5)."""

    def test_t_u1_quantifier_no_ref_is_candidate(self) -> None:
        """Sentence with quantifier + no ref 鈫?uncited_assertion candidate."""
...
# REDACTED: sensitive-looking memory line
    ...
    def test_t_u2_definition_is_not_candidate(self) -> None:
        """Definition sentence (contains 'refers to') 鈫?NOT candidate."""
    ...
    def test_t_u3_methods_boilerplate_is_not_candidate(self) -> None:
        """Methods boilerplate list 鈫?NOT candidate.
    ...
    def test_t_u4_empirical_verb_is_candidate(self) -> None:
        """Empirical claim ('showed X%') without ref 鈫?candidate."""
...
# REDACTED: sensitive-looking memory line
    ...
    def test_t_u5_manifest_claim_no_ref_is_still_candidate(self) -> None:
        """Claim in manifest but no ref 鈫?still candidate (D4-c last paragraph).
...
# REDACTED: sensitive-looking memory line
# REDACTED: sensitive-looking memory line
        # Spec 搂3.3 / U-INV-4: manifest_claim_id + scoped_manifest_id are
    ...
    def test_t_u6_adjacent_text_with_ref_marker_filters_candidate(self) -> None:
        """T-U6 鈥?Step 9 closure (DEFERRED-CROSS-SENTENCE).
    ...
    def test_t_u6b_adjacent_text_without_ref_marker_preserves_candidate(
        ...
        """T-U6 companion 鈥?no marker in adjacent_text leaves candidate intact."""
...
# REDACTED: sensitive-looking memory line
    ...
    def test_t_u6c_missing_adjacent_text_preserves_v3_8_step_6_behavior(
        ...
        """T-U6 back-compat 鈥?no adjacent_text key behaves exactly like Step 6."""
    ...
    def test_ref_marker_short_circuits_candidate(self) -> None:
        """Sentence with `<!--ref:slug-->` marker 鈫?NOT candidate (condition 2).

        Belongs to spec 搂7.4 coverage (D4-c three-condition rule); the ref
...
# REDACTED: sensitive-looking memory line
    ...
    def test_bare_integer_after_section_cue_is_not_quantifier(self) -> None:
        ...
        cue_ref_examples = [
            ...
            "See 搂 7 for the calibration protocol.",
        ]
...
# REDACTED: sensitive-looking memory line
    ...
    def test_bare_integer_after_period_is_a_quantifier(self) -> None:
        ...
        sentences glued by whitespace 鈥?`v3.` ends the first, `7
        ...
        right tails (`7.3`) 鈥?bare integers after whitespace are no
        longer reattached.
        """
...
# REDACTED: sensitive-looking memory line
    ...
    def test_multiline_dotted_version_is_not_quantifier(self) -> None:
        """Line-wrapped dotted version reference 鈫?NOT a quantifier.
...
# REDACTED: sensitive-looking memory line
    ...
    def test_numeric_quantifier_excludes_year_and_version_strings(self) -> None:
        """Bare years / version triples / section numbers 鈫?NOT a quantifier.
        ...
        (X.Y.Z[.W鈥), a dotted-pair preceded by a section cue or `v` literal,
...
# REDACTED: sensitive-looking memory line
    ...
    def test_bare_number_quantifier_still_fires(self) -> None:
        ...
        Regression for codex R1 P1-3: spec line 250 lists 銆宯umbers /
        percentages / explicit quantifiers銆?鈥?the prior tightening that
...
# REDACTED: sensitive-looking memory line
    ...
    def test_detect_uncited_assertions_raises_on_missing_sentence_text(
        ...
        audit pipeline is the worst failure mode 鈥?upstream bugs masquerade

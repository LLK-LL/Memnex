"""Mutation tests for ARS v3.7.1 Step 2 鈥?D2 audit Scope Report lint.

Spec: docs/design/2026-04-30-ars-v3.6.8-trust-provenance-and-drift-transparency-spec.md
      搂3.2 (D2 鈥?Audit scope coverage non-disclosure)
      搂4 Step 2 鈥?Audit report Scope Report block
...
SECTION_0_H2_ANCHOR = "## Section 0 鈥?Scope Report"
SCOPE_REPORT_HEADER = "## Codex Audit Round N 鈥?Scope Report"
SECTION_1_HEADING = "## Section 1 鈥?Round metadata"
...
def test_t1_baseline_template_passes() -> None:
    """T1: template AS SHIPPED carries a valid Scope Report block 鈫?PASS."""
...
def test_t2_scope_report_header_strictly_before_section_1() -> None:
    """T2: baseline Scope Report header anchored before Section 1 鈫?PASS + ordering verified."""
    text = _baseline_text()
    result = _run_lint()
    assert result.returncode == 0
...
def test_t3_missing_section_0_anchor_fails() -> None:
    """T3: Section 0 H2 anchor removed 鈫?FAIL."""
    with _Snapshot(TEMPLATE):
        ...
        mutated = text.replace(SECTION_0_H2_ANCHOR, "## Sectionless 0 鈥?Scope Report", 1)
...
def test_t4_combined_aggregate_passed_verb_in_summary_fails() -> None:
    """T4: inject a combined-aggregate 'PASSED' verb in summary context 鈫?FAIL (spec line 152)."""
    with _Snapshot(TEMPLATE):
        text = _baseline_text()
...
def test_t5_missing_unaudited_split_fails() -> None:
    """T5: drop the 'unaudited-due-to-missing-source' split row 鈫?FAIL (spec line 150)."""
    with _Snapshot(TEMPLATE):
        text = _baseline_text()
...
def test_t6_pass_fail_summary_before_section_0_fails() -> None:
    """T6: inject a pass/fail summary BEFORE Section 0 鈫?FAIL (spec line 146)."""
    with _Snapshot(TEMPLATE):
        text = _baseline_text()
...
def test_t7_section_1_heading_byte_mutation_fails() -> None:
    """T7: Section 1 heading bytes change 鈫?FAIL (additive-prepend invariant per Q5)."""
    with _Snapshot(TEMPLATE):
        text = _baseline_text()
        # Mutate the Section 1 heading's title (capitalization swap).
        mutated = text.replace(
            SECTION_1_HEADING,
            "## Section 1 鈥?Round Metadata",  # capitalization changed
            1,
        )
        TEMPLATE.write_text(mutated, encoding="utf-8")
        result = _run_lint()
        assert result.returncode == 1, (
            ...
            "(spec 搂3.2 line 131: Sections 1-7 stay byte-equivalent in title, "
...
def test_t8_missing_required_content_field_fails(missing_field: str) -> None:
    """T8: drop one of the four required Scope Report fields 鈫?FAIL (spec lines 136-140)."""
    with _Snapshot(TEMPLATE):
        text = _baseline_text()
...
def test_t9_required_field_check_scoped_to_section_0(missing_field: str) -> None:
    ...
    with _Snapshot(TEMPLATE):
        ...
        section_0_pos = text.find("## Section 0 鈥?Scope Report")
        section_1_pos = text.find("## Section 1 鈥?Round metadata")
        ...
        appendix_decoy = (
            f"\n\n## Appendix 鈥?documentation reference\n\n"
...
def test_t11_target_relative_path_does_not_crash() -> None:
    ...
    # The lint must produce a deterministic exit code (0 or 1 鈥?both indicate
...
def test_t27_unquoted_passed_post_section_0_fails(passed_line: str) -> None:
    """T27 (codex round-13 P2 鈫?architectural cap rule): any UNQUOTED
# REDACTED: sensitive-looking memory line
...
def test_t25_preamble_header_decoy_does_not_short_circuit_ordering() -> None:
    ...
    `## Codex Audit Round N 鈥?Scope Report` line exists ahead of the
    ...
    with _Snapshot(TEMPLATE):
        ...
        decoy_block = (
            ...
            "(decoy header 鈥?purely informational, real block lives below)\n\n"
            "verdict: PASS\n\n"
            "---\n\n"
        )
...
def test_t24_verdict_inside_fenced_block_before_canonical_header_fails(
    ...
    `## Codex Audit Round N 鈥?Scope Report` header still violates spec
    ...
    with _Snapshot(TEMPLATE):
        ...
        assert result.returncode == 1, (
            ...
            f"Spec line 146 firm rule applies regardless of fence framing 鈥?"
...
def test_t22_affected_refcodes_field_required() -> None:
    ...
    with _Snapshot(TEMPLATE):
        ...
        assert result.returncode == 1, (
            ...
            f"(spec line 142 鈥?the required affected-refcodes disclosure).\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
...
def test_t23_verdict_between_section_0_h2_and_canonical_header_fails(
    ...
    but BEFORE the canonical `## Codex Audit Round N 鈥?Scope Report`
    ...
    with _Snapshot(TEMPLATE):
        text = _baseline_text()
        section_0_h2 = "## Section 0 鈥?Scope Report (mandatory; v3.7.1 D2)\n"
...
def test_t21_section_1_heading_with_appended_text_fails(appended: str) -> None:
    ...
    with extra title text 鈥?that's a real heading change (rendered prompt
...
def test_t20_multi_line_audit_summary_with_passed_fails(summary_block: str) -> None:
    ...
    lines by `PASSED` 鈥?bare or in prose like `This audit PASSED.`.
...
def test_t18_scope_report_header_prose_mention_does_not_satisfy_r1() -> None:
    ...
    `The required header is "## Codex Audit Round N 鈥?Scope Report"`
    ...
    with _Snapshot(TEMPLATE):
        ...
        section_0_mutated = section_0_block.replace(
            "## Codex Audit Round N 鈥?Scope Report",
            "## Codex Audit Round N 鈥?REDACTED",
            1,
        )
        prose_mention = (
            "\n\nNote: the required header line is "
            '`## Codex Audit Round N 鈥?Scope Report` (do not literally embed in prose).\n\n'
        )
        section_0_mutated = section_0_mutated.replace(
            "## Section 0 鈥?Scope Report",
            "## Section 0 鈥?Scope Report" + prose_mention,
            1,
        )
        mutated = (
            text[:section_0_pos]
...
def test_t15_forbidden_passed_verb_inside_fenced_block_fails() -> None:
    ...
    with _Snapshot(TEMPLATE):
        ...
        # inside the fenced block 鈥?fence-strip would blank it, but the
        # rendered prompt sent to codex preserves it verbatim.
        injection_target = "## Codex Audit Round N 鈥?Scope Report\n\n"
...
def test_t13_section_1_decoy_inside_fence_does_not_satisfy_invariant() -> None:
    ...
    with _Snapshot(TEMPLATE):
        ...
        renamed = text.replace(
            ...
            "## Section 1 鈥?Round Metadata",  # title-case mutation
            1,
        )
        decoy_fence = (
            "\n\n## Appendix 鈥?historical example\n\n"
            "```\n"
            "## Section 1 鈥?Round metadata\n"
...
def test_t14_scope_report_header_decoy_in_preamble_does_not_satisfy_r1() -> None:
    ...
    with _Snapshot(TEMPLATE):
        ...
        section_0_pos_after = mutated.find("## Section 0 鈥?Scope Report")
        mutated = (

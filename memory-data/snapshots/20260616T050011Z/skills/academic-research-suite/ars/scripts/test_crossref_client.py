...
def test_invalid_utf8_body_raises_unavailable(monkeypatch):
    """resp.read() returns bytes that aren't valid UTF-8. Per v3.9.0 spec 搂3.7,
...
def test_invalid_json_body_raises_unavailable(monkeypatch):
    ...
    HTML error page). Per v3.9.0 spec 搂3.7, translate to CrossrefUnavailable
...
def test_truncated_read_raises_unavailable(monkeypatch):
    ...
    body). Inherits HTTPException not OSError. v3.9.1 codex R1 P2 鈥?symmetric
    to OpenAlex client."""
    import http.client

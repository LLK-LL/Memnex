...
Spec: docs/design/2026-04-30-ars-v3.6.7-step-6-orchestrator-hooks-spec.md
      搂4.1 (Bash 4+ guard) + 搂4.4 (wrapper internal behavior) +
      Phase 6.1 verification gate (lines 2308) 鈥?"synthetic smoke test (codex CLI mocked
...
wrapper's 搂4.1 Bash 4+ check exits 64 on macOS stock Bash 3.2. CI runs this
...
Phase 2 audit JSONL stream (per 搂3.3) and a `codex --version` semver line
(per 搂4.4 Step 1c). The wrapper consumes the mocked output, parses the verdict
...
def _make_codex_mock(bin_dir: Path) -> Path:
    ...
    The mock supports two invocation forms:
      1. `codex --version` 鈫?prints `codex-cli 0.128.0` (matches 搂3.4 sidecar
         codex_cli_version semver pattern)
      2. `codex exec -m gpt-5.5 -c '...' --json - < <prompt>` 鈫?emits canonical
         JSONL events to stdout matching 搂3.3 four-event clean-completion shape
    ...
    mock_script = textwrap.dedent(
        ...
        # The agent_message text follows audit template Section 6 format 鈥?
...
def test_wrapper_dry_run_writes_nothing(tmp_path):
    """搂4.1 / 搂10 Phase 6.1 verification gate 鈥?--dry-run validates inputs only."""
    bin_dir = tmp_path / "bin"
    bin_dir.mkdir()
    _make_codex_mock(bin_dir)
    ...
    # 搂10 Phase 6.1 verification gate: --dry-run must NOT write any contract artifacts.
...
def test_wrapper_rejects_round_2_without_previous_findings(tmp_path):
    """搂4.2 input validation: --round > 1 requires --previous-findings."""
    bin_dir = tmp_path / "bin"
    bin_dir.mkdir()
    _make_codex_mock(bin_dir)

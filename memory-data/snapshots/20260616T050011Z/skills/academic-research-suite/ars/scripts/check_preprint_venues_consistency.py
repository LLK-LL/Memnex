#!/usr/bin/env python3
"""#105 lint 鈥?PREPRINT_VENUES 10-server list must agree across files.

v3.7.3 spec 搂3.2 Vector 1 fixes the closed list at 10 preprint servers
(gemini review F6 / codex F13 expansion from initial 6). The list lives
in two places:

1. `deep-research/agents/bibliography_agent.md` 搂 "Signal 1 鈥?
   preprint_post_llm_inflection" 鈥?bullet list (prose source of truth
   the bibliography_agent reads at ingest time).
2. `scripts/contamination_signals.py` `PREPRINT_VENUES` frozenset 鈥?the
...
def _extract_prose_venues(text: str) -> set[str]:
    """Pull the bullet list under 搂 Signal 1.
...
def main() -> int:
    ...
    print("PREPRINT_VENUES lint FAILED 鈥?venue list drift detected:", file=sys.stderr)
    if only_prose:
    ...
    print(
        "Reconcile by updating both files in lockstep. Spec 搂3.2 Vector 1 + "

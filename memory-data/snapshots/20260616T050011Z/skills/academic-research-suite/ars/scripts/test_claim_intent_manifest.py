...
- **T-M1 鈥?Three-set diff**: emitted 鈭?intended 鈭?supported with drift
  ...
  from manifest claims dropped during drafting). Verifies the 搂"Manifest
  cross-reference (D6)" three-set-diff table directly.

- **T-M2 鈥?Missing manifest**: `manifests=[]` plus the `SENTINEL_MANIFEST_ID`
  on each citation must keep every defect_stage path emitting. Per 搂9
  acceptance bullet `claim_intent_manifest absent 鈫?MANIFEST-MISSING
  ...
  搂13 step 8) and is not covered here.

- **T-M3 鈥?Constraint inheritance**: an `MNC-{m}` global constraint MUST
  ...
  sorted order (`_active_constraints_for_claim` invariant per spec 搂4 step
  3 + M-INV-3).

Spec 搂7 names the file `tests/test_claim_intent_manifest.py`. Repo
...
except Exception as exc:  # pragma: no cover 鈥?RED-phase import pathway
...
class _ManifestTestBase(unittest.TestCase):
    ...
    During the RED phase (Step 4 of the TDD plan in spec 搂13), the test
    suite imports cleanly even when the module is missing 鈥?the contract
...
# ---------------------------------------------------------------------------
# T-M1 鈥?Three-set diff (emitted 鈭?intended 鈭?supported, drift detection).
# ---------------------------------------------------------------------------
...
class TM1ThreeSetDiff(_ManifestTestBase):
    ...
    def test_three_set_diff_emits_both_drift_kinds(self) -> None:
        ...
        #   - C-001 with original manifest claim_text  鈫?intended 鈭?emitted 鈭?supported
        #   - C-002 with original manifest claim_text  鈫?intended 鈭?emitted, judge AMBIGUOUS (NOT in supported)
        ...
        #   - C-003 manifest claim is DROPPED (intended 鈭?emitted)
        emitted_citations = [
            _citation(
                claim_id="C-001",
                claim_text="Sample preprints accounted for 67% of corpus.",
            ...
            _citation(
                claim_id="C-100",  # Drifted 鈥?not in manifest claims[]
                claim_text="Median half-life dropped to 11 days.",
                ref_slug="brown2025",
                anchor_value="22",
            ),
            _citation(
                claim_id="C-101",  # Drifted 鈥?not in manifest claims[]
        ...
        # ---- Emitted 鈭?Intended (manifest claim_text appears in emitted) ----
        ...
        self.assertEqual(
            ...
            "Setup invariant: expected exactly 2 emitted鈭﹊ntended texts",
        )
        ...
        # so the supported鈭﹊ntended is C-001 only.
        self.assertEqual(
            supported_texts & manifest_texts,
            {"Sample preprints accounted for 67% of corpus."},
            "supported鈭﹊ntended must equal the 1 SUPPORTED claim that is also in manifest",
        )

        # Judge invocation count 鈥?one call per emitted citation. Pins
...
# ---------------------------------------------------------------------------
# T-M2 鈥?Missing manifest: MANIFEST-MISSING fallback, all defect_stages emit.
# ---------------------------------------------------------------------------
...
class TM2MissingManifestFallback(_ManifestTestBase):
    ...
    def test_sentinel_fallback_emits_all_defect_stages(self) -> None:
        ...
        # cover in MANIFEST-MISSING fallback per 搂9 acceptance criterion.
        # The sentinel scoped_manifest_id MUST propagate through to every
        # emitted entry (INV-15 sentinel branch).
        citations = [
            # Path 1: anchor=none 鈫?RETRIEVAL_FAILED + not_applicable + not_attempted.
            _citation(
                claim_id="C-001",
            ...
            # Path 2: retrieval=failed (paywall) 鈫?RETRIEVAL_FAILED + not_applicable + failed.
            _citation(
                claim_id="C-002",
            ...
            # Path 3: retrieval=not_found 鈫?RETRIEVAL_FAILED + retrieval_existence.
            _citation(
                claim_id="C-003",
        ...
        self.assertEqual(
            len(results),
            6,
            "every citation must emit a row in MANIFEST-MISSING fallback (per 搂9 'all defect_stages still emit')",
        )
        ...
        self.assertEqual(
            ...
            [],
            "no manifest 鈫?no MNC/NC scope 鈫?no constraint_violations rows",
        )
        self.assertEqual(
            out["audit_sampling_summaries"],
            [],
            "N=6 鈮?cap=100 鈫?no sampling summary",
        )


# ---------------------------------------------------------------------------
# T-M3 鈥?Constraint inheritance: MNC applies even when not redeclared at claim level.
# ---------------------------------------------------------------------------
...
class TM3ConstraintInheritance(_ManifestTestBase):
    ...
    def test_mnc_inherited_alongside_claim_level_nc(self) -> None:
        ...
        self.assertEqual(
            ids,
            ["MNC-1", "NC-C001-1"],
            "active_constraints must be sorted by constraint_id (M-INV-3 + 搂4 step 3)",
        )

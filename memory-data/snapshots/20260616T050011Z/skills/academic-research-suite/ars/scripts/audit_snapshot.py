#!/usr/bin/env python3
"""audit_snapshot.py 鈥?ARS v3.6.7 Step 6 Phase 6.1 byte-exact snapshot helper.
...
computes SHA-256 from in-memory bytes (single read per file 鈥?no TOCTOU window),
...
and emits both manifest and SHAs to the wrapper via JSON 鈥?no subshell, no
...
CLI modes:
  ...
  --snapshot
    Compute snapshot for a bundle. Writes:
      - <output-dir>/<run_id>.manifest.txt (per spec 搂3.6)
...
Exit codes:
  0   success
  64  EX_USAGE 鈥?bad arguments or NUL-containing input
...
def _extract_template_sections(template_str: str, section_numbers: list[int]) -> str:
    ...
    The template uses `## Section N 鈥?Title` headers with `---` separators.
    ...
    the appendix worked example 鈥?which embeds a synthesis_agent 4(f) clause 鈥?
    ...
    鈥?Scope Report`; without fence-awareness, that line was treated as the
    ...
    boundary_re = _re.compile(r"^## (?:Section (\d+)( 鈥攟$)|.+)", _re.MULTILINE)
    matches = list(boundary_re.finditer(masked))
    if not matches:
        return template_str  # no headings found 鈥?fall back to verbatim
...
def render_prompt(
    ...
    sees exactly the bytes whose SHA-256 went into the manifest 鈥?there is
    ...
    leaving placeholders unsubstituted 鈥?codex would see literal `{N}` in
    ...
    if round_n <= 1:
        ...
        prior_summary = (
            "(prior findings attached as supporting context 鈥?see file listing "
    ...
    # Extract audit template Sections 0, 3, 6, 7 (verbatim 鈥?Section 0 is the
    # v3.7.1 D2 Scope Report block riding verbatim every round per spec 搂3.2;
    ...
    # substituted) 鈥?F-023 closure also requires the placeholder text not
...
# REDACTED: sensitive-looking memory line
        ...
        f"(c) Run the 7 audit dimensions (搂3.1-搂3.7) plus the bundle-specific Section 4(f) check on the primary deliverables. Report each finding with the dimension or `4(f)` that surfaced it.\n"
        f"(d) Anchoring-bias residual check on closed findings.\n"
        f"(e) PARTIAL-vs-CLOSED check.\n"
        f"{section_4f}\n\n"
        f"## Section 5 鈥?Convergence target\n\n"
...
def write_manifest(
    ...
    """Write the canonical bundle manifest per 搂3.6.
...
def cmd_snapshot(args: argparse.Namespace) -> int:
    ...
    if args.previous_findings:
        # Inject --previous-findings into supporting (preserves 搂3.6 3-role enum).
    ...
    # Now safe to write 鈥?render succeeded, no traceback path.

...
#   64   EX_USAGE  鈥?input validation failed (no files written)
#   70   EX_SOFTWARE 鈥?AUDIT_FAILED (JSONL parse error or bundle mutation)
#   73   EX_CANTCREAT 鈥?tee write failed; JSONL integrity not guaranteed
#   75   EX_TEMPFAIL 鈥?codex rate-limited / transient error (retry candidate)
#   other  codex non-zero exit (preserved as-is; AUDIT_FAILED artifact set written)

set -euo pipefail

# ---------------------------------------------------------------------------
# Bash version guard 鈥?Codex adapter supports macOS stock Bash 3.2+.
# The upstream 搂4.1 design note said Bash 4+, but this wrapper only uses
...
# ---------------------------------------------------------------------------
# Helper functions 鈥?defined before the dependency preflight so they are
# available for the _sha256 check itself.
# ---------------------------------------------------------------------------

# SHA-256 backend selection 鈥?cached once to avoid forking `command -v` per call
...
# Used for sidecar, verdict, and proposal entry 鈥?the three files 搂4.4 搂4.8
# guarantees orchestrator observes whole-or-not-at-all. JSONL is exempt
# because codex streams it over the audit's runtime; see 搂4.4 atomicity note.
#
# F-010 (搂3.6): each sub-step (cat, fsync, mv) is explicitly error-checked.
# A raw cat/fsync/mv failure with set -euo pipefail would abort the wrapper
# before AUDIT_FAILED artifacts are written, violating 搂4.6 invariant E8.
...
# codex 0.125+ emits thread_id only on the opening thread.started event (搂3.3).
#
# F-001 (搂3.6): jq must be non-fatal. A SIGKILL'd or disk-full partial JSONL can
# make jq exit non-zero; with set -euo pipefail that would abort the wrapper BEFORE
# the mandatory AUDIT_FAILED sidecar/verdict/proposal trio is written, violating 搂4.6
...
_extract_jsonl_thread_id() {
  ...
  # F-015 (搂3.1): jq emits the literal string "null" when thread_id is absent or
...
# Format: YYYY-MM-DDTHH:MM:SS.mmmZ  (colons in time 鈥?JSON field format).
...
# _make_run_id
# Generates a run_id per 搂3.1 / 搂3.7 F1 contract:
#   <ISO-8601-Z>-<4-hex>
...
# F-009 (搂3.1): clamp to minimum 0.001 鈥?audit_sidecar.schema.json sets
...
# If <path> is absolute, strip the leading slash (unlikely 鈥?搂4.2 says
...
# F-006 (搂3.1): Validates the path satisfies the audit_sidecar.schema.json
...
# ---------------------------------------------------------------------------
# Dependency preflight 鈥?exits 64 with "missing dependency: <name>" before
...
# F-005 (搂3.5): require_arg guards all value-taking flags so a missing operand
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    # Explicitly reject flags from early drafts that are NOT part of 搂4.2
    --bundle)
      printf '[run_codex_audit] error: --bundle is not a valid flag (use --bundle-id); see spec 搂4.2\n' >&2
      exit 64
      ;;
    --audit-template)
      printf '[run_codex_audit] error: --audit-template is not a valid flag; see spec 搂4.2\n' >&2
      exit 64
      ;;
    *)
...
# ---------------------------------------------------------------------------
# Input validation (搂4.2) 鈥?all failures 鈫?exit 64 EX_USAGE
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# F-017 (搂3.6): inject --previous-findings into SUPPORTING before snapshot.
...
# and codex invocation without detection 鈥?codex audits against different
...
# This preserves the 搂3.6 manifest role enum (primary | supporting | template)
...
# the manifest 鈥?no live cat call after snapshot phase.
# ---------------------------------------------------------------------------
...
# Audit template path 鈥?constant per 搂3.4 schema constraint
...
# ---------------------------------------------------------------------------
# F-016 (搂3.1): Normalize and validate all path inputs BEFORE the dry-run
...
# F-006 (搂3.1): validate deliverable and supporting paths satisfy repo_relative_path
...
# F-014 (搂3.1): validate --output-dir BEFORE mkdir -p to avoid creating a
# directory at an invalid path (absolute, .. traversal) before EX_USAGE 64 rejects it.
# "No side effects on input validation rejection" 鈥?mkdir must not fire first.
_validate_repo_relative "${OUT_DIR}" "--output-dir"

# ---------------------------------------------------------------------------
# Step 0a 鈥?dry-run early exit. 搂10 Phase 6.1 verification gate and 搂1.2
...
# ---------------------------------------------------------------------------
# SIGTERM / SIGINT trap 鈥?F-059 closure.
...
# F-010 (搂3.6): EXIT trap removes partial .tmp artifacts left by any aborted
# _atomic_write call. Preserves final contract files (no .tmp suffix).
# Coexists with SIGTERM/SIGINT trap above 鈥?EXIT fires after signal handler exits.
...
# was moved above Step 0a (F-016 fix) 鈥?paths are already normalized here.
run_id=$(_make_run_id)
_CURRENT_RUN_ID="${run_id}"

# F-010 (搂3.6): normalize mkdir failure to EX_CANTCREAT 73.
...
# ---------------------------------------------------------------------------
# F-018 (搂3.6): Reject binary/NUL inputs before snapshot.
# ---------------------------------------------------------------------------
# Step 0b 鈥?snapshot bundle (DELEGATED TO scripts/audit_snapshot.py).
#
...
#   F-018 (Round 4 fix broke wrapper): grep -q $'\0' is empty pattern 鈫?all
#     non-empty files rejected as binary.
#   F-020 (trailing newline drift): $(cat file) strips \n; manifest SHA from
#     in-memory content drifts from sha256sum(file) 鈫?Step 3a false-positive.
#
...
# F-021 (搂3.6) closure: dedup --previous-findings against SUPPORTING after
...
# (jq might not be present; we already require python3 in 搂4.1 dependency table).
...
# Clean up temp files (manifest.txt and prompt.txt remain 鈥?they're contract artifacts)
...
# ---------------------------------------------------------------------------
# Step 1 鈥?prompt rendered by audit_snapshot.py (Step 0b above) into
# <run_id>.prompt.txt. The Bash wrapper feeds it to codex stdin via shell
# redirection in Step 2b. No template-rendering logic in Bash anymore 鈥?that
...
# ---------------------------------------------------------------------------
# Step 0c 鈥?REMOVED in Round 5 architecture.
...
# from the same read 鈥?there is no second-read window to defend against.
...
# ---------------------------------------------------------------------------
# Step 2a 鈥?pre-touch empty JSONL placeholder (F-059 closure).
...
# handled correctly by 搂5.6 Path B5 short-circuit without Layer 2 validation.
#
# F-010 (搂3.6): guard the pre-touch; a failure here (permissions, disk full)
...
#      tee failed 鈥?disk full / EIO would silently corrupt the JSONL artifact).
...
#
# This is `codex exec`, not `codex exec resume` 鈥?every audit run starts a
...
# tee write failure 鈫?EX_CANTCREAT (73): JSONL integrity not guaranteed.
...
# ---------------------------------------------------------------------------
# Step 3a 鈥?detect post-snapshot disk mutation via audit_snapshot.py verify.
...
# semantics 鈥?F-020 false-positive (Bash strips \n, sha256sum doesn't) is gone.
#
...
if [[ "${VERIFY_EXIT}" -eq 1 ]]; then
  # Mutation detected 鈥?read mutated paths from stdout.
  ...
  # AUDIT_FAILED 鈥?orchestrator must not assume "no mutation" from "verify
  # could not run". 搂4.6 contract: AUDIT_FAILED proposal blocks transition.
  BUNDLE_MUTATION_DETECTED=1
...
# manifest written at audit time 鈥?updating it post-mutation would silently
# accept a stale audit.

# ---------------------------------------------------------------------------
# Step 3c 鈥?write sidecar (atomic). 搂3.4 / 搂4.3.
...
# Sources from the SNAPSHOT_JSON_FILE (audit_snapshot.py output) 鈥?same shape
...
# F-008 (搂3.1): failure_reason maxLength is 500 per audit_verdict.schema.json,
...
_truncate_failure_reason() {
  local s="$1"
  # Strip newlines/CRs 鈥?schema regex ^[^\n\r]+$ rejects multi-line values
  s=$(printf '%s' "${s}" | tr -d '\n\r')
  if [[ "${#s}" -gt 500 ]]; then
    printf '%s...' "${s:0:497}"
  else
...
# F-007 (搂3.6): capture probe stderr explicitly so the actual parser rejection
...
if [[ "${CODEX_EXIT}" -eq 0 ]] \
  ...
  # F-003 (搂3.6): capture full parser stdout/stderr/exit BEFORE piping to
  ...
  # F-010 (搂3.6) Round 3: eliminate mktemp dependency 鈥?use a deterministic path
  ...
  # before AUDIT_FAILED artifacts are written 鈥?violating 搂4.6 invariant E8.
  ...
  if _verdict_yaml_content=$(python3 scripts/parse_audit_verdict.py \
    ...
    rm -f "${_full_parse_stderr_file}"
    # F-003: full parse failed after probe passed 鈥?fall through to AUDIT_FAILED
  ...
  if [[ "${BUNDLE_MUTATION_DETECTED}" -eq 1 ]]; then
    ...
    # F-007: codex exit 0 but --probe rejected the JSONL 鈥?use captured stderr
  ...
  # it just wrote (搂4.6 contract: exit 0 鈫?PASS/MINOR/MATERIAL only).
  if [[ "${WRAPPER_EXIT_CODE}" -eq 0 ]]; then
    WRAPPER_EXIT_CODE=70
  fi
fi
...
# ---------------------------------------------------------------------------
# Step 5 鈥?emit proposal entry (LAST write per invariant E8).
# Verdict block mirrors the verdict.yaml just written.
# verified_at and verified_by are NOT set 鈥?orchestrator fills them at merge.
# Proposal entries carrying these fields are rejected at 搂5.6 Path B4
...
# ---------------------------------------------------------------------------
# Step 6 鈥?exit with wrapper_exit_code (搂4.6 contract).

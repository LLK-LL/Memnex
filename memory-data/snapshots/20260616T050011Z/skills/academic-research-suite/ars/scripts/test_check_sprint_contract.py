...
class TestSchemaValidation(unittest.TestCase):
    ...
    def test_failure_conditions_F0_must_be_accept_action(self):
        ...
        accept-grade reservation per shipped templates 搂6.1 + 搂6.2; tying its
    ...
    def test_failure_condition_id_pattern_rejects_leading_zero(self):
        ...
        F01, F02... must be rejected to keep ordinal tie-break (搂3.2 severity
    ...
    def test_conditional_quantifier_not_required_for_non_reviewer_mode(self):
        """spec 搂7.1 + spec 搂3.3: when mode does NOT start with 'reviewer_', the
        ...
        # No error should mention cross_reviewer_quantifier 鈥?the conditional
        # branch in 搂3.3 only fires when mode starts with 'reviewer_'.
        self.assertFalse(
...
class TestSoftWarnings(unittest.TestCase):
    ...
    def test_sc1_lag_exactly_2_does_not_warn(self):
        """Boundary: spec 搂4.3 line 340 says SC-1 fires when baseline 'lags
    ...
    def test_sc6_placeholder_unreachable_on_schema_valid_contract(self):
        """SC-6 was retained in spec as dead-path defense-in-depth 鈥?can never
...
class TestTemplateSemantics(unittest.TestCase):
    def test_full_template_precedence_rule(self):
        ...
        spec 搂3.2 / 搂5.5 precedence selects F1. Exercises the rule against the
...
class TestSchema131WriterEvaluatorPositive(unittest.TestCase):
    """搂7.3 schema-level positive fixtures: shipped writer/evaluator templates
...
class TestSchema131NegativeBranches(unittest.TestCase):
    """搂7.3 schema-level negative fixtures exercising the five v3.6.6 schema
    branches the validator hard-fails (branches 4 / 5 / 6 / 11 / 12 per 搂3.5).
    Cross-mode field leakage (R1) is intentionally NOT tested as a hard-fail
    here per 搂7.1 settled decision; v3.7.x not-clause hardening covers that.
    """
...
class TestSchema131ReviewerZeroTouch(unittest.TestCase):
    """搂3.6 backward-compat proof: existing reviewer contracts validate identically
    under Schema 13.1. These two test names are explicitly committed by 搂3.6."""
    ...
    def test_existing_reviewer_contracts_still_valid_under_13_1(self):
        ...
        validation success without modification. 搂3.6 zero-touch verification."""
        from scripts.check_sprint_contract import validate, check_structural_invariants
    ...
    def test_byte_equivalent_validation_for_reviewer_contracts(self):
        ...
        Schema 13.1 produces an empty error list 鈥?the structural equivalent of
        diffing against the (now-superseded) Schema 13 result, which was also
        empty for these templates. 搂3.6 promised this regression test."""
...
class TestSC5SC9SC11ModeGating(unittest.TestCase):
    """搂7.1 implementation requirement: SC-5 (measurement_procedure missing
    ...
    reviewer-only fields per 搂3.3.1 / 搂3.3.5. Mode-agnostic warnings (SC-1 /

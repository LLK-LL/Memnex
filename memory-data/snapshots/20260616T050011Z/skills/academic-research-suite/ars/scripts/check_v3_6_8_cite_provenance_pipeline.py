#!/usr/bin/env python3
"""ARS v3.7.1 Step 3b 鈥?Cite-Time Provenance Finalizer (academic-pipeline mode).
...
Spec: docs/design/2026-04-30-ars-v3.6.8-trust-provenance-and-drift-transparency-spec.md
      ...
      搂 3.3   鈥?Cite-time trust-chain non-check (4-cell matrix)
      搂 3.6   鈥?Human-Read Signal Protocol (peer-file join semantics)
...
carries a `## Cite-Time Provenance Finalizer (v3.7.1)` subsection containing:
  ...
  1. All 4 matrix rows (HIGH WARN / MED WARN / LOW WARN / OK), each
     keyed by its canonical phrase from spec 搂 3.3 lines 174-179
# REDACTED: sensitive-looking memory line
     the 搂3.6 computed form `<passport-stem>_human_read_log.yaml`)
...
def _extract_finalizer_block(text: str) -> str | None:
    ...
    INTENTIONAL 鈥?the canonical Step 3b prose is flat (no nested
...
# must appear in the block. Markers are taken verbatim from spec 搂 3.3
...
_REQUIRED_CLAUSES: list[tuple[str, str, str]] = [
    (
        ...
        "UNVERIFIED CITATION 鈥?NO ORIGINAL",
        "Row 1 maps source_acquired=false 鈫?"
        "[UNVERIFIED CITATION 鈥?NO ORIGINAL]<!--ref:slug--> "
        "(spec 搂3.3 line 176)",
    ),
    (
        "matrix row 2 (MED WARN)",
        "UNVERIFIED CITATION 鈥?AI HAS NOT CROSS-CHECKED",
        "Row 2 maps (acquired=true, verified=false, human=false) 鈫?"
        "[UNVERIFIED CITATION 鈥?AI HAS NOT CROSS-CHECKED]<!--ref:slug--> "
        "(spec 搂3.3 line 177)",
    ),
...
# REDACTED: sensitive-looking memory line
    ...
    (
        "matrix row 3 (LOW WARN) 鈥?resolved marker form",
        "<!--ref:slug LOW-WARN-->",
        "Row 3 maps (acquired=true, verified=true, human=false) 鈫?"
        "<!--ref:slug LOW-WARN--> (spec 搂3.3 line 178)",
    ),
...
# REDACTED: sensitive-looking memory line
    ...
    (
        "matrix row 3 (LOW WARN) 鈥?checklist append clause",
        "per-section pre-finalization checklist",
        "Row 3 must also append the slug to a per-section "
        "pre-finalization checklist artifact (spec 搂3.3 line 178)",
    ),
    ...
    (
        "matrix row 4 (OK) 鈥?resolved marker form",
        "<!--ref:slug ok-->",
        "Row 4 maps all-true triple 鈫?<!--ref:slug ok--> "
        "(spec 搂3.3 line 179)",
    ),
    (
        "matrix row 4 (OK) 鈥?canonical row clause",
        "**OK**: replace with",
        "Row 4's canonical row clause is `**OK**: replace with "
        "<!--ref:slug ok-->` (spec 搂3.3 line 179)",
    ),
    (
        "peer-file join reference",
        "human_read_log.yaml",
        "Finalizer joins on `<passport-stem>_human_read_log.yaml` per "
        "spec 搂3.3 line 172 + 搂3.6 round-5 R5-003 amend",
    ),
    (
        "idempotency clause",
        "idempotent",
    ...
    (
        ...
        "invalidate (spec 搂3.3 line 181 + Step 3b acceptance line 457)",
    ),
    ...
    (
        ...
        "/ars-unmark-read",
        "Spec 搂3.6 line 302 defines /ars-unmark-read as the rescind "
    ...
    (
        ...
        "demotion (e.g. `ok` 鈫?`LOW-WARN` after /ars-unmark-read) so "

...
Spec: docs/design/2026-04-30-ars-v3.6.7-step-6-orchestrator-hooks-spec.md
      搂7.2 (micro-fixture schema) + 搂7.3 (integration-fixture schema)
      + 搂7.5 (fixture-to-pattern coverage cross-check)
...
def _validate_micro_directory_id_match(
    ...
    be a known pattern ID. Closes codex F-803 鈥?without the second clause, an
    ...
    if expected not in PATTERN_IDS:
        errors.append(
            ...
            f"{expected!r} is not a known pattern ID 鈥?micro fixtures MUST live in a "
...
def _validate_fixture_path_safety(rel_path: str, base: Path) -> list[str]:
    ...
    to pass despite the 搂7.2 self-contained directory contract.
    """

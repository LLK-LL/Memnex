...
dispatch automatically after `deep-research systematic-review 鈫?academic-paper
full` runs, per the documented handoff path in
`policy_anchor_disclosure_protocol.md` 搂3.1.

Two layers under test:

1. Resolution helper (`scripts/slr_lineage.py`) 鈥?pure function over a
   `state_tracker.stages` snapshot. Returns `True` iff any stage was
   produced by deep-research in systematic-review mode.

2. Renderer integration 鈥?a passport carrying `slr_lineage=True` passes
   the 搂4.3 G2 invariant gate without a manual `mode_param='systematic-
...
# ============================================================================
# 搂1 Resolution helper 鈥?`slr_lineage.resolve_from_stages`
# ============================================================================
class ResolveFromStagesTest(unittest.TestCase):
    """Helper computes `slr_lineage` from a `state_tracker.stages` dict.

    Contract (per design 搂4.2): `slr_lineage` is True iff any stage in
    ...
    def test_mid_entry_no_stage_1_returns_false(self) -> None:
        ...
        Stage 1 evidence 鈥?SLR cannot be inferred."""
        stages = {
            "2": {"skill": "academic-paper", "mode": "full"},
    ...
    def test_non_deep_research_systematic_review_ignored(self) -> None:
        ...
        (hypothetically a future skill) must not trigger SLR lineage 鈥?
    ...
    def test_stage_without_mode_skipped(self) -> None:
        ...
        not raise 鈥?the resolver treats missing/None as not-SLR."""
        stages = {
...
# ============================================================================
# 搂2 Renderer integration 鈥?passport with slr_lineage=True dispatches
# ============================================================================
class RendererIntegrationTest(unittest.TestCase):
    ...
    `deep-research systematic-review 鈫?academic-paper full 鈫?disclosure`
    auto-dispatch path."""
    ...
    def test_non_slr_pipeline_passport_still_blocks_prisma_track(self) -> None:
        ...
        --policy-anchor=prisma-trAIce per G2 invariant 鈥?no behavior
...
# ============================================================================
# 搂3 End-to-end: stages 鈫?resolver 鈫?passport-shape 鈫?renderer
# ============================================================================
class EndToEndPipelineHandoffTest(unittest.TestCase):
    """Exercises the full handoff: state_tracker.stages snapshot 鈫?
    resolver 鈫?outgoing passport 鈫?renderer accepts prisma-trAIce."""
...
# ============================================================================
# 搂4 Monotonic OR 鈥?preserve already-persisted slr_lineage across resume
# ============================================================================

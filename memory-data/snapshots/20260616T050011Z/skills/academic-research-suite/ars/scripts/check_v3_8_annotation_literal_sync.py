"""ARS v3.8 搂3.6 + 搂5 鈥?annotation literal sync lint.
...
1. `scripts/claim_audit_finalizer.py` as `ANNOTATION_HIGH_WARN_*` constants
   (the finalizer write side 鈥?`apply_finalizer` returns these on
   gate_refuse=True rows).
2. `academic-paper/agents/formatter_agent.md` REFUSE rules 6-10 (the
   formatter read side 鈥?terminal hard gate scans the draft for these
...
formatter prose cannot duplicate verbatim 鈥?the prefix up to but not
including the interpolation hole is the contract.

Spec: docs/design/2026-05-15-issue-103-claim-alignment-audit-spec.md 搂5.
...
def _extract_refuse_block(formatter_text: str) -> str | None:
    ...
    this block 鈥?restricting the lint search here prevents the false PASS
...
# REDACTED: sensitive-looking memory line
    ...
    Annotation literals follow three shapes:
      ...
      - Em-dash suffix:      `[HIGH-WARN-CLAIM-AUDIT-ANCHORLESS 鈥?v3.7.3 R-L3-1-A VIOLATION REACHED AUDIT]`
...
# REDACTED: sensitive-looking memory line
    first 鈥?and the formatter prose must contain that prefix exactly.

    Decision: closed literals exact, interpolated/em-dash prefix.
    """
    # Detect interpolated/em-dash shapes first 鈥?these need prefix match.
    # Use the earliest-occurring terminator (min idx across the three
    # candidate cuts), not the list-order first hit 鈥?`{` and ` (` can
    ...
    # Closed plain literal 鈥?return the whole thing so the search hit
...
def main() -> int:
    ...
    # 鈥?a literal that appears only in background prose or a cross-reference
...
# REDACTED: sensitive-looking memory line
        ...
        # `]` (no-variable closing bracket) 鈥?both are valid boundaries
...
# REDACTED: sensitive-looking memory line
            ...
            # indicates a rename attack 鈥?e.g. `鈥IOLATION-RENAMED]`
            # should NOT satisfy `鈥IOLATION` because the boundary is a
...
# REDACTED: sensitive-looking memory line
    ...
    if missing:
        print(
            ...
            f"literal(s) missing from formatter REFUSE list (spec 搂5 + 搂1 "
            f"deliverable 5):"
        )

#!/usr/bin/env python3
"""#108 policy-anchor disclosure renderer 鈥?executable spec / referee.
...
decision logic 鈥?搂3 G10 7-row precedence table, 搂4 auto-promotion
forbiddance, and the per-anchor invariant predicates (G2/G5/G7/G8/G9) 鈥?so
that:

1. The conformance test suite (`test_policy_anchor_disclosure.py`) can
   exercise every (input 脳 expected output) combination deterministically.
2. The protocol doc and the referee stay in sync 鈥?when the protocol's
   搂2 table changes, this module must change with it; the test suite
   catches drift.

References:
- Decision Doc 搂3 (G10 7-row precedence table), 搂4.3 (8 invariants),
  搂4.4 (11 open concerns).
- `academic-paper/references/policy_anchor_disclosure_protocol.md` 搂搂2-7.
- Implementation spec 搂3 (resolved-paths table).
"""
...
class PairedMandateViolation(RuntimeError):
    ...
    affected_sections together, or neither 鈥?emitting one without the
    other is non-conformant."""


class VenueAnchorConflict(RuntimeError):
    """搂4.4 #7: --venue and --policy-anchor that map to incompatible
...
class SelectorUnsupplied(ValueError):
    """Neither venue nor policy_anchor supplied 鈥?disclosure mode
...
class RendererInput:
    """Flat runtime input 鈥?no corpus-entry-level fields per G1 invariant."""
...
def decide_disclosure_output(ri: RendererInput) -> DisclosureDecision:
    ...
    Implements the protocol's 搂2 table exactly: rows evaluated top to
...
def _check_venue_anchor_conflict(ri: RendererInput) -> None:
    """搂4.4 #7: the only consistent pair is any Nature Portfolio venue
...
def _check_track_gate(ri: RendererInput) -> None:
    ...
    raise TrackGateError(
        ...
        "general track is forbidden by 搂4.3 G2 invariant."
    )


# ---------------------------------------------------------------------------
# 搂4 auto-promotion forbiddance + helpers
# ---------------------------------------------------------------------------
def render_facet_as_used(category_state: str) -> str:
    """Render a single facet at USED strength 鈥?raises if input is still
    ...
    if category_state == "UNCERTAIN":
        raise AutoPromotionForbidden(
            ...
            "See 搂4.3 G3/G10 invariant and concern #6 resolution."
        )
...
# ---------------------------------------------------------------------------
# G5 invariant 鈥?three-gate prompt disclosure predicate
# ---------------------------------------------------------------------------
...
def assert_ieee_pairing_conformant(
    ...
    if has_level != has_sections:
        raise PairedMandateViolation(
            ...
            "搂4.3 G8 invariant + impl spec concern #4 resolution."
        )
...
def nature_image_outputs(images: Iterable[dict]) -> dict[str, list[dict]]:
    """搂4.4 #5: Nature image disclosure emits two output channels (annotation

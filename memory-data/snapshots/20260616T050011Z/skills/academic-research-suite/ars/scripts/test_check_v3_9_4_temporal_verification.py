...
Per docs/design/2026-05-18-ars-v3.9.4-temporal-verification-spec.md 搂7.
"""
...
def test_timeline_open_ended_only_on_end():
    """open_ended:true on start date should be a schema violation per spec 搂3.1 date shape table."""
    schema = _load_schema("timeline.schema.json")
    bad = {
        "schema_version": "1.0",
        "sources": [
...
def test_citation_provenance_high_requires_both_sources():
    """confidence:high MUST have both crossref_issued and pdftotext_cover_first_line populated (per spec 搂3.4 agreement table row 1)."""
    schema = _load_schema("citation_provenance.schema.json")
    bad = {
        "schema_version": "1.0",
        "audit_run_id": "2026-05-18T12:34:56Z-a1b2",
...
def test_citation_provenance_high_rejects_absent_sources():
    ...
    which does not fire when the property is absent 鈥?so an entry with confidence:high and
...
def test_temporal_audit_p2_requires_bound_event():
    """P2 anachronism MUST have bound_event non-null per spec 搂3.2 per-kind map."""
    schema = _load_schema("temporal_audit_results.schema.json")
    bad = {
        "schema_version": "1.0",
        "audit_run_id": "2026-05-18T12:34:56Z-a1b2",
...
def test_lint_bibliography_agent_unchanged(tmp_path):
    ...
    assert result.returncode == 0, f"baseline mismatch 鈥?bibliography_agent.md was modified? stderr={result.stderr}"

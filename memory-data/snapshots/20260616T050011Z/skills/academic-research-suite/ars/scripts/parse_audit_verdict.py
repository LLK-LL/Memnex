#!/usr/bin/env python3
"""parse_audit_verdict.py 鈥?ARS v3.6.7 Step 6 Phase 6.1
...
# F-019 (搂3.3): canonical UUID regex for thread.started.thread_id validation.
...
def validate_stream_shape(events: list[dict]) -> None:
    ...
    F-002 (搂3.6) 鈥?Round 3 fix: anchors checks 6, 7, 8 on the FIRST
    ...
    throughout, which allowed the multi-turn.completed forgery:
        ...
        thread.started 鈫?turn.started 鈫?real_agent_message 鈫?turn.completed_1
            鈫?forged_agent_message 鈫?turn.completed_2
    ...
    Fix: locate the FIRST turn.completed.  Per 搂3.3 a clean run has exactly ONE
    ...
    Per 搂3.3 canonical no-tool run sequence:
      thread.started 鈫?turn.started 鈫?item.completed(agent_message) 鈫?turn.completed

    Checks:
      1. Exactly one thread.started event (multiple 鈫?forgery surface).
      2. First event is thread.started.
      3. Second event is turn.started (per 搂3.3 canonical sequence).
      ...
      7. No events after the FIRST turn.completed (clean run ends with first TC;
         any trailing event 鈥?including a second TC 鈥?is a forgery indicator).
# REDACTED: sensitive-looking memory line
      9. thread.started.thread_id matches canonical UUID regex (F-019 搂3.3).
     10. All four turn.completed.usage fields present, integers, >= 0 (F-019 搂3.3).

    Raises ParseError if any check fails. Probe rejection cascades into the
    wrapper's AUDIT_FAILED branch per 搂4.6 case (b).
    """
    if not events:
        raise ParseError("stream is empty")

    # Check 1: exactly one thread.started event (搂3.3: thread_id emitted ONCE)
    ...
    # Check 3: second event is turn.started (per 搂3.3 canonical sequence)
    ...
    # Per 搂3.3 a clean run has exactly one turn.completed at the stream end.
    ...
    # Locate the last agent_message (verdict-bearing event per 搂3.3 contract).
    # F-027 (P3, R7) + F-030 (P3, R8/R9): malformed `item` on item.started/
    # item.completed events is rejected outright. Per spec 搂3.3 schema, `item`
    ...
    # here as a trailing event and be rejected.  Per 搂3.3 搂line 284, a clean codex run
    ...
    if trailing:
        ...
        raise ParseError(
            ...
            f"(stream must end at first turn.completed per 搂3.3)"
        )

    # Check 9 (F-019 搂3.3): thread_id must match canonical UUID format.
    ...
    # the key holds those types 鈥?feeding non-string to _THREAD_ID_RE.match
    ...
    # Check 10 (F-019 搂3.3): all four usage fields must be present, integers,
    # and non-negative.  Type validation runs FIRST (F-022 搂3.3) so non-int
...
# REDACTED: sensitive-looking memory line
    # Real codex always consumes input; zero 鈫?forgery indicator.
# REDACTED: sensitive-looking memory line
# REDACTED: sensitive-looking memory line
        raise ParseError(
# REDACTED: sensitive-looking memory line
...
def extract_verdict_text(events: list[dict]) -> str:
    ...
    Per 搂3.3 contract: tool-using runs emit intermediate agent_message events
...
#     "Round N: P1脳n1 / P2脳n2 / P3脳n3 (M total)"
...
# the same or adjacent line 鈥?we anchor on the ROUND value passed via CLI to
# REDACTED: sensitive-looking memory line

# Matches: Round N: P1脳n1 / P2脳n2 / P3脳n3 (M total)
#
# F-011 (搂3.6) Round 2 fixes:
...
#      鈫?AUDIT_FAILED rather than silently accepting an unchecked total.
...
_SUMMARY_A = re.compile(
    r"^\s*Round\s+(\d+)\s*:\s*P1[脳x脳](\d+)\s*/\s*P2[脳x脳](\d+)\s*/\s*P3[脳x脳](\d+)"
    r"\s*\((\d+)\s*total\)\s*$",
    re.IGNORECASE | re.MULTILINE,
)

# Matches: Round N: 0 findings of any severity. Convergence reached.
# F-011: anchored similarly for consistency 鈥?avoids substring hits in
...
#   1. **F-007** P3 搂3.7 chapter_4/synthesis.md:482 鈥?description. Fix: fix text.
...
#   - Dimension: 搂3.1鈥撀?.7 (with or without 搂 prefix) or 4(f)
...
_FINDING_HEADER = re.compile(
    ...
    r"\s+"
    r"(?:搂)?(3\.[1-7]|4\(f\))"              # dimension: 搂3.x or 4(f) (with or without 搂)
    r"\s+"
    r"([^\s:]+)"                             # file path (no spaces, stops before colon)
    r":([0-9]+)"                             # :line
    r"\s*[-鈥斺€揮+\s*"                          # separator: 鈥?or - or 鈥?
...
def _parse_finding_line(line: str) -> Optional[dict]:
    ...
    if len(fix_parts) == 2:
        ...
        # No "Fix:" separator found 鈥?description consumes entire tail,
        # suggested_fix is missing 鈫?malformed entry.
        raise ParseError(
...
# REDACTED: sensitive-looking memory line
    (avoiding "Previous rounds: Round 1: 鈥? recaps).  When None (probe mode),
...
# REDACTED: sensitive-looking memory line
        ...
        if ma:
            ...
            if current_round is None or r == current_round:
                ...
                # "Round 2: P1脳0 / P2脳3 / P3脳1 (5 total)" should reject because
                # 0+3+1=4 鈮?5. group(5) is None when parenthetical is absent.
                ...
                if total_group is not None:
                    ...
                    if total_claimed != total_computed:
                        raise ParseError(
                            ...
                            f"bucket sum {total_computed} (P1脳{n1}+P2脳{n2}+P3脳{n3})"
                        )
...
# REDACTED: sensitive-looking memory line
    ...
    if (p1_got, p2_got, p3_got) != (p1_exp, p2_exp, p3_exp):
        raise ParseError(
            ...
            f"(summary: P1脳{p1_exp}/P2脳{p2_exp}/P3脳{p3_exp}, "
            f"parsed: P1脳{p1_got}/P2脳{p2_got}/P3脳{p3_got})"
        )
...
# ---------------------------------------------------------------------------
# Status classification (搂3.2 cross-field rule)
# ---------------------------------------------------------------------------


def classify_status(finding_counts: dict) -> str:
    """Derive verdict_status from finding_counts per spec 搂3.2 cross-field rules.

    F-012 (搂3.2): original implementation omitted the p3 upper bound, classifying
    ...
    orchestrator escalate to user instead of BLOCKing 鈥?silent severity downgrade.
    """
...
# ---------------------------------------------------------------------------
# YAML serialisation (hand-rolled 鈥?standard library only)
# ---------------------------------------------------------------------------
...
def render_verdict_yaml(
    ...
    human-readable output matching the 搂3.5 example shape.
    """
    lines: list[str] = []
    ...
    # F-013 (搂3.5): ISO 8601 timestamps are parsed as Python datetime objects by
...
def cmd_probe(jsonl_path: str) -> int:
    ...
    Per spec 搂4.4 Step 4 and 搂4.6 case (b): probe does not require
    --round / --target-rounds.  Cross-field count validation is deferred to
# REDACTED: sensitive-looking memory line

    F-002 (搂3.6): validates stream shape (thread.started first, no error event,
...
def cmd_jsonl(
    ...
    F-002 (搂3.6): validates stream shape before classification to ensure a

...
External motivation: Zhao et al. arXiv:2605.07723 (2026-05) 鈥?corpus-scale
...
draft_writer_agent, report_compiler_agent) for:
  ...
  1. Every `<!--ref:slug-->` is followed by `<!--anchor:<kind>:<value>-->`
     where <kind> 鈭?{quote, page, section, paragraph, none}.
...
def _strip_fenced_code_blocks(text: str) -> str:
    ...
    handled 鈥?they are rare for Markdown contributors and unlikely to
...
# REDACTED: sensitive-looking memory line
    ...
    while True:
        ...
        if start == -1:
            break
        # First `-->` after the opening marker 鈥?this is where the HTML
        # parser thinks the comment ends.
        first_close = text.find("-->", start + len(open_marker))
        if first_close == -1:
            # Unterminated comment 鈥?separate violation outside this
            # check's scope. Skip.
            pos = start + len(open_marker)
            continue
        after_close = first_close + 3
        # Find the next `<!--` opener (or end of text) 鈥?that's the
        ...
        # text 鈥?that proves the quote value itself contained at least
...
# REDACTED: sensitive-looking memory line
        if "-->" in trailing:
            ...
            violations.append(
                ...
                f"v3.7.3 F10 鈥?percent-encode `-` as `%2D` per F1 rule"
            )
        pos = after_close
    return violations
...
# REDACTED: sensitive-looking memory line
    #   `<!--ref:slug-->`                                  (0 鈥?pre-finalizer)
    #   `<!--ref:slug ok-->` / `<!--ref:slug LOW-WARN-->`  (1 鈥?post-finalizer v3.7.1)
    #   `<!--ref:slug ok CONTAMINATED-PREPRINT-->`         (2 鈥?v3.7.3 contamination annotation)
    # v3.7.3 F8 closure (codex round-2): regex previously allowed only 0-1
# REDACTED: sensitive-looking memory line
    # entirely 鈥?violations on those refs went undetected.
        ...
        if kind == "quote":
            ...
            if word_count > QUOTE_WORD_CAP:
                violations.append(
                    ...
                    f"v3.7.3 R-L3-1-B 鈥?replace with page or section locator"
                )
...
# REDACTED: sensitive-looking memory line
    ...
    for m in broad_ref_pattern.finditer(text):
        ...
        if not covered:
            line_no = text.count("\n", 0, start) + 1
            violations.append(
                f"{path}:{line_no}: malformed ref marker 鈥?does not "
                f"match the v3.7.3 ref shape "
# REDACTED: sensitive-looking memory line
                f"v3.7.3 F14 鈥?fix the ref or remove it"
            )
    ...
    # HTML comment, not just refs 鈥?so `<!--note--><!--anchor:page:1-->`

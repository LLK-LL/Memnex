...
def test_audit_scaffold_returns_empty_findings_on_empty_draft(tmp_path):
    """Scaffold sanity: empty draft 鈫?0 findings, valid output shape."""
...
def test_p2_2026_handbook_governing_2022_event(tmp_path):
    """Mode 2: 2026 handbook cited for 2022 review cycle 鈫?anachronism."""
    result = _run_audit(
        tmp_path,
...
def test_freeze_regression_byte_identical_across_dates(tmp_path):
    """CC6: report_reference_date is frozen 鈥?output must be byte-identical regardless of wall-clock today.
...
def test_date_to_interval_parses_all_schema_valid_shapes(raw, expected):
    ...
    v3.9.4 only parsed day/year/prose-month 鈥?schema-valid month (YYYY-MM)
...
def test_p2_provenance_low_emits_metadata_missing_skips_anachronism(tmp_path):
    ...
    (not ANACHRONISTIC-CITATION) when confidence is low 鈥?spec 搂3.4 first-party safety check."""
    ...
    anachronism = [f for f in result["findings"] if f["finding_kind"] == "TEMPORAL-ANACHRONISTIC-CITATION"]
    assert anachronism == [], f"v3.9.4.1 fix #1 broken 鈥?anachronism emitted despite confidence:low: {anachronism}"
...
def test_p4_direct_date_causal_inversion_no_refs(tmp_path):
    ...
    (spec 搂3.2 P4: each side may bind to ref marker OR direct date capture)."""
    # No ref markers; both sides are bare dates. "enabled" requires left.date < right.date.
    # Here left=2026 right=2020 鈫?violation.
    result = _run_audit(
        tmp_path,

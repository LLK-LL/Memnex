...
Decision Doc reference:
  docs/design/2026-05-14-ai-disclosure-schema-decision.md 搂4.1 (item 6a)
  + 搂4.3 invariants.

Implementation spec reference:
  docs/design/2026-05-14-ai-disclosure-impl-spec.md 搂2.1 (file map) + 搂4
  (TDD test discipline) + 搂4.4 (Nature 鈫?v3.2 venue de-dup guard).

This lint enforces structural invariants on the anchor data table 鈥?the
...
Checks
------
1. Exactly four anchor sections present, slugs 鈭?{prisma-trAIce, icmje,
   nature, ieee}.
2. Each anchor section carries a Snapshot ref line of the form
   "**Snapshot:** `<slug>:wayback=<id>` 鈥?.
3. Each anchor table has 16 rows numbered 1..16 in canonical order.
4. Each row's source_strength cell 鈭?
...
ELISION_MARKERS = {"(quote elided)", "(snapshot deleted)", "鈥?}
...
def _split_anchor_sections(text: str) -> tuple[dict[str, str], list[str]]:
    ...
    (sections, duplicates) so the caller can flag duplicated headings 鈥?
...
def verify_nature_dedup_with_venue(
    ...
    *,
    ...
    consumer 鈥?a citation list at the file bottom would otherwise satisfy

#!/usr/bin/env python3
"""#111 slr_lineage resolver 鈥?pure function over state_tracker.stages.

Pipeline plumbing helper for the `deep-research systematic-review 鈫?
academic-paper full 鈫?disclosure --policy-anchor=prisma-trAIce` auto-
dispatch path. The orchestrator persists the resolved value on the
Schema 9 Material Passport at the Stage 1 鈫?Stage 2 handoff; the
...
Contract: policy_anchor_disclosure_protocol.md 搂3.1 (G2 invariant)
"""
...
# form. Either path resolves to the same module 鈥?single source of truth for
# SLR_MODES preserved.
try:
...
def resolve_from_stages(stages: Mapping[str, Mapping]) -> bool:
    ...
    specifically, mirroring the 搂4.3 G2 invariant track gate. A
    non-deep-research stage with mode='systematic-review' does NOT
    trigger SLR lineage 鈥?only the documented producer counts.
    """
    return any(

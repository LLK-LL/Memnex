# Code Runner Agent 鈥?Experiment Executor and Monitor
...
### 1. PARSE 鈥?Understand the Experiment
...
### 3. MONITOR 鈥?Watch for Problems
...
|-------|-----------|--------|
| Process alive | PID no longer running + exit code != 0 | 鈫?CRASHED |
| Process alive | PID no longer running + exit code == 0 | 鈫?COMPLETED |
| Output stall | Monitored files unchanged for 3 consecutive checks (90s) | 鈫?STALL_SUSPECTED (ADVISORY) |
| Resource anomaly | RSS memory > 3x initial | 鈫?RESOURCE_ALERT (ADVISORY) |
| Hard timeout | Duration exceeds timeout | 鈫?Kill process, report |
...
All detections except hard timeout are **ADVISORY** 鈥?notify user with options (continue / kill / adjust), never auto-act.

### 4. DECIDE 鈥?Handle Anomalies
...
1. **Never modify the user's command** 鈥?execute exactly as given
2. **Never auto-retry** 鈥?if it crashes, report and let user decide
3. **Never auto-kill** 鈥?only hard timeout kills. Always notify first.
4. **Never read files outside declared scope** 鈥?only monitor what user specified
5. **Confirm before execution** 鈥?always show the command and ask for go-ahead
...
Routed from SKILL.md based on user input (code execution keywords 鈫?this agent). Also called by validate mode for reproducibility re-runs.

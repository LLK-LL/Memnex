...
- Flag if p is borderline (.04-.05): "marginal significance 鈥?interpret with caution"
...
- Warn if significant but small effect: "statistically significant but practically small 鈥?consider whether this difference matters in context"

**Confidence Interval:**
- Does CI include 0 (or 1 for OR)? 鈫?result is not significant regardless of p-value
- Width: narrow CI = precise estimate; wide CI = uncertain estimate
- Asymmetric CI around point estimate 鈫?possible skewness
...
| Chi-squared | Expected frequencies >= 5 | Any cell < 5 鈫?for 2x2 tables use Fisher's exact; for larger tables use an exact/Monte Carlo approach or collapse categories with justification |
...
- **Reporting note**: describe what numbers say (overall vs per-group), do NOT recommend which to report 鈥?that is an editorial decision for the reviewer

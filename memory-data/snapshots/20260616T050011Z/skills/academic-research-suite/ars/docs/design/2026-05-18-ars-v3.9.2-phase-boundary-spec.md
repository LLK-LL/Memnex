# ARS Issue #133 鈥?Phase Scope Inflation: Design Doc (v4 鈥?HIGH-finding amendment)

**Date:** 2026-05-18
**Issue:** #133 鈥?Phase 2+ agents can silently inflate scope and skip independent Phase 3/5 checkpoints
...
**Related:** #134 (v3.10 active conductor architecture 鈥?long-term fix; hook + multi-phase schema + provenance now live there)

**Doc revision history:**
- v1: initial 鈥?single-track self-review. 9 issues identified.
...
- v4 (this doc): incorporated v3 review (codex broken corner + inline opus). HIGH-1 (3-bucket model insufficient 鈫?4-bucket A/B/C/D with out-of-scope inflation column) + HIGH-2 (multi-phase lite reminder is placebo 鈫?cut entirely, honest framing) addressed. MED/LOW carry to impl phase. Effort revised 18-22h 鈫?18-23h with Phase 0.2 bumped to 2-3h.
...
### L1 鈥?Routing failure (root cause)
...
### L2 鈥?Subagent scope inflation (downstream symptom)
...
### Blocker 2: Codex P1 鈥?forbidding `phase{N-1}_*/` reads breaks legitimate handoffs
...
### Blocker 3: Opus H3 鈥?L1 routes to orchestrator that can't handle cross-phase materials

Current `pipeline_orchestrator_agent.md` keyword-matches user phrasing (Stage 1 vs Stage 2 etc.). It does NOT have logic for "user dropped cross-phase artifacts, no passport, no explicit invocation 鈥?figure out where in the pipeline to resume from." That capability is the heart of #134 (conductor with structured intake), not a hot-fix patch.

v2's L1 strategy (route ambiguous input to orchestrator) was thus broken 鈥?destination was non-functional. v3 changes L1: instead of routing to orchestrator, ARS simply **clarifies** with the user.

### Blocker 4: Opus H2 + Codex P2 鈥?hook lookup + verifier provenance are missing infrastructure

- Hook needs `agent_type 鈫?ars_phase` lookup mechanism that doesn't exist (where it lives, regeneration, stale-safe).
...
Hot-fix scope:
- **L1**: routing clarification gate (not orchestrator detour 鈥?destination is broken)
- **Fix 1**: prompt hard fence per agent (writes only, reads allowed) 鈥?for **single-phase agents** only in v3.9.2; multi-phase agents get prose-only "be aware of your phase" reminder pending v3.10 schema
- **Fix 3**: SKILL.md phase-by-phase invocation contract
- **Fix 4 lite**: advisory-only verifier (heuristic, no provenance guarantee)

Cut from v3.9.2:
- ~~Fix 2 PreToolUse hook~~ 鈫?v3.10 (needs multi-phase schema + lookup mechanism + Phase 0.1's `agent_type` finding is preserved for v3.10)
- ~~Multi-phase `ars_phase` schema~~ 鈫?v3.10
- ~~Orchestrator cross-phase intake capability~~ 鈫?v3.10 (this is conductor's core feature anyway)
- ~~Deterministic verifier with provenance~~ 鈫?v3.10
...
- **0.1 鉁?DONE** Phase 0.1 from v2 preserved as note: CC PreToolUse payload contains `agent_type` field equaling agent frontmatter `name`. **Not used in v3.9.2** (hook deferred to v3.10). Recorded for v3.10 conductor design.
- **0.2 (NEW)** Audit each of the 38 unique agent .md files to classify into **4 buckets** (revised per v3 review opus HIGH-1):
  - **Bucket A 鈥?Single-phase** (e.g., `bibliography_agent` = P2 only, `research_question_agent` = P1 only) 鈫?gets v3.9.2 hard boundary prompt
  - **Bucket B 鈥?Multi-phase** (e.g., `devils_advocate_agent` = P1+P3+P5, `report_compiler_agent` = P4+P6) 鈫?**NO boundary added in v3.9.2** (per HIGH-2 honest framing 鈥?placebo prose would create illusion of enforcement). Recurrence acknowledged; v3.10 envelope is the fix.
  - **Bucket C 鈥?Phase-orthogonal** (e.g., `socratic_mentor_agent` operates on Socratic Layers 1-5 not numbered pipeline phases; `compliance_agent` runs at Stage 2.5 / 4.5 integrity gates cross-skill) 鈫?no boundary added. Note in classification doc whether the agent has any latent inflation risk class (e.g., socratic_mentor inflating into direct conclusions); v3.9.2 explicitly does not cover those classes.
  - **Bucket D 鈥?Cross-phase / meta** (e.g., `pipeline_orchestrator_agent`, `field_analyst_agent`, `intake_agent`) 鈫?no boundary added (legitimate cross-phase by design role)
  
  **Audit-flag (NOT a bucket):** `draft_writer_agent`, `literature_strategist_agent` already carry partial boundary language. Assigned to whichever real bucket they belong (likely both Bucket A 鈥?Phase 4 and Phase 2-equivalent respectively); audit existing language for v3.9.2 consistency. Do NOT classify these as a separate bucket.
...
# REDACTED: sensitive-looking memory line
  ...
  **Estimate revision:** 2-3h (not 1-1.5h per opus MED-5 鈥?38 agents 脳 careful cross-check against SKILL.md tables + frontmatter + actual usage is non-trivial).
...
#### L1.1 鈥?Routing discipline rule in `.claude/CLAUDE.md` (single source of truth)
...
1. **Explicit clear intent** (user names a specific skill via `/ars-*` command or unambiguous trigger keyword like "lit-review this", "review my paper"):
   鈫?Route directly; no clarification.

2. **Cross-phase materials detected** (user provides artifacts spanning 鈮?2 pipeline phases 鈥?e.g., pre-written abstract + pre-collected literature):
   鈫?**Clarify**. Do NOT auto-route to a single-phase agent. List candidate workflows as a-d options in markdown body (NOT via AskUserQuestion tool, per `feedback_no_askuserquestion_use_brainstorm.md`).
   鈫?Reason: clarification is the safest action when materials don't unambiguously identify intent. (v3.10 conductor will handle this via structured intake; for now, ask.)

3. **No materials, ambiguous request:**
   鈫?Clarify per `shared/references/intent_clarification_protocol.md`.

**Escape hatch:** Prefix `[direct-mode]` at start of first message 鈫?all clarification bypassed; ARS dispatches whatever single agent matches the literal trigger.
...
#### L1.2 鈥?Protocol doc: `shared/references/intent_clarification_protocol.md` (NEW file)
...
# REDACTED: sensitive-looking memory line
- **3-5 worked examples** of inputs 鈫?correct routing outcome
- **Forward note** referencing #134 / v3.10 active conductor

#### L1.3 鈥?Single-line backpointer in 4 SKILL.md files
...
#### L1.4 鈥?Behavioral smoke tests (NOT deterministic unit tests, honestly framed)

8 fixture cases covering:
- Cross-phase materials (abstract + lit) 鈫?asserts clarification triggers (NOT silent dispatch)
- Single-phase material (literature only) 鈫?asserts ARS proceeds with Phase 2 work (no clarification needed for single-phase)
- No materials + ambiguous request 鈫?asserts clarification with a-d options
- Explicit `/ars-lit-review` invocation 鈫?asserts skip clarification
- `[direct-mode] run bibliography_agent` 鈫?asserts skip clarification
- `[direct-mode]` mid-message (not byte-0) 鈫?asserts clarification still fires (escape hatch not honored)
- `[Direct-Mode]` case variant 鈫?asserts escape hatch honored (case-insensitive)
- Full draft + abstract + lit 鈫?assertion: clarification fires (cross-phase, no clear skill named)

**Acceptance criterion:** 100% pass on Opus 4.7; 鈮?75% pass on Sonnet 4.6 + GPT-5.5 (degradation flagged but non-blocking ship). Framed as behavioral smoke test + cross-model spot-check, not green/red unit test.

### Phase 1: Fix 1 鈥?Prompt hard fence per agent (~4-5h, NOT 5-7h, scope narrowed)

**Scope (per Phase 0.2 4-bucket classification, revised per v3 review):**
- **Bucket A 鈥?Single-phase agents** (~25 of 38, finalized in Phase 0.2): full hard boundary block as below
- **Bucket B 鈥?Multi-phase agents** (~8 of 38): **NO boundary added** (placebo prose creates false-enforcement illusion; HIGH-2 honest framing)
- **Bucket C 鈥?Phase-orthogonal agents** (~3 of 38): no boundary added; classification doc notes any latent inflation risk
- **Bucket D 鈥?Cross-phase / meta agents** (~2 of 38): no boundary added (legitimate cross-phase role)
- **Audit-flag (within Bucket A):** `draft_writer_agent`, `literature_strategist_agent` carry existing partial language 鈥?harmonize with v3.9.2 hard fence shape
...
You MUST NOT:
- WRITE files in `phase{M}_*/` directories where M 鈮?N (no inflate)
...
- 搂5 Failure modes 鈥?explicit "multi-phase inflation remains possible; recurrence expected for ~N agents until v3.10"
- README migration note 鈥?if user sees DA / RC / similar multi-phase agent producing out-of-scope content, the remediation is "report to issue #134, do not retry, switch to Mode A orchestrator-driven invocation"
...
### Phase 3: Fix 3 鈥?SKILL.md phase-by-phase invocation contract (~1h)
...
**Mode A 鈥?orchestrator-driven (default):** `pipeline_orchestrator_agent` runs all phases end-to-end with state tracking.
...
Routing into Mode B requires explicit user signal 鈥?`/ars-<mode>` slash command or `[direct-mode]` prefix. Ambiguous cross-phase input triggers clarification (per `.claude/CLAUDE.md` Routing Discipline).

**Enforcement:** prompt-level + advisory verifier in v3.9.2; deterministic gate in v3.10 active conductor (#134).
```

### Phase 4: Fix 4 lite 鈥?Advisory post-hoc verifier (~2h, simpler than v2 design)

File: `scripts/check_pipeline_integrity.py` (joins existing 28 `check_*.py` scripts).

**Logic (v3.9.2 lite 鈥?no provenance required):**
...
**Acceptance criterion:** verifier flags the #133 reported failure case (bibliography_agent producing phase3_*/phase4_*/phase5_*/phase6_*/ in single dispatch 鈥?they'll all have timestamps within ~minutes of each other).
...
- Advisory-only 鈥?does not block, only surfaces signals for user review

### Phase 5: Lint compat audit (~2h) 鈥?MOVED before Phase 1 per opus H5
...
- Run each on a single test agent .md with proposed v3.9.2 prompt fence body (no new frontmatter field 鈥?just new section in body) 鈥?confirm no break
...
- Per `feedback_ars_public_repo_boundary.md` 鈥?grep for hei-platform / Springer / HEEACT leakage in new files

### Phase 8: Documentation + migration (~1.5h)

- Spec doc `docs/design/2026-05-18-ars-v3.9.2-phase-boundary-spec.md` (final)
- CHANGELOG entry under `[Unreleased]`
- README migration note: existing in-flight projects 鈫?no break expected (we only added prompt sections and an advisory verifier; no behavior change for explicit-intent flows)
- `feedback_*` memory updates as warranted

---

## 4. Pipeline-wide silent drift inventory 鈥?MOVED to #134 (v3.10 conductor)
...
- **L1 over-asks** (codex v2 finding): less risk in v3 鈥?clarification only fires on cross-phase materials OR no-material ambiguous requests. Single-phase materials route directly. Risk reduced vs v2 (which routed everything to orchestrator).
...
- **`[direct-mode]` prefix edge cases** (per opus M1): specified in protocol doc 鈥?byte-0, case-insensitive, stripped before pass-through.
...
1. **Negative integration smoke test:** input "abstract + literature, see what you can do" 鈫?ARS clarifies with a-d options, does NOT silently dispatch. Verbatim transcript in `tests/fixtures/issue_133/`.
2. **L1 boundary case sweep:** 8 fixtures pass on Opus 4.7; degradation flagged on Sonnet + GPT-5.5.
3. **Verifier flags #133 case:** run on test fixture matching reported failure 鈫?鈮?4 INTEGRITY-ADVISORY lines flagged.
...
| 1 | Prompt fence 脳 ~25 Bucket A agents only (Bucket B/C/D no agent .md changes) | 3-4 |
| 3 | SKILL.md phase contract 脳 4 | 1 |
...
(v2 estimate was 22-32h with hook + provenance; v3 cut both. v4 adjustments 鈥?Phase 0.2 bump for 4-bucket audit + Phase 1 shrink because only Bucket A gets fence 鈥?net to 18-23h.)
...
| Multi-phase `ars_phase_writes` + `ars_phase_reads` schema | Cannot be retrofitted scalar 鈫?list cleanly; v3.10 will design correctly with envelope |
# REDACTED: sensitive-looking memory line
| Orchestrator cross-phase intake capability | Heart of v3.10 conductor architecture 鈥?passive routing 鈫?active intent classification |
| `agent_type 鈫?ars_phase` lookup mechanism | Will be obsoleted by envelope-carried phase info |
...
|---|---|---|
| Hook (Fix 2) | **DEFERRED to v3.10** | v2 dual-track review 鈥?Codex P1 (regress read break) + Opus H1 (multi-phase schema) + Opus H2 (lookup hand-wave) |
| Multi-phase `ars_phase` schema | **DEFERRED to v3.10** | Codex P1 (multi-phase agents exist) + Opus H1 |
| L1 routing default | **Clarify, not orchestrator detour** | Opus H3 鈥?orchestrator doesn't handle this case yet |
| Deterministic verifier with provenance | **DEFERRED to v3.10**; v3.9.2 ships advisory-only | Codex P2 鈥?provenance infrastructure missing |
| Prompt fence READ scope | **Allow upstream reads, only block writes** | Codex P1 鈥?v2 over-reached |
...
| **v4: Multi-phase agents get NO fence in v3.9.2** | Honest framing 鈥?placebo prose would create false-enforcement illusion. ~8 Bucket B agents explicitly unmitigated; v3.10 envelope is the fix. | v3 review opus HIGH-2 |
| **v4: Phase 0.2 estimate revised 1-1.5h 鈫?2-3h** | 38 agents 脳 4-bucket careful cross-check is non-trivial | v3 review opus MED-5 |

# ARS v3.6.5.1 鈥?SETUP.md Doc Correctness Fix 鈥?Implementation Brief
...
## 搂1 鈥?Non-negotiable invariants
...
鈹溾攢鈹€ deep-research/SKILL.md
鈹溾攢鈹€ academic-paper/SKILL.md
鈹溾攢鈹€ academic-paper-reviewer/SKILL.md
鈹斺攢鈹€ academic-pipeline/SKILL.md
```
...
Any install instruction that produces `<install-root>/academic-research-skills/<skill-name>/SKILL.md` (extra nesting level) is broken 鈥?Cowork / Claude Code / claude.ai will not discover the skill.

### 1.3 Method 4 (claude.ai) is two distinct sub-methods

**Sub-method 4a**: Real Skill upload via Settings 鈫?Capabilities 鈫?Skills, one zip per skill. This is the path that gives skill auto-loading and trigger phrase routing. Currently blocked by the 200-char description cap (fixed in v3.6.5.2, NOT v3.6.5.1).

**Sub-method 4b**: Project + GitHub integration loads files into Project knowledge (static retrieval context). This is NOT a Skill install 鈥?Claude can read the files but does not execute them as agentic skills.
...
# REDACTED: sensitive-looking memory line
   ...
   - Custom Skills packaging (incl. 鈮?00-char description cap): https://claude.com/docs/skills/how-to
...
2. **Issue #44 reply (already vetted UI wording, inlined below for self-contained reference):**
   ...
   > Cowork install 鈥?symlink approach the user already validated:
   > ```bash
   ...
   > "the GitHub integration loads files into Project knowledge, not Skills. Real Skill upload needs each skill zipped separately via Settings 鈫?Capabilities 鈫?Skills, and descriptions in this repo currently exceed the 200-char limit so they'd reject."

3. **Existing `docs/SETUP.md` text** 鈥?only for parts the audit (搂2 below) did not flag as drifted. Do not lift wording from sections F1-F7 covered by this brief.

Never invent a UI term. Never paraphrase a UI label as a "more readable" alternative 鈥?the literal label is what the user clicks. When Anthropic doc and issue #44 reply disagree on a UI label, Anthropic doc wins (it is the upstream truth as of the doc's last-updated date).

### 1.6 Out of bounds (do not touch)

The drafter must NOT modify these as part of v3.6.5.1:

- Any `SKILL.md` (description, frontmatter, body) 鈥?those are v3.6.5.2 scope
...
The drafter MAY modify (within v3.6.5.1 scope) 鈥?this list is the **complete allowed-edit set**:
...
**Sweep vs edit boundary** (resolves apparent conflict with 搂4): the 搂4 sweep grep MAY scan a broader path (e.g. all of `docs/`) to find install-method mentions, but the drafter MAY edit only the files in the allowed-edit set above. Hits outside the allowed-edit set are **report-only**: list them in the validation report for human review, do not auto-edit. Touching files outside the allowed-edit set requires explicit human approval (out of v3.6.5.1 scope).

The drafter does NOT decide:

- Whether v3.6.5.1 ships as a tag, a release, or just a merge 鈥?that is the user's call
- Whether the issue #44 follow-up comment goes out at v3.6.5.1 or v3.6.5.2 鈥?already decided (deferred to v3.6.5.2 per ROADMAP)
- Whether SKILL.md description trim is in this patch 鈥?already decided NO (v3.6.5.2)

---

## 搂2 鈥?Findings to fix in v3.6.5.1

Source: local Codex audit on `docs/SETUP.md` 2026-04-27.

### F1 (P0) 鈥?`docs/SETUP.md:213-229` Method 3 wrong path

**Problem**: Current Option A says clone repo to `.claude/skills/academic-research-skills/`. This produces:

```
.claude/skills/academic-research-skills/deep-research/SKILL.md   鈫?buried, not discovered
```

instead of the required:

```
.claude/skills/deep-research/SKILL.md   鈫?Cowork finds this
```
...
- **Option A 鈥?symlink (fastest, single-machine)**: `git clone` repo to a stable path (e.g. `~/academic-research-skills`), then `ln -s <repo>/<skill-name> ~/.claude/skills/<skill-name>` for each of the four skills. Must include a caveat: if the user syncs `~/.claude/skills` across machines via cloud folders, prefer `cp -R` because absolute-path symlinks break on a fresh checkout (this is a real ARS lesson from `feedback_relative_symlinks_in_repos.md`).
- **Option B 鈥?copy (cross-machine safe, no auto-update)**: `git clone` repo, then `cp -R <repo>/<skill-name> ~/.claude/skills/<skill-name>` for each of the four skills. Note that updates require re-running cp.
...
- Paid plan: Pro, Max, Team, or Enterprise (cite the Cowork article above for current plan availability 鈥?do not paraphrase the plan list, link to it because Anthropic adjusts plan tiers)

### F4 (P1) 鈥?`docs/SETUP.md:241` Method 4 prereqs missing
...
### F5 (P2) 鈥?`docs/SETUP.md:217-218` Cowork UI terminology drift
...
- "mode selector (Chat / Cowork)" 鈥?the toggle between Claude's two modes
...
### F6 (P2) 鈥?`docs/SETUP.md:231` Trigger phrase oversimplified

**Problem**: Current text says skills are invoked "via trigger phrases". True but incomplete 鈥?Cowork now also surfaces installed plugin Skills via the `/` command palette and the `+` (add capability) visual UI. The audit also notes that "trigger phrase" is itself a slight overstatement: Claude uses the skill description to decide relevance, not literal trigger-phrase matching.
...
### F7 (P2) 鈥?`docs/SETUP.md:243-255` Method 4 directory table omits `scripts/` + capacity outdated

**Problem**: Current Method 4 directory table omits `scripts/`, but adapter and validator workflows live there and break without it. The audit also notes the section's capacity rationale references a 200-file cap that conflicts with current Anthropic Project limits (the audit team did not actually find the 200-file claim on line 235-264 鈥?it may already have been removed in an earlier doc rev 鈥?but the surrounding capacity guidance is shaky and needs a refresh against current limits).
...
**Drafter responsibility**: prepare banner removal as part of the SETUP.md rewrite diff. The drafter does not commit (per 搂1.4 鈥?public repo, drafter never commits directly). The human maintainer reviews the diff and decides whether banner removal goes in the same commit as the F1-F7 rewrite (atomic) or as a separate commit immediately after (cleaner history). Recommended commit message if separate: "remove banners superseded by v3.6.5.1 SETUP rewrite".

---

## 搂4 鈥?Sweep checklist
...
Any hit inside the 搂1.6 allowed-edit set that contradicts the v3.6.5.1 SETUP.md must be aligned. Hits outside the allowed-edit set are **report-only** per 搂1.6: list them in the validation report for human review, do not auto-edit. Files known to potentially need touch (verify; update only if drifted):

- `README.md` 鈥?Quick Start / Installation section
- `README.zh-TW.md` 鈥?same as English
- `QUICKSTART.md` 鈥?if it exists and mentions Cowork or claude.ai
- `.claude/CLAUDE.md` 鈥?Skills Overview table is unlikely to need changes (no install instructions there), but check
- `docs/SETUP.zh-TW.md` 鈥?mirror EN changes if zh-TW exists (may or may not 鈥?check first)

Files NOT to touch in v3.6.5.1: anything under `*/agents/`, `shared/`, `scripts/`, `tests/`, individual `SKILL.md` files.

---

## 搂5 鈥?Validation gate
...
- **Backup + restore**: `mv ~/.claude/skills ~/.claude/skills.backup-pre-v3.6.5.1-validation`, run validation, then `rm -rf ~/.claude/skills && mv ~/.claude/skills.backup-pre-v3.6.5.1-validation ~/.claude/skills` after. Riskier 鈥?if the validation step fails partway, the backup might not get restored. Disposable HOME is preferred.
...
For each install method below, run the exact commands from the v3.6.5.1 SETUP.md draft (no edits, no shortcuts) inside the protected environment from 搂5.0:
...
# REDACTED: sensitive-looking memory line
# REDACTED: sensitive-looking memory line
6. **Method 4a live**: zip one skill folder per the SETUP.md instructions, attempt upload via Settings 鈫?Capabilities 鈫?Skills. Expected outcome **before v3.6.5.2 ships**: rejection with a description-too-long error, which validates that the documented upload path is the right one and that v3.6.5.2 description trim is the correct unblocker.
...
- Which 搂5.0 environment-protection approach was used
...
- **Zero P1 and zero P2 findings**: required. Every P1 and P2 finding must be fixed before ship. No "reject with rationale" path for P1/P2 鈥?they are blockers by definition. If a Codex P1 or P2 is technically wrong (e.g. Codex misread the brief), the response is to clarify the brief or argue the finding back, not to declare it rejected and ship.

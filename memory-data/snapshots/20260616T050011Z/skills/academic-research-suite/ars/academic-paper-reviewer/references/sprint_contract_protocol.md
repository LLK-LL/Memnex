...
>
> **v3.6.6 cross-reference**: this reviewer protocol is byte-equivalent across v3.6.2 鈫?v3.6.6 (zero-touch promise per 搂3.6 of `docs/design/2026-04-27-ars-v3.6.6-generator-evaluator-contract-design.md`). The v3.6.6 release adds a parallel generator-evaluator protocol inside `academic-paper` for the in-pair writer / evaluator pair (see `academic-paper/SKILL.md` 搂 "v3.6.6 Generator-Evaluator Contract Protocol" and design doc 搂5).
...
2. **Phase 1 call (paper-content-blind).**
   - System prompt: the `### Phase 1 鈥?Paper-content-blind pre-commitment` sub-section of the reviewer agent's `## v3.6.2 Sprint Contract Protocol` block.
...
3. **Phase 1 output lint.** See 搂4 below.
4. **Phase 2 call (paper-visible).**
   - System prompt: the `### Phase 2 鈥?Paper-visible review` sub-section of the same `## v3.6.2 Sprint Contract Protocol` block.
...
5. **Phase 2 output lint.** See 搂5 below.
6. **Panel cardinality invariant.** After all reviewers complete, verify `len(usable_phase2_outputs) == panel_size`. If any reviewer was dropped, emit `[PANEL-SHRUNK]` and abort the round (see 搂6).
7. Feed usable Phase 2 outputs into synthesizer (see 搂7).
...
- Paraphrase paragraph count 鈮?`measurement_procedure.paraphrase_minimum_dimensions` (for `"all"`, one paragraph per dimension; for integer `k`, at least `k` paragraphs each matching a distinct dimension).
...
**Lint is structural, not semantic.** A reviewer can in principle pass this lint by emitting generic boilerplate triggers 鈥?semantic judgement (whether triggers are concrete and discriminating) is deferred to a post-v3.6.2 judge-agent layer.
...
# REDACTED: sensitive-looking memory line
- `## Editorial Decision` is one of the `action` values derivable from `## Failure Condition Checks` via the synthesizer precedence rule (搂8 step 3). Inconsistency marks the reviewer unusable.
...
- **Panel cardinality invariant (搂2 step 6).** After all reviewers complete, if `len(usable_phase2_outputs) < panel_size`, abort the editorial round with `[PANEL-SHRUNK]`. Do not silently recompute `cross_reviewer_quantifier` thresholds against a smaller panel 鈥?the contract's published aggregation semantics bind on a specific `panel_size`.
...
| `reviewer_re_review`          | 鈥?         | not shipped in v3.6.2; continues pre-v3.6.2 behaviour |
| `reviewer_calibration`        | 鈥?         | not shipped in v3.6.2 |
| `reviewer_guided`             | 鈥?         | not shipped in v3.6.2 |
...
**Step 1 鈥?Build scoring matrix.** For each `acceptance_dimensions[i]`, gather N reviewers' `## Dimension Scores` for that dimension into a length-N array of `$defs.score` values. Dimensions resolved by `id`.

**Step 2 鈥?Evaluate each `failure_conditions[]`.** For each condition:

1. Parse `expression` against the recognised patterns (see 搂9 vocabulary). Unrecognised 鈫?emit `[EXPRESSION-UNRECOGNISED]`, abort synthesizer.
2. Apply `cross_reviewer_quantifier` with panel-relative thresholds:
   - `any`: fires if predicate holds for 鈮?1 of N reviewers.
   - `majority`: for N 鈮?3, fires if 鈮?`鈱圢/2鈱?+ 1`; for N == 2, fires if all 2; for N == 1, vacuous (SC-11 warns).
   - `all`: fires if predicate holds for all N reviewers.
3. Record `{condition_id, fired}`.

**Step 3 鈥?Precedence and decision.** Among fired conditions, pick the one with highest `severity`. Ties break by ordinal position (earliest in the `failure_conditions[]` array wins). Emit its `action` as `editorial_decision`.
...
- Synthesise substitute scores for reviewers marked unusable 鈥?the round is either complete with `panel_size` usable outputs or `[PANEL-SHRUNK]` aborted.
...
New expression forms require a PR updating both this 搂9 and the synthesizer prompt's recognised-pattern list.
...
| `[PROTOCOL-VIOLATION: multi_dissent=true]` | Phase 2 has 鈮?2 dissent entries, retry exhausted | mark reviewer unusable |
| `[PANEL-SHRUNK: usable=<k>, panel_size=<N>]` | 搂6 invariant failed | abort editorial round |

# Consumer Protocol 鈥?`literature_corpus[]` Reading
...
Step 2: Search-fills-gap 鈥?case A / B / B' / C
Step 3: Merge included + external_included for downstream
Step 4: Emit Search Strategy Report with PRE-SCREENED block
```

The full flow specification lives in spec 搂3.1. Each consumer agent's `agent.md` must include the full Step 0鈥? description and all four Step 2 case markers (case A / case B / case B' / case C). The lint enforces presence (L6).

## PRE-SCREENED block template

Every consumer emits the block in this exact shape (alphabetical ordering for citation_keys; truncate at 50 entries with appendix file). Spec 搂3.2 has the complete template plus truncation rules.
...
(1 entry silently disappeared from the pipeline: 3 + 1 鈮?5 鈥?Iron Rule 2 violation.)
```
...
### Iron Rule 3 鈥?No corpus mutation
...
### Iron Rule 4 鈥?Graceful fallback on parse failure
...
**Zero-hit note (F3).** When `pre_screened_included[]` is empty after Step 1 鈥?corpus is non-empty but no entry survived screening 鈥?the consumer emits a zero-hit note inside the PRE-SCREENED block listing the three plausible causes:
...
The note appears regardless of which Step 2 case fires next. Step 2 dispatch follows F3 in spec 搂4.1.

**Provenance reporting (F4a鈥揊4f).** `obtained_via` and `obtained_at` are optional in v3.6.4 schema. The PRE-SCREENED block's `Adapter:` and `Snapshot date:` lines reflect actual coverage; consumers never invent enum values or guess timestamps.
...
F4a/b/c are mutually exclusive by trigger. F4d applies only when zero entries declare `obtained_at`; F4e and F4f compose. See spec 搂4.2 for the full precedence reasoning.

...
## APA 7.0 Template (`apa7` Class) 鈥?Preferred for APA Output
...
% === Justify text (CRITICAL 鈥?apa7 man mode defaults to raggedright) ===
\usepackage{ragged2e}
\usepackage{etoolbox}
...
**NEVER** use bare `p{0.25\linewidth}` in longtable 鈥?this ignores inter-column padding and causes overflow.

**Correct formula**: `p{(\linewidth - N\tabcolsep) * \real{proportion}}`

Where N = `(number_of_columns - 1) 脳 2`
...
| `\autocite{Smith2024}` | (Smith, 2024) 鈥?adapts to style |
...
| Bibliography not appearing | Run: latex 鈫?bibtex 鈫?latex 鈫?latex |

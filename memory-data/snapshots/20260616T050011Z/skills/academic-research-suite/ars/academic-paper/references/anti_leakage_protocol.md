...
- The user is in `plan` or `socratic` mode (exploratory 鈥?LLM knowledge is expected)
...
5. The Methods section must describe ONLY what is documented in the user's materials 鈥?do not infer or interpolate experimental procedures
...
and discipline norms. The restriction applies only to FACTUAL CONTENT 鈥?claims, citations,
# REDACTED: sensitive-looking memory line
```
...
- Lu, C. et al. (2026). Towards end-to-end automation of AI research. *Nature* 651, 914-919. 鈥?Mode 6 (Methodology fabrication).

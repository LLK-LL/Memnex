...
# Intake Agent 鈥?Paper Configuration Interview
...
1. **Complete but efficient** 鈥?collect all necessary parameters without over-burdening the user
2. **Smart defaults** 鈥?suggest sensible defaults based on discipline and paper type
3. **Validate early** 鈥?catch incompatible configurations (e.g., 2000-word IMRaD is too short)
4. **Existing materials inventory** 鈥?understand what the user already has to avoid redundant work
5. **Bilingual awareness** 鈥?detect user language and set defaults accordingly
6. **Handoff awareness** 鈥?detect materials from deep-research and auto-import
...
2. Skip redundant questions:
   - Skip Step 1 (Topic & RQ) 鈥?already available
   - Skip parts of Step 8 (Existing Materials) 鈥?already available
...
- **Markdown** (default) 鈥?universal, easy to convert
- **LaTeX** (.tex + .bib) 鈥?for technical papers and journal submissions
- **DOCX** 鈥?for Word-based workflows
- **PDF** 鈥?final distribution format
- **Combined** 鈥?all of the above
...
4. Inform user: "I've analyzed your writing style. Key traits: [summary]. I'll use this as a soft guide 鈥?discipline conventions take priority."

...
| 3. Key Abstraction | Named concept 鈥?the paper's intellectual core | A coined term that captures the core insight. Appears ONLY in final versions 鈥?discovered through writing. |
| 4. Design Intuition | One-paragraph mental model | The "elevator pitch" 鈥?how the system works at the highest level. No algorithmic details. |
...
**Evolution**: Student drafts use Move 2 only (head-to-head). The advisor adds Moves 3鈥?. The Takeaway (Move 4) is the signature addition 鈥?absent from all early drafts, present in all final versions.
...
| Move | Function | What it looks like |
|------|----------|-------------------|
| 1. Category Clustering | Group prior work by approach type | 2鈥? categories max. Named by approach type, not by individual paper. |

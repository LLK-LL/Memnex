...
This guide helps synthesize **conceptual figures** 鈥?architecture diagrams, pipeline illustrations, comparison schematics, and other non-data figures. It does NOT handle data figures (CDFs, bar charts, scatter plots, heatmaps) 鈥?those go through the [data-visualization-skill](https://github.com/SNL-UCSB/data-visualization-skill) via `/viz`.
...
- `/paper-writing figure spec` 鈥?Classify non-data figures, walk through archetype-specific questions, produce a `figure_spec.md`
- `/paper-writing figure generate` 鈥?Route to AI prompt or TikZ, produce the figure
- `/paper-writing figure critique` 鈥?Review against claims, venue formatting, design principles
...
If no `project_context.md` exists, stop. The student must run the brainstorming workflow first (`/paper-writing` 鈫?brainstorming). Figures cannot be designed in a vacuum 鈥?they serve claims, and claims live in the project context.
...
|-----------|--------------|-----------------|-----------------|
| **Architecture Overview** | Full system pipeline, end-to-end flow with named stages, phase groupings | 搂3 Design (opener), full-width | AI image generation |
| **Pipeline / Process Flow** | Sequential transformation of data through stages, with intermediate representations | 搂3 Design (subsection) | TikZ |
| **Component Detail** | Internal structure of one pipeline stage, sub-components and their interactions | 搂3 Design (subsection) | TikZ |
| **Concept Illustration** | Abstract concept made concrete 鈥?before/after, spatial analogy, visual metaphor | 搂2 Background / Motivation | AI image generation |
| **Comparison Schematic** | Why approach A fails and approach B works, side-by-side structural contrast | 搂1 or 搂2 Motivation, 搂3 Design justification | AI image generation |
| **Taxonomy / Classification** | Hierarchical or matrix organization of categories, positioning the paper | 搂5 Related Work, 搂2 Background | TikZ |
| **Deployment / System Diagram** | Physical or logical infrastructure topology, how the system runs in practice | 搂4 Evaluation setup, 搂2 Background | AI image generation |
...
## Mode 1: Spec 鈥?"What does this figure need to show?"

### Step 1: Identify Non-Data Figures

Read `project_context.md` 鈫?"Key Figures Needed" section. For each figure listed:

1. **Classify as data or non-data.** Ask: "Does this figure require experimental data to render?"
   - Yes 鈫?route to `/viz`. Tell the student.
   - No 鈫?continue here.
...
For each non-data figure, ask the questions below. Skip questions the student has already answered in `project_context.md`. Push back on vague answers 鈥?every answer should be specific enough to appear in a generation prompt.
...
1. What is being transformed? (e.g., "raw time series 鈫?event representation")
...
3. What type of illustration best corrects that misconception?
   - **Before/after**: "Here's the problem state 鈫?here's the improved state"
...
4. What are the concrete elements in the illustration? (No abstract blobs 鈥?every shape must represent something named.)
5. Where is the "aha moment" 鈥?the point where the reader sees why the old way fails?
...
5. What structural element is different between the two? (Not just "ours is better" 鈥?what specific mechanism differs?)
...
5. What specific prior works go in each category? (Name them 鈥?"Work A [citation]".)
...
2. What are the infrastructure layers? (Client 鈫?Edge 鈫?Core 鈫?Cloud)
...
8. **Caption**: draft an interpretive caption following voice_profile.md rules 鈥?claim-first, states the takeaway.
...
## Mode 2: Generate 鈥?"Produce the figure"
...
## Mode 3: Critique 鈥?"Does this figure serve the paper?"
...
**Overall verdict:**
- **PASS** 鈥?all checks pass. The figure is ready for inclusion.
- **REVISE** 鈥?one or more checks failed. List the specific action items and suggest re-running generate mode with updated spec/prompt.
...
3. **Font and line weight**: Identical across all figures 鈥?use the shared styles from `venue_styles.md`.
...
**Stage 2 (Architecture):** When building the section architecture table, classify each figure from "Key Figures Needed" as data (鈫?`/viz`) or non-data (鈫?spec mode here). Run spec mode for all non-data figures during this stage.

**Stage 3 (Section Drafts):** When drafting the Design section, the architecture overview figure should already exist. Design Move 3 (Component Architecture) uses it as structural scaffolding 鈥?each subsection corresponds to a component in the overview figure. If the figure doesn't exist yet, generate it before drafting.

...
OVERALL FLOW: {{flow_direction 鈥?e.g., "left to right" or "top to bottom"}}
...
- {{connection.from}} 鈫?{{connection.to}}: "{{connection.label}}" ({{connection.style}} arrow)
{{end for}}
...
KEY CONTRIBUTION: The component [{{emphasis_component}}] is the paper's main contribution 鈥?make it visually prominent (thicker border, slightly larger, brighter color).
...
2. Identify which component is the key contribution (from the Styling 鈫?Emphasis field)
3. Set flow direction from Layout 鈫?Flow direction
4. Set width from Layout 鈫?Size target (typically 7.0in for architecture overview)
...
- Visual elements: {{what to draw 鈥?e.g., "scattered points with no structure, arrows pointing in random directions"}}
...
- Visual elements: {{what to draw 鈥?e.g., "organized clusters with clear boundaries, arrows aligned"}}
...
{{if spatial_analogy:}}
SPACE: {{describe the conceptual space 鈥?e.g., "2D embedding space"}}
ELEMENTS IN SPACE:
{{list of elements and their spatial positions/relationships}}
KEY INSIGHT: {{what the spatial arrangement reveals 鈥?e.g., "similar items cluster, dissimilar items separate"}}
{{end if}}

{{if visual_metaphor:}}
METAPHOR: {{the analogy 鈥?e.g., "funnel" for progressive filtering, "bridge" for connecting domains}}
CONCRETE MAPPING:
...
- Steps: {{step 1}} 鈫?{{step 2}} 鈫?{{step 3}}
...
4. Keep the illustration abstract and geometric 鈥?no realistic imagery
...
LEFT SIDE 鈥?"{{existing_approach_name}}" (the limitation):
- Components: {{list of components in the existing approach}}
- Data flow: {{how data moves through the existing approach}}
- FAILURE POINT: {{where and why it breaks 鈥?mark with red (#EE6677)}}
- Label: "{{short failure description 鈥?e.g., 'Misses bursty patterns'}}"

RIGHT SIDE 鈥?"{{proposed_approach_name}}" (the solution):
- Components: {{list of components in the proposed approach}}
- Data flow: {{how data moves through the proposed approach}}
- SUCCESS POINT: {{where the improvement happens 鈥?mark with green (#228833)}}
- Label: "{{short success description 鈥?e.g., 'Captures event boundaries'}}"
...
**Assembly instructions:**
1. The left side comes from the Problem Gap in `project_context.md` 鈥?the structural limitation of existing work
2. The right side comes from the key contribution 鈥?the insight that fixes the limitation
...
NETWORK CONNECTIONS:
{{for each connection:}}
- {{from}} 鈫?{{to}}: {{protocol or description}} ({{style: solid for data plane, dashed for control plane}})
{{end for}}

ANNOTATIONS:
- Latency/bandwidth labels on key links: {{list}}
- Replication indicators: {{e.g., "脳3 replicas" near a database component}}

...
description: AI builders digest 鈥?monitors top AI builders on X and YouTube podcasts, remixes their content into digestible summaries. Use when the user wants AI industry insights, builder updates, or invokes /ai-digest.
metadata:
  openclaw:
    requires:
      bins:
...
You are an AI-powered content curator that tracks the top builders in AI 鈥?the people
actually building products, running companies, and doing research 鈥?and delivers
...
"I'm your AI Builders Digest. I track the top builders in AI 鈥?researchers, founders,
PMs, and engineers who are actually building things 鈥?across X/Twitter and YouTube
...
updated centrally 鈥?you'll always get the latest sources automatically."
...
(Example: "8am, Pacific Time" 鈫?deliveryTime: "08:00", timezone: "America/Los_Angeles")
...
1. **Telegram** 鈥?I'll send it as a Telegram message (free, takes ~5 min to set up)
2. **Email** 鈥?I'll email it to you (requires a free Resend account)

Or you can skip this and just type /ai whenever you want your digest 鈥?but it
...
4. Choose a username (e.g. "myaidigest_bot") 鈥?must end in "bot"
# REDACTED: sensitive-looking memory line
6. Now open a chat with your new bot (search its username) and send it any message (e.g. "hi")
7. This is important 鈥?you MUST send a message to the bot first, otherwise delivery won't work
...
2. Sign up (free tier gives 100 emails/day 鈥?more than enough)
...
Set `delivery.method` to `"stdout"`. Tell them: "No problem 鈥?just type /ai
...
from a central feed 鈥?no API keys needed for that. You only need a key for
...
No need to edit any files 鈥?just tell me what you want."

### Step 8: Set Up Cron

Save the config (include all fields 鈥?fill in the user's choices):
```bash
...
- Daily at 8am 鈫?`"0 8 * * *"`
- Weekly on Monday at 9am 鈫?`"0 9 * * 1"`
...
- "Channel is required when multiple channels are configured" 鈫?you used `--channel last`, specify the exact channel
- "Delivering to X requires target" 鈫?you forgot `--to`, add the target ID
- "No agent" 鈫?add `--agent <agent-id>` if the OpenClaw instance has multiple agents
...
bypassing the agent entirely. The digest won't be remixed by an LLM 鈥?it will
...
This script handles ALL data fetching deterministically 鈥?feeds, prompts, config.
...
- `config` 鈥?user's language and delivery preferences
- `podcasts` 鈥?podcast episodes with full transcripts
- `x` 鈥?builders with their recent tweets (text, URLs, bios)
- `prompts` 鈥?the remix instructions to follow
- `stats` 鈥?counts of episodes and tweets
- `errors` 鈥?non-fatal issues (IGNORE these)
...
- `prompts.digest_intro` 鈥?overall framing rules
- `prompts.summarize_podcast` 鈥?how to remix podcast transcripts
- `prompts.summarize_tweets` 鈥?how to remix tweets
- `prompts.translate` 鈥?how to translate to Chinese

**Tweets (process first):** The `x` array has builders with tweets. Process one at a time:
1. Use their `bio` field for their role (e.g. bio says "ceo @box" 鈫?"Box CEO Aaron Levie")
...
2. Use `name`, `title`, and `url` from the JSON object 鈥?NOT from the transcript
...
- **"bilingual":** Interleave English and Chinese **paragraph by paragraph**.
  ...
  Box CEO Aaron Levie 璁や负 AI agent 灏嗕粠鏍规湰涓婇噸濉戣蒋浠堕噰璐?..
  https://x.com/levie/status/123

  Replit CEO Amjad Masad launched Agent 4...
  https://x.com/amasad/status/456

  Replit CEO Amjad Masad 鍙戝竷浜?Agent 4...
  https://x.com/amasad/status/456
  ```
...
### Schedule Changes
- "Switch to weekly/daily" 鈫?Update `frequency` in config.json
- "Change time to X" 鈫?Update `deliveryTime` in config.json
- "Change timezone to X" 鈫?Update `timezone` in config.json, also update the cron job

### Language Changes
- "Switch to Chinese/English/bilingual" 鈫?Update `language` in config.json

### Delivery Changes
- "Switch to Telegram/email" 鈫?Update `delivery.method` in config.json, guide user through setup if needed
- "Change my email" 鈫?Update `delivery.email` in config.json
- "Send to this chat instead" 鈫?Set `delivery.method` to "stdout"
...
- "Make summaries shorter/longer" 鈫?Edit `summarize-podcast.md` or `summarize-tweets.md`
- "Focus more on [X]" 鈫?Edit the relevant prompt file
- "Change the tone to [X]" 鈫?Edit the relevant prompt file
- "Reset to default" 鈫?Delete the file from `~/.follow-builders/prompts/`

### Info Requests
- "Show my settings" 鈫?Read and display config.json in a friendly format
- "Show my sources" / "Who am I following?" 鈫?Read config + defaults and list all active sources
- "Show my prompts" 鈫?Read and display the prompt files
...
When the user invokes `/ai-digest` or asks for their digest manually:
1. Skip cron check 鈥?run the digest workflow immediately
2. Use the same fetch 鈫?remix 鈫?deliver flow as the cron run

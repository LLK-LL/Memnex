#!/usr/bin/env python3
"""PaperSpine Integrity Audit 鈥?teaching-quality checkpoint before LaTeX.
...
Pattern: follows the *writing_rationale_matrix* philosophy 鈥?every row teaches.
"""
...
# ---------------------------------------------------------------------------
# Dimension 1 鈥?Artifact Chain Audit
# ---------------------------------------------------------------------------
    ...
    for i, artifact in enumerate(required, start=1):
        ...
        if not path.exists():
            ...
            dim.findings.append(AuditFinding(
                ...
                root_cause=f"`{owner}` did not produce `{artifact}` 鈥?either the skill was skipped or it failed silently",
                fix_action=f"Route back to `{owner}` and regenerate `{artifact}`. "
                           f"Do not hand-write this file 鈥?the owning skill produces it from research data.",
...
# ---------------------------------------------------------------------------
# Dimension 2 鈥?Reasoning Depth Audit
# ---------------------------------------------------------------------------
...
def audit_reasoning_depth(out_dir: Path, _config: dict) -> AuditDimension:
    ...
    if not matrix_path.exists():
        dim.findings.append(AuditFinding(
            ...
            what_was_found="writing_rationale_matrix.md not found 鈥?cannot audit reasoning depth",
    ...
    if not tables:
        dim.findings.append(AuditFinding(
            ...
            root_cause="The matrix was generated without a proper table structure 鈥?likely a format error in the writing step.",
        ...
        if len(first_row_text) < 300:
            dim.findings.append(AuditFinding(
                ...
                teaching_note="The whole-work framework row is the most important row in the matrix. "
                              "It's not a summary 鈥?it's a design justification.",
            ))
    ...
    if len(shallow) > max(2, (len(rows) - 1) * 0.3):
        dim.findings.append(AuditFinding(
            ...
            fix_action="For each shallow row, add: (a) the original problem this unit solves, "
                       ...
                       "A row that just says 'improve clarity' is not a rationale 鈥?it's an admission of not thinking.",
        ...
        dim.findings.append(AuditFinding(
            ...
            root_cause="Minor gaps in reasoning depth 鈥?likely rows that were left as placeholders.",
...
# ---------------------------------------------------------------------------
# Dimension 3 鈥?Evidence Chain Audit
# ---------------------------------------------------------------------------
        ...
        if evidence_bank.exists():
            ...
            if len(eb) < 300:
                dim.findings.append(AuditFinding(
                    ...
                    what_was_found="evidence_bank.md is very short 鈥?may lack sufficient evidence entries",
...
# ---------------------------------------------------------------------------
# Dimension 4 鈥?Integrity Pattern Scan
# ---------------------------------------------------------------------------
...
FAILURE_PATTERN_TEACHING: dict[str, str] = {
    "orphan_citation": (
        ...
        "This is one of the most common AI hallucinations 鈥?the model invents a plausible-sounding "
    ...
    "weak_assertion": (
        ...
        "In academic writing, if something is clear, you don't need to say it is 鈥?the evidence "
    ...
    "logical_leap": (
        ...
        "the bridge. Each leap should be unpacked into: observation 鈫?interpretation 鈫?implication."
    ),
}
    ...
    if p_matches:
        ...
        dim.findings.append(AuditFinding(
            ...
            fix_action="For each flagged p-value: add the test name (e.g., t-test, ANOVA), "
                       "the test statistic (t=, F=, 蠂虏=), degrees of freedom, and effect size. "
...
def to_markdown(report: IntegrityAuditReport) -> str:
    ...
    for dim in report.dimensions:
        ...
        for f in dim.findings:
            if f.severity == "INFO":
                lines.append(f"**{f.id}** 鉁?{f.what_was_found}")
                lines.append("")
                continue
            icon = "馃毇" if f.severity == "BLOCKER" else "鈿狅笍"
            lines.append(f"### {icon} {f.id} 鈥?{f.severity}")
            lines.append("")

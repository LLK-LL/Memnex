# SciWrite 鈥?Manuscript Writing Review
...
3. **Sentence architecture.** Evaluates sentence length, structure, and flow鈥攆lagging run-on sentences, misplaced modifiers, and paragraphs that lack a clear logical progression.
...
If you installed the skill account-wide via Customize (Option A above), it is already available in Cowork鈥攏o additional setup needed.
...
2. Go to [chat.openai.com](https://chat.openai.com), click your profile icon, then **My GPTs** 鈫?**Create a GPT**.
...
**Provide enough text for context.** A single isolated sentence is hard to review for keyword consistency or logical flow. A full section or chapter gives the skill enough material to identify patterns鈥攔epeated nominalizations, inconsistent terminology, or structural problems that span multiple paragraphs.
...
**Use the interactive mode for learning.** The paragraph-by-paragraph mode shows before/after revisions with explanations 鈥?useful for building long-term writing habits, not just fixing a single draft.
...
If you use this skill and find an edge case it doesn't handle well鈥攁 field-specific writing convention it misjudges, a common clutter pattern it misses, or a false positive that comes up repeatedly鈥攃ontributions are welcome. Open an issue describing the case, or submit a pull request with your proposed change.
...
The source material鈥擠r. Kristin Sainani's *Writing in the Sciences* video lectures鈥攊s published under a [Creative Commons Attribution (CC-BY)](https://creativecommons.org/licenses/by/4.0/) license.

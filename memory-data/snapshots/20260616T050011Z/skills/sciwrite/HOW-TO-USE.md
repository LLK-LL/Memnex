...
journal reviewer 鈥?once, in a file 鈥?so that every review it performs on your
manuscript follows the same rigorous protocol.

## Setup: Where to Put the File

### Option A 鈥?Project-level skill (recommended)
...
鈹溾攢鈹€ .claude/
鈹?  鈹斺攢鈹€ skills/
鈹?      鈹斺攢鈹€ manuscript-review/
鈹?          鈹斺攢鈹€ SKILL.md          鈫?put it here
鈹溾攢鈹€ draft.tex                     (or .md, .docx, etc.)
鈹溾攢鈹€ figures/
鈹斺攢鈹€ references.bib
```
...
### Option B 鈥?Global skill (available for all your projects)
...
Open your terminal in VS Code (`` Ctrl+` `` or **Terminal 鈫?New Terminal**).
...
```
Audit my Results section for keyword consistency 鈥?make sure group names
...
interactive mode is especially good for this 鈥?you approve each paragraph
...
complementary layer 鈥?your CLAUDE.md handles the computational context, while
...
- It does **not** generate new text 鈥?it only revises existing prose.

# Chart Types 鈥?Nature Figure Making
...
def plot_radar(methods, colors, subtask_names, value_matrix,
    ...
    Parameters
    ----------
    methods        : list[str]    鈥?one curve per method
    colors         : list[str]
    subtask_names  : list[str]    鈥?one spoke per subtask (may contain '\\n')
    value_matrix   : np.ndarray  鈥?shape (n_subtasks, n_methods)
    benchmark_radii: dict         鈥?{benchmark_name: [tick1, tick2, ...]} for normalization
    display_range  : (r_min, r_max) 鈥?polar radial display window
    """
...
**Key settings:**
- `ax.set_theta_zero_location('N')` 鈥?top-start convention
...
- [api.md](api.md) 鈥?PALETTE and core helper signatures
- [common-patterns.md](common-patterns.md) 鈥?Bar, trend, and layout patterns
- [design-theory.md](design-theory.md) 鈥?Rationale and color theory
- [tutorials.md](tutorials.md) 鈥?Full end-to-end walkthroughs

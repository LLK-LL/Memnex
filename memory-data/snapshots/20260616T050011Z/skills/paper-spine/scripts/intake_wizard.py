...
CHOICE_HELP = {
    "workflow": {
        "rewrite_existing": ("鏀硅繘宸叉湁鍒濈", "Improve an existing draft"),
        "build_from_materials": ("浠庣礌鏉愭枃浠跺す浠庨浂鏋勭瓚", "Build from a materials folder"),
    },
    "scene": {
        "journal": ("鏈熷垔璁烘枃", "Journal paper"),
        "conference": ("浼氳璁烘枃", "Conference paper"),
        "report_review": ("璇剧▼鎶ュ憡銆佹妧鏈姤鍛婃垨缁艰堪", "Report, technical report, or review"),
        "competition": ("绔炶禌璁烘枃鎴栫珵璧涙姤鍛?, "Competition paper or report"),
    },
    "tier": {
        "flash": ("杞婚噺璋冪爺锛?+3 绡囨牱渚嬪姞瀹樻柟瑕佹眰", "Light research: 3+3 examples plus official requirements"),
        "pro": ("娣卞害璋冪爺锛?+6 绡囨牱渚嬪姞瀹樻柟瑕佹眰", "Deep research: 6+6 examples plus official requirements"),
    },
    "output_language": {
        "en": ("鑻辨枃鏈€缁堢", "English final output"),
        "zh": ("涓枃鏈€缁堢", "Chinese final output"),
    },
    "word_output": {
        "none": ("涓嶉澶栫敓鎴?Word", "Do not generate Word"),
        "docx": ("鐢熸垚骞舵鏌?Word 鏂囦欢", "Generate and check DOCX"),
    },
    "translation_package": {
        "none": ("涓嶇炕璇?, "Do not translate"),
        "zh": ("鐢熸垚瀹屾暣涓枃缈昏瘧鍖?, "Generate complete Chinese translation package"),
    },
    "reference_mode": {
        "local_first": ("榛樿鍏堣鍙栨湰鍦?褰撳墠宸ヤ綔鏂囦欢澶癸紝鍐嶈ˉ鍏呯綉缁滄潵婧?, "Default: read local/current-folder references first, then supplement from web"),
        "specified_paths": ("鍙紭鍏堣鍙栫敤鎴锋寚瀹氱殑鏈湴鍙傝€冩枃鐚矾寰?, "Prefer user-specified local reference paths"),
        "web": ("涓昏浠庣綉缁滄绱㈠弬鑰冩潗鏂?, "Mainly collect references from the web"),
    },
    "ui_language": {
        "zh": ("涓枃鐣岄潰", "Chinese interface"),
        "en": ("English UI", "English interface"),
    },
    "humanize_tier": {
        "none": ("涓嶉檷 AI 鐥曡抗", "No humanization"),
        "light": ("杞诲害 鈥?鏇挎崲杩炴帴璇嶏紝寰皟鍙ュ紡", "Light 鈥?replace connectors, vary sentence length"),
        "medium": ("涓害 鈥?鍙ュ紡鎵撴暎 + 淇℃伅瀵嗗害 + 绗竴浜虹О", "Medium 鈥?break patterns + density + first-person"),
        "heavy": ("寮哄害 鈥?缁撴瀯涓嶈鏁?+ 鏈鍙樹綋锛堜繚鎸佸鏈姘旓級", "Heavy 鈥?structural variation + term variants (academic tone)"),
    },
    "detection_platform": {
        "cnki": ("鐭ョ綉 AIGC 妫€娴?, "CNKI AIGC detection"),
        "weipu": ("缁存櫘 AIGC 妫€娴?, "Weipu AIGC detection"),
        "general": ("閫氱敤绛栫暐 鈥?璺ㄥ钩鍙板吋瀹?, "General 鈥?cross-platform compatible"),
    },
}

LABELS = {
    "zh": {
        "banner": "PaperSpine 閰嶇疆鍚戝",
        "welcome": "Welcome back!",
        "tagline": "鍔ㄦ満椹卞姩鐨勮鏂?鎶ュ憡 Skill Suite",
        "flowline": "鍏堝涔犵洰鏍囧満鏅紝鍐嶇‘璁ゅ姩鏈猴紝鏈€鍚庢瀯绛戝彲瀹¤鐨?LaTeX 鎴愭灉",
        "why_1": "鎴戜滑鍋?PaperSpine锛屾槸涓轰簡璁?AI 鍏堝涔狅紝鍐嶅啓浣溿€?,
        "why_2": "涓嶆槸鎶婅鏂囨鼎鑹插緱鏇撮暱锛岃€屾槸鎶婂姩鏈恒€佽瘉鎹笌缁撴瀯杩炴垚涓€鏉℃竻鏅颁富绾裤€?,
        "why_3": "瀹冮潰鍚戣鏂囥€佹姤鍛婁笌绔炶禌鍐欎綔锛氳皟鐮旂洰鏍囧満鏅紝瀛︿範浼樼鏍蜂緥锛屽啀閫愭鐢熸垚銆?,
        "continue": "鎸変换鎰忛敭杩涘叆閰嶇疆",
        "workflow": "宸ヤ綔娴?,
        "scene": "鐩爣鍦烘櫙",
        "tier": "璋冪爺娣卞害",
        "output_language": "鏈€缁堣緭鍑鸿瑷€",
        "word_output": "Word 鐗堟湰",
        "translation_package": "鐢熸垚鑻辨枃浜х墿鍚庢槸鍚︾炕璇?,
        "ui_language": "鐣岄潰璇█",
        "humanize_tier": "闄?AI 鐥曡抗",
        "detection_platform": "鐩爣妫€娴嬪钩鍙?,
        "target_name": "鐩爣鍚嶇О",
        "draft_path": "鍒濈璺緞",
        "materials_dir": "绱犳潗鏂囦欢澶硅矾寰?,
        "user_motivation": "鍒濆鍔ㄦ満鍋囪",
        "official_urls": "瀹樻柟閾炬帴",
        "reference_mode": "鏂囩尞璇诲彇妯″紡",
        "reference_paths": "鏈湴鍙傝€冩枃鐚矾寰?,
        "citation_target_count": "鏈€缁堝紩鐢ㄧ洰鏍囨暟",
        "special_requirements": "鐗规畩瑕佹眰",
        "review": "妫€鏌ラ厤缃?,
        "confirm": "纭鍐欏叆閰嶇疆",
        "edit": "杈撳叆瑕佷慨鏀圭殑瀛楁缂栧彿锛屾垨鐩存帴鍥炶溅瀹屾垚",
        "invalid": "杈撳叆鏃犳晥锛岃閲嶆柊閫夋嫨銆?,
        "wrote": "宸插啓鍏?,
        "keyboard_help": "鈫?鈫?鍒囨崲閫夐」锛涒啈/鈫?鍒囨崲瀛楁锛汦nter 缂栬緫/纭锛汼 淇濆瓨锛決 閫€鍑?,
        "keyboard_subtitle": "涓婁笅鍒囨崲瀛楁锛屽乏鍙冲垏鎹㈤€夐」锛屾墍鏈夎矾寰勪笌娓呭崟瀛楁鍙寜 Enter 鐩存帴缂栬緫銆?,
        "progress": "杩涘害",
        "current_value": "褰撳墠鍊?,
        "fields_header": "閰嶇疆瀛楁",
        "previous": "涓婁竴涓?,
        "next": "涓嬩竴涓?,
        "current_marker": "褰撳墠",
        "choice_hint": "宸﹀彸閿垏鎹㈠€欓€夐」锛屽綋鍓嶉」浣嶄簬涓棿銆?,
        "text_field_hint": "璇ュ瓧娈靛凡灏介噺浠庡綋鍓嶆枃浠跺す鑷姩璇诲彇锛汦nter 鍙墜鍔ㄨ鐩栥€?,
        "save_hint": "妫€鏌ユ棤璇悗鎸?S 鎴?Enter 淇濆瓨閰嶇疆銆?,
        "text_help": "杈撳叆鏂板唴瀹广€傚垪琛ㄥ瓧娈靛彲鐢ㄥ垎鍙峰垎闅斻€傜洿鎺ュ洖杞︿繚鐣欏綋鍓嶅€笺€?,
        "last_field_hint": "鈫?鈫?鍒囨崲璇█锛屾寜 Enter 淇濆瓨骞堕€€鍑恒€?,
        "save": "淇濆瓨骞堕€€鍑?,
        "quit": "閫€鍑轰絾涓嶄繚瀛?,
        "auto": "鑷姩璇诲彇",
        "enter_edit": "Enter 淇敼",
        "empty": "绌?,
    },
    "en": {
...
def _enable_windows_vt() -> bool:
    ...
    box-drawing escape sequences as literal garbage (e.g. ``鈫怺0m``). Returns True
...
def print_centered_box(lines: list[str], width: int, accent: str = "38;5;208") -> None:
    print(ansi("鈺? + "鈹€" * (width - 2) + "鈺?, accent))
    for line in lines:
        print(ansi("鈹?, accent) + pad_ansi(line, width - 2) + ansi("鈹?, accent))
    print(ansi("鈺? + "鈹€" * (width - 2) + "鈺?, accent))
...
def print_welcome_screen(ui_language: str, wait: bool = False) -> None:
    ...
    title = [
        "鈻堚枅鈻堚枅鈻堚枅鈺? 鈻堚枅鈻堚枅鈻堚晽 鈻堚枅鈻堚枅鈻堚枅鈺?鈻堚枅鈻堚枅鈻堚枅鈻堚晽鈻堚枅鈻堚枅鈻堚枅鈺?鈻堚枅鈻堚枅鈻堚枅鈻堚晽鈻堚枅鈻堚枅鈻堚枅鈺?鈻堚枅鈺椻枅鈻堚枅鈺?  鈻堚枅鈺椻枅鈻堚枅鈻堚枅鈻堚枅鈺?,
        "鈻堚枅鈺斺晲鈺愨枅鈻堚晽鈻堚枅鈺斺晲鈺愨枅鈻堚晽鈻堚枅鈺斺晲鈺愨枅鈻堚晽鈻堚枅鈺斺晲鈺愨晲鈺愨暆鈻堚枅鈺斺晲鈺愨枅鈻堚晽鈻堚枅鈺斺晲鈺愨晲鈺愨暆鈻堚枅鈺斺晲鈺愨枅鈻堚晽鈻堚枅鈺戔枅鈻堚枅鈻堚晽  鈻堚枅鈺戔枅鈻堚晹鈺愨晲鈺愨晲鈺?,
        "鈻堚枅鈻堚枅鈻堚枅鈺斺暆鈻堚枅鈻堚枅鈻堚枅鈻堚晳鈻堚枅鈻堚枅鈻堚枅鈺斺暆鈻堚枅鈻堚枅鈻堚晽  鈻堚枅鈻堚枅鈻堚枅鈺斺暆鈻堚枅鈻堚枅鈻堚枅鈻堚晽鈻堚枅鈻堚枅鈻堚枅鈺斺暆鈻堚枅鈺戔枅鈻堚晹鈻堚枅鈺?鈻堚枅鈺戔枅鈻堚枅鈻堚枅鈺? ",
        "鈻堚枅鈺斺晲鈺愨晲鈺?鈻堚枅鈺斺晲鈺愨枅鈻堚晳鈻堚枅鈺斺晲鈺愨晲鈺?鈻堚枅鈺斺晲鈺愨暆  鈻堚枅鈺斺晲鈺愨枅鈻堚晽鈺氣晲鈺愨晲鈺愨枅鈻堚晳鈻堚枅鈺斺晲鈺愨晲鈺?鈻堚枅鈺戔枅鈻堚晳鈺氣枅鈻堚晽鈻堚枅鈺戔枅鈻堚晹鈺愨晲鈺? ",
        "鈻堚枅鈺?    鈻堚枅鈺? 鈻堚枅鈺戔枅鈻堚晳     鈻堚枅鈻堚枅鈻堚枅鈻堚晽鈻堚枅鈺? 鈻堚枅鈺戔枅鈻堚枅鈻堚枅鈻堚枅鈺戔枅鈻堚晳     鈻堚枅鈺戔枅鈻堚晳 鈺氣枅鈻堚枅鈻堚晳鈻堚枅鈻堚枅鈻堚枅鈻堚晽",
    ]
    ...
    lines.extend(
        [
            ...
            ansi("Bilibili: 褰琠2023 (ID: 450856661)", muted),
            "",
...
def find_candidate_draft() -> str:
    ...
    if not candidates:
        return ""
    preferred = ("draft", "manuscript", "paper", "main", "鍒濈", "璁烘枃")
...
def infer_motivation(text: str, ui_language: str) -> str:
    markers = ("鏍稿績鎯虫硶", "鏍稿績鍔ㄦ満", "motivation", "Motivation", "涓荤嚎", "鍒涙柊鐐?)
    for line in text.splitlines():
        clean = line.strip(" #锛?*-")
        if any(marker in line for marker in markers) and 12 <= len(clean) <= 180:
            return clean
    if ui_language == "zh":
        return "璇峰厛璋冪爺鐩爣鍦烘櫙鍜屼紭绉€鏍蜂緥锛屽啀鐢熸垚鍊欓€夊姩鏈哄苟鐢辩敤鎴风‘璁ゃ€?
...
def auto_config_project(config: PaperSpineConfig, args: argparse.Namespace) -> None:
    cwd = Path.cwd()
    materials_dir = find_first_existing_dir(("materials", "绱犳潗", "source_materials", "data"))
    ...
    if not args.reference_path:
        reference_dirs = [
            ...
            if path.is_dir()
            and path.name.lower()
            in {"reference_materials", "references", "literature", "papers", "citations", "鏂囩尞", "鍙傝€冩枃鐚?}
        ]
        config.reference_paths = reference_dirs or ["."]
    if not args.special_requirement:
        requirements = [
            "蹇呴』杈撳嚭 final_paper/main.tex锛涘鏋滄湰鏈烘湁 LaTeX 缂栬瘧鍣ㄥ垯缂栬瘧 paper.pdf銆?,
            "蹇呴』鐢熸垚璇︾粏 writing_rationale_matrix.md锛岄€愭瑙ｉ噴鍐欎綔閫昏緫銆?,
        ]
        figures_dir = material_root / "figures"
        if figures_dir.exists() and any(figures_dir.glob("*.*")):
            requirements.append("澶嶅埗骞跺紩鐢ㄧ礌鏉愬浘鐗囧埌鏈€缁?LaTeX 椤圭洰鐨?figures/銆?)
        if config.workflow == "build_from_materials":
            requirements.append("浠庣礌鏉愪粠闆舵瀯绛戯紝涓嶆妸鎶€鏈鏄庡綋鎴愬垵绋挎鼎鑹层€?)
...
def right_panel_lines(config: PaperSpineConfig, field: str, width: int, index: int, total: int, color: bool = True) -> list[str]:
    ...
    header = f"{tr(config.ui_language, 'progress')} {index + 1}/{total - 1} 路 {title}"
    lines = [
...
def render_keyboard_frame(
    ...
    top = style("鈺? + "鈹€" * (width - 2) + "鈺?, accent, color)
    split_top = style("鈹? + "鈹€" * left_w + "鈹? + "鈹€" * right_w + "鈹?, accent, color)
    split_mid = style("鈹? + "鈹€" * left_w + "鈹? + "鈹€" * right_w + "鈹?, accent, color)
    bottom = style("鈺? + "鈹€" * left_w + "鈹? + "鈹€" * right_w + "鈺?, accent, color)
    ...
    frame = [
        top,
        style("鈹?, accent, color) + pad_ansi(title, width - 2) + style("鈹?, accent, color),
        style("鈹?, accent, color) + pad_ansi(subtitle, width - 2) + style("鈹?, accent, color),
        style("鈹?, accent, color) + pad_ansi(cwd_line, width - 2) + style("鈹?, accent, color),
        split_top,
        style("鈹?, accent, color)

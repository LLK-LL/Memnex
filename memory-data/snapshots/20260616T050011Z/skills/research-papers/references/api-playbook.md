...
**Use the bundled `scripts/fetch_and_parse` helper** 鈥?do not hand-roll this cascade. The helper resolves any identifier, runs the cascade below, downloads the best available source, parses to layout-aware Markdown, and caches the result.
...
```
1. arXiv HTML        鈫?https://arxiv.org/html/{arxiv_id}        (skipped for old slash-form IDs)
2. arXiv PDF         鈫?https://arxiv.org/pdf/{arxiv_id}
3. PMC XML           鈫?efetch db=pmc&id={pmcid}&rettype=full&retmode=xml  (resolves PMID鈫扨MCID first)
4. PMC HTML          鈫?https://www.ncbi.nlm.nih.gov/pmc/articles/{pmcid}/
5. Unpaywall         鈫?https://api.unpaywall.org/v2/{doi}?email=...       (requires UNPAYWALL_EMAIL or OPENALEX_EMAIL)
6. OpenAlex OA       鈫?/works/doi:{doi} 鈫?best_oa_location.pdf_url / locations[].pdf_url
7. Semantic Scholar  鈫?/paper/{DOI:...}?fields=openAccessPdf 鈫?openAccessPdf.url
8. Crossref TDM      鈫?https://api.crossref.org/works/{doi} 鈫?message.link[content-type=application/pdf]
9. DOI landing       鈫?https://doi.org/{doi}    (paywall-prone; usually marks status=abstract_only)
10. Direct URL       鈫?user-supplied URL when no other identifier resolves
```
...
- `OPENALEX_EMAIL` 鈥?enables OpenAlex polite pool (`mailto=`); also used as Unpaywall fallback email.
- `UNPAYWALL_EMAIL` 鈥?required by Unpaywall (free; falls back to `OPENALEX_EMAIL`). Without this, step 5 is skipped.
...
1. `pymupdf4llm` 鈥?layout-aware Markdown (preserves headings, sections, tables, figure captions). Pre-installed in `scripts/.venv` on first run via `requirements.txt`.
2. `pymupdf` 鈥?text-only fallback.
3. `pdftotext` (poppler binary) 鈥?last resort.
...
- The script enforces retries with exponential backoff on 429/5xx 鈥?do not add your own retry logic on top.
...
- If `status` is `abstract_only`, the script has populated `abstract` (when available) 鈥?use that as your evidence source and tag claims accordingly.
...
- HTML (preferred): `https://arxiv.org/html/{arxiv_id}` 鈥?readable, structured, best for extraction
- PDF: `https://arxiv.org/pdf/{arxiv_id}` 鈥?fallback when HTML is unavailable
...
- `mailto=$OPENALEX_EMAIL` 鈥?always include when the env var is set (free polite-pool)
...
- Always include `mailto=$OPENALEX_EMAIL` in requests when the environment variable is available 鈥?this enters the free polite pool with better rate limits.
# REDACTED: sensitive-looking memory line
# REDACTED: sensitive-looking memory line
...
- For full-text resolution, prefer the `scripts/fetch_and_parse` helper 鈥?it already handles the OpenAlex OA cascade including `locations[]` fallback when `best_oa_location` is empty.
...
- Access is inconsistent 鈥?treat as best-effort. Bot protection may block scripted requests.
...
- `OPENALEX_EMAIL` 鈥?passed as `mailto=` to OpenAlex (free polite pool); also used as Unpaywall fallback email
- `UNPAYWALL_EMAIL` 鈥?passed to Unpaywall (required by their API; free). Falls back to `OPENALEX_EMAIL` if unset.
# REDACTED: sensitive-looking memory line
# REDACTED: sensitive-looking memory line
- `NCBI_EMAIL` 鈥?included in PubMed/PMC requests as good citizen identification
- `NCBI_TOOL` 鈥?included in PubMed/PMC requests as tool identification

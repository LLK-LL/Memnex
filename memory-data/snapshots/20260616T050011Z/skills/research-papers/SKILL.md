...
**Full-text reading is the default, not the exception.** For every shortlisted paper, you MUST attempt to fetch and read the *entire* paper content 鈥?every page from title to references 鈥?not just the abstract or introduction. Download PDFs when HTML full text is unavailable. Only fall back to abstract-based analysis when all full-text access methods have been exhausted and failed.
...
- **Shell / Bash / terminal**: Use `curl` for *discovery* against research APIs (arXiv, OpenAlex, DBLP, Semantic Scholar, PubMed). Once you have an identifier, switch to `fetch_and_parse` for the full text 鈥?do not hand-roll PDF downloads or pass raw PDFs through the Read tool.
...
1. arXiv HTML 鈫?arXiv PDF 鈫?PMC XML 鈫?PMC HTML 鈫?OpenAlex OA 鈫?Semantic Scholar OA 鈫?SSRN page 鈫?DOI landing
...
### 6. Read and extract evidence (CRITICAL 鈥?read the entire paper)
...
`<identifier>` can be a DOI, arXiv ID, PMID, PMCID, or URL 鈥?in any common form (`10.1038/...`, `arxiv:2301.08243`, `https://arxiv.org/abs/2301.08243`, `PMC4304851`, etc.). The script:

1. Resolves the identifier into canonical IDs.
2. Runs the OA cascade: arXiv (HTML鈫扨DF) 鈫?PMC (XML鈫扝TML) 鈫?Unpaywall 鈫?OpenAlex `best_oa_location` 鈫?Semantic Scholar `openAccessPdf` 鈫?Crossref TDM links 鈫?DOI landing.
...
5. For PDF parses, extracts embedded figures to `<cache>/images/`, drops images that repeat on 鈮?5% of pages (journal logos, page headers, watermarks), and emits inline `![](path)` references in the parsed Markdown for the rest.
...
- `path` 鈥?absolute path to a PNG you can pass to the Read tool. The Read tool is multimodal: `Read(path)` on a PNG renders the image inline.
- `size_kb` 鈥?file size hint.
- `instances` 鈥?copies in the source PDF; `>1` is unusual for content figures and may signal repeated decoration that slipped past the filter.
- `caption_nearby` 鈥?`true` when the parsed Markdown has a "Figure N" / "Table N" / "Scheme N" / etc. caption within ~500 chars of the image reference. **Advisory only** 鈥?many real figures (chemical structures, schematic insets, equation renders) lack such captions, so do not use this as a hard filter.

**Use judgment, not a rule.** Read figures whose content matters to the synthesis question:

- Always read figures cited by name in a passage you are about to summarize ("as shown in Figure 3, 鈥?).
...
- When `caption_nearby` is `false` and `size_kb` is small (under ~5KB), it is often an inline equation render or sub-component 鈥?read it only if the surrounding text indicates it carries meaning.
...
The script handles cross-source resolution internally 鈥?there is no need to manually try OpenAlex after arXiv fails, or to look up PMID鈫扨MCID, or to retry on 429s. If the script returns `failed`, it has exhausted the cascade. Inspect the `tried` array in the JSON to see what each step returned.
...
- Full text `[Full text]`
- Abstract only `[Abstract only 鈥?full text unavailable: {reason from tried log}]`

# `_shared/` 鈥?Common content for nature-* skills
...
Only when 鈮?2 skills need the same content. If only one skill needs it, keep it inside that skill's `static/`.

## When to keep content skill-local instead

The shared layer holds **definitions and reference material** (taxonomy, reader workflow, ethics rules). The **action layer** 鈥?how a specific skill diagnoses, drafts, or revises 鈥?stays inside each skill's `static/fragments/`. Two skills can use the same paper-type taxonomy but apply different actions on top of it.

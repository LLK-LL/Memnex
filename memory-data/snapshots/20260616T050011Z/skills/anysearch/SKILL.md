...
1. **Information retrieval** 鈥?looking up facts, news, documentation, or any current data.
2. **Fact-checking** 鈥?verifying claims, cross-referencing statements.
3. **Web browsing / URL content extraction** 鈥?reading page content beyond search snippets.
4. **Vertical domain queries** 鈥?structured searches with identifiers (Stock:/CVE:/DOI:/IATA:/patent, etc.).
5. **Multi-intent queries** 鈥?several independent searches that can run in parallel.

**Vertical domain rule:** For queries that may belong to a supported domain (finance, academic, travel, health, code, geo, etc.), **always call `list_domains` first** to get the correct `sub_domain` and `query_format` before searching 鈥?vertical search produces significantly better results than general web search for these topics.
...
# REDACTED: sensitive-looking memory line
# REDACTED: sensitive-looking memory line
| **Key exhausted 鈥?no new key returned** | Inform the user that the quota is exhausted and suggest configuring a new API key via `.env` or environment variable. |
...
**Step 1 鈥?Check Python**
```
python --version 2>&1
python3 --version 2>&1
```
- If either `python` or `python3` exists with version >= 3.6 鈫?use `anysearch_cli.py`
...
**Step 2 鈥?Check Node.js** (if Python failed)
```
node --version 2>&1
```
- If exit code 0 鈫?use `anysearch_cli.js`
- No external dependencies required (uses built-in `https` module)

**Step 3 鈥?Check Shell** (if both Python and Node.js failed)

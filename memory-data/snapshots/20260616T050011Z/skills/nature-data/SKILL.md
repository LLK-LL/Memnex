...
# REDACTED: sensitive-looking memory line
  ...
  selection while writing a paper, and Chinese phrasings like 鏁版嵁鍙敤鎬у０鏄庛€佹暟鎹彲鐢ㄦ€с€?
  鏁版嵁鍏变韩銆佷唬鐮佸彲鐢ㄦ€с€佸鏈啓浣滄暟鎹０鏄庛€佸啓鏁版嵁澹版槑銆佹暟鎹瓨鏀俱€佹暟鎹粨搴撻€夋嫨.
version: 2.0.0
# REDACTED: sensitive-looking memory line
---

# Nature Data Availability 鈥?Router
...
- `static/core/stance.md` 鈥?what the data-availability package is, the default stance, and the source hierarchy.
- `static/core/chinese-mode.md` 鈥?how to operate when the user writes in Chinese (accept Chinese, draft English, convert terms precisely).
- `static/core/workflow.md` 鈥?the eight-step workflow and the output format.

### 2. No content axis 鈥?confirm journal and language inline
...
- **journal/article type** 鈥?if journal-specific instructions conflict with this skill, follow the journal.
- **access route** 鈥?each dataset is classified into one route (public repository, controlled access, within paper, reused public, third-party restricted, justified request, or not applicable).
- **user language** 鈥?if the user writes Chinese, follow `core/chinese-mode.md` and add the 涓枃鏍稿 block.

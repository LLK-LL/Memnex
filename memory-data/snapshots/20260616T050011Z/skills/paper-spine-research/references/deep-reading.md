...
```markdown
## Deep Reading Paper N: [Title] 鈥?[SOTA / Prior-SOTA / Adjacent-SOTA]
...
**Goal**: Fill detailed extraction templates. Do NOT summarize 鈥?fill fields.
...
**Paragraph location**: [e.g., Introduction P4 鈥?Contribution statement]
...
**Move 3 鈥?Limitation Handling** (from Discussion):
```
Limitation 1: [acknowledged] 鈫?[hedged/mitigated]
Limitation 2: ...
Future direction: [limitations 鈫?future work]
```
...
- **Does this abstract contain specific numerical values?** [Yes/No 鈥?if Yes, list each]
...
**Critical for avoiding category errors** 鈥?putting Results-level detail into Abstract, or Abstract-level vagueness into Results.
...
| Results | Yes (metrics, 螖, p) | Yes | quantitative | [...] |
...
|------|-----------------------|-------|--------|
| **M1**: Establishing a territory | Claim centrality, review prior research | Centrality 鈫?Topic generalization 鈫?Literature review | |
| **M2**: Establishing a niche | Identify the gap | Gap indication, counter-claiming, question-raising | |
| **M3**: Occupying the niche | Present paper as filling gap | Outlining purpose 鈫?Announcing findings 鈫?Indicating structure | |
...
| Move | Communicative Purpose | Para # |
|------|-----------------------|--------|
| **M7**: Reporting results | Presenting overall 鈫?Highlighting specifics 鈫?Generalizing | |
| **M8**: Commenting on results | Interpreting 鈫?Comparing with prior work 鈫?Explaining 鈫?Noting limitations | |

**Discussion/Conclusion Moves** (Yang & Allison 2003):
| Move | Communicative Purpose | Para # |
|------|-----------------------|--------|
| **M9**: Reviewing present study | Restating purpose/methods 鈫?Summarizing findings | |
| **M10**: Evaluating findings | Stating significance 鈫?Suggesting applications | |
| **M11**: Addressing limitations | Acknowledging limitations 鈫?Proposing future directions | |
...
## Pass 3: Critical Synthesis 鈥?FAT Analysis (per paper)

```markdown
### Pass 3: FAT Analysis

#### F 鈥?Facts (objective restatement, no judgment)
- **Research question**: [1 sentence]
- **Key findings** (the data, not interpretation): [bullets]
- **Method** in one paragraph: [concise but complete]
# REDACTED: sensitive-looking memory line

#### A 鈥?Appreciation (critical evaluation)
...
#### T 鈥?Tie (relevance to our paper)

...
Prefer reordering over rewriting. The cleanest fix is usually to move the dependent content so it appears after the content it depends on. Large-scale reorderings鈥攎oving entire paragraphs or subsections鈥攁re acceptable and often necessary.
...
- The term consists of three or more words (e.g., *Large Language Model* 鈫?LLM, *Variational Autoencoder* 鈫?VAE).
...
All changes under this task鈥攔eordering, abbreviation replacement, appendix migration鈥攎ust preserve the paper's intended meaning exactly. If a restructuring would require changing what a claim says (not just where or how it appears), stop and flag it for the user rather than proceeding unilaterally.

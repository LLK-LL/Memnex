...
An Agent Skill that brings the fluid, conversational style of **Vibe Coding** to academic paper writing. Send it a chat log, email thread, or a voice-note dump鈥攁nd it integrates the ideas into your LaTeX paper with proper scholarly prose, automatically.
...
- **Chat logs & email threads** 鈥?Extracts only the ideas you explicitly endorsed, translates conversational language into academic prose, and flags anything ambiguous for your review.
- **Personal documents & notes** 鈥?Incorporates your drafts, memos, or bullet-point notes into the appropriate section of the paper.
- **Citations & references** 鈥?Integrates a new paper or finding, places it in the right location, and updates the bibliography.
- **Mathematical notation** 鈥?Writes or audits LaTeX math expressions against the paper's established symbol conventions.
- **Paper review** 鈥?Checks the full paper for internal contradictions, broken references, privacy issues, and consistency of claims.
- **Writing improvement** 鈥?Linearizes logic, enforces abbreviation policies, and moves verbose passages to an appendix.
...
```
vibe-paper-writing/
鈹溾攢鈹€ SKILL.md                  # Skill entry point (metadata + universal rules)
鈹溾攢鈹€ tasks/
鈹?  鈹溾攢鈹€ chat-and-email.md     # Endorsement-aware chat/email integration
鈹?  鈹溾攢鈹€ user-documents.md     # Personal drafts and notes
鈹?  鈹溾攢鈹€ citations.md          # Reference insertion
鈹?  鈹溾攢鈹€ math-notation.md      # LaTeX math
鈹?  鈹溾攢鈹€ paper-review.md       # Full-paper consistency check
鈹?  鈹溾攢鈹€ writing-improvement.md
鈹?  鈹斺攢鈹€ latex-compilation.md  # Compile-and-fix workflow
鈹斺攢鈹€ resources/
    鈹斺攢鈹€ math_commands.tex     # Shared math macro reference
```
...
The core challenge in academic writing assistance is not mechanical insertion鈥攊t is *intelligent integration*. This skill is built around three principles:

1. **Endorsement awareness.** Conversations contain many voices and tentative ideas. The skill incorporates only what you have explicitly agreed with or instructed it to use鈥攏ever hallucinated positions.

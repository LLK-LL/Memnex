...
A daily-updated collection of research papers on autonomous AI agents 鈥?systems that use LLMs for planning, reasoning, tool use, and multi-step task execution. Covers the full agent stack from foundational prompting techniques (ReAct, Chain-of-Thought) to multi-agent systems, memory architectures, and real-world deployments. Organized chronologically with category tags for easy navigation.

## Agent Taxonomy

```
Autonomous Agents
鈹溾攢鈹€ Planning & Reasoning
鈹?  鈹溾攢鈹€ Chain-of-Thought (CoT, ToT, GoT)
鈹?  鈹溾攢鈹€ ReAct (Reasoning + Acting)
鈹?  鈹溾攢鈹€ Reflexion (Self-reflection)
鈹?  鈹斺攢鈹€ LATS (Language Agent Tree Search)
鈹溾攢鈹€ Tool Use & Actions
鈹?  鈹溾攢鈹€ Function calling
鈹?  鈹溾攢鈹€ Code execution
鈹?  鈹溾攢鈹€ Web browsing
鈹?  鈹斺攢鈹€ API interaction
鈹溾攢鈹€ Memory Systems
鈹?  鈹溾攢鈹€ Short-term (context window)
鈹?  鈹溾攢鈹€ Long-term (vector stores)
鈹?  鈹溾攢鈹€ Episodic (experience replay)
鈹?  鈹斺攢鈹€ Procedural (learned strategies)
鈹溾攢鈹€ Multi-Agent Systems
鈹?  鈹溾攢鈹€ Debate/discussion (ChatDev, MetaGPT)
鈹?  鈹溾攢鈹€ Hierarchical (manager/worker)
鈹?  鈹溾攢鈹€ Collaborative (shared goals)
鈹?  鈹斺攢鈹€ Competitive (adversarial)
鈹斺攢鈹€ Applications
    鈹溾攢鈹€ Software engineering (SWE-agent, Devin)
    鈹溾攢鈹€ Scientific research (AI Scientist)
    鈹溾攢鈹€ Web automation (WebArena)
    鈹斺攢鈹€ Game playing (Voyager)
```

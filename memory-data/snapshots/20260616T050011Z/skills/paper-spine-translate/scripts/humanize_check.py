...
Self-contained 鈥?standard library only, no dependencies on other PaperSpine
...
AI_CONNECTORS_ZH = [
    "棣栧厛", "鍏舵", "鍐嶆", "鏈€鍚?, "缁间笂鎵€杩?, "鎬昏€岃█涔?, "鎬荤殑鏉ヨ",
    "姝ゅ", "鍙﹀", "涓嶄粎濡傛", "鍊煎緱娉ㄦ剰鐨勬槸", "闇€瑕佹寚鍑虹殑鏄?, "涓嶅蹇借鐨勬槸",
    "鍏锋湁閲嶈鎰忎箟", "鍏锋湁閲嶈鐨勭悊璁烘剰涔?, "鍏锋湁閲嶈鐨勭幇瀹炴剰涔?,
    "涓衡€︹€﹀瀹氬熀纭€", "鍦ㄢ€︹€︾殑杩囩▼涓?,
]
AI_CONNECTORS_EN = [
...
def sentence_lengths(text: str) -> list[int]:
    sents = re.split(r"[.銆?锛?锛?锛沑n]+", text)
    return [
        len(s.strip())
        for s in sents
...
def check_matrix(matrix_path: Path, manuscript_text: str, lang: str) -> HumanizeCheckResult:
    ...
    if result.matrix_rows > 2 and severity_counts["high"] == 0:
        result.findings.append("No high-severity patterns found 鈥?matrix may be under-reporting")
    ...
    if len(lengths) > 2:
        ...
        if result.sentence_length_stddev < MIN_SENTENCE_LENGTH_STDDEV:
            result.findings.append(
                f"Sentence length stddev = {result.sentence_length_stddev} 鈥?too uniform. "
    ...
    dash_pattern = re.search(r"[鈥擻-鈥斺€昡{3,}", manuscript_text)
    if dash_pattern:
        result.findings.append(
            "Long dash separators detected (e.g. '鈥斺€斺€斺€?). "
            "These are a strong AI-generation signal 鈥?replace with section headings or blank lines."
        )

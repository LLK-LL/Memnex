...
- adapted or redrawn content: label as `鏁寸悊鑷猔 or `鏀圭粯鑷猔,
...
- `output/ppt_outline_cn.md` 鈥?Chinese outline: paper information, paper type, central argument, slide structure, slide purpose.
- `output/figure_plan.md` 鈥?figure selection plan: figure / panel, what it shows, why it matters, recommended slide, Chinese caption, interpretation.
- `output/ppt_script_cn_with_figures.md` 鈥?slide-by-slide script (Purpose / Layout / On-slide bullets / Figure-Table / Chinese caption / Core takeaway / Speaker note per slide).
- `output/rendered/` 鈥?rendered slide previews only when a reliable headless renderer is available or the user requests visual QA.

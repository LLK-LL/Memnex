...
- `discovery` 鈥?discovery / mechanism papers (question-to-evidence arc). Default.
- `methods` 鈥?methods / AI / tool / algorithm papers (problem-to-solution arc).
- `resource` 鈥?resource / dataset / atlas / omics / benchmark papers (workflow-to-validation arc).
- `clinical` 鈥?clinical / population / intervention studies (design-to-inference arc).
- `materials` 鈥?materials / chemistry / physics / engineering papers (property-to-mechanism / design-to-performance arc).
- `review` 鈥?reviews / perspectives / commentaries / meta-analyses (evidence-map arc).
...
1. Core principles (`core/principles.md`) 鈥?the argument is the spine; lean operating mode; accepted inputs; Chinese-by-default language rule.
2. Toolchain policy and fast path (`core/toolchain.md`) 鈥?cross-platform Python-first stack, default fast path.
3. Paper-type arc (the loaded `paper_type` fragment) 鈥?narrative order and slide structure for this paper.
4. Workflow (`core/workflow.md`) 鈥?run the 9 steps end to end.
5. Output and quality rules (`core/output-and-quality.md`) 鈥?deliverables, quality gates, fallbacks.
...
- composing/auditing slide layout, visual rhythm, typography, anti-template design, archetypes, on-slide text budget 鈫?`references/design-and-layout.md`.
- selecting, extracting, cropping, and quality-checking figure/table assets 鈫?`references/figure-assets.md`.
- running the self-review/corrective revision loop, severity grading, programmatic python-pptx checks, rendered-preview policy, and final verification 鈫?`references/self-review.md`.

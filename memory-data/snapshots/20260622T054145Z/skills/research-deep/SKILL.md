...
### Step 0: Preconditions and Fallback
- Search the current working directory recursively for `outline.yaml` or `outline.yml`.
- If no outline file exists, do **not** start the batch JSON workflow and do **not** pretend this skill can do generic web research by itself.
- For explicit deep-research requests without an outline, first create/ask for an outline with the `research` skill workflow, or route the task to the web/search skill that matches the user's request.
- If an `outline.yml` file exists, treat it the same as `outline.yaml`.

### Step 1: Auto-locate Outline
Find `*/outline.yaml` or `*/outline.yml` file in current working directory, read items list, execution config (including items_per_agent).
...
- `{fields_path}`: absolute path to fields.yaml next to the discovered outline file, or {topic}/fields.yaml when that project layout is used

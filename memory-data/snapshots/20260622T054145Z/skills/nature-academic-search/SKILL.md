...
description: >-
  ...
  citation, converting citation files, and Chinese phrasings like 鏂囩尞妫€绱€佹煡鏂囩尞銆佹壘鏂囩尞銆?
  鏂囩尞缁艰堪妫€绱€佹煡璁烘枃銆佸紩鏂囨牳瀵广€佸弬鑰冩枃鐚鐞嗐€佹枃鐚幓閲?
version: 2.0.0
# REDACTED: sensitive-looking memory line
---

# Academic Search 鈥?Router
...
- `static/core/tools.md` 鈥?the MCP tool inventory (core search, extended search, PubMed utilities) and the shared-module map.
- `static/core/routing-and-ops.md` 鈥?the T1鈫扵2鈫扵3 source routing quick guide, environment setup, error handling, and limitations.

### 2. Detect the workflow

Map the user's need to one or more `workflow` values:

- `multi-source-search` 鈥?find literature across sources.
- `citation-verification` 鈥?verify citations extracted from a document.
- `mesh-strategy` 鈥?build a MeSH/PubMed search strategy.
- `citation-file-mgmt` 鈥?convert/manage `.nbib`/`.ris`/`.bib` files.
- `reference-mgmt` 鈥?BibTeX, related-article discovery, ID conversion.
...
1. Core tools and routing (`core/tools.md`, `core/routing-and-ops.md`) 鈥?which MCP tool for which need, and the T1鈫扵2鈫扵3 fallback chain that is the standard execution order across all workflows.
2. The workflow fragment 鈥?its specific steps.

...
Pattern: follows the writing_rationale_matrix philosophy 鈥?every review finding
...
# ---------------------------------------------------------------------------
# review generation (prepares structure 鈥?LLM fills findings)
# ---------------------------------------------------------------------------
...
def generate_structured_review(out_dir: Path, manuscript_path: Path) -> StructuredReviewReport:
    ...
    if methods_rows:
        for mr in methods_rows[:3]:
            ...
            methods.findings.append(ReviewFinding(
                ...
                evidence_detail="Evidence bank has content 鈥?verify specific evidence for this method claim."
                if evidence["has_content"] else "evidence_bank.md is missing or empty 鈥?methods cannot be verified.",
    ...
    for cr in contrib_rows[:3]:
        ...
        contribution.findings.append(ReviewFinding(
            ...
            teaching_note="A contribution is not what you did 鈥?it's what the community gains. "
# REDACTED: sensitive-looking memory line
    ...
    if total_paras < 20:
        clarity.findings.append(ReviewFinding(
            ...
            what=f"Manuscript has {total_paras} paragraphs across {len(sections)} sections 鈥?fairly concise",
            rationale_row=None,
            evidence_status="n/a",
            evidence_detail="",
...
def dispatch_review(out_dir: Path, manuscript_path: Path) -> dict:
    ...
    dispatch_md = (
        ...
        "Each agent reads only its own prompt file and the manuscript 鈥?"
...
def to_markdown(report: StructuredReviewReport) -> str:
    lines = [
        ...
        "and provides a concrete revision command 鈥?not just 'improve this'.",
        "",
        "---",
        "",
    ]
    ...
    for reviewer in report.reviewers:
        ...
        if not reviewer.findings:
            lines.append("*No findings 鈥?this reviewer has nothing to flag.*")
            lines.append("")
            lines.append("---")
            lines.append("")
            continue
        ...
        for f in reviewer.findings:
            lines.append(
                f"| {f.id} | {f.severity} | {f.what[:80]} | {f.rationale_row or '鈥?} | "
        ...
        for f in reviewer.findings:
            if f.severity == "OBSERVATION":
                continue
            icon = "馃敶" if f.severity == "CRITICAL" else "馃煛" if f.severity == "MAJOR" else "馃數"
            lines.append(f"#### {icon} {f.id}")
            lines.append("")
            lines.append(f"**Finding:** {f.what}")
            lines.append("")
            if f.rationale_row:
                lines.append(f"**Rationale matrix row:** {f.rationale_row} 鈥?compare against the planned function, "

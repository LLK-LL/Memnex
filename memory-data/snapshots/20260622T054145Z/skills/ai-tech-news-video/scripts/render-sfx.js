...
// 鈹€鈹€ Sound synthesis functions (deterministic, no Math.random) 鈹€鈹€
...
async function renderSound(name, duration, renderFn) {
  ...
  console.log(`  鉁?${name}.wav (${(duration * 1000).toFixed(0)}ms, ${(wav.byteLength / 1024).toFixed(1)}KB)`);
  return outPath;
}

// 鈹€鈹€ Sound: Whoosh (transition sweep) 鈹€鈹€
...
// 鈹€鈹€ Sound: Pop (notification) 鈹€鈹€
...
// 鈹€鈹€ Sound: Click (UI feedback) 鈹€鈹€
...
// 鈹€鈹€ Sound: Sparkle (reveal/achievement) 鈹€鈹€
...
// 鈹€鈹€ Sound: Rise (tension builder) 鈹€鈹€
...
async function main() {
  ...
  console.log(`馃攰 Rendering SFX to ${OUTPUT_DIR}/\n`);
  ...
  console.log('\n鉁?Done! Use these WAV files in HyperFrames <audio> tags.');
}

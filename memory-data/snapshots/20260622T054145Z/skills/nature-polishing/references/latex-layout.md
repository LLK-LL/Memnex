...
Deep reference for **typesetting/layout** requests (鎺掔増): loose pages, stranded
...
Golden rule: **change 鈫?compile 鈫?render to image 鈫?look 鈫?iterate.** Never judge
...
1. Compile and read the log for the three signals that matter:
   - `Float too large for page by Xpt` 鈥?a float (often figure+caption) exceeds the text height.
   - `Overfull \vbox` 鈥?content ran past the bottom margin (often `[H]` placement).
   - `Reference ... undefined` / `Citation ... undefined` 鈥?re-run needed (and for `xr-hyper`, compile the SI first, then the main file twice).
...
3. Measure figure aspect ratios from the source PDFs (`pypdf` mediabox, or pymupdf):
   `aspect = width/height`. Then `displayed_height_pt 鈮?textwidth_pt / aspect`.
   For a 1-in-margin `article` on US Letter, `\textwidth 鈮?468 pt`, usable
   `\textheight 鈮?620鈥?50 pt`. Budget every page against that height.

---

## 1. The "loose / 鏉炬暎" float page 鈥?top-align the glue
...
```latex
\makeatletter
\setlength{\@fptop}{0pt}            % no stretch at top 鈫?floats start at top
...
## 2. Wide-and-short figures can never fill a portrait page 鈥?fix at the source

**Symptom:** a figure (e.g. a 1脳N strip of per-dataset bars) sits at full width but is
short (aspect 3:1鈥?:1), leaving ~40鈥?0% of the page empty; it also tends to strand the
next heading (see 搂4).
...
**Correct fix 鈥?regenerate the figure with a taller aspect** (target ~**1.9:1鈥?.2:1**)
so two stacked panels + caption fill the portrait page. In matplotlib this is just the
`figsize` height, e.g. `figsize=(17, 4.1)` 鈫?`figsize=(17, 6.8)`. Then:
...
- **Font-size caveat:** the on-page text size is set by the *display width* (figure
  ...
  fonts 鈥?it only adds vertical extent (taller bars, more breathing room). To enlarge
  labels you must *narrow* the figure (more scale-up to `\textwidth`) or bump the
  in-plot font, not add height.
- **Mind the caption budget.** A 5鈥? line caption is ~80 pt. Size the figure so
  `heading + intro + 2 panels + subcaptions + caption 鈮?usable height`. A long-caption
...
reader to turn the page. **Prefer a taller redraw (搂2) over `\rotatebox` /
...
## 4. Float backlog 鈫?stranded section headings
...
**Fix 鈥?bind each figure section into a "heading + figure" unit:**

1. Make the figure short enough to *share a page* with its heading + intro
   (`heading + intro + figure 鈮?usable height`; see 搂2 caption budget).
...
flushes the page early 鈥?if the previous section ended with a big floating figure whose
...
## 5. `[H]` and `placeins` 鈥?sharp edges
...
- **`[H]`** (float package) does not float. If the figure doesn't fit in the remaining
  ...
  (`Overfull \vbox`). Safe only when you guarantee room 鈥?hence pair it with `\clearpage`
...
- **`\usepackage[section]{placeins}`** puts a `\FloatBarrier` at every `\section`, keeping
  a section's floats inside it. Useful against backlog 鈥?**but** it flushes the previous
  ...
  until figures were also shortened. Use it *with* 搂2/搂4 sizing, not as a standalone cure.
- **Don't blanket-`\clearpage` every section.** Section-boundary clearpages create
  ...
  搂4), not everywhere. (Removing 8 blanket clearpages once took an SI 23鈫?1 pages with
  even density.)

---

## 6. Multi-panel main-text figures 鈥?stack, don't cram

- A 2脳2 grid shrinks panels; for **wide** panels prefer a **single-column vertical stack**
...
- Tune inter-panel space with `\vspace` between `subfigure`s (e.g. `0.3em鈥?.8em`).
...
## 7. Page-count is not the metric 鈥?*fullness* is
...
- [ ] Top-aligned float glue in the preamble (搂1).
- [ ] Wide-short figures regenerated to ~2:1, SI branch only, faithfulness pixel-checked (搂2).
- [ ] Upright, not landscape (搂3).
- [ ] Each figure section: `\clearpage` + `[H]` heading-figure unit; trailing figure of the
      previous section also `[H]` and sized to fit (搂4).
- [ ] No blanket section clearpages; no stranded headings (搂5).
- [ ] Compile clean: no "Float too large", no "Overfull \vbox", no undefined refs.
- [ ] Contact-sheet the final PDF and eyeball every page (搂0).

...
5. **Ban long dash separators** (e.g. "鈥斺€斺€斺€? or 3+ consecutive em-dashes/hyphens).
...
|--------|----------------|------------------|---------------|----------|---------------|-----------------|---------------|
| 1 | Abstract | 3 consecutive sentences 18-22 words | D1-Sentence Length | High | Split sentence 2 into 5-word + 25-word | Breaks bell-curve distribution | D1 is the strongest detection signal 鈥?fix first |
...
### D1 Sentence Length
Every 3-4 sentences, one must differ significantly (鈮?8 words or 鈮?30 words). No 3 consecutive sentences with length difference under 6 words.
...
Use different templates per paragraph. Ban "claim鈫抏xplain鈫抏xample鈫抯ummary" loop:
- Question-driven: question鈫抋nalysis鈫抍onclusion
- Compare-judge: view A鈫抳iew B鈫抎ifferentiate鈫抪osition
- Causal chain: observation鈫抍ause鈫抜nference鈫抳erification
- Abrupt end: deep argument, then stop 鈥?no summary
...
- Density difference between consecutive paragraphs 鈮?15%
...
### First-Person
"We / Our study found / We observed" at method explanations and result analyses. 鈮?2 per 2000 words.
...
Introduction: may lead with core problem, not "background鈫抔ap鈫抋pproach鈫抍ontributions". Methods: may interleave rationale with description. Discussion: may leave 1-2 questions open.

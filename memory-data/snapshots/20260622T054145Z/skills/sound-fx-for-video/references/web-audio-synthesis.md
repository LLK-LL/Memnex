...
```
Gain: 0 鈫?0.3 (instant) 鈫?0.001 (80ms, exponential)
```

### Medium decay (notifications, reveals)
```
Gain: 0 鈫?0.4 (5ms) 鈫?0.001 (200ms, exponential)
```

### Long fade (whooshes, ambience)
```
Gain: 0 鈫?0.5 (100ms, linear) 鈫?0 (400ms, linear)
```

### Rise & fall (tension builders)
```
Gain: 0.01 鈫?0.3 (1.5s, linear) 鈫?stop immediately
```
...
### Impact + Debris
- Layer 1: Low sine sweep (150鈫?0Hz, 300ms)
- Layer 2: Noise burst (100ms, highpass 1kHz)
- Layer 3: Sub hit (40Hz sine, 500ms)

### Whoosh + Pass-by
- Layer 1: Bandpass noise sweep (200鈫?k鈫?00Hz)
...
- **Want variation?** Add slight random delay (卤30ms) and pitch shift (卤5%) for repeated sounds
- **Need stereo?** Use `StereoPannerNode` 鈥?`pan.value` from -1 (left) to 1 (right)

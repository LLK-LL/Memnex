...
## Principle 1: Introduction-Twice (Draft 0 鈫?Evaluation 鈫?Final Introduction)

The introduction is written twice. **Draft 0** is a framing scaffold 鈥?stakes, problem gap, rough contribution claims 鈥?written early to set guardrails for the evaluation. It clarifies thinking and establishes what the paper is *trying* to show. Draft 0 is explicitly disposable; it probably will not survive. **The final introduction** is written after the evaluation is complete, constrained by what the evidence actually supports. Each claim in the final introduction must map to a specific evaluation subsection.

The key insight: writing is a thinking tool, not just a communication tool. A preliminary introduction forces the student to externalize their framing *before* designing experiments. Without Draft 0's guardrails, the evaluation is written without constraints 鈥?the student doesn't know what they're trying to show. But without evaluation-constrained rewriting, the introduction promises what the student *hoped* to show rather than what they *did* show.

**Observed consequence of skipping Draft 0**: Students who went straight to evaluation without framing guardrails produced evaluations that were technically complete but narratively incoherent 鈥?the experiments didn't build toward a unified argument because there was no argument to build toward.
...
**Rule**: Every baseline, metric, and abstraction should have a proper name. If the paper introduces a new concept, NAME it 鈥?the name becomes citable.

---

## Principle 3: The What 鈫?Why 鈫?So-What Heading Progression
...
3. **SO-WHAT headings** (final version): claim-first, stating the conclusion. Example: "System X outperforms all baselines by 2鈥?脳"
...
**Normal compression range**: 30鈥?0% reduction in the final pass. Compression above 50% usually signals a framing problem 鈥?content was cut because the narrative shifted fundamentally. Compression below 15% suggests the draft was already near publication-ready.
...
**Why this matters**: Method-first openings position the paper as "another ML application" 鈥?competing against every other ML-for-X paper. Problem-first openings position the paper as solving a domain problem 鈥?competing against other solutions to that specific problem. The latter is a smaller, more winnable competitive frame.
...
After every experiment or group of related experiments, insert a Takeaway paragraph that states the IMPLICATION, not just the result. The Takeaway compresses results into interpretation 鈥?it tells the reader the PATTERN, not the NUMBERS.
...
**Rule**: Every evaluation subsection ends with a Takeaway paragraph. It should be interpretable without reading the detailed results 鈥?it is the skim-reader's entry point.
...
**Rule**: Before writing, read 3鈥? recent accepted papers at the target venue. Adapt surface conventions while maintaining the underlying principles.

---

## Principle 11: Rejection Drives Abstraction

Rejection forces papers to a higher level of abstraction. Each rejection identifies a framing mismatch 鈥?the paper claims to be X but delivers Y. The revision process forces alignment between claim and evidence.

**Observable pattern**: Papers that resolved their identity crises BEFORE submission were accepted. Papers whose identity was still evolving at submission were rejected. The number of pre-submission rewrites correlates with acceptance 鈥?not because rewrites are inherently good, but because they indicate the identity discovery happened in the writing room rather than during peer review.
...
The student's initial comprehensive draft is valuable raw material, not a failure to be fixed. The issue is not that students write too much 鈥?it's that knowing WHAT to keep requires understanding the paper's competitive framing, which is a skill developed through experience.

**The division of labor**: Student writes comprehensively 鈫?Advisor restructures 鈫?Advisor compresses 鈫?Joint polishing. Students should write comprehensive first drafts without self-censoring. Asking students to "be more concise" in their first draft is counterproductive 鈥?it eliminates material the advisor might need.
...
**Implication for advising**: Less experienced writers can succeed with measurement papers (stable identity, question-driven structure). Systems papers require framing skill that develops through experience 鈥?they benefit from early advisor involvement in framing.
...
| # | Principle | Impact |
|---|-----------|--------|
| 1 | Introduction-twice (Draft 0 鈫?Evaluation 鈫?Final intro) | High 鈥?prevents both frameless evaluation and overcommitment |
| 2 | Named over vague | High 鈥?precision correlates with acceptance |
| 3 | What 鈫?Why 鈫?So-What headings | High 鈥?enables skim-reading |
| 4 | Compress after expanding | Medium 鈥?improves focus |
| 5 | Structural rewrites over polishing | High 鈥?fundamental to the process |
| 6 | Labeled paragraphs as narrative contracts | Medium 鈥?stabilizes structure |
| 7 | Problem-first, not method-first | High 鈥?determines competitive frame |
| 8 | Takeaway paragraphs | Medium 鈥?aids skim-reading |
| 9 | Dual evaluation for dual claims | Medium 鈥?prevents evaluation confusion |
| 10 | Venue-specific adaptation | Medium 鈥?surface conventions matter |
| 11 | Rejection drives abstraction | High 鈥?explains multi-venue journeys |
| 12 | Student draft is material | Meta 鈥?defines collaboration model |
| 13 | Measurement vs. systems identity stability | Meta 鈥?determines advising strategy |
| 14 | Technical claims require citations | High 鈥?uncited claims read as fluff |

---

## Principle 14: Technical Claims Require Citations

Every technical claim in a paper must be grounded with a citation or an explicit cross-reference. When section N references content that was established with citations in section M, it must either carry forward the relevant `\cite{}` commands or use an explicit forward/backward reference (`\S\ref{}`). Uncited technical claims 鈥?no matter how well-established 鈥?read as unsupported assertions to reviewers.
...
For **advisor simulation**: Apply in order: Principle 1 (check whether the introduction promises match evaluation evidence 鈥?if not, rewrite the introduction from scratch) 鈫?Principle 7 (reframe problem-first) 鈫?Principle 3 (rewrite headings) 鈫?Principle 2 (replace generic terms) 鈫?Principle 14 (verify citations) 鈫?Principle 4 (compress).

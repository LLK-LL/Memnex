...
**Draft 0** is written early 鈥?before the evaluation 鈥?as a framing scaffold. It covers Moves 1鈥? (Stakes through Contributions) in rough form. Its purpose is to set guardrails: what is the paper trying to show? Draft 0 clarifies thinking and constrains what experiments need to be run. It is explicitly disposable.

**The final introduction** is rewritten from scratch after the evaluation is complete. Now Move 6 (Results Preview) can be filled with real numbers, and Moves 3鈥? can be sharpened to match what the evidence actually supports. The final introduction promises exactly what the paper delivers 鈥?no more, no less. Draft 0 is reference material for the rewrite, not a starting point for editing.
...
**Critical insight**: The key abstraction appears ONLY in final versions of papers. It is discovered through writing, not planned in advance. If you don't have one yet, that's normal 鈥?it will emerge as you write the evaluation and design sections.
...
**How to do it**: Include a headline number (e.g., "2鈥?脳 improvement") and reference a headline figure. This move is left blank in Draft 0 and filled only in the final introduction 鈥?after the evaluation is complete.

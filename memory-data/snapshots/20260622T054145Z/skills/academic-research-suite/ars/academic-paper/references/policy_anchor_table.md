...
**Nature 鈫?v3.2 venue de-dup**: the Nature anchor below shares its substantive policy content with the v3.2 Nature venue renderer in `venue_disclosure_policies.md`. Both files cross-reference the canonical source pointer `shared/policy_data/nature_policy.md` so a future single-source-of-truth refactor can byte-compare imported substrings without breaking either consumer. The dedup pointer presence is lint-enforced via `verify_nature_dedup_with_venue` in `check_policy_anchor_table.py`.

**Forbidden by Decision Doc 搂4.3 (renderer must honour these):**
- G1 鈥?no `ai_disclosure` field is added to the corpus entry schema; the renderer reads this table at runtime, no schema change.
- G9 鈥?image-rights regimes stay anchor-specific; the renderer MUST NOT unify field #16 into a single boolean across the four tables.
...
| 7 | Date(s) of use | not-addressed | 鈥?| 鈥?| 鈥?|
...
| 9 | Human oversight method | explicit-recommend | "Describe the process of human interaction with and oversight of the AI tool(s) at each stage: a. How many reviewers interacted with/validated the AI outputs for each task? b. Did reviewers work independently when validating AI outputs?" | Table 1, M8.a鈥揼 | narrative |
| 10 | Human responsibility statement | not-addressed | 鈥?| 鈥?| 鈥?|
...
| 15 | Copyediting exemption predicate | not-addressed | 鈥?| 鈥?| 鈥?|
...
- Per-(tool 脳 task) prompt scope: M6.a says verbatim "for each specific task" 鈥?the renderer produces one prompt record per (tool, task) tuple across M3.a's six SLR stages (search / screening / data extraction / Risk of Bias / synthesis / drafting). Missing tuples emit "not supplied" annotation per tuple, not per tool.
- PRISMA M6 prompt disclosure fires only when **all three** G5 gates hold: (i) PRISMA-trAIce track selected, (ii) `tool_type 鈭?{LLM, GenAI}`, (iii) AI use is methodological-in-SLR (not pure copyediting). Gate (iii) fail routes to the G7 copyediting carve-out path.

---

## Anchor: icmje

**Snapshot:** `icmje:wayback=20260513075516` (sha256: 52f9e6bc鈥?
# REDACTED: sensitive-looking memory line

| # | Field | Source strength | Verbatim quote | Locator | Value type |
|---|---|---|---|---|---|
# REDACTED: sensitive-looking memory line
| 2 | AI tool version | not-addressed | 鈥?| 鈥?| 鈥?|
| 3 | AI tool developer / manufacturer | not-addressed | 鈥?| 鈥?| 鈥?|
# REDACTED: sensitive-looking memory line
| 5 | Specific task within stage | implicit | (inference passage 鈥?same source as #4) "how they used it" implies task-level description without explicit field. | 搂V.A, paragraph 1 | narrative |
| 6 | Affected manuscript sections / content locator | not-addressed | 鈥?| 鈥?| 鈥?|
| 7 | Date(s) of use | not-addressed | 鈥?| 鈥?| 鈥?|
| 8 | Prompts | not-addressed | 鈥?| 鈥?| 鈥?|
# REDACTED: sensitive-looking memory line
| 10 | Human responsibility statement | explicit-mandate | "Therefore, humans are responsible for any submitted material that included the use of AI-assisted technologies." | 搂V.A, paragraph 1 | narrative |
| 11 | Performance evaluation method | not-addressed | 鈥?| 鈥?| 鈥?|
| 12 | Performance evaluation results | not-addressed | 鈥?| 鈥?| 鈥?|
| 13 | Limitations / known failure modes | not-addressed | 鈥?| 鈥?| 鈥?|
# REDACTED: sensitive-looking memory line
| 15 | Copyediting exemption predicate | not-addressed | 鈥?| 鈥?| 鈥?|
...
- Disclosure location: two-channel 鈥?cover letter (cite ICMJE language verbatim) + manuscript "appropriate section". Renderer emits separate paragraphs per channel, not a single duplicated paragraph.
...
**Snapshot:** `nature:wayback=20260513075542` (sha256: cf691cba鈥?
...
# REDACTED: sensitive-looking memory line
# REDACTED: sensitive-looking memory line
| 16 | AI-generated image / figure / content rights | explicit-mandate | "Springer Nature journals are unable to permit its use for publication." + "All exceptions must be labelled clearly as generated by AI within the image field." | 搂Generative AI images | narrative |
...
- Field #15 carve-out semantics: **eliminate** strength 鈥?copyediting-only use produces **no disclosure paragraph**, only an internal log entry. Renderer MUST NOT collapse this into a boolean shared with IEEE's downgrade semantics (Decision Doc 搂4.3 G7 invariant).
# REDACTED: sensitive-looking memory line
...
**Snapshot:** `ieee:wayback=20260513075605` (sha256: 3ab8db50鈥?
...
| 7 | Date(s) of use | not-addressed | 鈥?| 鈥?| 鈥?|
| 8 | Prompts | not-addressed | 鈥?| 鈥?| 鈥?|
| 9 | Human oversight method | not-addressed | 鈥?| 鈥?| 鈥?|
| 10 | Human responsibility statement | not-addressed | 鈥?| 鈥?| 鈥?|
| 11 | Performance evaluation method | not-addressed | 鈥?| 鈥?| 鈥?|
| 12 | Performance evaluation results | not-addressed | 鈥?| 鈥?| 鈥?|
| 13 | Limitations / known failure modes | not-addressed | 鈥?| 鈥?| 鈥?|
...
- Disclosure location: acknowledgments section only 鈥?the tightest closed enum across all four anchors. Renderer emits placement instruction citing the verbatim "acknowledgments section of any article submitted to an IEEE publication" language. **No Methods placement**, no cover-letter channel.
- Field #5 + field #6 are a paired mandate (Decision Doc 搂4.3 G8 invariant). The renderer emits **both** a per-section locator input (free-form list with recommended IMRaD exemplars `"Introduction" | "Methods" | "Results" | "Discussion" | "Abstract" | "Title" | free-form-other`) **and** a `level_of_involvement` narrative annotation (G8 design). Emitting one without the other is non-conformant.
- Field #15 carve-out semantics: **downgrade-not-eliminate** strength 鈥?copyediting-only use still produces a disclosure paragraph at the same acknowledgments location, but with recommend-strength rather than mandate language. Renderer MUST NOT collapse this into a boolean shared with Nature's eliminate semantics (Decision Doc 搂4.3 G7 invariant).
...
- Discovery doc (verbatim quote source-of-truth): `docs/design/2026-05-13-ai-disclosure-schema-discovery.md` 搂4.3-4.6

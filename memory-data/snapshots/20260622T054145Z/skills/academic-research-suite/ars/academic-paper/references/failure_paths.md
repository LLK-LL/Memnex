# Failure Paths 鈥?Academic Paper Writing Failure Path Map
...
2. Classify the nature of the problems:
   a. Fixable (writing, formatting, minor logic issues) 鈫?Recommend Major Revision
   b. Structural issues (argument architecture needs reorganization) 鈫?Return to Phase 3 for restructuring
   c. Fundamental issues (RQ infeasible, insufficient data) 鈫?Return to Phase 0 for re-evaluation
...
2. Assess the impact of missing materials:
   a. Missing Bibliography 鈫?Need Phase 1 (literature_strategist)
   b. Missing Synthesis 鈫?Can continue, Phase 3 handles it additionally
   c. Missing Methodology Blueprint 鈫?Need Phase 0 supplementary questions
3. Recommend:
...
```
F1 (Insufficient research foundation) 鈫?Recommend deep-research 鈫?May encounter F9 (incomplete materials) upon return
F2 (Wrong structure) 鈫?Return to Phase 2 鈫?May cascading affect F3/F4 (word count issues)
F5 (All citations wrong) 鈫?May be a downstream effect of F2 (wrong format selected)
F7 (Rejection) 鈫?Analysis may require returning to F2 (structure) or F1 (foundation)
F8 (Non-convergence) 鈫?May evolve into F10 (abandonment)
```
...
**General Protocol**:
1. Do NOT take desk-reject personally 鈥?30-50% of submissions to top journals are desk-rejected

...
1. **Every figure must add value** 鈥?do not visualize data that is better expressed in a sentence or table
...
5. **Figures must be interpretable without reading the text** 鈥?include all necessary context in the caption

### Caption Format

```
Figure [N]                          鈫?Bold, on its own line
[Descriptive Title]                 鈫?Italic, sentence case, next line
Note. [Explanation if needed]       鈫?Plain text, starts with "Note."
```
...
1. Never rely on color alone 鈥?pair with shape, pattern, or label

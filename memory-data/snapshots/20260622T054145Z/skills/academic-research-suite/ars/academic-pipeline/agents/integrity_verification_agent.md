...
#### Five-Type Taxonomy (GPTZero 脳 NeurIPS 2025; Adams et al., 2026)
...
# REDACTED: sensitive-looking memory line
2. **Venue Exploitation** (PH+PAC): Real journal/conference name + fake article details 鈥?passes "is this a real journal?" heuristic
3. **Mashup Fabrication** (PH): Elements from 2-3 real papers blended into one fake reference 鈥?each fragment is real, but the combination never existed
# REDACTED: sensitive-looking memory line
...
- **In paper**: Lin, Y. H., Hou, A. Y. C., & Chiang, T. L. (2020). "Quality assurance in higher education in Taiwan: Past, present, and future." In A. Curaj et al. (Eds.), *European higher education area* (pp. 589鈥?06). Springer.
- **Reality**: The real chapter is Lin, **A. S. R.**, Hou, A. Y. C., **Chan, S. J.**, & Chiang, T. L. (2021). "Quality Assurance in Taiwan Higher Education: **Regulation, Model Shift, and Future Prospect**." In Hou et al. (Eds.), ***Higher Education in Taiwan*** (pp. **65鈥?1**). Springer. DOI: 10.1007/978-981-15-4554-2_4
# REDACTED: sensitive-looking memory line
...
| GPTZero 脳 NeurIPS (2026) | 100+ hallucinated citations in 53 papers passed 3+ peer reviewers |
...
#### A0. Semantic Scholar API Batch Verification 鈥?NEW v3.3
...
| S2 Result | Action |
|-----------|--------|
| `S2_VERIFIED` | Proceed to A2 (bibliographic accuracy) 鈥?skip A1 WebSearch |
| `S2_NOT_FOUND` | Proceed to A1 (WebSearch existence check) as normal |
| `DOI_MISMATCH` | Flag as SERIOUS 鈥?possible DOI Misdirection (Compound Deception Pattern #5) |
| `API_UNAVAILABLE` | Skip A0, proceed to A1 for all references |

A0 is additive 鈥?it does not replace A1. The audit trail must record both A0 and A1 results.
...
- NOT_FOUND: Cannot find any match after 3 different search queries 鈥?suspected fabrication 鈫?MUST be flagged as SERIOUS issue
# REDACTED: sensitive-looking memory line

鈿狅笍 CRITICAL: There is NO "uncertain" or "difficult to verify" category. If you cannot positively verify a reference exists with its exact bibliographic details, it is either NOT_FOUND or MISMATCH. Both require correction.
```
...
### Mode 1: Initial Verification (Stage 2.5 鈥?Pre-Review Integrity)
...
### Mode 2: Final Verification (Stage 4.5 鈥?Post-Revision Final Check)
...
- **鈿狅笍 Phase A must be a FRESH full verification of ALL references, not just re-checking Stage 2.5 fixes.** The Stage 2.5 check may have missed references (sampling gaps, gray-zone classifications). Stage 4.5 is the last line of defense 鈥?it must independently verify every reference as if Stage 2.5 never happened.
...
- 鉂?"difficult to independently verify" 鈥?this is not a verdict, classify as NOT_FOUND or MISMATCH
- 鉂?"real organizations but specific documents are difficult to verify" 鈥?verify the specific document, not just the organization
- 鉂?Listing references in a "partially verified" or "plausible but unconfirmed" bucket without flagging them for correction
- 鉂?Passing a reference in Phase B (context check) without first passing it in Phase A (bibliographic check)
...
|---|----------|----------|------------------|--------------------|----|
| 1 | Reference | 搂References | [description] | [correct value] | [verification source URL] |
...
- Disagreements 鈫?`[CROSS-MODEL-DISAGREEMENT]` 鈫?prioritized for human review

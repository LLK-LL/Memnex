...
| `resume_from_passport=<hash>` (any continuation phrasing) | Resume Mode (see 搂"Resume Mode: `resume_from_passport`" below) |
...
- When user enters mid-pipeline, check for Material Passport 鈥?see "Mid-Entry Material Passport Check" below

#### Resume Mode: `resume_from_passport`

**Trigger:** user input starts with or contains `resume_from_passport=<12-hex>`.

**Contract:** full spec in [`../references/passport_as_reset_boundary.md`](../references/passport_as_reset_boundary.md) 搂"`resume_from_passport` mode contract".

**Orchestrator obligations:**
1. **Acquire passport lock.** Before reading the ledger or checking for a prior consuming entry, acquire an exclusive advisory lock on the passport file (see `references/passport_as_reset_boundary.md` 搂"Concurrency model"). Hold the lock across the read, the no-prior-resume check, and the append. Release after the append is durable on disk. Do NOT release between steps.
...
4. Load `reset_boundary[]`. Find the entry with `kind: boundary` and matching `hash`. No match 鈫?hard error: "Passport hash `<hash>` not found in `<path>`. Cannot resume."
...
3. If checkpoint_type is FULL or SLIM: invoke collaboration_depth_agent on the just-completed stage's dialogue range (advisory only; non-blocking). If MANDATORY: SKIP this step 鈥?integrity gates must not be diluted. See "Collaboration Depth Observer" section below.
...
**IRON RULE**: the user's response handling above considers only the checkpoint's metrics, deliverables, and integrity results. The `collaboration_depth_agent` output is **advisory only and must never appear in the blocking criteria** 鈥?it is inserted for the user's reflection, not the orchestrator's decision logic.
...
|------------|------|-----------------------------|
| unset / `=0` | any | Continuation (pre-v3.6.3 default) 鈥?no reset tag |
# REDACTED: sensitive-looking memory line
...
```
鈹佲攣鈹?Stage [X] [Name] Complete 鈹佲攣鈹?
...
Collaboration Depth (advisory, Wang & Zhang 2026 鈥?never blocks):
...
See [`../references/passport_as_reset_boundary.md`](../references/passport_as_reset_boundary.md) 搂"Reset-boundary emission sequence".

#### SLIM Checkpoint Template

```
鈹佲攣鈹?[OK] Stage [X] [Name] -> Stage [Y] [Name] ready 鈹佲攣鈹?
Collaboration Depth (advisory): Zone [1|2|3] 路 DI [N] / CV [N] / CR [N] 路 rubric: shared/collaboration_depth_rubric.md
...
2. **Short-stage guard**: if the stage's user-turn count is less than 5, skip the dispatch and inject a static `Collaboration Depth: insufficient_evidence (stage had N user turns; rubric needs 鈮?)` block. This avoids a full-model call just to receive the agent's own `insufficient_evidence` answer.
3. Otherwise, dispatch `collaboration_depth_agent` with the range pointer. It reads live conversation turns 鈥?**do not** pass a summary.
...
**Decision policy.** First check verdict status. If `AUDIT_FAILED` (Path B5 short-circuit per spec 搂5.6), BLOCK without running the eleven gating checks; surface `verdict.failure_reason`; user must dispatch a fresh wrapper run. Otherwise, validate against the eleven gating verification checks (spec 搂5.2), then apply ship/block per verdict status (spec 搂5.3 鈥?rows evaluated top-to-bottom, first matching row wins):

- `PASS` (`p1 == 0 AND p2 == 0 AND p3 == 0`) 鈫?proceed to next stage; append `[Audit: PASS at round N]` line to FULL checkpoint.
- `MINOR` (`p1 == 0 AND p2 == 0 AND p3 <= 3`) 鈫?MANDATORY checkpoint with finding details; user choice required: `continue` (ship) / `iterate` (dispatch revision) / `pause` (stop).
- `MATERIAL + acknowledgement` (latest entry's `verdict.status == "MATERIAL"` AND latest entry carries an `acknowledgement` whose `finding_ids` covers every current `findings[].id`) 鈫?proceed to next stage; emit FULL checkpoint with `[Audit: MATERIAL at round N, residue acknowledged by user at <acknowledged_at>]` line. This row has higher precedence than plain `MATERIAL`.
- `MATERIAL` (`p1 > 0 OR p2 > 0 OR p3 > 3`, AND latest entry either has no `acknowledgement` OR its `acknowledgement.finding_ids` does not cover every current `findings[].id` 鈥?defense-in-depth: the partial-coverage branch is unreachable under 搂5.4 lint rules but fails closed on hand-edit or lint bypass) 鈫?BLOCK; surface findings; re-invoke producing agent with revision prompt; deployment runs the wrapper at `--round N+1 --previous-findings <prior verdict.yaml>` for the next round audit. **Only after `round == target_rounds` (spec 搂5.4 default 3) still MATERIAL** does the gate emit the ESCALATION block offering `ship_with_known_residue` / `another_round` (raises cap by 1) / `abort_stage` 鈥?these three choices are escalation-only, not the default MATERIAL response.

# REDACTED: sensitive-looking memory line

- **Path A** re-verifies an already-merged persisted entry (recovery on resume / re-transition). Failure phases 鈥?all fall through to Path B with reason carried 鈥?are: `P-PA-precond` (no matching persisted entry), `P-PA-schema` (schema validation), `P-PA-gate` (eleven-gate failure), `P-PA-verdict-schema` (verdict file schema), `P-PA-verdict-mirror` (verdict mirror drift, Pattern C3 evidence), `P-PA-stale-late` (late freshness recheck), `P-PA-supersede-preempt` (A1.5 found a higher-round proposal). All seven defined inline in spec 搂5.6 inventory; this prompt cites them by ID only.
# REDACTED: sensitive-looking memory line

**Hard rules.**

- Audit gate cannot be skipped 鈥?there is no "skip audit" option in checkpoint command vocabulary.
...
- A `verdict_status: PASS` does NOT imply integrity check is skipped. Stage 2.5 / 4.5 integrity gates remain mandatory per existing 搂3 Hard boundaries rule 9.

**Failure surfacing.** Any block message uses the standard FULL/MANDATORY checkpoint visual (鈹佲攣鈹?separator). Block message MUST include: why blocked (which check failed / which severity finding triggered), where to look (file:line for findings; artifact path for verification failures), what to do next (re-audit command, revision dispatch, escalation options).

**Cross-references.**

- Spec: `docs/design/2026-04-30-ars-v3.6.7-step-6-orchestrator-hooks-spec.md` 搂5.6 (full procedure), 搂5.2 (eleven gating checks), 搂5.3 (verdict semantics), 搂5.4 (round upper bound + escalation).
...
**Trigger.** Stage 4 鈫?Stage 5 transition, in the same handoff slot as the v3.7.1 Cite-Time Provenance Finalizer. The audit dispatches AFTER the Cite-Time Provenance Finalizer pass (anchor-presence settled per v3.7.3 搂3.1) and BEFORE `formatter_agent` runs its hard gate at the start of Stage 5. Mirrors the 搂3.5 audit-between-deliverable-and-consumption ordering. Spec: `docs/design/2026-05-15-issue-103-claim-alignment-audit-spec.md` 搂5 + 搂1 deliverable 4.

**Why not Stage 5鈫?:** `formatter_agent`'s terminal hard gate runs **during** Stage 5. Dispatching at Stage 5鈫? would produce `claim_audit_results[]` after the gate has already passed; HIGH-WARN-CLAIM-NOT-SUPPORTED could not block output. Stage 4鈫? is the only slot where (a) the draft prose carries resolved v3.7.3 anchors, (b) the cite finalizer has settled anchor presence, and (c) the formatter hard gate has NOT yet run.
...
- The `claim_intent_manifests[]` aggregate from the writing-stage agents (per spec 搂3.2 + the v3.8 "Claim Intent Manifest Emission" sibling sections on `synthesis_agent` / `draft_writer_agent` / `report_compiler_agent`)
- The `literature_corpus[]` aggregate (retrieval input)
# REDACTED: sensitive-looking memory line

**Outputs feeding formatter hard gate (same Stage 5 pass).**

- `claim_audit_results[]` 鈥?drives the 8-row matrix annotations below
- `constraint_violations[]` 鈥?drives `[HIGH-WARN-CONSTRAINT-VIOLATION-UNCITED ({violated_constraint_id})]` annotation. MUST be passed alongside `claim_audit_results[]` 鈥?without this the uncited HIGH-WARN gate-refuse path silently disappears (no claim_audit_result row exists for uncited constraint violations per 搂3.5 schema split)
- `uncited_assertions[]` 鈥?drives `[UNCITED-ASSERTION]` LOW-WARN advisory
- `uncited_audit_failures[]` (v3.8.2 / #118) 鈥?drives `[CLAIM-AUDIT-TOOL-FAILURE-UNCITED 鈥?<fault-class>]` MED-WARN advisory annotation. MUST be passed alongside `claim_audit_results[]` so the formatter sees uncited-path judge outages 鈥?without this hand-off the operational signal stays silent in production (mirrors cited-path INV-14 but uses a dedicated aggregate because `claim_audit_result.ref_slug` is required). Gate passes; retry-next-pass remediation. See `claim_ref_alignment_audit_agent.md` Output emission table and spec 搂3.6.
- `claim_drifts[]` 鈥?drives `[LOW-WARN-CLAIM-DRIFT 鈥?kind=...]` LOW-WARN advisory (per D4-a 鈥?drift never gate-refuses)
- `audit_sampling_summaries[]` 鈥?drives paper-level `[CLAIM-AUDIT-SAMPLED 鈥?k/N audited]` annotation when audited_count < total_citation_count (S-INV-3)
...
- Per-stage `defect_stage` histogram appendix (renders when 鈮? completed entries via `scripts/claim_audit_finalizer.py:render_stage6_histogram`) 鈥?added to the existing Stage 6 AI Self-Reflection Report after gate pass.

**Finalizer matrix (8-row).** The matrix discriminates the previously-conflated paywall vs anchorless cases by reading `ref_retrieval_method` alongside `(judgment, defect_stage)`. Rows are evaluated top-to-bottom, first match wins. Spec source-of-truth: 搂5. Implementation: `scripts/claim_audit_finalizer.py:classify_claim_audit_result`.
...
| RETRIEVAL_FAILED | not_applicable | not_attempted | `[HIGH-WARN-CLAIM-AUDIT-ANCHORLESS 鈥?v3.7.3 R-L3-1-A VIOLATION REACHED AUDIT]` | HIGH-WARN | gate-refuse (defense-in-depth) |
| RETRIEVAL_FAILED | not_applicable | failed | `[CLAIM-AUDIT-UNVERIFIED 鈥?REFERENCE FULL-TEXT NOT RETRIEVABLE]` | LOW-WARN | pass (paywall 鈥?D2) |
| RETRIEVAL_FAILED | not_applicable | audit_tool_failure | `[CLAIM-AUDIT-TOOL-FAILURE 鈥?<fault-class>]` | MED-WARN | pass (retry next pass) |

**Why three rows for `(RETRIEVAL_FAILED, not_applicable)`:** anchor=none (INV-6/INV-11), paywall (INV-10), and audit-tool failure (INV-14) all emit this `(judgment, defect_stage)` pair but mean three different things. Anchorless is a contract violation that v3.7.3 should have already gate-refused upstream 鈥?defense-in-depth row HIGH-WARN gate-refuse. Paywall is a stable access restriction 鈥?legitimate tool/access failure, LOW-WARN advisory pass. Audit-tool failure is a transient infrastructure outage (judge timeout, retrieval 5xx, network error) 鈥?MED-WARN advisory pass with retry-next-pass remediation. The `ref_retrieval_method` field discriminates them; INV-10 / INV-11 / INV-14 jointly enforce that these three are the only `(not_applicable)` paths AND they're mutually exclusive on `ref_retrieval_method`.
...
1. **Paper writing** 鈥?delegate to `academic-paper`
2. **Research** 鈥?delegate to `deep-research`
3. **Review** 鈥?delegate to `academic-paper-reviewer`
4. **Citation verification** 鈥?delegate to `integrity_verification_agent`
5. **Decisions** 鈥?offer suggestions and options; final decisions are the user's
# REDACTED: sensitive-looking memory line

## Hard boundaries (never violate)

7. **Do not fabricate materials** 鈥?if a stage's output does not exist, surface the gap; do not invent
8. **Do not skip checkpoints** 鈥?explicit user confirmation is required after each stage
9. **Do not skip integrity checks** 鈥?Stage 2.5 and 4.5 are mandatory, no override
...
2. Launch Residual Coaching (EIC guides via Socratic dialogue):
   ...
   - "This is the last revision opportunity 鈥?which items can be marked as study limitations?"
...
When `academic-pipeline` mode is active, the orchestrator runs the **Cite-Time Provenance Finalizer** at every Stage 4 鈫?Stage 5 transition (and on every revision loop pass back through Stage 4) to resolve the two-layer citation markers emitted by `synthesis_agent`, `draft_writer_agent`, and `report_compiler_agent` per Step 3a.
...
| true              | false                             | 鈥?                  | **MED WARN**: PDF in repo but AI has not cross-checked (regardless of whether the user has read it; AI verification is the gating condition). Replace with `[UNVERIFIED CITATION 鈥?AI HAS NOT CROSS-CHECKED]<!--ref:slug-->` |
...
**Revision loops:** on revision loops (Stage 4 鈫?reviewer 鈫?Stage 4 revise; or `academic-paper` Phase 6 鈫?Phase 4 loops), the finalizer re-runs against the current draft, resolves any newly-emitted bare `<!--ref:slug-->` comments introduced in the revision pass, and re-applies the matrix to existing resolved markers per the idempotency rule above. Resolved markers **do not invalidate** in the sense that nothing about the revision-loop mechanism itself perturbs them 鈥?only a change in the joined evidence (acquire / verify / `/ars-mark-read` / `/ars-unmark-read`) can move a marker. When evidence is unchanged across a revision pass, every marker is preserved byte-identical.
...
## Cite-Time Provenance Finalizer 鈥?v3.7.3 extension (5-cell + contamination annotation)

Extends the v3.7.1 4-cell matrix above with two additive checks. External motivation: Zhao et al. arXiv:2605.07723 (2026-05). Spec: `docs/design/2026-05-12-ars-v3.7.3-claim-faithfulness-and-contaminated-source-spec.md` 搂3.1 + 搂3.2.
...
- If `<kind>` = `none`, the finalizer resolves the citation to **MED-WARN-NO-LOCATOR** regardless of the underlying trust state. Replace the marker pair with `[UNVERIFIED CITATION 鈥?NO QUOTE OR PAGE LOCATOR]<!--ref:slug--><!--anchor:none:-->`.
- If `<kind>` 鈭?`{quote, page, section, paragraph}`, the finalizer proceeds to the 4-cell matrix above.

NO-LOCATOR is MED severity (not HIGH) because the citation may still point at a real verified source 鈥?only the claim-anchor is missing. Treating it as HIGH would conflate two distinct defects (no source vs no anchor). The fix is locator emission by re-running the upstream agent or manual editing, not source acquisition.

**`/ars-mark-read` does NOT clear NO-LOCATOR.** The precedence-zero rule stops BEFORE applying the trust-state matrix on `(source_acquired, source_verified_against_original, human_read_source)`. Acknowledgment via `/ars-mark-read` only affects `human_read_source`, which is part of the 4-cell matrix that NO-LOCATOR bypasses. The only remediation is re-emitting the citation with a valid (`<kind>` 鈮?`none`) anchor. This asymmetry is intentional: a locator is a structural property of the prose, not an evidence-state property of the source. v3.7.3 codex review P2-2 closure.
...
The contamination annotation does NOT apply to HIGH-WARN / MED-WARN / MED-WARN-NO-LOCATOR rows 鈥?those already block at the gate and the user must address the higher-severity problem before contamination becomes relevant.
...
Per-pass resolution counts gain ten new columns: NO-LOCATOR (precedence-zero hits, v3.7.3 搂3.1), CONTAMINATED-PREPRINT (v3.7.3 搂3.2), CONTAMINATED-UNMATCHED (v3.7.3 搂3.2 legacy single-S2 case), CONTAMINATED-PREPRINT+UNMATCHED (v3.7.3 搂3.2 legacy combination), CONTAMINATED-COVERAGE-NOISE (v3.9.0 搂3.3 k=1 k_max鈮? OR k=1 k_max=1 with non-S2 single index), CONTAMINATED-PREPRINT+COVERAGE-NOISE (v3.9.0 composition), CONTAMINATED-PARTIAL-UNMATCH (v3.9.0 搂3.3 k=2), CONTAMINATED-PREPRINT+PARTIAL-UNMATCH (v3.9.0 composition), CONTAMINATED-TRIANGULATION-UNMATCHED (v3.9.0 搂3.3 k=3), CONTAMINATED-PREPRINT+TRIANGULATION-UNMATCHED (v3.9.0 composition). All ten surface in the Stage 4.5 integrity-check report alongside the existing HIGH / MED / LOW / OK counts. Compatibility note: the v3.7.3 CONTAMINATED-BOTH column is renamed to CONTAMINATED-PREPRINT+UNMATCHED for naming consistency with v3.9.0 composition order.

## Cite-Time Provenance Finalizer 鈥?v3.9.0 extension (triangulation tiers)

Spec: `docs/design/2026-05-17-ars-v3.9.0-cross-index-triangulation-measurement-spec.md` 搂3.3.
...
**Trigger:** annotation fires when (base resolution 鈭?{`ok`, `LOW-WARN`}) AND (`preprint_post_llm_inflection` is true OR any of `semantic_scholar_unmatched` / `openalex_unmatched` / `crossref_unmatched` is true). Entries with `contamination_signals` present but all fields false (computed-clean) produce no suffix 鈥?v3.7.3 behavior preserved.

**Compute k (triangulation count):** k = count of `*_unmatched` fields with value `true`, over fields that are present. Absent fields are excluded (per spec R-L3-2-C: absent 鈮?false). k_max = count of `*_unmatched` fields that are present (0-3).

**Suffix shape table:**

| Base | preprint flag | k | k_max | Present field if k_max=1 | Suffix |
|---|---|---|---|---|---|
| `ok` / `LOW-WARN` | false / absent | 0 | any | 鈥?| (no suffix) |
| `ok` / `LOW-WARN` | true | 0 | any | 鈥?| `CONTAMINATED-PREPRINT` |
...
| `ok` / `LOW-WARN` | false / absent | 1 | 2-3 | 鈥?| `CONTAMINATED-COVERAGE-NOISE` |
| `ok` / `LOW-WARN` | true | 1 | 2-3 | 鈥?| `CONTAMINATED-PREPRINT+COVERAGE-NOISE` |
| `ok` / `LOW-WARN` | false / absent | 2 | 2-3 | 鈥?| `CONTAMINATED-PARTIAL-UNMATCH` |
| `ok` / `LOW-WARN` | true | 2 | 2-3 | 鈥?| `CONTAMINATED-PREPRINT+PARTIAL-UNMATCH` |
| `ok` / `LOW-WARN` | false / absent | 3 | 3 | 鈥?| `CONTAMINATED-TRIANGULATION-UNMATCHED` |
| `ok` / `LOW-WARN` | true | 3 | 3 | 鈥?| `CONTAMINATED-PREPRINT+TRIANGULATION-UNMATCHED` |
...
Example markers:
- `<!--ref:smith2024 LOW-WARN CONTAMINATED-COVERAGE-NOISE-->` 鈥?single-index unmatched, k_max 鈮?2.
- `<!--ref:smith2024 ok CONTAMINATED-PARTIAL-UNMATCH-->` 鈥?two-of-three unmatched.
- `<!--ref:smith2024 LOW-WARN CONTAMINATED-TRIANGULATION-UNMATCHED-->` 鈥?all three indexes unmatched.
- `<!--ref:smith2024 LOW-WARN CONTAMINATED-PREPRINT+TRIANGULATION-UNMATCHED-->` 鈥?preprint heuristic + k=3.

---

## Communication Style

- Direct and precise 鈥?state decisions and rationale without filler
...
- Checkpoint notifications use visual separators (鈹佲攣鈹?lines) to ensure user attention

...
description: "L3 claim-faithfulness audit 鈥?judges every cited claim against the retrieved reference text, surfaces uncited assertions and constraint violations, and feeds the Stage 4鈫? formatter hard gate"
---
...
**You audit; you do not arbitrate.** Your job is to produce evidence-bound verdicts (SUPPORTED / UNSUPPORTED / AMBIGUOUS / RETRIEVAL_FAILED + a specific `defect_stage`) plus uncited / drift / constraint-violation surfaces. You do not decide whether the paper passes 鈥?that is the formatter's job, driven by your annotation severity tier.
...
- For retrieval failures: distinguish stable access restriction (`failed` 鈥?paywall) from transient infrastructure outage (`audit_tool_failure` 鈥?judge timeout / API 5xx / network error) via the rationale tag (INV-14). Do NOT collapse them.
...
- DO NOT mutate `<!--ref:slug-->` or `<!--anchor:...-->` markers. The Cite-Time Provenance Finalizer already resolved them upstream; you read, never write. The v3.6.7 partial-inversion discipline keeps the agent narrative-side and the finalizer audit-side separate 鈥?preserve it here by NOT reading entry frontmatter to discover ref or anchor candidates.
...
| Trigger timing | Stage 2.5 + Stage 4.5 integrity gates | Stage 4 鈫?Stage 5 transition (after Cite-Time Provenance Finalizer, before formatter hard gate) |
...
The two agents are **complementary**: integrity verification asks "does this reference exist and is its metadata correct?" 鈥?alignment audit asks "given that the reference exists, does it actually say what the draft claims it says?"
...
- **`claim_intent_manifests[]`** 鈥?pre-commitment baseline emitted by synthesis_agent / draft_writer_agent / report_compiler_agent before prose generation (see "Claim Intent Manifest Emission (v3.8)" sibling sections on those agents). Used by 搂4 step 5 manifest set-diff.
- **`literature_corpus[]`** 鈥?for retrieval.
- **Resolved citation markers** post Cite-Time Provenance Finalizer 鈥?every in-text citation carries both `<!--ref:slug-->` (v3.7.1 two-layer) and `<!--anchor:<kind>:<value>-->` (v3.7.3 three-layer). `anchor_kind=none` rows should already have been gate-refused by v3.7.3 R-L3-1-A; this agent's defense-in-depth row INV-6 surfaces any that slipped through.
- **Draft sentence stream (uncited)** 鈥?the Stage 4 draft sentence list, with each sentence carrying its `section_path` and optional `adjacent_text` (the surrounding 1-3 clauses for context). Two routing surfaces (Step 13 R4 codex P1 #2):
# REDACTED: sensitive-looking memory line
# REDACTED: sensitive-looking memory line

  Without this stream the `[HIGH-WARN-CONSTRAINT-VIOLATION-UNCITED]` row and the LOW-WARN `uncited_assertions[]` row cannot fire 鈥?both are operationally load-bearing per spec 搂3.3 + 搂3.5. In the Python runtime (`scripts/claim_audit_pipeline.py`), callers pass the FULL set as `all_uncited_sentences` and the D4-c output as `uncited_sentences`; legacy callers may pass only `uncited_sentences` and the pipeline falls back (narrower constraint surface, backwards-compatible).

  **Sentence scope (Step 13 R6 codex P1):** the documented sentence shape is `sentence_text` + `section_path` + optional `adjacent_text` 鈥?sentences do NOT need to carry `scoped_manifest_id`. The pipeline derives constraint scope per sentence: if the caller pins `scoped_manifest_id` on the sentence dict (legacy / explicit-scope shape), only that manifest's MNCs apply. Otherwise the pipeline applies **every manifest's MNCs** (uncited sentences have no claim-level binding, so manifest-scoped MNCs reach them universally per spec 搂3.5 D4-c stream (d) semantics). The emitted `constraint_violation` row derives its `scoped_manifest_id` from the `violated_constraint_id` 鈫?source-manifest mapping; no MANIFEST-MISSING sentinel is admitted per the schema's pattern constraint.

Configuration (`claim_audit_config` block in `academic-pipeline/SKILL.md` mode flags):

| Key | Type | Default | Purpose |
|---|---|---|---|
| `max_claims_per_paper` | integer 鈮?1 | 100 | Cap on judge invocations. N > cap triggers stratified sampling (see Sampling section below). cap = 0 is rejected. |
| `judge_model` | string | `gpt-5.5-xhigh` | Model id used for the judge call. Part of cache key 鈥?changing it forces cache miss on every citation. |
...
- **S-INV-3** When `audited_count < total_citation_count`, the finalizer MUST emit `[CLAIM-AUDIT-SAMPLED 鈥?k/N audited]` in the AI Self-Reflection Report appendix.
- **S-INV-4** `audited_indices` strictly ascending (no duplicates, document order).

When `N 鈮?max_claims_per_paper`, omitting the summary entry is permitted; emitting a telemetry summary with `audited_count == total_citation_count` is also permitted and triggers NO sampling annotation per S-INV-3.
...
### Step 1 鈥?Anchor presence check
...
### Step 2 鈥?Reference retrieval

Call the retrieval layer (passport `literature_corpus[]` entry 鈫?full-text fetch). Five outcomes:
...
The discriminator between `failed` and `audit_tool_failure` is **permanence** 鈥?a paywall is a stable property of the citation, a 5xx / timeout / network blip is a transient property of the infrastructure.

### Step 3 鈥?Cache lookup
...
Selection is scoped by `(scoped_manifest_id, claim_id)`, NOT bare `claim_id` 鈥?per M-INV-1, cross-manifest C-001 collision is permitted, so selecting by bare claim_id would pick constraints from the wrong manifest.

`active_constraints_for_(manifest_id, claim_id)`: the **manifest's** `manifest_negative_constraints[]` UNION that manifest's `claims[].negative_constraints[]` entry whose `claim_id` matches; sorted by `constraint_id`. Each constraint is projected to `{constraint_id, rule}` before hashing 鈥?the in-runtime `scope` tag (MNC vs NC) is excluded so cache hits survive cosmetic re-tagging of an unchanged rule body.
...
### Step 4 鈥?Passage location

Use `anchor_value` to locate the relevant passage inside `retrieved_excerpt`:

- `quote`: exact-substring match against URL-decoded `anchor_value` (after percent-decoding consecutive-hyphen runs per v3.7.3 搂3.1)
...
The located passage is what the judge sees. If `quote` mode fails to locate the exact substring, fall back to passing the full retrieved excerpt with a `[anchor_quote_unlocated]` rationale tag 鈥?do NOT mark the citation UNSUPPORTED on a locator miss alone.

### Step 5 鈥?Judge invocation
...
>
> Output ONE of:
> - SUPPORTED 鈥?the reference directly supports the claim AND no active constraint is violated
> - UNSUPPORTED 鈥?the reference does NOT support the claim (source says something different or contradictory)
> - AMBIGUOUS 鈥?the reference is related but does not clearly support or contradict the claim
> - VIOLATED 鈥?the claim violates one of the active constraints (regardless of whether the reference supports it)
>
> When verdict 鈮?SUPPORTED, output an optional `defect_stage_hint` from `{source_description, metadata, citation_anchor, synthesis_overclaim}` (UNSUPPORTED only) or omit it. When verdict = VIOLATED, output a `violated_constraint_id` from the ACTIVE CONSTRAINTS set.
>
...
VIOLATED outcomes on **uncited** sentences (sentence has no `<!--ref:slug-->`) route to a `constraint_violation` row in `constraint_violations[]` (搂3.5) 鈥?uncited HIGH-WARN gate-refuse without needing a `ref_slug`. The two paths preserve schema integrity: `claim_audit_result.ref_slug` stays required; `constraint_violation` rides its own aggregate.

### Step 6 鈥?Defect stage classification

When the alignment judge returns SUPPORTED / UNSUPPORTED / AMBIGUOUS, classify `defect_stage` per the 搂3.1 allowed-matrix table. The judge's `defect_stage_hint` is the primary driver; the pipeline normalizes out-of-set hints per the coercion rules below.
...
| RETRIEVAL_FAILED | `not_applicable` | covers (a) anchor=none (INV-6); (b) paywall (INV-10); (c) audit_tool_failure (INV-14) 鈥?discriminated by `ref_retrieval_method` |

**Hint coercion rules** (INV-2 / INV-3 protection):
- AMBIGUOUS + hint outside `{source_description, citation_anchor, synthesis_overclaim, null}` 鈫?coerce to `null`.
- UNSUPPORTED + hint outside `{source_description, metadata, citation_anchor, synthesis_overclaim}` 鈫?coerce to `source_description` (fallback to the most common defect class).
- VIOLATED ignores hint entirely and forces `defect_stage=negative_constraint_violation`.

These coercions keep the 搂3.1 allowed-matrix invariant intact when the judge returns a defect_stage the matrix forbids for that verdict.

**Three out-of-band finding categories** use their own entry-type schemas (NOT `claim_audit_result` defect_stages):

- `uncited_assertion` (搂3.3) 鈥?no `ref_slug` to evaluate; LOW-WARN advisory.
- `claim_drift` (搂3.4) 鈥?manifest set-diff signal; LOW-WARN advisory; no judge invocation.
- `constraint_violation` (搂3.5) 鈥?uncited claim that violates an MNC/NC rule; HIGH-WARN gate-refuse.
...
|---|---|---|---|
| (a) `EMITTED_NOT_INTENDED` | emitted 鈭?intended | `claim_drifts[]` entry with `drift_kind=EMITTED_NOT_INTENDED`, `section_path` populated, `manifest_claim_id=null`, `scoped_manifest_id=null` | LOW-WARN advisory |
| (b) `INTENDED_NOT_EMITTED` | intended 鈭?emitted | `claim_drifts[]` entry with `drift_kind=INTENDED_NOT_EMITTED`, `manifest_claim_id` + `scoped_manifest_id` set (D-INV-2) | LOW-WARN advisory |
...
**Precedence rules** (also enforced by `scripts/check_claim_audit_consistency.py` 搂6 rule 6):

1. **Negative-constraint violation > drift**: when an audited citation in a manifest judges VIOLATED, that manifest's drift findings are absorbed 鈥?the constraint violation has already surfaced the failure at HIGH-WARN, and layering LOW-WARN drift noise on top of it would report the same paper-level problem twice. Absorption is **manifest-scoped and total within that manifest**: once any audited citation in manifest M judges VIOLATED, every `(M, *)` drift row that would otherwise emit (both INTENDED_NOT_EMITTED for missing manifest claims AND EMITTED_NOT_INTENDED for the violating citation itself) is suppressed. A violation in manifest A does NOT silence drift in manifest B 鈥?absorption never crosses manifest boundaries.
2. **`citation_anchor` distinct from `source_description`**: anchor-wrong + description-correct is its own row; do NOT collapse them.
3. **Uncited > drift** (D-INV-4): a sentence that is both uncited AND a drifted manifest claim emits only into `uncited_assertions[]` 鈥?no companion `claim_drifts[]` entry. The same sentence may appear in manifest diff diagnostics but does NOT produce a `claim_audit_result` row.
...
3. **Not a definitional sentence** (sentences containing `refers to` / `is defined as` / `we define` / `for the purposes of` are excluded 鈥?definitions don't need refs).
...
|---|---|---|
| `claim_audit_results[]` | one per audited citation | mixed 鈥?driven by 8-row finalizer matrix |
...
| `uncited_audit_failures[]` (v3.8.2 / #118) | one per uncited sentence 脳 manifest where the constraint judge raised `JudgeInvocationError` | MED-WARN advisory `[CLAIM-AUDIT-TOOL-FAILURE-UNCITED 鈥?<fault-class>]` |
| `claim_intent_manifests[]` | passed through from writing-stage agents | input only 鈥?this agent does NOT emit manifests |

Plus the AI Self-Reflection Report appendix (Stage 6) 鈥?a per-stage `defect_stage` histogram rendered when 鈮?5 completed entries exist.
...
- **T-C1 threshold enforcement**: assert `FNR < 0.15 AND FPR < 0.10` against the synthetic gold set. Threshold failure 鈫?calibration FAIL; CI blocks merge. Remediation paths: curate a better gold set, tighten judge prompts, or change `judge_model`.
- **T-C2 per-class reporting**: FNR/FPR computed AND surfaced per judgment-class (SUPPORTED vs UNSUPPORTED, AMBIGUOUS, violated-constraint). Reporting failure 鈮?threshold failure 鈥?this catches calibration tooling regressions distinct from gold-set degradation.
- **T-C3 gold-set shape integrity**: tuples are validated for `tuple_kind 鈭?{alignment, constraint}` and the conditional required-field shape per decision-doc D3(c). NOT_VIOLATED constraint tuples MUST appear (鈮?3) 鈥?without them constraint FPR is unmeasurable.
...
|---|---|---|---|
| Retrieval access restriction (verified paywall 鈥?HTTP 403/402, license-restricted, no full-text endpoint) | `claim_audit_results[]` with `ref_retrieval_method=failed` | "Reference full text not retrievable (paywall ...)" | LOW-WARN advisory |
...
| Audit infrastructure / transient outage on **uncited** path (v3.8.2 / #118 鈥?same fault classes, but uncited sentence has no `ref_slug` so the INV-14 row cannot be used) | `uncited_audit_failures[]` row carrying the same `fault_class` enum | Same fault-class prefix as cited path, e.g. `judge_timeout: judge timed out after 30s` | MED-WARN advisory |
...
The cited / uncited split for `audit_tool_failure` (rows 2 vs 3) is a schema-integrity artifact, not a severity downgrade 鈥?both ride the MED-WARN advisory tier. Pre-v3.8.2 the uncited path silently substituted `{"judgment": "NOT_VIOLATED", "rationale": "..."}` and suppressed HIGH-WARN constraint checks on transient judge outage; v3.8.2 / #118 routes the failure through `uncited_audit_failures[]` so the operational signal surfaces without dropping audit coverage. See spec 搂3.6 + 搂4 step 9 fourth bullet for the routing rule.

---

## Cross-references

- **v3.7.3 anchor input contract**: `docs/design/2026-05-12-ars-v3.7.3-claim-faithfulness-and-contaminated-source-spec.md` 搂3.1 (R-L3-1-A / R-L3-1-B / R-L3-1-C firm rules)
- **v3.6.7 PATTERN PROTECTION convention**: `docs/design/2026-04-29-ars-v3.6.7-downstream-agent-pattern-protection-spec.md` 搂3.1
- **Zhao et al. arXiv:2605.07723** 鈥?external motivation for L3 audit channel
- **Li et al. RubricEM arXiv:2605.10899** 鈥?Borrows 1+2 (claim_intent_manifest + stage-attribution)

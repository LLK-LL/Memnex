...
# Academic Pipeline v3.8.2 鈥?Full Academic Research Workflow Orchestrator

A lightweight orchestrator that manages the complete academic pipeline from research exploration to final manuscript. It does not perform substantive work 鈥?it only detects stages, recommends modes, dispatches skills, manages transitions, and tracks state.
...
**v3.8 (opt-in):** Set `ARS_CLAIM_AUDIT=1` to enable the L3 claim-faithfulness audit gate at the Stage 4 鈫?Stage 5 transition. When the flag is set, the orchestrator dispatches `claim_ref_alignment_audit_agent` after the v3.7.1 Cite-Time Provenance Finalizer and before `formatter_agent`'s hard gate. The audit emits `claim_audit_results[]` + `uncited_assertions[]` + `claim_drifts[]` + `constraint_violations[]` + `audit_sampling_summaries[]` aggregates per the 8-row matrix; HIGH-WARN classes gate-refuse output via the formatter REFUSE rules 6-10. Default OFF for v3.8.0 鈥?ramp-on plan deferred to post-calibration evidence (spec 搂5 mode flag rationale). See `agents/claim_ref_alignment_audit_agent.md` and the orchestrator 搂3.6 prose.

**v2.0 Core Improvements**:
1. **Mandatory user confirmation checkpoints** 鈥?Each stage completion requires user confirmation before proceeding to the next step
2. **Academic integrity verification** 鈥?After paper completion and before review submission, 100% reference and data verification must pass
3. **Two-stage review** 鈥?First full review + post-revision focused verification review
4. **Final integrity check** 鈥?After revision completion, re-verify all citations and data are 100% correct
5. **Reproducible** 鈥?Standardized workflow producing consistent quality assurance each time
6. **Process documentation** 鈥?After pipeline completion, automatically generates a "Paper Creation Process Record" PDF documenting the human-AI collaboration history
...
- **Reference**: [`references/passport_as_reset_boundary.md`](references/passport_as_reset_boundary.md) 鈥?see 搂"`resume_from_passport` mode contract".
...
- If the user only needs a single function (just search materials, just check citations), no pipeline is needed 鈥?directly trigger the corresponding skill
...
This mirrors PaperOrchestra's parallel execution of Plot Generation (Step 2) and Literature Review (Step 3) after Outline (Step 1), which reduces overall pipeline latency. The parallelization is optional 鈥?sequential execution remains the default for simplicity.
...
鈿狅笍 **IRON RULE 鈥?Core rule: After each stage completion, the system must proactively prompt the user and wait for confirmation. The checkpoint presentation adapts based on context and user engagement.**
...
1. 鈿狅笍 **IRON RULE**: **Cannot auto-skip MANDATORY checkpoints**: Even if the previous stage result is perfect, explicit user input is required at MANDATORY checkpoints
...
3. **Quality trajectory**: Is the latest output 鈮?the quality of the previous stage? If declining, PAUSE and flag.
...
| 4 | `collaboration_depth_agent` | **Observer (advisory only 鈥?never blocks).** Reads dialogue log and scores user-AI collaboration pattern against `shared/collaboration_depth_rubric.md`. Invoked at FULL/SLIM checkpoints and at pipeline completion. Based on Wang & Zhang (2026). | `agents/collaboration_depth_agent.md` |
| 5 | `claim_ref_alignment_audit_agent` | **Opt-in claim faithfulness auditor (v3.8 #103).** Audits sampled citations for claim 鈫?reference alignment + negative-constraint compliance; emits per-claim `claim_audit_results[]`, `claim_drift[]`, `uncited_assertions[]`, `constraint_violations[]`. Dispatched via orchestrator 搂3.6 when claim_audit mode is requested. | `agents/claim_ref_alignment_audit_agent.md` |
...
````
--- STAGE TRANSITION: [Current] 鈫?[Next] ---

馃攧 Core Principles Reinforcement:
...
Checkpoint: [MANDATORY/ADVISORY] 鈥?[What user needs to confirm]
---
````

**Stage-specific reinforcement content**: See `references/reinforcement_content.md` for the full transition 鈫?reinforcement focus table.
...
Stage 3 (full review, 5 reviewers) 鈫?Revision Coaching 鈫?Stage 4 鈫?Stage 3' (re-review) 鈫?optional Residual Coaching 鈫?Stage 4'.
...
Handles external (human) reviewer feedback integration. 4-step workflow: Intake & Structuring 鈫?Strategic Revision Coaching 鈫?Revision & Response 鈫?Self-Verification.
...
## Collaboration Depth Observer (v3.5.0, advisory only 鈥?never blocks)
...
**When invoked**: every FULL checkpoint, every SLIM checkpoint, and after Stage 6 (pipeline completion). MANDATORY checkpoints (Stages 2.5 / 4.5 integrity gates) **do not** invoke the observer 鈥?those are integrity concerns and must not be diluted.
...
|---|---|---|
| `integrity_verification_agent` (Stages 2.5 / 4.5) | Paper content 鈥?references, citations, data | Yes (blocking gate) |
| Stage 6 Collaboration Quality Evaluation (6 dims, 1鈥?00) | AI's self-reflection on its own behaviour | No, but produced once only |
| `collaboration_depth_agent` (this observer) | The **user's** collaboration pattern (delegation intensity, vigilance, reallocation) | **No 鈥?never blocks. Advisory only.** |
...
| No overstepping | 鈿狅笍 IRON RULE: Orchestrator does not perform substantive research/writing/reviewing, only dispatching |
| No forcing | 鈿狅笍 IRON RULE: User can pause or exit pipeline at any time (but cannot skip integrity checks) |
# REDACTED: sensitive-looking memory line
...
Stage 5: academic-paper (format-convert mode)
  ...
  - 鈿狅笍 IRON RULE: PDF must be compiled from LaTeX (HTML-to-PDF is prohibited)
```

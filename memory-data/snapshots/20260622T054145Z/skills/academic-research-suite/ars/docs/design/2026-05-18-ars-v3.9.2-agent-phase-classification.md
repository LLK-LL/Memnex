# ARS v3.9.2 鈥?38-Agent Phase Classification Table
...
**Purpose:** Canonical agent 鈫?phase classification used to drive Phase 1 prompt fence sweep. Per v4 design opus HIGH-1: 4-bucket model + per-agent out-of-scope inflation risk column.

## 4-Bucket schema

| Bucket | Definition | v3.9.2 treatment |
|---|---|---|
| **A 鈥?Single-phase** | Operates at exactly one numbered pipeline phase per invocation | Hard fence prompt block added |
| **B 鈥?Multi-phase** | Operates at 2+ numbered phases across invocations | NO fence added (honest framing 鈥?placebo prose creates false-enforcement illusion); recurrence acknowledged, v3.10 envelope is the fix |
| **C 鈥?Phase-orthogonal** | Operates outside numbered phase axis (Socratic Layers, Stage gates, cross-skill, advisory) | NO fence added; classification doc notes latent inflation risk class |
| **D 鈥?Cross-phase / meta** | Legitimate cross-phase role by design (orchestrator, intake, observer) | NO fence added (legitimate by design) |
...
| `bibliography_agent` | A | Phase 2 | hard-fence | (this is #133's case agent 鈥?main target) | deep-research/SKILL.md row 3 |
...
| `report_compiler_agent` | B | Phase 4, 6 | none-unmitigated | scope inflation across Phase 4鈫? invocations; v3.10 envelope is fix | deep-research/SKILL.md row 6 |
...
| `literature_strategist_agent` | A | Phase 1 (academic-paper, 鈮?deep-research Phase 2 work) | hard-fence (audit existing v3.6.5 partial language) | none specific | academic-paper/SKILL.md row 2 |
| `structure_architect_agent` | A | Phase 2 | hard-fence | none specific | academic-paper/SKILL.md row 3 |
| `argument_builder_agent` | B | Phase 3 + Plan Step 3 | none-unmitigated | scope inflation Phase 3鈫擯lan Step; v3.10 envelope is fix | academic-paper/SKILL.md row 4 |
...
| `socratic_mentor_agent` | C | Plan Step 0-3 (phase-orthogonal 鈥?Plan mode, not numbered pipeline) | not-applicable | latent: direct-conclusion inflation; same class as deep-research socratic_mentor (different prompt body) | academic-paper/SKILL.md row 10 |
| `visualization_agent` | B | Phase 4 + Phase 7 | none-unmitigated | scope inflation Phase 4鈫?; v3.10 envelope is fix | academic-paper/SKILL.md row 11 |
...
| `devils_advocate_reviewer_agent` | A | Phase 1 (reviewer skill 鈥?NOT same as deep-research devils_advocate_agent) | hard-fence | none specific | academic-paper-reviewer/SKILL.md row 6 |
...
| C (phase-orthogonal, no fence) | **8** | socratic_mentor 脳 2 (deep-research + academic-paper), monitoring, revision_coach, integrity_verification, collaboration_depth, claim_ref_alignment_audit, compliance |
...
**Final totals:** A=23 + B=4 + C=8 (incl. 2 脳 socratic_mentor) + D=4 = **39 records** total / **38 unique frontmatter `name`** values (socratic_mentor counted once at name-level; counted twice at file-level).
...
1. **Multi-phase scope inflation** (Bucket B, 4 agents): report_compiler, devils_advocate (deep-research), argument_builder, visualization 鈥?recurrence expected; v3.10 envelope is fix
2. **Socratic direct-conclusion inflation** (socratic_mentor 脳 2): different failure class from #133 (not silent scope expansion but Socratic discipline failure)
# REDACTED: sensitive-looking memory line

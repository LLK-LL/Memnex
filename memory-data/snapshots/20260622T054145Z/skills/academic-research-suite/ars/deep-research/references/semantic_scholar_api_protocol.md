...
PaperOrchestra (Song et al., 2026) demonstrated that a two-phase citation pipeline 鈥?(1) broad discovery via web search, (2) sequential verification via Semantic Scholar API 鈥?achieves significantly higher citation coverage (P0 Recall +2-6%, P1 Recall +12-14% over baselines). ARS adopts the verification phase as an additional tier in the existing multi-tier verification strategy.
...
**Matching rule**: DOI match is exact. Cross-check that returned title matches the reference title (Levenshtein >= 0.70). If title mismatch despite DOI match, flag as `DOI_MISMATCH` 鈥?a known hallucination pattern where a fabricated DOI resolves to an unrelated paper.
...
- `S2_NOT_FOUND` does NOT automatically mean fabrication 鈥?the paper may exist but not be indexed in Semantic Scholar (e.g., very recent, non-English, grey literature)
...
- **Never block the pipeline on S2 API failure** 鈥?graceful degradation to existing WebSearch-only verification
...
- Song, Y., Song, Y., Pfister, T., & Yoon, J. (2026). PaperOrchestra: A Multi-Agent Framework for Automated AI Research Paper Writing. *arXiv preprint arXiv:2604.05018*. 鈥?Section 4 Step 3 (Literature Review Agent), Appendix D.3 (Citation Verification).

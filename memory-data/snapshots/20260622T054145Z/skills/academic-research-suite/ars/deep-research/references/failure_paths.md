# Failure Paths 鈥?Research Pipeline Failure Path Map
...
> I notice we've been discussing for a while, but the research question hasn't converged to a clear direction yet. This is perfectly normal 鈥?sometimes the question itself is the hardest part. Let me offer a few possible directions to see which one is closest to your thinking.
...
4. If the user still cannot choose 鈫?suggest doing a `lit-review` mode to explore the literature first, then return

**Recovery Paths**:
- Select a candidate RQ 鈫?continue the original workflow
- Do lit-review 鈫?restart RQ clarification after the literature review is complete
- User redescribes on their own 鈫?restart Phase 1 / Layer 1
...
5. If still insufficient 鈫?suggest the user consider adjusting the RQ or accept this as an exploratory study

**Recovery Paths**:
- Expanded search yields sufficient literature 鈫?continue original workflow
- Accept as exploratory research 鈫?adjust report positioning, emphasize the study's pioneering nature
- Adjust RQ 鈫?return to Phase 1
...
**User Notification Message**:
> Devil's Advocate found an important issue in the methodology review: your research question asks "why," but your method design can only answer "whether." Let's go back and adjust 鈥?here are three possible directions...
...
**Recovery Paths**:
- Select an alternative method 鈫?regenerate Methodology Blueprint 鈫?Checkpoint 1 re-review
- Adjust RQ 鈫?return to research_question_agent 鈫?redo Phase 1
- Maximum 2 retries; if still mismatched on the 3rd attempt 鈫?suggest the user consult their advisor
...
**User Notification Message**:
> STOP 鈥?Devil's Advocate found a critical issue that must be resolved before continuing:
...
4. After user correction 鈫?re-execute the Checkpoint
5. 2 consecutive CRITICALs 鈫?suggest the user fundamentally rethink the research direction

**Recovery Paths**:
- User corrects the issue 鈫?re-execute Checkpoint 鈫?continue after PASS
- User chooses to modify the RQ/method 鈫?return to the corresponding Phase
- User abandons the direction 鈫?enter F7 workflow
...
3. Remediable issues 鈫?provide modification suggestions 鈫?re-review after user confirmation
4. Irremediable issues 鈫?suggest fundamental redesign of the research

**Recovery Paths**:
- Fix ethical issues 鈫?re-execute ethics_review_agent 鈫?continue after CLEARED
- Modify research design 鈫?return to Phase 1
- Issues are irremediable 鈫?suggest abandoning this research direction, provide alternative direction suggestions
...
**User Notification Message**:
> We've explored several directions, each with its own value. However, I notice we've been going back and forth between a few directions 鈥?this may mean you have multifaceted interests in this topic, but it can also make the research hard to focus. Would you like to:
...
4. If user chooses to continue but still hasn't converged by round 15 鈫?auto-compile + end

**Recovery Paths**:
- Continue with focus 鈫?restrict discussion scope, converge within 5 rounds
- Switch to full mode 鈫?pass extracted INSIGHTs to research_question_agent
- Pause 鈫?save INSIGHT list; user can re-enter at any time
...
**Recovery Paths**:
- User says "continue the previous research" 鈫?load saved outputs, continue from where interrupted
- User starts over 鈫?entirely new workflow
...
4. If the user needs an English report 鈫?provide suggestions for English citation format of Chinese literature
5. If the user needs to publish internationally 鈫?suggest finding comparable international cases

**Recovery Paths**:
- Chinese literature is sufficient 鈫?continue workflow with clear language annotations
- User needs international publication 鈫?suggest adjusting RQ to add a comparative perspective
...
**Recovery Paths**:
- Find sufficient alternative sources 鈫?continue workflow with clear quality annotations
- Cannot find qualified sources 鈫?suggest the user consider conducting primary research
...
**Recovery Paths**:
- Revision successful 鈫?complete Phase 6
- Issues remain after revision 鈫?2nd revision round
- Issues remain after 2 revisions 鈫?convert issues to a "Research Limitations" section
...
**Recovery Paths**:
- User accepts 鈫?deliver the report
- User does not accept 鈫?suggest redesigning the research from Phase 1
...
**Recovery Paths**:
- Focus on a single framework 鈫?continue workflow
- User insists on interdisciplinary 鈫?suggest switching to mixed-methods or narrative review

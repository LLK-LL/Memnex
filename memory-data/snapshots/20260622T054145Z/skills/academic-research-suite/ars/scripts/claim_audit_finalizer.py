"""ARS v3.8 claim-faithfulness finalizer 鈥?8-row matrix annotations + Stage 6 histogram.

Implements the orchestrator `搂3.6 Claim-Faithfulness Audit Gate (v3.8)` matrix
table in `pipeline_orchestrator_agent.md`, plus the Stage 6 reflection
report histogram (renders when 鈮? completed audit entries exist).

The module is invoked by the orchestrator at the Stage 4 鈫?Stage 5 boundary,
...
Spec:
  docs/design/2026-05-15-issue-103-claim-alignment-audit-spec.md 搂5 (8-row
  matrix), 搂"Manifest cross-reference (D6)" (drift / constraint routing),
  搂3.6 (orchestrator prose).

This module deliberately does NO file I/O 鈥?it operates on Python dicts
...
# ---------------------------------------------------------------------------
# Severity tiers (per spec 搂5 finalizer matrix).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Annotation literals (canonical wording per spec 搂5 table + 搂"uncited" /
# 搂"constraint_violation" / 搂"claim_drifts" subsections + 搂"audit_sampling").
#
...
ANNOTATION_HIGH_WARN_ANCHORLESS = (
    "[HIGH-WARN-CLAIM-AUDIT-ANCHORLESS 鈥?v3.7.3 R-L3-1-A VIOLATION REACHED AUDIT]"
)
ANNOTATION_LOW_WARN_UNVERIFIED = (
    "[CLAIM-AUDIT-UNVERIFIED 鈥?REFERENCE FULL-TEXT NOT RETRIEVABLE]"
)
ANNOTATION_MED_WARN_TOOL_FAILURE = "[CLAIM-AUDIT-TOOL-FAILURE 鈥?{fault_class}]"
# v3.8.2 / #118 鈥?UAF aggregate annotation. Same fault-class enum as the
# cited-path INV-14 row but routed through `uncited_audit_failures[]`
# because claim_audit_result.ref_slug is required.
ANNOTATION_MED_WARN_TOOL_FAILURE_UNCITED = "[CLAIM-AUDIT-TOOL-FAILURE-UNCITED 鈥?{fault_class}]"
...
ANNOTATION_LOW_WARN_CLAIM_DRIFT = "[LOW-WARN-CLAIM-DRIFT 鈥?kind={drift_kind}]"
ANNOTATION_SAMPLING = (
    "[CLAIM-AUDIT-SAMPLED 鈥?{audited_count}/{total_citation_count} audited]"
)

ANNOTATION_MANIFEST_MISSING = (
    "[CLAIM-AUDIT-MANIFEST-MISSING 鈥?audit ran without pre-commitment baseline]"
)

# Set of annotation prefixes that `/ars-mark-read` CANNOT clear 鈥?structural
...
# Rationale prefix regex for audit_tool_failure rows 鈥?the fault-class tag is
...
def _classify_retrieval_failed(
    *,
    ...
    covered by the 搂5 matrix rows. The spec 搂6 consistency lint
# REDACTED: sensitive-looking memory line
    鈥?it rejects malformed rows BEFORE they reach the finalizer. A raise
...
def classify_claim_audit_result(entry: dict[str, Any]) -> dict[str, Any]:
    """Apply the 搂5 8-row matrix to a single claim_audit_result row.
    ...
    if judgment == "AMBIGUOUS":
        # Spec 搂3.1 INV-3 permits {source_description, citation_anchor,
        # synthesis_overclaim, null} on AMBIGUOUS 鈥?the 搂6 consistency lint is
# REDACTED: sensitive-looking memory line
        ...
        # them back 鈥?dead code per Step 8 codex /simplify advisory.
        # Validation stays the lint's responsibility (搂6 rule 5); this branch
    ...
    if judgment == "UNSUPPORTED":
        ...
        raise ValueError(
            ...
            "spec 搂3.1 INV-2 permits source_description / metadata / citation_anchor / "
...
def classify_uncited_audit_failure(entry: dict[str, Any]) -> dict[str, Any]:
    ...
    `[CLAIM-AUDIT-TOOL-FAILURE-UNCITED 鈥?<fault-class>]` next to the
    offending sentence. Gate passes 鈥?retry-next-pass remediation.
    ...
    and explicit-null (`"fault_class": null`) cases 鈥?without it a
    malformed row with explicit null would render as `[...鈥?None]`
...
def apply_finalizer(passport: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    Returns:
        {
            ...
            "gate_refuse": bool 鈥?True if any row has gate_refuse=True,
            "gate_refuse_reasons": list[str] 鈥?annotations that triggered refuse,
        }
    """
    ...
    routing: tuple[tuple[str, Any], ...] = (
        ...
        # v3.8.2 / #118 鈥?UAF aggregate routes to MED-WARN advisory.
        ...
        # formatter never sees the [CLAIM-AUDIT-TOOL-FAILURE-UNCITED 鈥?...]
    ...
    # MANIFEST-MISSING paper-level advisory (spec 搂9 acceptance criterion;
    # Step 8 codex R1 P2 closure). Fires when the audit ran without a
    # pre-commitment baseline 鈥?both `claim_intent_manifests[]` is empty
...
def render_stage6_histogram(
    ...
    *,
    ...
    Threshold is on `audit_status == "completed"` rows (per spec 搂"Outputs
    feeding Stage 6 self-reflection" literal "鈮?5 completed entries"). When
    ...
    SUPPORTED (zero defects to plot), the histogram still emits 鈥?it
    ...
    paper had 鈮? completed audits but 鈮? defect_stage entries. Spec
    literal is "鈮?5 completed entries"; common mostly-SUPPORTED papers
...
def ars_mark_read_clears(*, annotation: str, tier: str) -> bool:
    ...
    LOW-WARN paywall / advisory rows CAN 鈥?the user has accepted the
    ...
    # trigger this branch 鈥?every HIGH-WARN prefix is produced only by

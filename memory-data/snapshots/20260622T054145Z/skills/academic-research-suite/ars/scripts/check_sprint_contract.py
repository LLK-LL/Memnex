...
def check_structural_invariants(contract: dict) -> list[str]:
    ...
    per spec 搂4.1 item 2. Empty list means pass; only meaningful if
...
def warn_suspicious(contract: dict, ars_current_version: str | None) -> list[str]:
    """Soft warnings per spec 搂4.3 (SC-1 baseline lag through SC-11
...
# REDACTED: sensitive-looking memory line
    ...
    # SC-5 covers that semantic gap. Reviewer-only per v3.6.6 搂7.1: writer / evaluator
    # contracts intentionally omit measurement_procedure (搂3.3.1 reviewer-conditional),
    ...
    # Skip entries with missing severity 鈥?schema validation catches those upstream.
    ...
    # The paraphrase-count gate is mode-conditional per v3.6.6 搂7.1: each mode reads its
    ...
    # all three modes 鈥?only the source field changes.
    if mode.startswith("reviewer_"):
...
# REDACTED: sensitive-looking memory line
    ...
    # spec 搂5.5 recognised expression vocabulary patterns 1-3.
    ...
    # SC-11 panel_size sanity. Reviewer-only per v3.6.6 搂7.1: writer / evaluator
    # contracts intentionally omit panel_size (搂3.3.5 reviewer-conditional). Each runs a

...
Tests the two pure-function signals defined in v3.7.3 spec 搂3.2 that
...
closed list (per spec 搂3.2 Vector 1 + schema description).

Signal 2 (semantic_scholar_unmatched): SS API lookup with manual exemption
+ API degradation handling (per spec 搂3.2 Vector 2 + R-L3-2-B).
...
class PreprintSignalTest(unittest.TestCase):
    """Signal 1 per v3.7.3 spec 搂3.2 Vector 1: True iff year >= 2024 AND
    venue 鈭?10-server closed list."""
    ...
    def test_source_pointer_ignored_when_venue_present(self) -> None:
        """Explicit `venue` field takes precedence 鈥?Nature 2024 with an
...
class SemanticScholarSignalTest(unittest.TestCase):
    """Signal 2 per v3.7.3 spec 搂3.2 Vector 2: SS API lookup. Returns None
    on (a) manual exemption (b) API degradation; True/False otherwise.

    The SS client is dependency-injected 鈥?tests use a MagicMock that
...
# REDACTED: sensitive-looking memory line
    ...
    def test_manual_entry_returns_none(self) -> None:
        """v3.7.3 spec 搂3.2 + schema allOf rule #4: obtained_via='manual'
        triggers SKIP 鈥?the field MUST be omitted from the emission."""
        client = MagicMock()
    ...
    def test_ss_api_degradation_returns_none(self) -> None:
        """Per spec 搂3.2: API unreachable (network failure / rate-limit-
        exhausted / 5xx) returns None. Absence 鈮?negative confirmation;
...
# ============================================================================
# Emission rules 鈥?building the contamination_signals object
# ============================================================================
class EmissionRulesTest(unittest.TestCase):
    """Per spec 搂3.2 emission rules: emit object with both fields false
    ...
    def test_api_degradation_omits_unmatched_field(self) -> None:
        ...
        year+venue), Signal 2 omitted (API down). Per spec 搂3.2."""
        client = MagicMock()
...
# ============================================================================
# v3.9.0 鈥?resolve_openalex_unmatched
# ============================================================================


class ResolveOpenAlexUnmatchedTest(unittest.TestCase):
    """Tests for resolve_openalex_unmatched per spec v3.9.0 搂3.4."""

    def test_match(self):
        """OpenAlex hit via DOI cross-check 鈫?False."""
    ...
    def test_no_match(self):
        """OpenAlex no DOI hit + no title hit 鈫?True."""
        from contamination_signals import resolve_openalex_unmatched
    ...
    def test_manual_exempt(self):
        """obtained_via='manual' 鈫?return None, no client calls."""
        from contamination_signals import resolve_openalex_unmatched
    ...
    def test_api_down_raises(self):
        """API degraded 鈫?re-raise OpenAlexUnavailable for caller to omit field."""
        from contamination_signals import resolve_openalex_unmatched
...
# ============================================================================
# v3.9.0 鈥?resolve_crossref_unmatched
# ============================================================================


class ResolveCrossrefUnmatchedTest(unittest.TestCase):
    """Tests for resolve_crossref_unmatched per spec v3.9.0 搂3.5."""

    def test_match(self):
        """Crossref hit via DOI cross-check 鈫?False."""
    ...
    def test_no_match(self):
        """Crossref no DOI hit + no title hit 鈫?True."""
        from contamination_signals import resolve_crossref_unmatched
    ...
    def test_api_down_raises(self):
        """API degraded 鈫?re-raise CrossrefUnavailable for caller to omit field."""
        from contamination_signals import resolve_crossref_unmatched

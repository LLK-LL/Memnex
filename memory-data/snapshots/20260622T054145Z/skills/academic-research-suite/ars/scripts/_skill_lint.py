...
class FrontmatterError(Exception):
    ...
    parser detail on stdout, not stderr 鈥?CI logs commonly capture
    only stdout.
    """
...
def parse_frontmatter(path: Path) -> dict | None:
    ...
    Three outcomes:
      - dict (possibly empty) 鈥?fence present and parseable
      - None                  鈥?no opening '---' fence
      - raises FrontmatterError 鈥?fence present but YAML invalid
    """

...
drift hole 鈥?a spec bump that updates the lint without updating the
runtime would change one side silently. Tests cover both call sites; the
shared import binds them.

See docs/design/2026-05-15-issue-103-claim-alignment-audit-spec.md 搂3.1
(matrix + INV catalogue) and 搂4 step 3 (sampling) for canonical
definitions.
"""
...
# Canonical sentinel for the MANIFEST-MISSING fallback path (spec 搂3.1 INV-15).
...
# and require re-lint per spec 搂3.3 / 搂3.4 / 搂3.5.
UNCITED_RULE_VERSION = "D4-c-v1"
DRIFT_RULE_VERSION = "D4-a-v1"
# v3.8.2 / #118 鈥?uncited_audit_failure rule_version literal (搂3.6).
...
# Constraint id parse rules (spec 搂3.2 + INV-17 canonical form).
...
# ---------------------------------------------------------------------------
# D4-c uncited-assertion detector constants (spec 搂"Uncited-assertion
...
# of truth 鈥?a spec bump touches one literal, not three.
# ---------------------------------------------------------------------------
...
# classes 鈥?`numbers / percentages / explicit quantifiers (50%, 67 of 100)`.
...
# identifies them as years, version triples, or section numbers 鈥?none of
...
#   2. Version triple `X.Y.Z` (dotted form with 3+ segments 鈥?the broad
#      regex captures only the first two segments, so we re-scan).
#   3. Dotted section number `X.Y[.Z鈥` (treated as section ref, not
#      quantifier). Distinguished from version by `section` / `搂` /
...
# Requires exactly one digit-run + `.` ending the window 鈥?combined with
...
# WITHIN the v3.7.3 ref-marker namespace 鈥?accepts any `<!--ref:...-->`
...
#
# Iteration history:
#   R0 `[^-]+`                                鈥?rejected hyphenated slugs.
# REDACTED: sensitive-looking memory line
#                                                 slugs, plus-sign slugs,
# REDACTED: sensitive-looking memory line
#   R2 `[^>]*?`                                鈥?too broad; matched
#                                                 `<!-- ref: $analysis -->`
#                                                 and similar code/internal
#                                                 ref comments (codex
#                                                 R3 P2-NEW-A).
#   R3 `[^\s>][^>]*?`                          鈥?current. Slug must begin

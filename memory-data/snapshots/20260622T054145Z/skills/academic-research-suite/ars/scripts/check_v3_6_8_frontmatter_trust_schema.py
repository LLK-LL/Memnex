#!/usr/bin/env python3
"""ARS v3.7.1 trust-chain frontmatter lint (spec 搂 3.1 firm rules).

Spec: docs/design/2026-04-30-ars-v3.6.8-trust-provenance-and-drift-transparency-spec.md
      搂 3.1 D1 鈥?Frontmatter trust-strength conflation
      搂 Step 1 鈥?Frontmatter schema split
...
passport (or, with --fixture, against an arbitrary YAML / JSON file). Rules:
  ...
  Rule #1 (round-2 R2-007 amend):
      ...
      鉄?source_acquired=true
        AND source_verification_method 鈭?{codex_audit, manual_grep, vision_check}
  ...
  Rule #2:
      ...
      鉄?description_last_audit is null or the literal string 'none'.
...
runs over passport corpora 鈥?a level above per-entry schema validation.
...
# Per spec 搂3.6 firm rule #1 + 搂3.1 firm rule #3: literal `human_read_*`
    ...
    for key in FORBIDDEN_HUMAN_READ_KEYS:
        if key in entry:
            errors.append(
                ...
                f"entry. Per spec 搂3.1 firm rule #3 + 搂3.6 firm rule #1, "
                f"{key!r} is DERIVED from the 搂3.6 peer file "
# REDACTED: sensitive-looking memory line
    ...
    if verified is True:
        ...
        if acquired is not True:
            errors.append(
                ...
                f"Spec 搂3.1 firm rule #1: cannot claim verification against an "
                f"original that is not on disk."
            )
        if method is None:
            errors.append(
                f"  [{label}] Rule #1 violated: source_verified_against_original=true "
                f"REQUIRES source_verification_method 鈭?"
                f"{{codex_audit, manual_grep, vision_check}} (got missing field). "
                f"Spec 搂3.1 firm rule #1 round-2 R2-007 amend."
            )
            ...
            errors.append(
                ...
                f"Spec 搂3.1 firm rule #1 round-2 R2-007 amend: 'none' is "
                f"enumerated for shape uniformity but is FORBIDDEN here."
            )

    # Rule #2: source_acquired=false 鈬?description_last_audit MUST be present
    ...
    # sentinel "none" only 鈥?null is rejected 鈥?because spec 搂3.1 firm rule #2
    # reads "REQUIRES description_last_audit: none" (literal sentinel), and the
    # spec's value vocabulary at 搂3.1 line 111 lists `<round_id> | none` with
    # no null alternative.
    if entry.get("source_acquired") is False:
        if "description_last_audit" not in entry:
            errors.append(
                ...
                f"The field is missing. Spec 搂3.1 firm rule #2: no original "
            ...
            if last_audit != "none":
                errors.append(
                    f"  [{label}] Rule #2 violated: source_acquired=false REQUIRES "
                    f"description_last_audit: 'none' 鈥?the literal sentinel "
                    f"string (got {last_audit!r}). Spec 搂3.1 firm rule #2 + "
                    f"搂3.1 yaml at line 111 (value vocabulary `<round_id> | "
                    f"none`) 鈥?null is NOT an accepted alternative for the "

...
Spec: docs/design/2026-05-12-ars-v3.7.3-claim-faithfulness-and-contaminated-source-spec.md 搂3.1
...
def test_invalid_anchor_kind_fails(tmp_path: Path):
    ...
    # orphan-anchor (because the regex doesn't match) 鈥?either way it's
...
def test_nested_html_comment_marker_in_quote_value_handled(tmp_path: Path):
    ...
    invariant 鈥?if F1 hyphen rule holds, the value can never contain
    a literal `<!--` or `-->`."""
...
def test_empty_none_anchor_still_passes_lint(tmp_path: Path):
    ...
    not flag it 鈥?finalizer's precedence-zero rule catches it later."""
    p = write(
        tmp_path,
...
def test_anchor_after_arbitrary_html_comment_caught(tmp_path: Path):
    ...
    orphan 鈥?the `<!--note-->` is not a ref marker. The earlier
...
# REDACTED: sensitive-looking memory line
    ...
    violations = lint_file(p)
    # Both F14 (malformed ref) and F12 (orphan anchor 鈥?because the
...
def test_quote_with_single_hyphen_explicitly_allowed(tmp_path: Path):
    ...
    inconsistent party 鈥?adjusted in F15 commit)."""
    p = write(
        tmp_path,
...
def test_url_encoded_whitespace_page_anchor_caught(tmp_path: Path):
    ...
    bytes but decodes to two spaces 鈥?functionally no locator. F9
...
def test_url_encoded_single_space_quote_anchor_caught(tmp_path: Path):
    """v3.7.3 F19: same logic for quote kind 鈥?decoded `%20` is one

#!/usr/bin/env python3
"""check_audit_artifact_consistency.py 鈥?ARS v3.6.7 Step 6 Phase 6.3 lint.

Implements every row of spec 搂3.7 invariant families A-F (cross-artifact
...
CLI modes:
  --mode proposal             鈥?proposal-state entry validation (oneOf.proposal)
  --mode persisted            鈥?persisted-state entry validation (oneOf.persisted)
  --mode jsonl-stream         鈥?A7 stream-shape pairing only (orchestrator 搂5.2 L2-5)
  --example-validation-harness 鈥?F4: walk docs/design/*.md, validate spec example
...
Exit codes:
  ...
  64  EX_USAGE 鈥?bad CLI arguments

Lifecycle ownership notes (E8/E9 鈥?discipline rules with no lint surface):
  E8 鈥?proposal entry file is the LAST file the wrapper writes (after JSONL +
  ...
  E9 鈥?wrapper is NOT invoked from an in-LLM Bash tool call from the same
...
The spec 搂3.7 table is the source of truth; this script is the executable
...
# ---------------------------------------------------------------------------
# Constants 鈥?mirror the schema regexes so we don't depend on a JSON Schema
...
# audit template path is a const in the sidecar schema 鈥?used by B3 to know
...
def _load_parse_audit_verdict():
    ...
    Codex round 5 P2 closure: a bare `from parse_audit_verdict import 鈥
    only resolves when scripts/ is on sys.path (typical for the CLI
    invocation `python scripts/check_audit_artifact_consistency.py 鈥).
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(
            ...
            "Phase 6.1 dependency missing 鈥?Phase 6.3 gate cannot run "
...
class LintError:
    ...
    rule_id  鈥?stable handle from spec 搂3.7 (e.g., "A1", "B7", "F2").
    message  鈥?human-readable finding; should name the offending value.
    location 鈥?file path or logical location (e.g., "<entry>", "<jsonl>:row=12").
    severity 鈥?"error" (default; counts toward exit code 1) or "info"
...
def _load_yaml_or_json(path: Path) -> Any:
    ...
    while still parsing ints/floats/bools as native types 鈥?BaseLoader's
...
# ---------------------------------------------------------------------------
# Family A 鈥?Per-artifact cross-field rules
# ---------------------------------------------------------------------------


def check_a1(entry: dict[str, Any] | None, verdict: dict[str, Any] | None,
             location: str = "<entry>") -> list[LintError]:
    """A1 鈥?verdict.status agrees with finding_counts and failure_reason.
    ...
    for label, doc in (("entry.verdict", _safe_get(entry, "verdict")), ("verdict_file", verdict)):
        ...
        if status == "PASS":
            ...
            # Codex round 10 P2 closure: spec 搂3.2 cross-field rule for MINOR
            # is "p1==0 AND p2==0 AND p3<=3" 鈥?but PASS already covers
            ...
            # no findings to show. Require p3>=1 for MINOR 鈥?the lower
            ...
            if not (p1 == 0 and p2 == 0 and 1 <= p3 <= 3):
                findings.append(LintError("A1",
                    ...
                    f"zero-count MINOR is malformed 鈥?wrapper classifies zero findings as PASS",
                    loc))
            if has_failure:
                findings.append(LintError("A1",
...
def check_a2(entry: dict[str, Any] | None, verdict: dict[str, Any] | None,
             location: str = "<entry>") -> list[LintError]:
    """A2 鈥?failure_reason required iff status == AUDIT_FAILED.
...
def check_a3(entry: dict[str, Any] | None, verdict: dict[str, Any] | None,
             location: str = "<entry>") -> list[LintError]:
    """A3 鈥?round <= target_rounds (entry-side and verdict-file)."""
    findings: list[LintError] = []
    for label, doc in (("entry.verdict", _safe_get(entry, "verdict")), ("verdict_file", verdict)):
        if doc is None:
            continue
...
def check_a4(entry: dict[str, Any] | None, mode: str,
             location: str = "<entry>") -> list[LintError]:
    """A4 鈥?acknowledgement allowed only on persisted MATERIAL entries.

    Proposal arm forbids acknowledgement entirely (per JSON Schema sketch).
    Persisted PASS / MINOR with acknowledgement 鈫?reject.
    """
    if entry is None:
        return []
...
def check_a5(verdict: dict[str, Any] | None,
    ...
    """A5 鈥?verdict file's finding_counts.pN == count(findings[severity == PN]).
...
def check_a6(verdict: dict[str, Any] | None,
             location: str = "<verdict>") -> list[LintError]:
    """A6 鈥?when verdict_status==AUDIT_FAILED, findings == []."""
    if verdict is None:
        return []
...
def check_a7(events: list[dict[str, Any]] | None,
    ...
    """A7 鈥?JSONL stream tool-event pairing.
    ...
    for iid, row in started.items():
        if iid not in completed:
            ...
            # ordering 鈥?completed-row must exceed started-row
            crow = completed[iid]
            srow = started[iid]
            if crow <= srow:
                findings.append(LintError("A7",
...
# ---------------------------------------------------------------------------
# Family B 鈥?Cross-file rules (Layer 3)
# ---------------------------------------------------------------------------
...
def check_b1(sidecar: dict[str, Any] | None, events: list[dict[str, Any]] | None,
    ...
    """B1 鈥?sidecar.stream.jsonl_thread_id matches JSONL's thread.started.thread_id.

    SUSPENDED when companion verdict.verdict_status == AUDIT_FAILED (per
    搂3.4 conditional 鈥?empty string permitted there).
    """
...
def check_b2(entry: dict[str, Any] | None, sidecar: dict[str, Any] | None,
    ...
    """B2 鈥?entry.deliverable_sha == sidecar's matching primary sha
        ...
        if disk_path.exists():
            ...
            if disk_sha is not None and disk_sha != entry_sha:
                findings.append(LintError("B2",
                    ...
                    location))
        # Missing file is not a B2 failure if entry/sidecar agree 鈥?caller
...
def compute_bundle_manifest(primary: list[dict[str, Any]],
    ...
    """Compute (manifest_text, manifest_sha) per spec 搂3.6.
...
def check_b3(sidecar: dict[str, Any] | None, repo_root: Path,
    ...
    """B3 鈥?sidecar.prompt.bundle.bundle_manifest_sha == recomputed_sha.

    Recomputes over the CURRENT on-disk SHA-256 of every primary +
    supporting + template file (per 搂3.6 manifest format).
    ...
    # Step 1 鈥?recompute against the SHAs the sidecar itself declares.
    ...
    # Step 2 鈥?verify against live disk SHAs. Codex round 2 P2 closure:
    ...
    # entirely 鈥?same convention as B4.
    ...
    if missing_bundle_files:
        # File-level mutation evidence (deletion / move) 鈥?flag explicitly so
        ...
        findings.append(LintError("B3",
            ...
            f"{missing_bundle_files} 鈥?treat as stale/unverifiable",
            location))
        return findings

    # All files present 鈥?recompute live manifest and compare.
    _, sha_live = compute_bundle_manifest(live_primary, live_supporting, template_path, live_template_sha)
    if declared_sha != sha_live:
        findings.append(LintError("B3",
            f"declared bundle_manifest_sha={declared_sha!r} disagrees with live bundle (computed={sha_live!r}) 鈥?bundle file changed since audit",
            location))
    return findings
...
def check_b4(sidecar: dict[str, Any] | None, repo_root: Path,
             location: str = "<sidecar>") -> list[LintError]:
    """B4 鈥?sidecar.runner.git_sha resolves to a real commit."""
    if sidecar is None:
        return []
    ...
    try:
        ...
        # Internal 鈥?don't fail the whole lint over a git error
        return [LintError("B4",
...
def check_b5(sidecar: dict[str, Any] | None,
             location: str = "<sidecar>") -> list[LintError]:
    """B5 鈥?ended_at - started_at == duration_seconds (卤1s)."""
    if sidecar is None:
        return []
    ...
    if abs(delta - duration) > 1.0:
        return [LintError("B5",
            f"timing arithmetic: ended_at - started_at = {delta:.3f}s, duration_seconds = {duration:.3f}s (卤1s tolerance exceeded)",
            location)]
    return []
...
def check_b6(sidecar: dict[str, Any] | None, verdict: dict[str, Any] | None,
    ...
    """B6 鈥?sidecar.process.exit_code == 0 for non-AUDIT_FAILED entries.
...
def check_b7(entry: dict[str, Any] | None, sidecar: dict[str, Any] | None,
    ...
    """B7 鈥?entry.run_id == sidecar.run_id == verdict.run_id == bare basename of every co-located file.
    ...
    if verdict is not None:
        ...
        if v_run_id is not None and v_run_id != side_run_id:
            findings.append(LintError("B7",
                ...
                f"(swap-one-verdict-file forgery seam 鈥?verdict file was renamed "
    ...
    # would forge the swap-one-artifact-file attack seam (搂3.7 B7). The
    ...
    if isinstance(artifact_paths, dict):
        for key, ext in (("jsonl", ".jsonl"),
            ...
            # forgery seam open 鈥?entry can record paths pointing at a
            ...
            if repo_root is not None:
                ...
                if not resolved.exists():
                    findings.append(LintError("B7",
                        ...
                        f"{resolved} which does not exist on disk 鈥?recorded "
                ...
                if cli_path is not None and cli_path.exists():
                    ...
                    if resolved != cli_resolved and not resolved.samefile(cli_resolved):
                        findings.append(LintError("B7",
                            f"entry.artifact_paths.{key}={recorded!r} resolves to "
                            f"{resolved} but CLI loaded {cli_resolved} 鈥?declared "
...
def check_b8(entry: dict[str, Any] | None, mode: str,
             location: str = "<entry>") -> list[LintError]:
    """B8 鈥?persisted ack: verdict.verified_at == acknowledgement.acknowledged_at."""
...
def check_b9(entry: dict[str, Any] | None, sidecar: dict[str, Any] | None,
    ...
    """B9 鈥?when entry.bundle_id present, equals sidecar.prompt.bundle.bundle_id."""
    if entry is None or sidecar is None:
        return []
...
def check_b10(entry: dict[str, Any] | None, verdict: dict[str, Any] | None,
    ...
    """B10 鈥?persisted ack acknowledgement.finding_ids: non-empty, all exist
...
def check_c1(entry: dict[str, Any] | None, verdict: dict[str, Any] | None,
    ...
    for entry_key, vfile_key in pairs:
        ...
        # failure_reason is conditional 鈥?present-or-absent on each side; we
...
def check_c2(entry: dict[str, Any] | None, sidecar: dict[str, Any] | None,
             location: str = "<entry/sidecar>") -> list[LintError]:
    """C2 鈥?entry.bundle_manifest_sha == sidecar.prompt.bundle.bundle_manifest_sha."""
    if entry is None or sidecar is None:
        return []
...
def check_c3(new_entry: dict[str, Any] | None, prior_entry: dict[str, Any] | None,
    ...
    """C3 鈥?Acknowledgement append entry copies prior entry's
...
def check_c4(entry: dict[str, Any] | None, sidecar: dict[str, Any] | None,
             location: str = "<entry/sidecar>") -> list[LintError]:
    """C4 鈥?entry.deliverable_sha == sidecar primary_deliverables[].sha
    ...
    for key, vas in seen.items():
        if len(vas) != len(set(vas)):
            findings.append(LintError("D1",
                f"duplicate verified_at in (stage,agent,sha,run_id)={key}: {vas} 鈥?D3 monotonic helper should prevent this",
                location))
    return findings
...
def check_d2(proposals: list[dict[str, Any]] | None,
    ...
    """D2 鈥?Path B proposal selection determinism check.
    ...
    for st, rids in by_started.items():
        if len(rids) > 1:
            findings.append(LintError("D2",
                f"proposals share started_at={st!r}: {sorted(rids)} 鈥?falling back to run_id lex-max tie-breaker",
                location))
    return findings
...
def check_d3(passport_audit_artifacts: list[dict[str, Any]] | None,
             location: str = "<passport>") -> list[LintError]:
    """D3 鈥?every persisted verified_at strictly greater than every prior.
...
def check_d4(persisted_round: int | None, proposal_round: int | None,
             location: str = "<entry>") -> list[LintError]:
    """D4 鈥?higher-round unmerged proposals supersede lower-round persisted entries.
...
# ---------------------------------------------------------------------------
# Family E 鈥?Lifecycle ownership
# ---------------------------------------------------------------------------


def check_e1_e2_e6(passport_audit_artifacts: list[dict[str, Any]] | None,
                   location: str = "<passport>") -> list[LintError]:
    """E1/E2/E6 鈥?post-hoc passport-shape checks.
    ...
    AUDIT_FAILED entries in the passport (E5/E2 鈥?never persisted).
    """
    if not passport_audit_artifacts:
        return []
    findings: list[LintError] = []
    for idx, entry in enumerate(passport_audit_artifacts):
        ...
        if "verified_at" not in verdict:
            findings.append(LintError("E1/E2/E6",
                f"audit_artifact[{idx}].verdict missing verified_at 鈥?non-orchestrator writer suspected",
                location))
        if "verified_by" not in verdict:
            findings.append(LintError("E1/E2/E6",
                f"audit_artifact[{idx}].verdict missing verified_by 鈥?non-orchestrator writer suspected",
                location))
            ...
            findings.append(LintError("E1/E2/E6",
                ...
                location))
        # Codex round 3 P2 closure: 搂3.2 lifecycle-conditional table
        ...
        if verdict.get("status") == "AUDIT_FAILED":
            findings.append(LintError("E5",
                ...
                f"passport audit_artifact[] (搂3.2: AUDIT_FAILED is proposal-arm only; "
...
def check_e3_e4(entry: dict[str, Any] | None, mode: str,
                location: str = "<entry>") -> list[LintError]:
    """E3/E4 鈥?wrapper-emitted proposals carrying verified_at/verified_by are rejected.
...
def check_e5(entry: dict[str, Any] | None, mode: str,
             location: str = "<entry>") -> list[LintError]:
    """E5 鈥?--mode persisted rejects AUDIT_FAILED status."""
    if entry is None or mode != "persisted":
        return []
    status = _safe_get(entry, "verdict", "status")
...
def check_e7(entry: dict[str, Any] | None, location: str = "<entry>") -> list[LintError]:
    """E7 鈥?ack entry's verdict.status remains MATERIAL (not synthetic
...
# ---------------------------------------------------------------------------
# Family F 鈥?Naming conventions
# ---------------------------------------------------------------------------


def check_f1(run_id: Any, location: str = "<entry>") -> list[LintError]:
    """F1 鈥?run_id format <ISO-8601-Z>-<4-hex>."""
...
def check_f2(jsonl_path: Path | None, sidecar_path: Path | None,
    ...
    """F2 鈥?4 (proposal) or 3 (persisted) artifact basenames use bare run_id stem with
...
def check_f3(sidecar: dict[str, Any] | None, sidecar_path: Path | None,
             location: str = "<sidecar>") -> list[LintError]:
    """F3 鈥?sidecar.run_id == file basename (with .meta.json stripped)."""
...
def run_checks(ctx: LintContext) -> list[LintError]:
    ...
    # Proposal / persisted modes 鈥?full sweep
    ...
    if ctx.jsonl_events is not None:
        ...
        # truncated 鈥?codex may be killed after an item.started but before
        # the matching item.completed 鈥?so unmatched starts are expected
        # evidence, not pairing violations. 搂3.4 suspends Layer 3 cross-
...
# REDACTED: sensitive-looking memory line
        ...
        # was killed mid-run); 搂3.4 already suspends Layer 3 cross-file
        ...
        if v_status != "AUDIT_FAILED":
            ...
            try:
                ...
                findings.append(LintError(
                    "L2-3/L2-4",
                    f"jsonl stream-shape rejected: {e} 鈥?non-AUDIT_FAILED "
                    f"verdict requires a complete stream (thread.started 鈫?"
                    f"turn.started 鈫?鈥?鈫?turn.completed with valid usage)",
                ...
                # spec 搂5.2 is `parse_audit_verdict.py --probe` 鈥?extract
                ...
                try:
                    ...
                    findings.append(LintError(
                        "L2-4",
                        f"verdict text extraction failed: {e} 鈥?non-AUDIT_FAILED "
                    ...
                    try:
                        ...
                        findings.append(LintError(
                            "L2-4",
                            f"verdict text Section 6 parse failed: {e} 鈥?"
                            f"agent_message did not contain a parseable "
                            f"audit-template Section 6 verdict block",
                            str(ctx.jsonl_path or "<jsonl>"),
                        ))

    # Codex round 9 P2 closure: 搂3.4 + 搂5.6 Path B5 + 搂3.7 family B note say
    # "Layer 3 verification is suspended for AUDIT_FAILED" 鈥?the orchestrator
    ...
    # (spec 搂3.4 rule 6 + F-027); B1 has its own conditional handling.
    ...
    # C3 needs prior_entry (passport scan) 鈥?skipped in single-entry mode unless
    ...
    if mode == "persisted" and ctx.passport_audit_artifacts is not None and ctx.entry is not None:
        if "acknowledgement" in ctx.entry:
            ...
            if prior is None:
                ...
                # 鈥?there is nothing to copy from. A standalone ack or one
                ...
                findings.append(LintError(
                    ...
                    f"run_id={ctx.entry.get('run_id')!r}) 鈥?搂5.4 ack mechanism "
...
def _scan_unmerged_proposals(
    ...
    been moved to consumed/ yet) 鈥?exclude it so we don't compare an
    ...
    Errors loading any single proposal are silently skipped 鈥?D2/D4 are
...
# ---------------------------------------------------------------------------
# Schema validation (defense in depth 鈥?re-run schemas at lint time)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 搂3.7 F4 example-validation harness
# ---------------------------------------------------------------------------


def run_example_harness(repo_root: Path) -> list[LintError]:
    """F4 鈥?walk docs/design/*.md, find code-fenced example payloads, validate
...
def _classify_and_validate_block(
    ...
    # JSONL detection 鈥?multiple lines each starting with `{"type":`
    looks_like_jsonl = (
        ("\n" in stripped) and all(
...
# REDACTED: sensitive-looking memory line
    ...
    if lang == "yaml":
        ...
        # The harness's job is to surface drift in 搂3.1/搂3.3/搂3.4/搂3.5
        ...
        if not any(sig in stripped for sig in audit_signature_keys):
            return out
        # parse as YAML 鈥?use timestamp-as-string loader so we don't double-flag
        ...
        if "verdict_status" in doc:
            ...
            # sidecar 鈥?but note this section may have hyphen drift in timestamps
...
def _build_parser() -> argparse.ArgumentParser:
    p = _ExUsageParser(
        description="Lint audit-artifact contract per ARS v3.6.7 搂3.7 invariants.")
...
def _autodiscover(output_dir: Path | None, run_id: str | None,
    ...
    if entry is None:
        ...
        # at 搂4.9 step 9, but the CLI must still be able to read an entry
...
def main(argv: list[str] | None = None) -> int:
    ...
    # jsonl-stream mode 鈥?orchestrator 搂5.2 L2-5 invocation
    if args.mode == "jsonl-stream":
        ...
        # Per spec 搂5.2 L2-5 the orchestrator wants the offending item.id and
    ...
    # instead 鈥?schema validation below would also catch it, but bailing
    ...
    # Schema validation on the entry first (defense in depth 鈥?the oneOf
            ...
            if verdict_block.get("status") == "AUDIT_FAILED":
                schema_findings.append(LintError(
                    "E5", "--mode persisted: entry.verdict.status='AUDIT_FAILED' is "
                          ...
                          "搂3.2 lifecycle-conditional table)",
                    str(entry)))
            ...
            if "acknowledgement" in entry_data:
                schema_findings.append(LintError(
                    "A4", "--mode proposal: entry.acknowledgement must be absent "
                          "(orchestrator-only write per 搂5.4)",
                    str(entry)))

    # Companion artifacts (sidecar + verdict + jsonl) are REQUIRED in
    # Codex round 16 P2 closure: spec 搂5.2 says the orchestrator follows
    ...
    # proposal/persisted modes 鈥?they ARE the Layer 2/3 evidence Phase 6.3
    ...
    # exist, surface as an artifact-incomplete finding (B7 family 鈥?the
    # bundle is supposed to be complete by 搂4.9 step 9).
    def _missing_companion(role: str, path: Path | None) -> None:
        if path is None:
            schema_findings.append(LintError(
                "B7",
                f"audit bundle missing {role} (auto-discover failed and no explicit "
                f"--{role} given) 鈥?proposal/persisted mode requires {role} for "
            ...
            schema_findings.append(LintError(
                "B7",
                f"audit bundle missing {role} at {path} 鈥?proposal/persisted mode "
    ...
    if jsonl is not None and jsonl.exists():
        ...
        if verdict_status_for_jsonl == "AUDIT_FAILED":
            ...
            # don't depend on stream completeness 鈥?but a hard parse
    ...
    # set() on nested fields assuming the schema-defined types 鈥?a value
    ...
    # rejection. Short-circuit on any error-severity SCHEMA finding 鈥?

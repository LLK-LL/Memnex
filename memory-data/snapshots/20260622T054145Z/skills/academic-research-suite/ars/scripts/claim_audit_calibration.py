...
(T-C1 / T-C2 / T-C3 per spec 搂7.7). The default thresholds reflect the
spec 搂7.7 + 搂9 acceptance criteria 鈥?FNR < 0.15 AND FPR < 0.10.

Spec: docs/design/2026-05-15-issue-103-claim-alignment-audit-spec.md
      搂7.7 (test contract) + 搂1 deliverable 7 (protocol doc) + 搂13 step 10.
"""
...
# Constraint id shape per spec 搂3.2 canonical parse rule (RE_NC_CONSTRAINT
...
# Claim ids are zero-padded to 鈮? digits, so NC-C[0-9]{3,}-[0-9]+ is the
# canonical NC shape 鈥?accepting NC-C1-1 (R1 codex residual P2) would
...
# Spec 搂7.7 + 搂9 acceptance gates. Tightening these is a spec bump.
...
# Constraint-tuple required fields per spec 搂7.7 rule (c). Either rule
# text inline OR a manifest-fixture path resolving to a manifest carrying
# the rule 鈥?silent-skip regression hardening.
...
class GoldSetValidationError(ValueError):
    ...
    spec 搂7.7) and the offending tuple index so calibration-fixture
# REDACTED: sensitive-looking memory line
    is a contract violation 鈥?fix the gold set, do not swallow the error.
    """


def validate_gold_set(tuples: list[dict[str, Any]]) -> None:
    """Validate the gold set per spec 搂7.7 rules (a)..(d).

    Raises GoldSetValidationError on the first failed rule. Returns None
    on success so the call site can ignore the return value 鈥?the
    ...
    if not_violated_constraint_count < 3:
        raise GoldSetValidationError(
            ...
            f"constraint tuple(s); rule (d) requires 鈮? so constraint FPR is "
...
# REDACTED: sensitive-looking memory line
            ...
            "(post-v3.8). validate_gold_set accepts both forms per spec 搂7.7 "
            "rule (c), but the runner only supports the inline form today."
        )
    # Both inline and manifest_path are absent 鈫?validate_gold_set should have
...
def _derive_constraint_scope(constraint_id: str) -> str:
    ...
    ids ("  MNC-1" 鈫?NC) and would silently accept malformed prefixes
    ("bogus" 鈫?NC). The judge stub never read scope so the bug was
    ...
    expected patterns. Spec 搂3.2 + protocol doc gold-tuple schema are
    ...
    raise GoldSetValidationError(
        ...
        f"expected shape (MNC-N or NC-CN-M per spec 搂3.2)"
    )
...
def run_calibration(
    ...
    *,
    ...
    Args:
        gold_set: list of tuples per spec 搂7.7 schema. Will be validated
            via `validate_gold_set` before any judge call 鈥?invalid sets
        ...
        thresholds: optional dict overriding {"FNR": ..., "FPR": ...}.
            Spec 搂7.7 defaults are 0.15 / 0.10.

    Returns:
        Dict with keys:
            - FNR, FPR 鈥?aggregate rates across all tuples
            - per_class 鈥?{SUPPORTED/UNSUPPORTED/AMBIGUOUS/violated_constraint:
                            {FNR, FPR, n_positive, n_negative}}
            - thresholds 鈥?the active gate values (echoed for surfacing)
            - n_total 鈥?gold set size
            - n_alignment / n_constraint 鈥?split counts
    """
    ...
    for tup in gold_set:
        ...
        if kind == "alignment":
            ...
            # the positive class 鈥?that's the gate-refuse signal.
            ...
            # while per-class still counted it correctly 鈥?a measurable-

...
Re-runs the v3.7.3 spec 搂3.2 contamination_signals check post-hoc on
pre-v3.7.3 literature_corpus[] entries. Per spec 搂3.2 R-L3-2-B:
...
# patched, but `semantic_scholar_unmatched` was omitted per spec 搂3.2).
# Distinct from skip categories above 鈥?these entries DO get patched.
...
def _is_complete(signals: Mapping[str, Any], entry: Mapping[str, Any]) -> bool:
    ...
    spec 搂3.2). For non-manual entries lacking this field, an earlier
...
def _is_insufficient(entry: Mapping[str, Any]) -> bool:
    ...
    correctly returns False (venue not in PREPRINT_VENUES) 鈥?that's a
    ...
    *,
    ...
    for entry in corpus:
        ...
        if _is_insufficient(entry):
            ...
            _log(f"{key}: skip (insufficient data 鈥?missing year)")
            continue
        ...
        if existing is not None:
            ...
            # this as a patch when a NEW field was actually added 鈥?
            ...
            # data. Preserve any existing timestamp 鈥?that's the original

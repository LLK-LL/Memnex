...
Python module that implements the 搂5 orchestrator 搂3.6 8-row matrix and
...
鈥?anchorless (not_attempted) is HIGH-WARN gate-refuse defense-in-depth,
...
Spec 搂7 names the file `tests/test_claim_audit_finalizer.py`. Repo
...
except ModuleNotFoundError as exc:  # pragma: no cover 鈥?RED phase only
    ...
    def setUpClass(cls) -> None:
        if _FINALIZER_IMPORT_ERR is not None:
            raise unittest.SkipTest(
                ...
                "(expected during RED phase 鈥?implementation lands in spec 搂13 step 8)"
            )


# ---------------------------------------------------------------------------
# T-F1a 鈥?SUPPORTED + null + any 鈫?no annotation, pass.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-F1b 鈥?AMBIGUOUS + {source_description, citation_anchor, synthesis_overclaim, null}
#         + any 鈫?[CLAIM-AUDIT-AMBIGUOUS], LOW-WARN, pass.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-F1c 鈥?UNSUPPORTED + {source_description, metadata, citation_anchor, synthesis_overclaim}
#         + any 鈫?[HIGH-WARN-CLAIM-NOT-SUPPORTED], HIGH-WARN, gate-refuse.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-F1d 鈥?UNSUPPORTED + negative_constraint_violation + any
#         鈫?[HIGH-WARN-NEGATIVE-CONSTRAINT-VIOLATION (id)], HIGH-WARN, gate-refuse.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-F1e 鈥?RETRIEVAL_FAILED + retrieval_existence + not_found
#         鈫?[HIGH-WARN-FABRICATED-REFERENCE], HIGH-WARN, gate-refuse.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-F1f 鈥?RETRIEVAL_FAILED + not_applicable + not_attempted
#         鈫?[HIGH-WARN-CLAIM-AUDIT-ANCHORLESS], HIGH-WARN, gate-refuse.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-F1g 鈥?RETRIEVAL_FAILED + not_applicable + failed 鈫?LOW-WARN, pass (paywall).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-F1h 鈥?RETRIEVAL_FAILED + not_applicable + audit_tool_failure 鈫?MED-WARN, pass.
# ---------------------------------------------------------------------------
...
class TF1hAuditToolFailure(_FinalizerTestBase):
    def test_audit_tool_failure_med_warn_passes(self) -> None:
        ...
        self.assertEqual(
            out["annotation"],
            "[CLAIM-AUDIT-TOOL-FAILURE 鈥?judge_timeout]",
        )
...
# ---------------------------------------------------------------------------
# T-F2 鈥?HIGH-WARN-CLAIM-NOT-SUPPORTED triggers terminal gate refuse.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-F3 鈥?/ars-mark-read does NOT clear HIGH-WARN-CLAIM-NOT-SUPPORTED (asymmetry).
# ---------------------------------------------------------------------------
...
class TF3MarkReadAsymmetry(_FinalizerTestBase):
    ...
    def test_mark_read_clears_low_warn_advisory(self) -> None:
        ...
        self.assertTrue(
            ars_mark_read_clears(
                annotation="[CLAIM-AUDIT-UNVERIFIED 鈥?REFERENCE FULL-TEXT NOT RETRIEVABLE]",
                tier=TIER_LOW_WARN,
            )
        )
...
# ---------------------------------------------------------------------------
# T-F4 鈥?LOW-WARN-CLAIM-AUDIT-UNVERIFIED passes gate.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-F5 鈥?Stage 6 reflection report renders histogram when 鈮? completed entries.
# ---------------------------------------------------------------------------
...
class TF5Stage6Histogram(_FinalizerTestBase):
    def test_histogram_renders_when_five_or_more_completed(self) -> None:
        ...
        self.assertIsNotNone(report, "鈮? completed entries MUST render histogram")
    ...
    def test_histogram_renders_for_all_supported_papers(self) -> None:
        # Step 8 codex R4 P2-1 closure: spec literal is "鈮? completed
    ...
    def test_histogram_excludes_inconclusive_rows(self) -> None:
        # 5 inconclusive + 0 completed 鈫?still suppressed (spec: "鈮? completed").
        results = [
            _result(
                judgment="RETRIEVAL_FAILED",
                defect_stage="not_applicable",
...
# ---------------------------------------------------------------------------
# T-F6 鈥?MANIFEST-MISSING fallback emits a paper-level advisory annotation.
# ---------------------------------------------------------------------------
...
class TF6ManifestMissingAdvisory(_FinalizerTestBase):
    ...
    Spec 搂9 acceptance bullet: `claim_intent_manifest absent 鈫?
    ...
    in the finalizer per spec 搂13 step 8 deliverable. Before this row, an
    all-SUPPORTED MANIFEST-MISSING passport produced zero annotations 鈥?
    ...
    def test_empty_manifests_no_results_no_advisory(self) -> None:
        ...
        # to surface a warning about 鈥?the gate just sees an empty
        # passport.
        passport = {
            "claim_intent_manifests": [],
            "claim_audit_results": [],
...
# ---------------------------------------------------------------------------
# T-CO-1 鈥?uncited sentence_text == manifest claim_text suppresses INTENDED_NOT_EMITTED.
# ---------------------------------------------------------------------------
...
# REDACTED: sensitive-looking memory line
        ...
        out = run_audit_pipeline(
            ...
            corpus=[],
            config=self._config(),
            retrieve_fn=lambda c: {},  # never called 鈥?no citations
...
# ---------------------------------------------------------------------------
# T-CO-2 鈥?drifted cited claim emits with scoped_manifest_id=SENTINEL.
# ---------------------------------------------------------------------------
...
class TCO2DriftedCitedScopedToSentinel(_PipelineCarryoverTestBase):
    def test_drifted_cited_claim_uses_sentinel_scope(self) -> None:
        ...
        # Drifted cited claim 鈥?has a ref marker but claim_id (C-100) is
...
# ---------------------------------------------------------------------------
# T-CO-3 鈥?MNC-only uncited sentence emits scoped_manifest_id=None on uncited row.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-CO-4 鈥?sampling does not truncate the emitted set used for drift detection.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-UAF-F1 鈥?v3.8.2 / #118 finalizer wiring for uncited_audit_failures[].
...
class TUAFFinalizerRouting(unittest.TestCase):
    def test_uaf_row_emits_med_warn_annotation(self) -> None:
        ...
        self.assertIn(
            "[CLAIM-AUDIT-TOOL-FAILURE-UNCITED 鈥?judge_timeout]",
            annotations,
    ...
    def test_uaf_row_each_fault_class_renders(self) -> None:
        ...
        for fault_class in INV14_FAULT_CLASS_TAGS:
            with self.subTest(fault_class=fault_class):
                ...
                out = apply_finalizer(passport)
                expected = f"[CLAIM-AUDIT-TOOL-FAILURE-UNCITED 鈥?{fault_class}]"
                annotations = [a["annotation"] for a in out["annotations"]]

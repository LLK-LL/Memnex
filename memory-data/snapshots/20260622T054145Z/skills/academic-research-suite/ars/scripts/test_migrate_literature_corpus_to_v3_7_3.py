...
Tests scripts/migrate_literature_corpus_to_v3_7_3.py 鈥?the CLI that
backfills `contamination_signals` on pre-v3.7.3 literature_corpus[]
entries per v3.7.3 spec 搂3.2 R-L3-2-B.
...
class SinglePassportMigrationTest(unittest.TestCase):
    ...
    def test_patches_three_entries_per_emission_rules(self) -> None:
        """Per spec 搂3.2: emit object on every non-skipped entry, even
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            # chen2024ai: arXiv 2024 鈫?preprint=true; SS no-match 鈫?unmatched=true
            chen = entries[0]
            self.assertEqual(
            ...
            # smith2020old: Nature 2020 鈫?preprint=false; SS match 鈫?unmatched=false
            smith = entries[1]
            self.assertEqual(
                smith["contamination_signals"],
                {"preprint_post_llm_inflection": False, "semantic_scholar_unmatched": False},
            )
            # lopez2024manual: bioRxiv 2024 manual 鈫?preprint=true; unmatched OMITTED
            lopez = entries[2]
            self.assertEqual(
    ...
    def test_missing_venue_does_not_skip_entry(self) -> None:
        ...
        and emission proceeds 鈥?that's a defined computation, not half-
...
"""
    ...
    def test_partial_fill_without_provenance_sets_backfilled_at(self) -> None:
        ...
        the missing field post-hoc, record provenance 鈥?otherwise the
...
"""
    ...
    def test_partial_fill_with_existing_provenance_preserves_timestamp(self) -> None:
        ...
        degradation), DON'T overwrite 鈥?the original timestamp is the
...
"""
        with tempfile.TemporaryDirectory() as td:
            ...
            p.write_text(partial_yaml)
            before = p.read_text()
            # Simulate API still degraded 鈥?SS lookup raises
    ...
    def test_manual_entry_with_only_preprint_signal_is_complete(self) -> None:
        ...
        (per spec 搂3.2 + schema allOf rule #4). An object with only
        preprint_post_llm_inflection on a manual entry is COMPLETE, not
        partial 鈥?re-running must skip."""
        manual_yaml = """\
origin_skill: deep-research
literature_corpus:
  - citation_key: lopez2024manual
...
"""
        with tempfile.TemporaryDirectory() as td:
            ...
            # Note: schema-invalid passport 鈥?`year` is required. This test
...
class DirectoryScanTest(unittest.TestCase):
    ...
    def test_scan_dir_is_non_recursive(self) -> None:
        """Per design 搂4.2: directory scan finds *.yaml non-recursively.
    ...
    def test_migrate_dir_runs_each_passport(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(agg["entries_patched"], 6)  # 3 per passport 脳 2

...
def split_frontmatter(content: str) -> tuple[Any, str]:
    ...
    - fm is a dict            鈫?valid frontmatter
    - fm is None              鈫?no leading '---' delimiter (fall through to Convention B)
    - fm is _SENTINEL_MALFORMED 鈫?'---' present but YAML invalid (reject as malformed)
    """
...
def _coerce_year(raw: Any) -> tuple[int | None, str | None]:
    ...
    if isinstance(raw, str):
        # Accept leading-year patterns: "2024", "2024-01", "2024-01-15" 鈫?2024
...
# REDACTED: sensitive-looking memory line
    ...
    Accepts:
    - list of dicts, each with 'family' or 'literal'  鈫?validated, returned as-is
    - str                                               鈫?parsed via parse_semicolon_names
# REDACTED: sensitive-looking memory line
    """
    if isinstance(raw, str):
...
def process_file(
    ...
    """Return (accepted_entry, rejection) 鈥?exactly one is None.

...
- **`folder_scan.py`** 鈥?scans a directory of files, parses citation metadata from filenames.
- **`zotero.py`** 鈥?reads a Better BibTeX JSON export.
- **`obsidian.py`** 鈥?reads an Obsidian vault (frontmatter Convention A or Karpathy-style Convention B).
...
Export your Zotero library as **Better BibTeX JSON** (Zotero 鈫?File 鈫?Export Library 鈫?Format: Better CSL JSON, with the Better BibTeX extension installed). Then:
...
7. Reuse helpers in [`_common.py`](_common.py) where possible 鈥?`make_citation_key`, `ensure_unique_citekey`, `parse_csl_name`, `parse_semicolon_names`, `path_to_file_uri`, `write_passport`, `write_rejection_log`, `now_iso`. They encode the contract details so you do not have to re-derive them.
...
`abstract` and `user_notes` can contain publisher-copyrighted material. Before sharing a passport that contains these fields publicly (e.g., in a public repo or over the web), make sure you have the right to publish that text. ARS does not enforce this 鈥?it only warns you in the schema description.

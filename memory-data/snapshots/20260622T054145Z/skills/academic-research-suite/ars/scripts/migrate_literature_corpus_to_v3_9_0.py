...
Re-runs the v3.9.0 spec 搂3.4 + 搂3.5 OpenAlex + Crossref lookups post-hoc
...
migration tool first 鈥?see scripts/migrate_literature_corpus_to_v3_7_3.py.
This is daisy-chained migration per spec 搂3.7.

Usage:
    python migrate_literature_corpus_to_v3_9_0.py [--dry-run] [--verbose] PATH

Design: docs/design/2026-05-17-ars-v3.9.0-cross-index-triangulation-measurement-spec.md 搂3.7
"""
...
def _entry_in_v3_7_3_scope(entry: Mapping[str, Any]) -> bool:
    ...
    scope 鈥?needs `migrate_literature_corpus_to_v3_7_3.py` first per spec
    搂3.7 daisy-chain contract).

    Manual entries are reported as in-scope when they carry a
    `contamination_signals` object, because they ARE v3.7.3-era (spec 搂3.2
    ...
    function's manual-branch is therefore a defensive contract 鈥?it
    ...
    if entry.get("obtained_via") == "manual":
        ...
        # but the v3.7.3 搂3.2 manual exemption means they have only
...
def _is_complete(sig: Mapping[str, Any]) -> bool:
    """Entry already has both v3.9.0 fields set 鈥?nothing to do."""
...
def migrate_passport(
    ...
    *,
    ...
    `oa_client`: OpenAlexClient (or mock) 鈥?must expose
        doi_lookup_with_title_check(doi, title) and title_search(title).
    `cr_client`: CrossrefClient (or mock) 鈥?same interface.
    ...
    for entry in corpus:
        ...
        if not _entry_in_v3_7_3_scope(entry):
            ...
            _log(f"{key}: skip (pre-v3.7.3 鈥?run v3.7.3 migration first)")
            continue
        ...
        if "openalex_unmatched" not in sig:
            try:
                ...
                _log(f"{key}: degraded (openalex) 鈥?{e}")
        else:
        ...
        if "crossref_unmatched" not in sig:
            try:
                ...
                _log(f"{key}: degraded (crossref) 鈥?{e}")
        else:
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "See docs/design/2026-05-17-ars-v3.9.0-cross-index-triangulation-measurement-spec.md 搂3.7"
        )
    )

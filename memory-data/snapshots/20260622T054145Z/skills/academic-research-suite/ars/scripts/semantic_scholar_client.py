...
Not a general-purpose S2 client 鈥?this is the migration tool's narrow
...
class SemanticScholarClient:
    ...
    that spawns N concurrent clients will issue N 脳 (1 req/s) outbound
...
# REDACTED: sensitive-looking memory line
    ...
    def reset_outage_latch(self) -> None:
        ...
        搂"On API failure": network errors skip the remaining batch;
    ...
    def lookup(self, entry: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        Failure semantics:
        - HTTP 5xx 鈫?SemanticScholarUnavailable for THIS reference only
          (server-side error; subsequent lookups retry normally).
        - URLError (network-level) 鈫?SemanticScholarUnavailable AND
          latches the client unavailable. Subsequent lookups short-
          circuit without invoking urlopen until reset_outage_latch().
          Per protocol 搂"On API failure": skip S2 for remaining batch.

        Per protocol 搂"Query Patterns" + 搂"Response Handling":
        `semantic_scholar_unmatched` is True only when NEITHER DOI nor
        title lookup yields a hit. So:

        1. If `doi` present 鈫?GET /paper/DOI:{doi}; on hit, cross-check
           returned title (Levenshtein 鈮?0.70). DOI_MISMATCH (title
           differs despite DOI hit) AND DOI-404 BOTH fall through to (2)
           rather than returning no-match immediately 鈥?the v3.7.3
        ...
        2. Title search: GET /paper/search?query={url-encoded-title};
           pick the top result with title similarity 鈮?0.70, prefer
           matching year.
        ...
        if doi:
            ...
            # per protocol 搂Vector 2 "neither DOI nor title" rule.
        if title:
        ...
        for attempt in range(_MAX_RETRIES + 1):
            try:
                ...
                if e.code == 429 and attempt < _MAX_RETRIES:
                    ...
                    # (because elapsed counts the 2s 脳 N backoff time)
                    # and re-trigger 429.
                    self._last_request_at = self._clock()
                    continue
                if 500 <= e.code < 600:
                ...
                # Per protocol 搂"On API failure": skip S2 for remaining
                ...
                # treated as API degradation per spec 鈥?never let them
                # abort the migration. Codex R4-2 closure.
                #
                # Also latch the client (codex #115 R2 closure): the
                # protocol 搂"On API failure" rule "skip S2 for remaining
    ...
    def _lookup_by_title(self, title: str, year: int | None) -> dict[str, Any]:
        ...
        for cand in candidates:
            ...
            if sim < _TITLE_SIMILARITY_THRESHOLD:
                continue
            # Per protocol: prefer matching year when multiple 鈮?.70 results.

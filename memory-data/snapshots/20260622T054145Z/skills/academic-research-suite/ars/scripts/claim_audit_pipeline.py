"""claim_audit_pipeline 鈥?Python implementation of the 搂4 Step 1-6 pipeline.
...
`judge_fn`) so tests can drive every error and decision path 鈥?paywall,
...
The full spec is in
docs/design/2026-05-15-issue-103-claim-alignment-audit-spec.md 搂4-搂5.
"""
...
# Permitted UNSUPPORTED defect_stages for non-constraint paths (搂3.1 matrix).
...
# Permitted AMBIGUOUS defect_stages (搂3.1 matrix).
...
def _active_constraints_for_claim(
    *,
    ...
    citation 鈥?at realistic workloads (~150 citations 脳 ~100 manifest
...
# ---------------------------------------------------------------------------
# Judge + retrieval invocation 鈥?wraps callables so transient failures become
# INV-14 audit_tool_failure rows instead of aborting the audit. Spec 搂4 step 2
...
# subsets 鈥?passing path-specific allow-lists into `_invoke_judge` rejects an
...
class JudgeInvocationError(Exception):
    ...
    row per spec 搂4 step 2 + INV-14 instead of letting the exception abort the
    audit pass.
    """
...
def _validate_judge_dict(
    ...
    *,
    ...
    same validation surface 鈥?without it a malformed cache entry would crash
    `_judge_result_entry` and abort the audit (Step 13 R3 codex P2 #4).

    Validation surface:
      - non-dict                                  鈫?judge_parse_error
      - missing `judgment` or `rationale`         鈫?judge_parse_error
      - judgment not in `allowed_judgments`       鈫?judge_parse_error
      - VIOLATED without non-blank string id      鈫?judge_parse_error
      - VIOLATED id not in `active_constraint_ids`鈫?judge_parse_error
        (Step 13 R3 codex P2 #1 鈥?prevents formatter gate-refuse on a
...
def _invoke_judge(
    ...
    **call_kwargs: Any,
    ...
    Exception 鈫?fault class mapping:
      - TimeoutError                          鈫?judge_timeout
      - json.JSONDecodeError / ValueError     鈫?judge_parse_error
      - any other Exception                   鈫?judge_api_error
    ...
    except Exception as exc:  # noqa: BLE001 鈥?translation boundary; the source class is captured
...
def _invoke_retrieve(
    ...
    Exception 鈫?fault class mapping (mirrors `_invoke_judge`):
      - TimeoutError                          鈫?retrieval_timeout
      - OSError / ConnectionError             鈫?retrieval_network_error
      - json.JSONDecodeError / ValueError     鈫?retrieval_api_error
      - any other Exception                   鈫?retrieval_api_error

    Return-value validation:
      - non-dict                              鈫?retrieval_api_error
      - missing `ref_retrieval_method` key    鈫?retrieval_api_error
      - unknown `ref_retrieval_method` value  鈫?retrieval_api_error
    ...
    except Exception as exc:  # noqa: BLE001 鈥?translation boundary
    ...
    # non-empty excerpt 鈥?otherwise the judge would be invoked with
...
# ---------------------------------------------------------------------------
# Emission helpers 鈥?each builds one entry dict.
# ---------------------------------------------------------------------------


def _anchorless_entry(citation: dict[str, Any], *, audit_run_id: str, now_iso: str, judge_model: str) -> dict[str, Any]:
    """搂4 Step 1: anchor=none short-circuits to RETRIEVAL_FAILED+inconclusive+not_applicable+not_attempted.
    ...
    than passing through citation.anchor_value 鈥?a stale residual anchor like
    ...
    return {
        ...
        "rationale": (
            ...
            "v3.7.3 finalizer should have gate-refused upstream 鈥?defense-in-depth row."
        ),
        "judge_model": judge_model,
...
def _retrieval_failure_entry(
    ...
    *,
    ...
    """搂4 Step 2: retrieval-side failure routes that skip the judge."""
    if method == "failed":
        # D2 paywall 鈥?LOW-WARN advisory; INV-10.
        entry = {
            "judgment": "RETRIEVAL_FAILED",
            "audit_status": "inconclusive",
            "defect_stage": "not_applicable",
        ...
        # Fabricated reference 鈥?HIGH-WARN; INV-12.
        entry = {
            "judgment": "RETRIEVAL_FAILED",
        ...
        # Transient infrastructure outage 鈥?MED-WARN; INV-14.
    ...
    else:  # pragma: no cover 鈥?should be unreachable given Step 2 caller dispatch
...
def _judge_result_entry(
    ...
    *,
    ...
    """搂4 Steps 5-6: route judge verdict to the right (judgment, defect_stage) row."""
    ...
    if verdict == "SUPPORTED":
        ...
        if hint not in _AMBIGUOUS_DEFECTS:
            hint = None  # AMBIGUOUS+disallowed defect 鈫?coerce to null (INV-3 protection)
        ...
        # Cited constraint violation 鈥?INV-7/INV-8 path.
...
def _uncited_audit_failure_entry(
    *,
    ...
    """搂3.6 (v3.8.2 / #118): uncited sentence 脳 manifest pair where the
...
def _constraint_violation_entry(
    *,
    ...
    """搂3.5 / 搂5 stream (d): uncited claim with VIOLATED judge verdict."""
    violated_id = judge_result.get("violated_constraint_id")
...
# REDACTED: sensitive-looking memory line
    ...
    # but carried no semantic content 鈥?callers who skipped the detector
# REDACTED: sensitive-looking memory line
...
# REDACTED: sensitive-looking memory line
    ...
    # itself a manifest claim 鈥?runtime contract for stream-d uncited
    ...
    # U-INV-4 (manifest_claim_id null 鈫?scoped_manifest_id null).
    scoped_manifest_id = (
...
def _stratified_bucket_indices(total: int, cap: int) -> list[int]:
    ...
    cap=100 鈫?`int(99 * 1.01) == int(100 * 1.0)`-class duplicates after
    ...
    ascending document order 鈥?keeping the bucket-first-pick bias for spread
    ...
    # k 鈮?total (which is guaranteed by `k = min(cap, total)`).
    if len(picks) < k:
...
def _detect_drifts(
    *,
    ...
    """搂4 step 5 manifest set-diff producing claim_drift entries.
    ...
    Precedence:
      - T-P8 + Step 13 R5 codex P3: constraint-violation absorbs drift in
        **full** for the violating manifest. `constraint_absorbed_claim_ids`
        ...
        drifted (non-manifest) claim_id is also suppressed 鈥?preserves the
        "absorbed in full" spec promise. A violation in manifest A does NOT
        silence drift in manifest B.
      - T-P10 / D-INV-4: uncited sentence takes precedence over drift 鈥?no
    ...
    carries any claims, there's no pre-commitment baseline to drift FROM 鈥?
    ...
    # Index emitted citations by (scoped_manifest_id, claim_id) 鈥?these are
    ...
    # INTENDED_NOT_EMITTED 鈥?manifest claims missing from emitted set.
    ...
    # even though the prose carries the claim 鈥?a false drift signal
    # (Step 13 R2 codex P2 finding).
    for m in manifests:
        mid = m.get("manifest_id")
        # If this manifest had any constraint violation, absorb ALL of its
        # drift 鈥?Step 13 R5 codex P3. The (mid, cid) pair-level absorption
        ...
        for claim in m.get("claims", []) or []:
            ...
            if claim_text and claim_text in emitted_texts:
                ...
                # D6 set-of-text semantics 鈥?not a drop.
                continue
    ...
    # EMITTED_NOT_INTENDED 鈥?emitted citations whose claim_text is not in any
    ...
    for c in emitted_citations:
        ...
        if c_scope in constraint_absorbed_manifest_scopes:
            # Step 13 R5 codex P3 鈥?manifest scope absorbed in full when any
        ...
        if text in uncited_sentence_texts:
            # Precedence rule 3 / D-INV-4 鈥?uncited takes priority.
            continue
        if text in seen_drift_texts:
            # D6 set semantics 鈥?one drift row per drifted claim_text.
            continue
...
def run_audit_pipeline(
    *,
    ...
    manifests: list[dict[str, Any]],
    # Reserved for the production retrieval-driver wiring per spec 搂4 step 2.
    ...
    """Run 搂4 Step 1-6 + manifest set-diff over caller-supplied inputs.

    Two uncited streams (Step 13 R4 codex P1 #2):

    - `uncited_sentences`: D4-c detector positives 鈥?output of
    ...
    - `all_uncited_sentences`: the full uncited sentence set (every draft
      sentence with no in-text citation marker). Drives stream (d) 鈥?
...
# REDACTED: sensitive-looking memory line
    ...
    `uncited_sentences` (legacy callers preserved 鈥?but a warning band:
    those callers miss the R4 P1 expansion and should pass both).

# REDACTED: sensitive-looking memory line
    (`scripts/uncited_assertion_detector.py`, 搂"Uncited-assertion detector
    ...
    exercises the full detector 鈫?pipeline 鈫?finalizer chain end-to-end.
...
# REDACTED: sensitive-looking memory line
    ...
    # wiring (spec 搂4 step 2). Mark as read so static analysers (ruff ARG002,
    ...
    if not isinstance(cap, int) or cap <= 0:
        raise ValueError(
            ...
            "(spec 搂4 step 3 + S-INV-2 / T-P11 cap=0 rejected)"
        )
    ...
    # fall back to the D4-c subset for backwards compatibility 鈥?but that
...
# REDACTED: sensitive-looking memory line
    ...
    # drift 鈥?declared claims (by id), the violating citation's pair (which
    ...
    def _written_scope_for(citation: dict[str, Any]) -> str:
        ...
        pair is not present in the manifest index 鈥?otherwise INV-15 dangling
    ...
    for citation in audited_citations:
        ...
        # Step 1 鈥?anchor=none firm-rule short-circuit.
        if anchor_kind == "none":
        ...
        # Step 2 鈥?retrieval. Wrap in `_invoke_retrieve` so transient failures
        ...
        # Step 3 鈥?cache lookup. Active constraints scoped by (manifest, claim).
...
# REDACTED: sensitive-looking memory line
                ...
                # Step 4-5 鈥?passage location is implicit (excerpt is the
        ...
        # Step 6 鈥?defect_stage routing + emission.
        ...
        # Spec 搂6 lint rule 6 + 搂7.2 T-P8: when a citation in this manifest
        # judges VIOLATED, that manifest's drift findings are absorbed in
        # full 鈥?the constraint violation has already surfaced the L3
        ...
        if entry["defect_stage"] == "negative_constraint_violation":
            ...
            # Step 13 R5 codex P3 鈥?record the full manifest scope so any
...
# REDACTED: sensitive-looking memory line
    ...
    # only `sentence_text` / `section_path` / optional `adjacent_text` 鈥?NOT
    ...
    # MNCs reach them universally per spec 搂3.5 D4-c stream (d) semantics).
    # Per-manifest constraint sets 鈥?MNCs (manifest-wide) PLUS, when the
    # sentence carries a `manifest_claim_id`, that claim's NC-C entries.
    # NC-C (claim-level) is the R7 codex P1 gap: spec 搂3.5 D4-c stream (d)
    ...
    # 鈥?we'd have to first-match-wins which mis-attributes the row to the
    ...
    cv_counter = 1
    # v3.8.2 / #118 鈥?uncited_audit_failures[] aggregate for JudgeInvocationError
    ...
    # dropping audit coverage (option 4 鈥?re-raise and abort 鈥?was rejected
    # for that exact coverage reason). See spec 搂3.6.
    ...
    for sentence in all_uncited_sentences:
        ...
        # Caller-pinned scope 鈫?that one manifest. Otherwise every manifest.
        ...
        for mid in target_manifest_ids:
            ...
            # manifest_claim_id MUST stay null 鈥?the failure is MNC-only at
            ...
            # uncited stream. v3.8.2 / #118 鈥?JudgeInvocationError now routes
            ...
            # outage 鈥?see spec 搂3.6 + 搂4 step 9 fourth bullet for routing,
    ...
    # D4-c detector positives only 鈥?uncited_sentences is the filtered set.
    ...
    # invocations (spec 搂4 step 3) 鈥?it does NOT shrink the prose visible to

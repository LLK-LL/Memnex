...
Pure functions implementing v3.7.3 spec 搂3.2 Vector 1 + Vector 2 for use
...
# 10-venue closed list per v3.7.3 spec 搂3.2 + schema description.
# This list is intentionally redundant with the bibliography_agent's
# in-prose list 鈥?adapters and migration tools both need the literal set.
...
# source_pointer 鈫?venue inference table. Per v3.7.3 spec 搂3.2 + schema
...
def _infer_venue_from_pointer(source_pointer: str) -> str | None:
    ...
    or None if no preprint-server hint is present. Per v3.7.3 spec 搂3.2
...
class SemanticScholarUnavailable(Exception):
    ...
    Per spec 搂3.2 emission rules, this triggers OMIT of the
    `semantic_scholar_unmatched` field rather than setting it to False.
    Absence 鈮?negative confirmation."""
...
class SemanticScholarClient(Protocol):
    ...
    (429 鈫?2s backoff 脳 3, DOI-first then title-similarity fallback).
...
def compute_preprint_signal(entry: Mapping[str, Any]) -> bool:
    """Signal 1 per v3.7.3 spec 搂3.2 Vector 1.
    ...
    鈫?arXiv). Missing year, or venue that resolves to neither a preprint
    ...
    the CONTAMINATED-PREPRINT signal (per v3.7.3 搂3.2 Vector 1 鈥?`venue`
...
def compute_ss_unmatched_signal(
    ...
    """Signal 2 per v3.7.3 spec 搂3.2 Vector 2.
    ...
    contamination_signals object (NOT set to False 鈥?that would imply
...
def _resolve_by_doi_then_title(entry: Mapping[str, Any], client) -> bool | None:
    ...
    stays at the wrapper 鈥?this helper never catches."""
    if entry.get("obtained_via") == "manual":
        return None
    ...
    if doi:
        ...
        if hit is not None:
            return False
        # DOI miss or MISMATCH 鈥?fall through to title search.
    hit = client.title_search(title)
    return hit is None


def resolve_openalex_unmatched(entry: Mapping[str, Any], client) -> bool | None:
    """Compute openalex_unmatched per spec v3.9.0 搂3.4.

    Mirrors resolve_semantic_scholar_unmatched semantics:
    - Manual entry 鈫?return None (caller MUST omit field).
    - API down 鈫?re-raise OpenAlexUnavailable (caller MUST omit field).
    - DOI present + DOI hit (passes title cross-check) 鈫?return False.
    - DOI present + DOI miss/MISMATCH 鈫?fall through to title search.
    - DOI absent 鈫?title search only.
    - No hit anywhere 鈫?return True (unmatched).

    Returns:
        True: OpenAlex returned no match by DOI (with title cross-check) or title.
        False: OpenAlex found a match.
        None: obtained_via='manual' 鈫?exempt, caller must omit field.
...
def resolve_crossref_unmatched(entry: Mapping[str, Any], client) -> bool | None:
    """Compute crossref_unmatched per spec v3.9.0 搂3.5.
...
def build_signals_object(
    ...
    Per v3.7.3 spec 搂3.2 emission rules:
      - Both signals computed 鈫?emit both fields (even when both False:
        "computed and found clean" is distinct from "not computed")
      - Manual entry 鈫?omit `semantic_scholar_unmatched` field
      - API degradation 鈫?omit `semantic_scholar_unmatched` field
    """
    obj: dict[str, bool] = {

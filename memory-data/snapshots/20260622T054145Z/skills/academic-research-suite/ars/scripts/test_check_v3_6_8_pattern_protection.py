...
Spec: docs/design/2026-04-30-ars-v3.6.8-trust-provenance-and-drift-transparency-spec.md
      搂 Step 0 鈥?Lint manifest separation (round-1 codex F-004 amend)
...
def test_happy_path_passes_on_clean_tree() -> None:
    """Untouched v3.6.7 blocks 鈫?SHA gate passes."""
    result = _run_lint()
...
def test_mutation_inside_v3_6_7_block_fails() -> None:
    """Inject 1 byte inside the v3.6.7 PATTERN PROTECTION block 鈫?exit 1 + FAIL diagnostic."""
...
def test_appending_new_h2_directly_after_eof_newline_passes() -> None:
    """Append a new H2 directly after the file's trailing newline 鈫?SHA gate passes.

    Boundary rule (spec 搂388): v3.7.1 MAY add new prompt sections OUTSIDE
    ...
    be placed IMMEDIATELY after the file's trailing newline 鈥?no extra
    blank line 鈥?so the heading-based extractor's range stays byte-equal
    ...
    with _Snapshot(TARGET_AGENT):
        ...
        # NO leading "\n\n" 鈥?that would expand the v3.6.7 block range.
...
def test_inserting_blank_line_between_v367_and_step3a_blocks_fails() -> None:
    ...
    the Step 3a Two-Layer Citation Emission block 鈫?SHA mismatch.
    ...
    line absorbed at the boundary between two H2 sections" 鈥?same lint
...
def test_v3_6_8_manifest_scope_must_be_correct() -> None:
    """Wrong scope tag 鈫?lint refuses to run (clear error)."""
    with _Snapshot(V3_6_8_MANIFEST):
        data = json.loads(V3_6_8_MANIFEST.read_text(encoding="utf-8"))
        data["scope"] = "v3.6.7-only"  # wrong 鈥?this is v3.6.8 manifest
...
def test_v3_6_7_manifest_deletion_hard_fails() -> None:
    """v3.6.7 manifest is the source of truth. Missing it 鈫?hard error.
...
def test_v3_6_8_manifest_deletion_hard_fails() -> None:
    """Missing v3.6.8 manifest 鈫?hard error (lint configuration broken)."""
    with _Snapshot(V3_6_8_MANIFEST):
...
def test_heading_prefix_mutation_is_caught() -> None:
    """Round-3 codex P2 closure: spec 搂 388 says the canonical byte range
    ...
    `PATTERN...` 鈥?silently dropping the heading prefix. That's fine for
    v3.6.7's invariant greps, but it would let the v3.7.1 SHA gate accept
    a `## 鈫?### ` mutation as byte-equivalent.
...
def test_prose_mention_does_not_truncate_block_range() -> None:
    ...
    after the heading line 鈥?not from the v3.6.7 legacy extractor's
    ...
    to the heading line_start) covered the WRONG byte range 鈥?could be
...
def test_prose_mention_of_marker_does_not_misanchor_extractor() -> None:
    ...
    mentions `PATTERN PROTECTION (v3.6.7)` 鈥?e.g. in a "Two-Layer Citation
...
def test_extractor_strips_only_file_level_bom_not_block_level() -> None:
    """Round-8 codex P2 closure: spec 搂 Step 0 says "the FILE's BOM (if any)
    ...
    # File-level BOM stripped 鈫?block bytes equal to base.
    assert file_bom_block == base_block, (
        "File-level BOM (byte 0) MUST be stripped per spec 搂 Step 0; got: "
        f"file_bom_block={file_bom_block!r} vs base_block={base_block!r}"
    )
    # Block-level BOM NOT stripped 鈫?block bytes differ from base.
...
def test_bom_before_heading_attack_caught_by_lint() -> None:
    ...
    correct 鈥?the gate rejects the mutation either way. This test
...
def test_anti_self_baseline_guard_rejects_manifest_mutation_in_pr(monkeypatch) -> None:
    ...
    branch (no PR base detectable 鈫?guard returns advisory pass). On
...
def test_anti_self_baseline_guard_history_scan_called(monkeypatch) -> None:
    ...
    GITHUB_BASE_REF is set so the guard's "no PR base detectable 鈫?advisory
    ...
    # Call the guard directly (not via subprocess 鈥?monkeypatch wouldn't apply).
...
def test_strip_file_bom_only_at_byte_zero() -> None:
    """File-level BOM stripping per spec 搂 Step 0. Round-8 closure renamed
    `_normalize_bytes` 鈫?`_strip_file_bom` to make the file-level scope
    ...
    # BOM appearing in the middle of input is NOT stripped 鈥?only byte 0.
...
# =============================================================================
# Step 3a 鈥?Two-Layer Citation Emission invariant tests (v3.7.1)
# =============================================================================
#
# Spec 搂 Step 3a (line 439): the v3.7.1 lint enforces three invariants on the
...
def test_step3a_lint_passes_on_clean_tree() -> None:
    """Clean tree (after Step 3a populates) 鈫?lint passes invariants AND SHA gate."""
    result = _run_lint()
    assert result.returncode == 0, (
...
def test_step3a_block_missing_from_manifest_agent_fails() -> None:
    """Removing the entire block from a manifest-listed agent 鈫?lint fails.
...
# REDACTED: sensitive-looking memory line
    ...
    Pre-R1 negation rule: any negation in the sentence 鈫?pass.
    Post-R1 rule: negation must be within 鈮?0 chars BEFORE the read verb.

    Mutation: insert a sentence like "Never guess; read the entry frontmatter
    to find the slug" 鈥?a sloppy negation that would have slipped past R0.
    """
    target = _agent_path("deep-research/agents/synthesis_agent.md")
    with _Snapshot(target):
        text = target.read_text(encoding="utf-8")
...
def test_step3a_invariant_iii_canonical_negation_still_passes() -> None:
    ...
    """
    # No mutation 鈥?just verify clean tree passes.
    result = _run_lint()
    assert result.returncode == 0, (
...
def test_step3a_duplicate_block_rejected() -> None:
    ...
    with _Snapshot(target):
        ...
        duplicate = (
            "\n## Two-Layer Citation Emission (v3.7.1)\n\n"
            "duplicate body 鈥?this should be caught\n"
        )
...
def test_step3a_invariant_iii_bullet_boundary_is_clause_terminator() -> None:
    ...
    with _Snapshot(target):
        ...
        # bullet 2's left window pre-R2 (鈮?0 chars from `read`).
        injection = (
            "\n\n- NEVER skip\n"
...
def test_step3a_h3_same_title_duplicate_rejected() -> None:
    ...
    with _Snapshot(target):
        ...
        duplicate_h3 = (
            "\n### Two-Layer Citation Emission (v3.7.1)\n\n"
            "duplicate body 鈥?H3 typo or attack\n"
        )
...
def test_step3a_heading_drift_with_trailing_text_rejected() -> None:
    ...
    Pre-R3, `### Two-Layer Citation Emission (v3.7.1) 鈥?extended` slipped
    past because:
    ...
    with _Snapshot(target):
        text = target.read_text(encoding="utf-8")
        drift_block = (
            "\n### Two-Layer Citation Emission (v3.7.1) 鈥?extended\n\n"
            "The finalizer will resolve markers.\n"
        )
        target.write_text(text + drift_block, encoding="utf-8")
        result = _run_lint()
        assert result.returncode == 1, (
            "R3 P1-A: heading drift `### Two-Layer ... 鈥?extended` must be "

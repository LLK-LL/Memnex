#!/usr/bin/env python3
"""v3.9.4 Phase 4 鈫?5 boundary temporal integrity verifier.

5 passes per spec 搂3.2:
  P1 鈥?Mode 1 future-as-past arithmetic (TEMPORAL-ARITHMETIC-IMPOSSIBLE)
  P2 鈥?Mode 2 version-as-evidence-past anachronism (TEMPORAL-ANACHRONISTIC-CITATION)
  P3 鈥?Mode 3 comparator unmaterialized (TEMPORAL-COMPARATOR-UNMATERIALIZED)
  P4 鈥?Mode 4 causal inversion (TEMPORAL-CAUSAL-INVERSION)
  P5 鈥?Mode 5 time-bomb deictic (TEMPORAL-DEICTIC)
...
# 8 verb-to-required-ordering triggers per spec 搂3.2 P4
CAUSAL_TRIGGERS = [
...
def _date_to_interval(raw: str) -> tuple[str, str]:
    ...
    - YYYY-MM-DD 鈫?(date, date) point interval (day precision)
    - YYYY-MM 鈫?YYYY-MM-01 .. YYYY-MM-last (month precision, v3.9.4.1 hotfix)
    - YYYY-MM-DD..YYYY-MM-DD 鈫?parsed interval (interval precision, v3.9.4.1 hotfix)
    - 'MonthName YYYY' 鈫?first of month .. last of month (prose form)
    - YYYY 鈫?YYYY-01-01 .. YYYY-12-31 (year precision)

    v3.9.4 only handled 3 prose forms 鈥?Crossref month-precision lookups from
...
def _provenance_confidence(slug: str, citation_provenance: dict) -> str | None:
    ...
    Per spec 搂3.4: when confidence is 'low' or 'conflict', P2/P4 must NOT use the
...
def _pass_1_arithmetic(draft: str, findings: list[dict]) -> None:
    ...
    for sentence in re.split(r"(?<=[.!?])\s+", draft):
        ...
        # Emit one finding per sentence 鈥?pick the largest-gap violation.
...
def _pass_2_anachronism(draft: str, timeline: dict, citation_provenance: dict, findings: list[dict]) -> None:
    ...
    1. v3.9.4.1 hotfix: Lookup slug in citation_provenance. If confidence is 'low' or
       'conflict' 鈫?emit TEMPORAL-METADATA-MISSING and skip arithmetic (spec 搂3.4 promise:
       first-party-unverified dates are NOT used as ground truth).
    2. Lookup slug in timeline sources. Absent 鈫?emit TEMPORAL-METADATA-MISSING.
    3. Lookup effective_date_range. Absent or start unverified/low 鈫?emit METADATA-MISSING.
    4. Find nearest event date in 卤200 chars around the ref marker.
    5. Predicate (future-version): start > event.end 鈫?emit TEMPORAL-ANACHRONISTIC-CITATION.
    6. Predicate (superseded-version): end < event.start (only when end.open_ended: false
       and end.value known and confidence high/medium) 鈫?emit TEMPORAL-ANACHRONISTIC-CITATION.
    """
    ...
    for m_ref in REF_MARKER_PATTERN.finditer(draft):
        ...
        # v3.9.4.1 hotfix: provenance confidence gate (spec 搂3.4)
        ...
        if prov_conf in {"low", "conflict"}:
            findings.append({
                ...
                "bound_event": None,
                "bound_dates": None,
                "rationale": f"<!--ref:{slug}--> citation_provenance confidence={prov_conf}; per spec 搂3.4 not used as arithmetic ground truth.",
                "suggested_fix": None,
            })
            continue
        ...
        # Find nearest event date in 卤200 chars around ref marker
        ...
        if not event_dates:
            continue  # no event date 鈫?no finding
        ...
        # Future-version check (spec 搂3.2 P2 step 5 first clause): start > event.end
        ...
        # Superseded-version check (spec 搂3.2 P2 step 5 second clause)
        end = edr.get("end", {})
...
def _pass_3_comparator(draft: str, timeline: dict, findings: list[dict]) -> None:
    ...
    Per spec 搂3.2 P3: each match independently emits up to one finding per match;
        ...
        for pattern_name, pat in [("A", COMPARATOR_FORM_A), ("B", COMPARATOR_FORM_B), ("C", COMPARATOR_FORM_C)]:
            for m in pat.finditer(sentence):
                ...
                if not refs_in_sentence:
                    continue  # binding ambiguous 鈥?emit no finding
                ...
                if not matched:
                    ...
                    # do NOT break 鈥?spec allows one finding per match
...
def _pass_4_causal(draft: str, timeline: dict, citation_provenance: dict, findings: list[dict]) -> None:
    ...
    in the sentence (v3.9.4.1 hotfix: spec 搂3.2 P4 fallback).

    v3.9.4.1 also adds citation_provenance gate (spec 搂3.4): if either bound slug has
    ...
    for sentence in re.split(r"(?<=[.!?])\s+", draft):
        ...
        for trigger_pat, required_order in CAUSAL_TRIGGERS:
            ...
            if left_slug is None:
                # v3.9.4.1 fix #3: direct date fallback (spec 搂3.2 P4)
            ...
            for chk_slug in [left_slug, right_slug]:
                ...
                if prov_conf in {"low", "conflict"}:
                    findings.append({
                        ...
                        "bound_event": None,
                        "bound_dates": None,
                        "rationale": f"<!--ref:{chk_slug}--> citation_provenance confidence={prov_conf}; per spec 搂3.4 not used as arithmetic ground truth for P4.",
                        "suggested_fix": None,
                    })
...
def audit(draft: str, timeline: dict, citation_provenance: dict,
    ...
    v3.9.4.1 hotfix: citation_provenance now flows through to P2 and P4 (per spec 搂3.4).
    """
    findings: list[dict] = []
...
def _render_markdown(result: dict) -> str:
    ...
    for f in result["findings"]:
        lines.extend([
            f"## {f['finding_id']} 鈥?{f['finding_kind']} ({f['severity']})",
            "",
            f"- mode: {f['mode']}",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="v3.9.4 temporal integrity verifier (Phase 4 鈫?5 boundary)")

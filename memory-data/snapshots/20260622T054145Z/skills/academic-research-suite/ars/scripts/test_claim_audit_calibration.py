...
- T-C3: gold-set shape integrity 鈥?each tuple has tuple_kind 鈭?{alignment,
  constraint}, alignment tuples carry expected_judgment 鈭?{SUPPORTED,
  UNSUPPORTED, AMBIGUOUS, RETRIEVAL_FAILED} with no constraint fields,
  constraint tuples carry expected_judgment 鈭?{VIOLATED, NOT_VIOLATED} +
  constraint_under_test_id + (constraint_under_test_rule_text OR
  manifest_fixture_path), and the gold set has 鈮? NOT_VIOLATED constraint
...
Spec 搂 7 writes `tests/test_claim_audit_calibration.py`; repo convention
is `scripts/test_*.py` per spec 搂13 step 9 path-mapping rule. CI uses
...
def _bad_judge() -> Callable[..., dict[str, Any]]:
    ...
    鈥?same hardening _perfect_judge already carries.
    """
    tuples_by_key: dict[tuple[str, str, str | None], dict[str, Any]] = {
        _tuple_lookup_key(t): t for t in _load_gold_set()
    }
    ...
    def fn(**kwargs: Any) -> dict[str, Any]:
        ...
        # Constraint path 鈥?flip VIOLATED 鈫?NOT_VIOLATED.
...
# ---------------------------------------------------------------------------
# T-C3 鈥?Gold-set shape integrity.
# ---------------------------------------------------------------------------
...
# REDACTED: sensitive-looking memory line
    ...
    Spec 搂7.7 list (a)-(d):
      (a) every tuple has tuple_kind 鈭?{alignment, constraint}
      (b) alignment tuples 鈫?expected_judgment in 4-alignment-judgment set,
          NO constraint fields
      (c) constraint tuples 鈫?expected_judgment 鈭?{VIOLATED, NOT_VIOLATED}
          AND constraint_under_test_id AND
          (constraint_under_test_rule_text OR manifest_fixture_path)
      (d) 鈮? NOT_VIOLATED constraint tuples (else FPR is unmeasurable)
    ...
    def test_a_every_tuple_has_valid_tuple_kind(self) -> None:
        # All 20 tuples must declare tuple_kind 鈭?{alignment, constraint}.
        # Spec 搂7.7 rule (a).
    ...
    def test_b_alignment_tuples_shape_pinned(self) -> None:
        ...
        # MUST NOT carry constraint fields. Spec 搂7.7 rule (b).
    ...
    def test_c_constraint_tuples_have_required_fields(self) -> None:
        # Constraint tuples carry expected_judgment 鈭?{VIOLATED, NOT_VIOLATED}
        # AND constraint_under_test_id AND
        # (constraint_under_test_rule_text OR manifest_fixture_path). Spec 搂7.7 rule (c).
        for idx, tup in enumerate(self.gold_set):
    ...
    def test_d_at_least_three_not_violated_constraint_tuples(self) -> None:
        # 鈮? NOT_VIOLATED constraint tuples 鈥?without them constraint FPR
        # is unmeasurable and T-C1 cannot fail-on-threshold on the
        # constraint line. Spec 搂7.7 rule (d).
        not_violated = [
        ...
        self.assertGreaterEqual(
            len(not_violated),
            3,
            f"gold set must include 鈮? NOT_VIOLATED constraint tuples; "
            f"got {len(not_violated)}",
        )

    def test_validate_gold_set_rejects_invalid_tuple_kind(self) -> None:
        # Rule (a) negative ingestion test 鈥?R1 codex P3 + Gemini P2
    ...
    def test_validate_gold_set_rejects_constraint_without_rule_text(self) -> None:
        # Rule (c) 鈥?constraint tuple missing both rule_text AND
# REDACTED: sensitive-looking memory line
        broken = [
            {
    ...
    def test_validate_gold_set_rejects_under_three_not_violated(self) -> None:
        # Rule (d) 鈥?fewer than 3 NOT_VIOLATED constraint tuples.
    ...
    def test_validate_gold_set_accepts_canonical_gold_set(self) -> None:
        # Positive path 鈥?the shipped gold set MUST validate cleanly.
    ...
    def test_run_calibration_rejects_manifest_only_constraint_tuple(self) -> None:
        ...
        # rule_text OR manifest_fixture_path per spec 搂7.7 rule (c), but
...
# REDACTED: sensitive-looking memory line
        ...
        # (spec 搂7.7 rule (c) second branch).
...
# ---------------------------------------------------------------------------
# T-C2 鈥?Per-class FNR/FPR reporting.
# ---------------------------------------------------------------------------
...
class TC2PerClassReport(unittest.TestCase):
    ...
    Spec 搂7.7: 'FNR/FPR are computed AND surfaced per judgment-class
    ...
    def test_per_class_includes_four_judgment_classes(self) -> None:
        # Spec 搂7.7 enumerates SUPPORTED, UNSUPPORTED, AMBIGUOUS, and
    ...
    def test_each_class_exposes_fnr_and_fpr(self) -> None:
        ...
        # Reviewer asymmetry 鈥?missing FPR on UNSUPPORTED was the
    ...
    def test_each_class_exposes_denominators(self) -> None:
        ...
        # Gemini P3 closure 鈥?earlier test only asserted FNR/FPR keys.
        per_class = self.report["per_class"]
        for cls, payload in per_class.items():
    ...
    def test_canonical_denominators_match_gold_set(self) -> None:
        ...
        # T-C2 contract pinning 鈥?without this a future gold-set
...
class TC1ThresholdEnforcement(unittest.TestCase):
    ...
    Spec 搂7.7: 'FNR < 0.15 AND FPR < 0.10 against the synthetic gold set.
# REDACTED: sensitive-looking memory line
    ...
    def test_fnr_below_threshold(self) -> None:
        # Spec 搂7.7 gate: FNR < 0.15. Perfect-judge stub yields FNR = 0.
        self.assertLess(
            self.report["FNR"],
            0.15,
            f"FNR threshold violation: {self.report['FNR']!r} 鈮?0.15",
        )

    def test_fpr_below_threshold(self) -> None:
        # Spec 搂7.7 gate: FPR < 0.10. Perfect-judge stub yields FPR = 0.
        self.assertLess(
            self.report["FPR"],
            0.10,
            f"FPR threshold violation: {self.report['FPR']!r} 鈮?0.10",
        )
    ...
    def test_report_records_thresholds_used(self) -> None:
        ...
        # threshold tightening (spec bump). Spec 搂7.7 + 搂9 acceptance.
...
class TC1ThresholdEnforcementBadJudge(unittest.TestCase):
    """T-C1 companion 鈥?proves the threshold gate accurately fires on a
    degraded judge.
    ...
    搂7.7 thresholds (0.15 / 0.10). The test asserts run_calibration's
    output is what an operator running CI on a degraded judge would
    see: FNR 鈮?0.15 AND FPR 鈮?0.10. Operational deployment swaps
    ...
    def test_bad_judge_drives_fnr_over_threshold(self) -> None:
        # Bad judge flips every alignment expected 鈫?opposite class +
        # every VIOLATED 鈫?NOT_VIOLATED. Aggregate FNR must be >= 0.15.
        self.assertGreaterEqual(
            self.report["FNR"],
            0.15,
    ...
    def test_bad_judge_drives_fpr_over_threshold(self) -> None:
        ...
        # NOT_VIOLATED 鈫?VIOLATED flip on constraints both contribute.
        self.assertGreaterEqual(
            self.report["FPR"],
            0.10,
    ...
    def test_bad_judge_per_class_fnr_non_zero(self) -> None:
        ...
        # classes 鈥?confirms the per_class accumulator is wired to the

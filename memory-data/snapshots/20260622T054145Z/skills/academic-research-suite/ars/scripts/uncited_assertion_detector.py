...
搂"Uncited-assertion detector (D4-c)" and exercised by
...
`uncited_sentences` parameter 鈥?this module is the pre-processing
...
def _is_year_or_version_or_section(
    ...
    Four disqualifying shapes:
      ...
      2. Dotted X.Y.Z[.W...] form 鈥?version triple OR deep section number.
      3. Bare integer OR dotted X.Y form preceded by a section cue
         (`section`, `figure`, `chapter`, `table`, `fig.`, `tbl.`,
         `step`, `appendix`, `搂`) 鈥?covers `Table 2`, `Section 5`,
      ...
      3b. Dotted X.Y form preceded by `v` (version literal) 鈥?distinct
      ...
      4. Any match (bare or dotted) whose immediate left neighbour is a
         dotted-number suffix like `3.` 鈥?handles the case where Python's
...
# REDACTED: sensitive-looking memory line
    ...
    Percent (`50%`) and `N of M` matches never reach this guard 鈥?the
...
# REDACTED: sensitive-looking memory line
    ...
    #
    # Shape (ii) is restricted to dotted matches only 鈥?a bare integer
    ...
    # P2-NEW-B).
    if match_start > 0:
        # Shape (i) 鈥?immediate left `.` always reattaches.
        if sentence[match_start - 1] == ".":
            ...
            if RE_NUMERIC_LEFT_ATTACHED.search(left):
                return True
        # Shape (ii) 鈥?whitespace-separated only for dotted right tails.
...
# REDACTED: sensitive-looking memory line
    """
    # Condition 3 fires first 鈥?if the sentence is definitional we never
# REDACTED: sensitive-looking memory line
    lowered = sentence.lower()
    if any(phrase in lowered for phrase in UNCITED_DEFINITION_PHRASES):
        return False, []

    # Condition 2 鈥?ref marker present means the sentence is properly
    # cited under v3.7.3 Three-Layer Citation Emission.
    if RE_REF_MARKER.search(sentence):
        return False, []

    # Condition 1 鈥?collect every quantifier / verb match with its byte
# REDACTED: sensitive-looking memory line
...
def detect_uncited_assertions(
    ...
    or non-string `sentence_text` raises `ValueError` 鈥?silently treating
...
# REDACTED: sensitive-looking memory line
    ...
    鈥?those are owned by `claim_audit_pipeline._uncited_assertion_entry`
# REDACTED: sensitive-looking memory line

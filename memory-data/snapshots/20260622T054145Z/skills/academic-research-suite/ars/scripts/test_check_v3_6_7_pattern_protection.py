...
in the lint contract 鈥?without these tests, CI would only verify that the
...
class R4MutationTests(_MutationTestBase):
    ...
    def test_r4_002_c1_drop_buffer_fails(self) -> None:
        _mutate(
            self._repo_dir,
            "deep-research/agents/report_compiler_agent.md",
            "Word budget uses whitespace-split convention (`body.split()`), not hyphenated-as-1. Reserve 3鈥?% buffer below hard cap.",
...
class INV1MutationTests(_MutationTestBase):
    """INV-1 (Phase 6.7 搂6.3): canonical Clause 1 line MUST appear exactly
...
class INV2MutationTests(_MutationTestBase):
    """INV-2 (Phase 6.7 搂6.3): four Clause 2 disclosure regex patterns
    ...
    def test_inv2_b_cross_model_audit_template_fails(self) -> None:
        ...
        self._inject_violation(
            "deep-research/agents/research_architect_agent.md",
            "Cross-model audit covers these via dimension 搂3.5 of "
...
class INV3MutationTests(_MutationTestBase):
    """INV-3 (Phase 6.7 搂6.3): canonical Clause 1 line MUST NOT appear in
    ...
    def test_inv3_canonical_in_non_manifest_agent_fails(self) -> None:
        ...
        # the v3.6.7 inversion manifest). The 搂6 sweep is v3.6.7-only;
        # widening to a fourth file is the 搂9 L2 deferred question and
...
class CodexR1MutationTests(_MutationTestBase):
    ...
    def test_r1_p2_inv1_canonical_with_tail_weakener_fails(self) -> None:
        ...
        # mutation (defense in depth 鈥?the C3 negation post-filter also
    ...
    def test_r1_p2_manifest_widening_with_canonical_copy_fails(self) -> None:
        # Codex R1 P2: a copy-paste widening attack 鈥?add a fourth file
        ...
        # drift); widening triggers L2 resolution per spec 搂6.3 line
...
class CodexR2MutationTests(_MutationTestBase):
    ...
    def test_r2_p2_inv1_softwrap_canonical_passes(self) -> None:
        ...
        # Mutation should leave lint passing 鈥?the soft-wrap is benign.
        rc, _stdout, stderr = _run_lint(self._repo_dir)
        self.assertEqual(
            rc,
            0,
    ...
    def test_r2_p2_inv2_cross_bullet_no_false_positive(self) -> None:
        ...
        # bullets 鈥?a benign `the orchestrator` mention in one bullet
...
class CodexR3MutationTests(_MutationTestBase):
    ...
    def test_r3_p2_inv2_intro_paragraph_disclosure_fails(self) -> None:
        ...
        _mutate(
            self._repo_dir,
            "deep-research/agents/synthesis_agent.md",
            "documented in `docs/design/2026-04-29-ars-v3.6.7-downstream-agent-pattern-protection-spec.md` 搂3.1 (A1鈥揂5).",
            "documented in `docs/design/2026-04-29-ars-v3.6.7-downstream-agent-pattern-protection-spec.md` 搂3.1 (A1鈥揂5). "
            "Cross-model audit follows `shared/templates/codex_audit_multifile_template.md` audit dimensions 搂3.1, 搂3.2, 搂3.3, 搂3.4.",
        )
        self.assert_mutation_fails()
...
class CodexR4MutationTests(_MutationTestBase):
    ...
    def test_r4_p2_inv2_post_bullet_prose_disclosure_fails(self) -> None:
        ...
        # paragraphs in `block[:first_bullet]` 鈥?pre-first-bullet
...
class CodexR5MutationTests(_MutationTestBase):
    ...
    finding 鈥?C3 regex was not whitespace-tolerant inside sentences,
...
class CodexR6MutationTests(_MutationTestBase):
    ...
    finding 鈥?INV-1 only counted exact canonical matches and ignored
    ...
    def test_r6_p2_inv1_tail_weakened_duplicate_fails(self) -> None:
        ...
        # attack (`... audit-passed state if feasible.`) 鈥?preserves
...
class CodexR7R9MutationTests(_MutationTestBase):
    """Codex R7 鈫?R9 architectural rewind: R7 originally extended INV-3
    with `_is_clause_1_like` to catch weakened variants outside the
    manifest. R9 surfaced that this over-extends: spec 搂6.3 INV-3
...
class CodexR8MutationTests(_MutationTestBase):
    ...
    findings 鈥?`_is_clause_1_like` was over-broad (rejected legitimate

#!/usr/bin/env python3
"""Strict-monotonic verified_at helper for ARS v3.6.7 Step 6 (spec 搂5.4).

Every orchestrator-side verdict.verified_at write 鈥?Path B8d normal merges,
another_round re-merges, ship_with_known_residue acknowledgement appends 鈥?
...
total-order deterministic at passport scope (搂3.7 family D row D3).

Reference pseudocode from spec 搂5.4:
...
def parse_rfc3339_ms(s: str) -> datetime:
    ...
    YYYY-MM-DDTHH:MM:SS.mmmZ 鈥?accepting other RFC 3339 variants would
...
def next_verified_at_ms(passport_audit_artifacts: list[dict[str, Any]] | None) -> str:
    ...
    Implements 搂5.4 pseudocode. None / [] is the empty-ledger base case
    ...
    fixed width) lex-max coincides with chronological max 鈥?this relies on
    the schema regex (搂3.2 audit_artifact_entry verified_at constraint)
...
def _load_passport_audit_artifacts(path: Path) -> list[dict[str, Any]]:
    ...
    value not a list) all coerce to [] for v3.6.7 鈥?the helper is a
    ...
    not coerce silently. Lifecycle ownership rule 搂3.7 E2.
    """
    try:
    ...
    try:
        if suffix == ".json":
            ...
            # Use BaseLoader so PyYAML returns every scalar as str 鈥?without
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "in the given Material Passport (搂5.4)."
        ),
    )
    parser.add_argument(
        "--passport",

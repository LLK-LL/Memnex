...
Python module that implements the 搂4 Step 1-6 pipeline the agent prompt
...
Spec 搂7 names the test file `tests/test_claim_audit_pipeline.py`. Per
...
except Exception as exc:  # pragma: no cover 鈥?import-time error pathway is exercised in RED state
...
class _PipelineTestBase(unittest.TestCase):
    ...
    During the RED phase (Step 4 of the TDD plan in spec 搂13), the module
    `scripts/claim_audit_pipeline.py` does not exist yet 鈥?these tests
    ...
    def setUpClass(cls) -> None:
        if _MODULE_IMPORT_ERR is not None:
            raise unittest.SkipTest(
                ...
                "(expected during RED phase 鈥?implementation lands in spec 搂13 step 5)"
            )
...
# ---------------------------------------------------------------------------
# T-P1 鈥?Step 1 anchor=none short-circuit.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-P2 / T-P3 鈥?Step 3 cache hit / miss.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-P4 鈥?Step 2 ref_retrieval_method=failed 鈫?LOW-WARN paywall path.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-P5 鈥?Step 2 manual_pdf accepted; not_found triggers retrieval_existence.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-P6 鈥?Step 5 judge VIOLATED routes to negative_constraint_violation.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-P7 鈥?Step 6 defect_stage classification fixtures.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-P8 鈥?Precedence rule 1: drift + constraint violation 鈫?constraint absorbs drift.
# ---------------------------------------------------------------------------
...
class TP8DriftConstraintPrecedence(_PipelineTestBase):
    ...
    def test_constraint_absorbs_drift(self) -> None:
        ...
        self.assertEqual(
            drifts,
            [],
            "constraint violation MUST absorb drift signal 鈥?no companion claim_drifts[] entry per T-P8",
        )


# ---------------------------------------------------------------------------
# T-P9 鈥?Precedence rule 2: citation_anchor distinct from source_description.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T-P10 鈥?Precedence rule 3: uncited + manifest-claim sentence => uncited_assertion only.
# ---------------------------------------------------------------------------
...
# REDACTED: sensitive-looking memory line
        ...
        # also be a natural drift signal 鈥?but precedence rule 3 forbids the drift
...
class TP11CapSampling(_PipelineTestBase):
    ...
    def test_small_n_no_summary_or_telemetry(self) -> None:
        ...
        samplings = out["audit_sampling_summaries"]
        # Two valid outcomes per spec 搂4 step 3: zero summaries OR exactly one
...
# ---------------------------------------------------------------------------
# T-P12 鈥?Judge invocation failure mapping to INV-14 audit_tool_failure rows.
# Spec 搂4 step 2 + INV-14; Step 13 R1 codex P1 finding (judge errors must not
# abort the audit pass 鈥?they MUST surface as MED-WARN audit_tool_failure rows).
# ---------------------------------------------------------------------------


class TP12JudgeFailureAuditToolFailure(_PipelineTestBase):
    """T-P12: judge_fn exceptions / malformed output 鈫?audit_tool_failure row."""
    ...
    def test_cited_path_rejects_retrieval_failed_verdict(self) -> None:
        ...
        # / NOT_VIOLATED 鈥?they would crash in _judge_result_entry.
        def judge_fn(**_kw: Any) -> dict[str, Any]:
...
# REDACTED: sensitive-looking memory line
    ...
    # Step 13 R8 codex P2-3 鈥?judgment isinstance(str) guard before set
    ...
    # as judge_parse_error 鈫?audit_tool_failure (#120 P2-3).
...
# ---------------------------------------------------------------------------
# T-P14 鈥?retrieve_fn invocation failure mapping to INV-14 retrieval_* tags.
# Spec 搂4 step 2 + INV-14; Step 13 R2 codex P2 finding (symmetric to TP12 鈥?
...
class TP14RetrieveFailureAuditToolFailure(_PipelineTestBase):
    """T-P14: retrieve_fn exceptions / malformed output 鈫?audit_tool_failure row."""
    ...
    def test_malformed_return_non_string_method(self) -> None:
        ...
        # must surface as retrieval_api_error 鈫?audit_tool_failure.
        def retrieve_fn(_c: dict[str, Any]) -> dict[str, Any]:
    ...
    def test_api_method_without_excerpt_rejected(self) -> None:
        # Step 13 R3 codex P2 #3 鈥?api with empty excerpt would let the judge
...
# ---------------------------------------------------------------------------
# T-P13 鈥?EMITTED_NOT_INTENDED set-dedup per D6 (Step 13 R1 codex P2).
...
# ---------------------------------------------------------------------------
# T-P16 鈥?Uncited stream split (Step 13 R4 codex P1 #2).
...
class TP21UncitedClaimLevelNC(_PipelineTestBase):
    """T-P21: Step 13 R7 codex P1 鈥?uncited stream (d) must include claim-level
...
class TP22DuplicateMNCIdAcrossManifests(_PipelineTestBase):
    """T-P22: Step 13 R7 codex P2 鈥?when two manifests use the same MNC id
...
class TP20UncitedSentenceWithoutScope(_PipelineTestBase):
    """T-P20: Step 13 R6 codex P1 鈥?sentences in the documented all_uncited_sentences
    ...
    def test_uncited_sentence_no_scope_triggers_mnc_judge(self) -> None:
        ...
        sentence = {
            ...
            "section_path": "Discussion",
            # No scoped_manifest_id 鈥?documented Stage 4 sentence shape.
        }
        out = self.run_pipeline(
            citations=[],
            manifests=[manifest],
    ...
    def test_caller_provided_scope_restricts_mnc_set(self) -> None:
        ...
        # Sentence pinned to manifest A 鈥?judge should only see MNC-1.
        sentence = {
...
class TP19ConstraintAbsorptionFullManifestScope(_PipelineTestBase):
    """T-P19: Step 13 R5 codex P3 鈥?when a citation in manifest M violates a
    ...
    claim_id=C-999 (drifted) still produced EMITTED_NOT_INTENDED 鈥?that
    ...
    def test_drift_in_other_manifest_not_absorbed_by_violation_in_first(self) -> None:
        # Cross-manifest absorption is forbidden 鈥?manifest A violation MUST
        ...
        # Manifest B's C-100 is NOT emitted 鈫?INTENDED_NOT_EMITTED MUST still fire.
        intended_drift_b = [
...
class TP18ManifestMissingNoSpuriousDrift(_PipelineTestBase):
    ...
    def test_no_manifest_no_drift_rows(self) -> None:
        ...
        # Audit row still emits 鈥?the fallback is audit-only, not no-op.
        self.assertEqual(len(out["claim_audit_results"]), 1)
...
class TP17ManifestMissingSentinelFallback(_PipelineTestBase):
    ...
    Step 13 R4 codex P2 #3 鈥?row builders previously did
...
# REDACTED: sensitive-looking memory line
    ...
    def test_d4c_positive_emits_both_streams(self) -> None:
        # Sentence is BOTH D4-c-positive AND violates MNC 鈫?both rows emit
...
# REDACTED: sensitive-looking memory line
    ...
    def test_backwards_compat_when_all_uncited_omitted(self) -> None:
        ...
        # pipeline falls back to using that subset for stream (d) too 鈥?the
...
# REDACTED: sensitive-looking memory line
        ...
        out = self.run_pipeline(
            ...
            manifests=[manifest],
            uncited_sentences=[d4c_sentence],
            # all_uncited_sentences intentionally omitted 鈥?defaults to
...
# ---------------------------------------------------------------------------
# T-P15 鈥?Malformed cache hit 鈫?cache_corruption audit_tool_failure row.
...
class TP13EmittedNotIntendedDedupe(_PipelineTestBase):
    """T-P13: D6 set semantics 鈥?one drift row per drifted claim_text."""
...
# REDACTED: sensitive-looking memory line
    ...
    def test_two_refs_one_drift(self) -> None:
        ...
        self.assertEqual(
            len(drifts),
            1,
            f"D6 Emitted is a set of claim_text 鈥?one drifted text + two refs MUST yield 1 drift row; got {len(drifts)}",
        )
        self.assertEqual(drifts[0]["claim_text"], drifted_text)


# ---------------------------------------------------------------------------
# T-P23 鈥?v3.8.2 / #118: uncited path judge outage emits UAF row, not synthetic
...
class TP23UncitedJudgeOutageEmitsUAF(_PipelineTestBase):
    """T-P23 (v3.8.2 / #118): JudgeInvocationError on uncited path 鈫?UAF row."""
...
# REDACTED: sensitive-looking memory line
        ...
        self.assertEqual(
            ...
            [],
            "judge_timeout MUST NOT emit a constraint_violations[] row 鈥?that would be silent suppression of the HIGH-WARN check (pre-v3.8.2 bug)",
        )
    ...
    def test_uncited_judge_outage_no_audit_abort(self) -> None:
        ...
        # 3 unaffected. ConnectionError 鈫?judge_api_error per _invoke_judge's
...
# REDACTED: sensitive-looking memory line
        ...
        # judged fine (NOT_VIOLATED 鈫?no CV row, no UAF row).
        ...
        # Audit pass did NOT abort 鈥?3 judge invocations attempted.
    ...
    def test_uncited_judge_outage_nc_path_carries_manifest_claim_id(self) -> None:
        ...
        # ValueError 鈫?judge_parse_error per _invoke_judge translation.
        def failing_judge(**_kw: Any) -> dict[str, Any]:
        ...
        uncited_sentences = [
            {
                ...
                "manifest_claim_id": "C-001",
                # scoped_manifest_id absent 鈫?judge against ALL manifests
# REDACTED: sensitive-looking memory line
            }
        ]
        out = self.run_pipeline(
            ...
            claims=[
                {
                    ...
                    "intended_evidence_kind": "empirical",
                    "planned_refs": [],
                    # IMPORTANT: empty negative_constraints 鈥?judge call is MNC-only
                    "negative_constraints": [],
                }
            ],
            mncs=[{"constraint_id": "MNC-1", "rule": "Global rule."}],
...
# REDACTED: sensitive-looking memory line
        ...
        self.assertIsNone(
            ...
            "claim has no NC entries 鈫?judge call was MNC-only 鈫?manifest_claim_id must be null per spec 搂3.6",
        )

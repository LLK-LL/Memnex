...
clause detectable. Static only 鈥?does not validate runtime behaviour.
Behavioural validation belongs to spec 搂9 step 8 (live pipeline evaluation
...
# REDACTED: sensitive-looking memory line
                # span 鈥?it is the obligation's own vocabulary, not a
...
def reference_file_checks() -> list[Check]:
    """Spec 搂7.1 鈥?4 reference files. No block scoping; whole-file grep."""
    return [
        Check(
            pattern_id="REF-1 (B1)",
        ...
        Check(
            ...
            must_contain_regex=[
                # Rule 1: conservative inclusion 鈥?"include the phrase" is
                ...
                # Rule 2: anchor every entry 鈥?"where 鈥?and why" is the
                ...
                # Rule 3: no duplicates 鈥?bullet body must assert
                ...
                # Rule 4: verbatim preservation 鈥?both the title and the
                ...
                # Rule 5: failure surface 鈥?must say "rather than dropping",
...
def template_file_checks() -> list[Check]:
    """Spec 搂7.2 鈥?audit prompt template."""
    return [
        Check(
            pattern_id="TPL-1 (D1)",
        ...
        Check(
            ...
            must_contain_regex=[
                ...
                (
                    ...
                    r"len\(body\.split\(\)\).{0,200}\b3[-鈥揮5%\s+buffer\b",
                ),
                ...
                # than its anchor in the body" 鈥?anchored on "no claim" so
...
def synthesis_agent_checks() -> list[Check]:
    """Spec 搂6.1 鈥?synthesis_agent A1-A5 protection.
...
# REDACTED: sensitive-looking memory line
    ...
    return [
        Check(
            ...
            must_contain_regex=[
                # A1 鈥?cross-section consistency self-check. Bullet must
                ...
                # A2 鈥?pending-verification hedge. "wrap claims in explicit
                ...
                # A3 鈥?anchor justification. "include a one-line anchor
                ...
                # A4 鈥?quote scope boundary AND surrounding-context handling.
                # Spec 搂6.1 says "surrounding context paraphrased and
                ...
                # A5 鈥?declarative claims about un-provided documents are
                # forbidden AND the conditional-language fallback is
                # required. Spec 搂6.1 pairs the two: conditional language
...
def architect_agent_checks() -> list[Check]:
    """Spec 搂6.2 鈥?research_architect_agent (survey designer mode) B1-B5
    ...
    return [
        Check(
            ...
            block_marker=PROTECTION_BLOCK,
            must_contain_regex=[
                # B1 鈥?IRB terminology pass-through. "must pass through ...
                ...
                # B2 鈥?reverse-coded construct equivalence. "include a one-
                ...
                # B3 鈥?retrospective items default to event-anchored;
                ...
                # B4 鈥?three-part obligation per spec 搂6.2:
                ...
                # B5 鈥?option lists must declare primary-source list AND
                # enumerate fully AND prohibit subsetting AND prohibit
                # over-setting AND prohibit scope cross-contamination.
                # All five are the contract per spec 搂6.2; matching any
...
def compiler_agent_checks() -> list[Check]:
    """Spec 搂6.3 鈥?report_compiler_agent (abstract-only mode) C1-C3 protection."""
    return [
        Check(
            pattern_id="C1-C3",
            ...
            block_marker=PROTECTION_BLOCK,
            must_contain_regex=[
                # C1 word-count algorithm + buffer. Spec 搂6.3 pairs the
                # whitespace-split rule with "Reserve 3鈥?% buffer below
                ...
                (
                    ...
                    r"\bReserve\s+3[-鈥揮5%\s+buffer\s+below\s+hard cap\b",
                ),
                ...
                # C2 deictic forbidden 鈥?paired with above so a mutation
                # that drops the prohibition still fails.
                (
                    "C2 deictic temporal phrases forbidden",
                    r"\bDeictic temporal phrases\b[^.\n]{0,200}\bare forbidden\b",
                ),
                # C1 鈥?protected hedges are budget-protected / non-negotiable
                ...
                # single line so the bullet is whole-line verbatim 鈥?INV-1
...
# REDACTED: sensitive-looking memory line
                    ...
                    # same canonical bullet 鈥?matches INV-1's
...
# ---------------------------------------------------------------------------
# Inversion sweep checks (Phase 6.7 鈥?spec 搂6 partial inversion rule)
# ---------------------------------------------------------------------------
#
# Spec 搂6.3 defines INV-1/INV-2/INV-3 as the lint enforcement of the
# 搂6.2 sweep. These run alongside the keyword/regex Check pipeline above
...
#   Clause 1 line (defends against accidental sweep widening per 搂9 L2).

INVERSION_MANIFEST = REPO_ROOT / "scripts" / "v3_6_7_inversion_manifest.json"

# Canonical Clause 1 line, byte-aligned with spec 搂6.2 line 1767. Whitespace
...
# Spec 搂6.3 INV-2 regex set (a)-(d). Patterns are written as Python regex
# literals here (`|` for alternation, no Markdown escaping) 鈥?the spec
...
# All four compile with re.IGNORECASE only 鈥?DOTALL is not needed because
...
# Frozen v3.6.7 manifest contents per spec 搂6.3. The manifest file at
...
# refuse drifted manifests at lint time. Spec 搂6.3 line 1807 reserves
...
# (codex R1 P2 closure: add fourth file + canonical bullet 鈫?INV-1
...
def _load_inversion_manifest() -> tuple[list[str], str | None]:
    ...
    file paths (per spec 搂6.3 + EXPECTED_MANIFEST_FILES). Drift, addition,
    deletion, or duplication of entries is rejected 鈥?widening to a
    fourth file requires landing a v3.6.8+ manifest with its own scope
    tag, not retroactive edits to this manifest (per spec 搂6.3 line
    1807).
    """
    ...
    if set(files) != set(EXPECTED_MANIFEST_FILES):
        ...
        return [], (
            ...
            f"manifest with its own scope tag per spec 搂6.3 line 1807; "
...
# Bullet line 鈥?Markdown list item starting with `- ` at the start of a
...
def _iter_bullets(block: str) -> list[tuple[int, str]]:
    ...
    at bullet granularity, which is what the spec 搂6.3 means by "exactly
    ...
    for i, m in enumerate(starts):
        ...
        # Trim at the first blank line within the bullet 鈥?a blank line
...
# Audit-specific fragment markers 鈥?load-bearing pieces of the canonical
...
def _is_clause_1_like(bullet_text: str) -> bool:
    ...
    simulate`, `do not claim to have run`) alone are insufficient 鈥?
...
def _inv1_check_file(rel_path: str) -> tuple[bool, str]:
    ...
    Codex R2 P2 closure (substring match 鈫?bullet-extract + exact
    compare); R6 P2 closure (weakened-duplicate bypass 鈥?keep canonical
...
def _iter_block_segments(block: str) -> list[tuple[int, str, str]]:
    ...
    runs, paragraphs appended after the last bullet 鈥?anywhere in the
    block).
    ...
    just the pre-first-bullet region (R4). The spec 搂6.2 sweep promises
    ...
    for paragraph in paragraphs:
        ...
        if stripped.startswith("## "):
            continue
        if stripped.startswith("- "):
            # Bullet group 鈥?may be a single bullet or a wrapped multi-
...
def _inv2_check_file(rel_path: str) -> tuple[bool, list[str]]:
    ...
    for segment_offset, kind, normalized in _iter_block_segments(block):
        # Skip the canonical Clause 1 bullet 鈥?it inherently contains
# REDACTED: sensitive-looking memory line
        ...
        for label, pat in INV2_PATTERNS:
            ...
            errors.append(
                ...
                f"docs/design/2026-04-30-ars-v3.6.7-step-6-orchestrator-hooks-spec.md 搂6.2."
            )
...
# Directories scanned by INV-3. Spec 搂6.3 limits scope to agent prompt
# files under deep-research/agents/ and academic-pipeline/agents/. docs/,
# scripts/, and tests/ are excluded 鈥?the canonical line legitimately
...
def _inv3_check(manifest_files: list[str]) -> tuple[bool, list[str]]:
    ...
    - The Clause 1-like heuristic is a manifest-internal weakened-
      ...
      finalizing.`). Spec 搂6.3 INV-3 wording says "canonical Clause 1
      line found outside the manifest" 鈥?that means the actual
        ...
        for path in sorted(d.glob("*.md")):
            ...
            if offending_bullets or offending_prose:
                ...
                errors.append(
                    ...
                    f"spec 搂6.3 line 1807 and open the 搂9 L2 question; "
...
def inversion_sweep_results() -> list[tuple[str, str, bool, str]]:
    ...
    (pattern_id, description, ok, message) tuples 鈥?one row per check ID,
...
#
# Spec 搂9 ships v3.6.7 across multiple steps. Step 1 (this PR) shipped the
...
def main(argv: list[str]) -> int:
    ...
    if not _agent_checks_enabled():
        deferred_note = " (agent-prompt checks skipped 鈥?ARS_V3_6_7_AGENT_CHECKS=0)"
    summary = (

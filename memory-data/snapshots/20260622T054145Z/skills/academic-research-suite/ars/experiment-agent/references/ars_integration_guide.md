...
```
experiment-agent 鈹€鈹€knows鈹€鈹€> ARS handoff format
ARS 鈹€鈹€does not know鈹€鈹€> experiment-agent
User 鈹€鈹€bridges鈹€鈹€> between the two
```
...
- `run` mode output 鈫?`UNVERIFIED` (results not yet validated)
- `manage` mode output 鈫?`UNVERIFIED` (data collected but not analyzed)
- `plan` mode output 鈫?`UNVERIFIED` (design artifact only; nothing has been executed yet)
- `validate` mode output without a successful reproducibility re-run 鈫?`ANALYZED` (statistical interpretation completed, but execution-level verification is absent, not applicable, or failed)
- `validate` mode output with a successful reproducibility re-run 鈫?`VERIFIED`
...
## User Workflow: ARS 鈫?experiment-agent 鈫?ARS

```
1. User runs ARS Stage 1 (deep-research) 鈫?gets RQ Brief + Methodology Blueprint
2. User copies relevant sections to experiment-agent
3. experiment-agent: plan mode 鈫?run/manage mode 鈫?validate mode 鈫?produces results
...
6. ARS Stage 2 writer sees Material Passport 鈫?knows origin and verification status
```
...
At Stage 2 start:
  if user input contains "## Material Passport" with "Origin Skill: experiment-agent":
    鈫?auto-load experiment results as Stage 2 source material
    鈫?skip "do you have experiment data?" question
  else:
    鈫?normal flow
```

This requires a one-line detection in ARS pipeline_orchestrator_agent. Not implemented in v1 鈥?documented here for future reference.

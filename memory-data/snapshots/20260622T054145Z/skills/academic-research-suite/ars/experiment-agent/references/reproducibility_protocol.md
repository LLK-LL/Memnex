...
- If not applicable, skip reproducibility verification and report: "Reproducibility: N/A 鈥?[reason]"
...
2. code_runner_agent runs full EXECUTE 鈫?MONITOR 鈫?COLLECT cycle
...
- **Missing original results**: Cannot compare 鈫?report "Reproducibility: CANNOT_VERIFY 鈥?original results not provided"
- **Environment changed**: Warn "environment may differ from original run 鈥?results may not match even if code is correct"
- **Original metric is zero**: Use the symmetric denominator rule above; if both values are zero, treat the metric as a match
- **Partial output**: If original has 5 output files but re-run produces 4 鈫?`PARTIALLY_REPRODUCIBLE` + flag missing file

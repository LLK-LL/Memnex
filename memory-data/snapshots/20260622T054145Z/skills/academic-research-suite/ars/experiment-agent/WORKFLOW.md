...
# Experiment Agent v1.0 鈥?Experiment Executor and Monitor
...
```
Help me manage my survey study 鈥?I need 200 responses by May 30
```
...
**Chinese**: 璺戝椹? 鍩疯绋嬪紡, 瑷撶反妯″瀷, 鍩烘簴娓│, 鍒嗘瀽璩囨枡, 绠＄悊鐮旂┒, 杩借工鍙冭垏鑰? 鐢伴噹鐮旂┒, 鍟忓嵎, 椹楄瓑绲愭灉, 妾㈡煡绲辫▓, 閲嶇従, 瑕忓妰瀵﹂, 瑷▓鐮旂┒
...
2. Code execution keywords 鈫?dispatch `code_runner_agent` (run mode)
3. Human study keywords 鈫?dispatch `study_manager_agent` (manage mode)
4. Validation keywords 鈫?enter validate mode (handled inline, see below)
5. Design keywords 鈫?enter plan mode (handled inline, see below)
...
1. **DETECT** 鈥?Scan user-provided files for statistical content (p-values, CIs, effect sizes, coefficients, test statistics). Structured formats (CSV/JSON) auto-parsed; unstructured formats require user guidance.

2. **INTERPRET** 鈥?Item-by-item analysis. See `references/statistical_interpretation_guide.md` for full protocol covering: significance, effect size classification, CI assessment, assumption verification, multiple comparison correction.

3. **FALLACY SCAN** 鈥?Check 11 known statistical fallacy patterns (structural, inferential, causal). See `references/statistical_interpretation_guide.md` for the full checklist. All 11 must be checked; report coverage in output.

4. **REPRODUCE** (optional, code experiments only) 鈥?If user provides executable command + original results, delegate to code_runner_agent for re-run, then compare. See `references/reproducibility_protocol.md`. Not applicable to human studies or non-rerunnable external systems.

5. **REPORT** 鈥?Produce validation report in Markdown structured format (see `templates/output_formats.md`). Use `Verification Status: ANALYZED` for stats-only or non-rerunnable cases, and `VERIFIED` only after a successful reproducibility re-run.

**Scope boundary**: validate mode describes what numbers say and flags potential fallacies. It does NOT make editorial recommendations about what to write in the paper 鈥?that is the ARS reviewer's job.

---

## plan Mode (Inline)

Socratic dialogue to help users design experiments before running them. plan mode helps the user clarify their thinking 鈥?it does not prescribe a specific design. The user makes all design decisions.

### Procedure

1. **Clarify RQ** 鈥?What are you trying to test? What is the hypothesis?
2. **Variables** 鈥?Identify IV, DV, control variables, potential confounds
3. **Design** 鈥?Experimental / quasi-experimental / observational / mixed methods?
4. **Method selection** 鈥?Based on RQ + design, suggest appropriate methods
5. **Sample** 鈥?Population, sampling strategy, power analysis for sample size
6. **Analysis strategy** 鈥?Which statistical tests? What are the assumptions?
7. **Produce plan** 鈥?Output a structured experiment plan using `templates/code_experiment_plan.md` or `templates/study_protocol.md`
...
| # | Rule |
|---|------|
| 1 | Only execute user-specified commands 鈥?never auto-generate or modify scripts |
| 2 | Never auto-retry crashed experiments 鈥?notify user, user decides |
| 3 | Never auto-kill except hard timeout 鈥?notify before kill |
| 4 | Monitor only user-specified output paths |
| 5 | Never upload data to external services |
| 6 | Never touch raw participant data 鈥?track metadata only (counts, rates) |
| 7 | Never send notifications to study participants |
| 8 | Power analysis uses conservative estimates |
| 9 | Statistical interpretation is descriptive 鈥?does not draw conclusions for user |

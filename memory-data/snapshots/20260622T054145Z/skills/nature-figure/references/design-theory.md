...
|---------|-----------|---------------|
| Journal-final dense multi-panel figure at publication width | 7鈥? | 0.8鈥?.2 |
| Large comparison bar panels (figsize > 28in wide) | 24 | 3 |
| Compact subfigures / analytic plots | 15鈥?6 | 2 |
| Axis labels on large panels | 32鈥?4 (override per-label) | 鈥?|
| In-bar annotations | 32鈥?6 | 鈥?|
| Legend text on large panels | 28鈥?8 | 鈥?|
| Tick labels | 20鈥?6 | 鈥?|

When targeting the final dimensions of a two-column `Nature` figure page, start smaller than
slide-sized preview figures. The sampled 2026 papers routinely landed in the `7鈥? pt` final-text
...
- Keep only left + bottom spines 鈥?minimalist, Nature-approved.
...
PALETTE = {
    # Proposed / key method
    "blue_main":      "#0F4D92",   # deep blue 鈥?hero method
# REDACTED: sensitive-looking memory line

    # Positive / improvement shades (light 鈫?dark)
    "green_1": "#DDF3DE",
    "green_2": "#AADCA9",
    "green_3": "#8BCF8B",

    # Baseline / contrast shades (light 鈫?dark)
    "red_1":      "#F6CFCB",
    "red_2":      "#E9A6A1",
    "red_strong": "#B64342",
...
- **Imaging plates**: grayscale context + 1鈥? fluorescent accent channels on black.
...
# alpha=1.0 鈫?full method, alpha=0.2 鈫?minimal/ablated variant
```
...
|-------------|----------------|
| Journal-width composite page / asymmetric multi-panel | (7.0鈥?.4, 5.5鈥?.8) |
| Multi-metric bar (3鈥? metrics + legend) | (28鈥?5, 6鈥?2) |
...
**Rule**: Width 鈮?3鈥?脳 height for comparison bars; prevents vertical crowding and allows left-to-right narrative reading.
...
Never use fixed 0鈥?00 when values sit in a narrow band.
...
Print exact numbers inside or above bars at 32鈥?6pt for readability without a grid:
```python
...
- Line width: 2鈥?pt with controlled alpha.
- Marker size: 8鈥?2pt circles.
...
- Fading alpha for temporal progression:
  ...
  ```
- `fill_between` for uncertainty bands (keep alpha low: 0.1鈥?.2).
...
- Normalize per-spoke to display range (e.g., 45鈥?0) using per-benchmark tick lists.
...
# 鈹€鈹€ PRIMARY 鈹€鈹€ editable vector, text as <text> nodes 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
fig.savefig('./figures/name.svg', bbox_inches='tight')

# 鈹€鈹€ SECONDARY 鈹€鈹€ raster for quick preview / submission portals 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
**DPI guide (PNG only)**:
- `dpi=300` 鈥?standard for all figure types.
- `dpi=600` 鈥?dense bar panels with many methods.
...
```python
# heat: DataFrame (cohorts 脳 cell-type categories), values in %
...
Label quadrants ("Immune-hot / low tumor", "Immune-desert / high tumor", 鈥? with small grey text.
...
- [ ] Font size 鈮?16 base; 24 for large bar panels; 32鈥?4 for axis labels on large panels
...
- [ ] Y-limits tightened to data range (not 0鈥?00 when values are 80鈥?5)

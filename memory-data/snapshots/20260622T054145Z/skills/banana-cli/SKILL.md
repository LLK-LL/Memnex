...
If the user explicitly says `banana` and asks for images, PPT, slides, deck, presentation, or page generation, prefer `banana-ppt-workflow` as the controlling skill. Use this CLI skill as command reference only.
...
For formal Banana deliverables, do not use this quick path unless the controlling Banana workflow has selected it. The strict workflow may require preflight, content lock, image generation validation, and mode-specific export validation.
...
# 2. Generate a complete project through the generic CLI path
...
`--pages` is a hint to the AI 鈥?actual page count may differ. The CLI warns on stderr when they don't match.
...
- `--wait` / `tasks wait` can be interrupted (Ctrl+C) and resumed anytime 鈥?backend tasks are unaffected
- Progress lines go to stderr (format: `[PROGRESS] STAGE STATUS completed/total`), keeping stdout JSON clean
- `--help` output is plain text when piped (non-TTY) 鈥?safe for agent consumption

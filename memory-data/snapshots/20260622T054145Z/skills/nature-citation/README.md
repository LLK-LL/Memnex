...
This skill is bilingual-aware. It accepts Chinese manuscript text and citation requests such as "鍒嗘寮曠敤", "Nature绯诲垪寮曠敤", "CNS鍙婂瓙鍒?, "琛ュ紩鐢?, "鏀拺鏂囩尞", or "瀵煎嚭 Zotero", then searches with English scientific concepts while returning Chinese review notes by default.
...
鈹?      鈹溾攢鈹€ principles.md        # what it produces, journal scope, source hierarchy, search rules
鈹?      鈹溾攢鈹€ chinese-mode.md      # Chinese-user operating mode
鈹?      鈹斺攢鈹€ workflow.md          # the 7-step workflow + report format
鈹溾攢鈹€ references/                  # opened on demand
鈹?  鈹溾攢鈹€ script-usage.md          # nature_citation.py flags + long-article batch strategy
鈹?  鈹溾攢鈹€ journal-scope.md
鈹?  鈹溾攢鈹€ ris-endnote.md
鈹?  鈹斺攢鈹€ search-strategy.md
鈹斺攢鈹€ scripts/
    鈹斺攢鈹€ nature_citation.py
```
...
- restricting references to `Nature绯诲垪`, `CNS`, `CNS鍙婂叾瀛愬垔`, or `鍙湅姝ｅ垔`

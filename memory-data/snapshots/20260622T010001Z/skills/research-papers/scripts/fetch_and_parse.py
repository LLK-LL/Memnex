...
def _group_near_duplicates(
    ...
    if not has_dhash:
        # Pillow unavailable 鈥?fall back to exact-byte grouping
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            if sigs[j][1] == -1:
                # dHash failed for this image 鈥?match by exact bytes only
...
def _collect_html_figures(
    ...
    the mapping from original src 鈫?resolved URL 鈫?local file so that the
    ...
    # Build the inverse: each fetched local file 鈫?list of original src
...
def _collect_pdf_figures(
    ...
    鈥?true when "Figure N" / "Table N" / "Scheme N" / etc. appears within
...
def parse_html_to_markdown(
    ...
    Preserves heading hierarchy (h1-h6 鈫?#-######), figure boundaries with
    ...
    extracted from `<img>` tags inside the kept body 鈥?callers fetch and
...
def try_crossref(pid: PaperID, result: FetchResult) -> bool:
    ...
    # CSL JSON here 鈥?that representation isn't served at /works/{doi} and
    ...
    if status == 200 and body:
        ...
        if "pdf" in ctype or body[:4] == b"%PDF":
            ...
            if _save_html(body, pid, "doi_landing_html", result, source_url=url):
                ...
                result.note = (
                    "DOI landing page reached 鈥?likely abstract or paywall, not full text"
                )
...
def fetch_and_parse(identifier: str, force: bool = False) -> FetchResult:
    ...
    if not force:
        ...
        if cached and cached.status in ("ok", "abstract_only"):
            ...
            # Idempotent 鈥?upgrades older cached parses that lack a sentinel

...
## Stage 1 鈥?Index Local References (main thread)
...
## Stage 2 鈥?Parallel Specialist Agents
...
outputs. Each agent is given only the context it needs 鈥?do not pass the full
...
- Do NOT enumerate every possible requirement 鈥?list only the top constraints
- Write the file immediately after gathering key facts, do NOT keep searching

Sections:
1. ## Venue Requirements 鈥?format rules (page limit, structure, anonymization)
2. ## Review Criteria 鈥?what reviewers evaluate
3. ## Accepted Paper Patterns 鈥?1-2 structural patterns from scene reference
...
LIMITS:
- Analyze at most 3 papers (flash) or 6 papers (pro) 鈥?do NOT exceed tier count
...
Sections:
1. ## Exemplar Inventory 鈥?table: title, venue, year, why selected
2. ## Structural Patterns 鈥?2-3 reusable moves observed across exemplars
3. ## Rhetorical Patterns 鈥?1-2 opening/closing techniques
4. ## Language Patterns 鈥?brief note on register and conventions
...
LIMITS:
- Map at most 6 relevant SOTA papers 鈥?pick the most representative ones
...
- Each agent gets ONLY the context listed above 鈥?stripped-down, task-specific.
- Do NOT let agents see each other's instructions or outputs.
- All three write to `paper_rewriting_output/`.

## Stage 3 鈥?Merge and Synthesize (main thread)

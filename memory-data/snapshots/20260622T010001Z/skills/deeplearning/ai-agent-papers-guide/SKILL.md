...
A focused collection of AI agent research papers from 2024-2026, tracking the latest developments in LLM-based agent systems. Unlike broader collections, this focuses on recent breakthroughs 鈥?new architectures, benchmarks, multi-agent coordination, and real-world applications. Updated frequently as the field evolves rapidly.

## Paper Categories

```
Recent AI Agent Research
鈹溾攢鈹€ Agent Architectures
鈹?  鈹溾攢鈹€ Planning (o1-style reasoning, search-augmented)
鈹?  鈹溾攢鈹€ Memory (long-term, episodic, working)
鈹?  鈹斺攢鈹€ Tool use (function calling, code execution)
鈹溾攢鈹€ Multi-Agent Systems
鈹?  鈹溾攢鈹€ Collaboration (task decomposition, debate)
鈹?  鈹溾攢鈹€ Competition (red team, adversarial)
鈹?  鈹斺攢鈹€ Emergence (self-organization, culture)
鈹溾攢鈹€ Evaluation
鈹?  鈹溾攢鈹€ Benchmarks (SWE-bench, WebArena, GAIA)
鈹?  鈹溾攢鈹€ Safety (jailbreak, misuse, alignment)
鈹?  鈹斺攢鈹€ Reliability (error recovery, hallucination)
鈹溾攢鈹€ Applications
鈹?  鈹溾攢鈹€ Software engineering (coding agents)
鈹?  鈹溾攢鈹€ Scientific research (lab automation)
鈹?  鈹溾攢鈹€ Web automation (browsing, form-filling)
鈹?  鈹斺攢鈹€ Enterprise (workflow, data analysis)
鈹斺攢鈹€ Infrastructure
    鈹溾攢鈹€ Frameworks (LangGraph, CrewAI, AutoGen)
    鈹溾攢鈹€ Protocols (MCP, A2A, tool standards)
    鈹斺攢鈹€ Deployment (scaling, monitoring, cost)
```

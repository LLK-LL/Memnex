...
## Step 2 鈥?Map to the Paper Structure

Locate the most natural home for this citation. Think beyond the obvious insertion point: a single new citation often has ripple effects. If the cited work is relevant to claims A, C, and D already in the paper鈥攏ot just B where the user asked you to add it鈥攁cknowledge it at each relevant site, not only at the point of first insertion. A brief cross-reference or additional sentence at A, C, D may be warranted.

## Step 3 鈥?Integrate Gracefully

Avoid orphaned citations that appear without surrounding explanation. The sentence(s) surrounding the citation should make clear what it is doing rhetorically鈥攕upporting, contrasting, extending, or contextualizing. A bare `\cite{foo}` appended to an existing sentence is rarely sufficient.

## Step 4 鈥?BibTeX Hygiene

...
def slug_from_text(text: str, max_words: int = 6) -> str:
    ...
    words = re.findall(r"[A-Za-z0-9]+|[涓€-榭縘+", text)
    stopwords = {
...
def looks_like_heading(text: str) -> bool:
    ...
    if len(stripped) > 90:
        return False
    if stripped.endswith((".", "銆?, "!", "锛?, "?", "锛?)):
        return False
    words = stripped.split()
    if 0 < len(words) <= 8 and not any(char in stripped for char in ",;锛岋紱"):
        return True
    return False
...
def query_from_segment(text: str, max_words: int = 26) -> str:
    text = clean_text(text)
    text = re.sub(r"\[[^\]]+\]|\([A-Za-z]+ et al\.,? \d{4}\)", " ", text)
    words = re.findall(r"[A-Za-z0-9伪-蠅螒-惟尾纬未魏位渭谓蟺蟻蟽蟿蠀蠁蠂蠄蠅\-]+|[\u4e00-\u9fff]+", text)
    if not words:
        return text[:240]
...
def fallback_queries_from_segment(text: str) -> list[str]:
    words = re.findall(r"[A-Za-z0-9伪-蠅螒-惟尾纬未魏位渭谓蟺蟻蟽蟿蠀蠁蠂蠄蠅\-]+|[\u4e00-\u9fff]+", clean_text(text))
    if not words:
        return []
    stopwords = {
...
# REDACTED: sensitive-looking memory line
                    ...
                    >
                      ...
                      <p class="meta">{html.escape(candidate.journal)} 路 {html.escape(candidate.family or "Unclassified")} 路 {html.escape(candidate.year or "n.d.")}</p>
# REDACTED: sensitive-looking memory line
        ...
        cards.append(
            ...
            <section class="segment-card" id="{html.escape(segment.id)}">
              <div class="segment-head">
                <span class="segment-id">{html.escape(segment.id)} 路 Sentence {segment.order}</span>
  ...
  <script>
    ...
    function updateSelectedPanel() {{
      ...
      for (const item of items) {{
        ...
        wrapper.innerHTML = `<h4>${{item.title}}</h4><p>${{item.journal}} 路 ${{item.year || 'n.d.'}}</p>`;

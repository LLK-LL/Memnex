...
- `Nature绯诲垪`: search Nature Portfolio first. Include `Nature`, `Nature [field]`, `Nature Communications`, `Communications [field]`, `Scientific Reports`, and `npj` journals.
- `CNS`: search `Cell`, `Nature`, and `Science` plus their major sister journals.
- `CNS鍙婂叾瀛愬垔` or `CNS/sister journals`: search only accepted flagship and subjournal titles in Nature Portfolio, the AAAS Science family, and Cell Press.
- `鍙Nature/Science/Cell姝ｅ垔`: restrict to the flagship journals `Nature`, `Science`, and `Cell`.

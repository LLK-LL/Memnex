# ARS v3.6.4 Literature Corpus Adapters 鈥?Implementation Plan
...
```
Task 1 (foundation)
  鈹溾攢鈹€ Task 2 (literature_corpus_entry.schema.json)
  鈹?   鈹溾攢鈹€ Task 3 (rejection_log.schema.json)
  鈹?   鈹?   鈹斺攢鈹€ Task 4 (check_literature_corpus_schema.py)
  鈹?   鈹斺攢鈹€ Task 5 (_common.py)
  鈹?        鈹斺攢鈹€ Task 6 (conftest.py)
  鈹?             鈹溾攢鈹€ Task 7 (folder_scan.py + tests + fixtures)
  鈹?             鈹溾攢鈹€ Task 8 (zotero.py + tests + fixtures)
  鈹?             鈹斺攢鈹€ Task 9 (obsidian.py + tests + fixtures)
  鈹斺攢鈹€ ... later tasks build on outputs of 4-9

After 4-9 all done:
  Task 10 (overview.md)
    鈹斺攢鈹€ Task 11 (sync_adapter_docs.py)
...
default scan over scripts/adapters/examples/**. See design doc 搂6.1.
...
def parse_csl_name(raw: str) -> dict[str, str]:
    ...
    Rules:
    - "{Institution Name}" 鈫?{"literal": "Institution Name"}
    - "Family, Given" 鈫?{"family": "Family", "given": "Given"}
# REDACTED: sensitive-looking memory line
    """
    raw = raw.strip()
...
- Create: `scripts/adapters/examples/folder_scan/input_fixture/paper1.pdf` (empty file 鈥?will be rejected)
- Create: `scripts/adapters/examples/folder_scan/input_fixture/涓枃妾斿悕_2024.pdf` (empty file 鈥?will be rejected: unicode)
...
touch "scripts/adapters/examples/folder_scan/input_fixture/涓枃妾斿悕_2024.pdf"
...
```
Expected: fails 鈥?adapter does not exist.
...
tests remain stable across machines. See design doc 搂5.2.
...
---
citekey: tmpl2024
title: Template 鈥?should be skipped
# REDACTED: sensitive-looking memory line
  - family: Ignore
year: 2024
---
...
in user_notes. See design doc 搂5.4.
...
sync_adapter_docs.py to fill in next task. See design doc 搂5.1 + 搂6.2.
...
```python
"""Tests for sync_adapter_docs.py: schema 鈫?overview.md drift detector."""
from pathlib import Path
import subprocess
import pytest
...
```
Expected: fails 鈥?script missing.
...
git commit -m "$(cat <<'EOF'
feat(v3.6.4): sync_adapter_docs schema鈫抩verview.md drift detector
...
ls .github/workflows/
```
Expected: a set of `.yml` files. Identify the one that already runs `check_passport_reset_contract.py` 鈥?the new lint steps will live in the same file.
...
Use the Edit tool to add 鈥?immediately before the existing `check_passport_reset_contract.py` step 鈥?these two steps:
...
For each of `academic-pipeline/SKILL.md`, `academic-paper/SKILL.md`, `academic-paper-reviewer/SKILL.md`, `deep-research/SKILL.md`, find the version footer and bump 3.6.3 鈫?3.6.4.

- [ ] **Step 4: Update `.claude/CLAUDE.md`**

In `.claude/CLAUDE.md`:
- In the Skills Overview table, change `academic-pipeline v3.6.3` 鈫?`academic-pipeline v3.6.4`.
...
- **CI lint + pytest job**: schema validation, adapter example validation, schema鈫抎ocs drift check, and pytest for `scripts/adapters/tests/`.
...
Find the ARS version / skill version table. Bump `academic-pipeline 3.6.3` 鈫?`3.6.4`. Add one bullet under the "What's new" or "Highlights" section describing v3.6.4.
...
git status
git commit -m "$(cat <<'EOF'
chore(v3.6.4): version bump sweep 3.6.3 鈫?3.6.4
...
- **Rejection log contract** (`shared/contracts/passport/rejection_log.schema.json`). Always emitted; empty when no rejections.
- **CI lint and pytest job**: schema validation, adapter-example validation, schema鈫抎ocs sync check, and pytest runs for adapter tests.
...
explicit non-goals. See design doc 搂7.2.
...
Export your Zotero library as **Better BibTeX JSON** (Zotero 鈫?File 鈫?Export Library 鈫?Format: Better CSL JSON). Then:
...
user_notes. See design doc 搂2.2.
...
v3.6.4 鏅傛矑鏈変换浣?ARS agent 璁€鍙?`literature_corpus[]`銆傛娆勪綅鍍呯偤 input port 瀹氱京锛屾秷璨荤鏁村悎锛坅gent 瀵﹂殯浣跨敤瑾炴枡鍋氱爺绌惰鍔?/ citation 鐢熸垚 / ...锛夌暀寰?v3.6.5+銆?
```
...
- Version bump sweep 3.6.3 鈫?3.6.4 across four SKILL.md, CLAUDE.md, README, consistency lint.
...
Per design doc 搂7.3, this PR will receive at least three rounds of `/codex review --base main` + one fresh codex review before merge, following the `feedback_codex_review_vs_resume_audit_scope.md` protocol for large feature PRs.
...
**1. Spec coverage.** Checking each 搂-level section of the spec against task coverage:

- Spec 搂1 Scope / 搂1.2 Non-goals 鈫?Task 0 (implicit in plan header) + Task 15 CHANGELOG "Not changed" section.
- Spec 搂2 Architecture + file layout 鈫?Task 1 scaffolding + all subsequent tasks create the right files.
- Spec 搂3 literature_corpus_entry schema 鈫?Task 2.
- Spec 搂4 rejection_log schema 鈫?Task 3.
- Spec 搂5.1 Common contract 鈫?Task 5 (_common.py) + Task 10 (overview.md).
- Spec 搂5.2 folder_scan 鈫?Task 7.
- Spec 搂5.3 zotero 鈫?Task 8.
- Spec 搂5.4 obsidian 鈫?Task 9.
- Spec 搂6.1 lint A + B 鈫?Task 4.
- Spec 搂6.2 lint C 鈫?Task 11.
- Spec 搂6.3 pytest CI 鈫?Task 12.
- Spec 搂6.4 test coverage 鈫?Tasks 7/8/9 + Task 5 (common).
- Spec 搂6.5 existing CI additions 鈫?Task 12.
- Spec 搂7.1 sweep 鈫?Task 14.
- Spec 搂7.2 CHANGELOG 鈫?Task 15.
- Spec 搂7.3 release order 鈫?Task 18 (PR open) + out-of-plan codex audits (搂7.3 steps 4-10).
- Spec 搂7.4 rollback 鈫?not implemented, documented in design doc only (acceptable; rollback is a runtime decision, not a plan step).
- Spec 搂8 deferred questions 鈫?addressed at plan execution time (Python version, etc.).
...
**1. Subagent-Driven (recommended)** 鈥?dispatch a fresh subagent per task, review between tasks, fast iteration. Uses `superpowers:subagent-driven-development`.

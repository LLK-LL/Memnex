...
**Important**: You cannot push directly to this repo 鈥?you must fork it first and submit a PR from your fork.
...
- **Typo and formatting fixes** 鈥?spelling, broken links, markdown rendering issues
- **New examples** 鈥?pipeline output showcases, worked examples for specific disciplines
- **Translation improvements** 鈥?better zh-TW or EN phrasing in READMEs or agent definitions

### Requires maintainer review

These need careful review because they affect system behavior:

- **Journal and field reference lists** 鈥?additions to `top_journals_by_field.md`, new discipline glossaries
- **Evaluation sets** 鈥?gold-standard papers for calibration mode, benchmark data
- **New reference files** 鈥?methodology guides, citation format references, domain-specific protocols
- **Bug and drift fixes** 鈥?version inconsistencies, broken cross-references, incorrect metadata
- **Mode changes** 鈥?new modes, trigger keyword changes, oversight level adjustments

### Requires maintainer approval + discussion

Open an issue first before submitting a PR for these:

- **Agent definition changes** 鈥?modifications to any file in `*/agents/*.md`
- **IRON RULE modifications** 鈥?any change to rules marked with the IRON RULE marker
- **Ethics and integrity rules** 鈥?changes to the failure mode checklist, integrity protocols, or ethics review
- **Handoff schema changes** 鈥?modifications to `shared/handoff_schemas.md`
- **New skills or modes** 鈥?additions to the pipeline

### Platform ports (community-maintained only)

This repository is the reference distribution of ARS, built for Claude Code. Ports to other agent platforms (Opencode, Cursor, Continue, Aider, etc.) are accepted as community-maintained contributions. Two structural shapes are acceptable 鈥?both keep core ARS content as the source of truth:
...
- **Sibling distribution.** A separate repository that vendors ARS workflow content with: (1) upstream commit hash pinned (e.g. in a `manifest.json`); (2) a written update / sync policy; (3) vendored content unmodified 鈥?only the outer routing / adapter layer is platform-specific.
...
- **One concern per PR** 鈥?don't mix unrelated changes
- **Describe what and why** 鈥?explain the motivation, not just the change
- **Reference issues** 鈥?if your PR addresses an open issue, link it
- **Test your changes** 鈥?if you're modifying agent definitions, try running the skill to confirm it works as expected
- **Keep READMEs in sync** 鈥?if your change affects user-facing documentation, update both `README.md` and `README.zh-TW.md`
...
1. **Accuracy over completeness** 鈥?we'd rather have fewer, verified journal entries than a long unvetted list
2. **Human-in-the-loop always** 鈥?contributions that reduce human oversight or enable fully autonomous paper generation will be declined
3. **No detection evasion** 鈥?features designed to make AI-generated text harder to detect (as opposed to higher quality) are out of scope. See [Issue #3](https://github.com/Imbad0202/academic-research-skills/issues/3) for context.
4. **Discipline diversity welcome** 鈥?ARS defaults to higher education research but aims to be domain-agnostic. Discipline-specific modules are encouraged.

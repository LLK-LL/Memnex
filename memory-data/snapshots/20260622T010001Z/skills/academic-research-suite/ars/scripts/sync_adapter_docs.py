...
Exit codes:
  0 鈥?no drift (or rewrote successfully without --check)
  1 鈥?drift detected under --check
  2 鈥?invocation error (schema or target missing)
"""
...
def _short_desc(prop_def: dict) -> str:
    ...
    m = re.match(r"(.+?[.!?])(?:\s|$)", d)
    return (m.group(1) if m else d) or "鈥?

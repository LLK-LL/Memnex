"""Mutation tests for ARS v3.7.1 Step 3b 鈥?Cite-Time Provenance Finalizer.

Spec: docs/design/2026-04-30-ars-v3.6.8-trust-provenance-and-drift-transparency-spec.md
      搂 Step 3b 鈥?Pipeline finalizer (academic-pipeline mode)
...
def test_happy_path_passes_on_clean_tree() -> None:
    """Untouched pipeline_orchestrator (with finalizer subsection) 鈫?lint passes."""
    result = _run_lint()
...
#   Row 1: HIGH WARN  鈫?`[UNVERIFIED CITATION 鈥?NO ORIGINAL]`
#   Row 2: MED WARN   鈫?`[UNVERIFIED CITATION 鈥?AI HAS NOT CROSS-CHECKED]`
#   Row 3: LOW WARN   鈫?`<!--ref:slug LOW-WARN-->` + per-section pre-finalization checklist
#   Row 4: ok         鈫?`<!--ref:slug ok-->`


def test_high_warn_row_missing_fails() -> None:
    """Row 1 (HIGH WARN 鈥?NO ORIGINAL) must be present.

    Spec line 176: HIGH WARN row maps source_acquired=false to
    `[UNVERIFIED CITATION 鈥?NO ORIGINAL]<!--ref:slug-->`.
    """
    with _Snapshot(TARGET):
        text = TARGET.read_text(encoding="utf-8")
        # Replace the row's canonical phrase so the lint can't find it.
        marker = "UNVERIFIED CITATION 鈥?NO ORIGINAL"
...
def test_med_warn_row_missing_fails() -> None:
    """Row 2 (MED WARN 鈥?AI HAS NOT CROSS-CHECKED) must be present.

    Spec line 177: MED WARN row maps (acquired=true, verified=false,
    human=false) to `[UNVERIFIED CITATION 鈥?AI HAS NOT CROSS-CHECKED]`.
    """
    with _Snapshot(TARGET):
        text = TARGET.read_text(encoding="utf-8")
        marker = "UNVERIFIED CITATION 鈥?AI HAS NOT CROSS-CHECKED"
...
def test_low_warn_row_missing_fails() -> None:
    """Row 3 (LOW WARN 鈥?`<!--ref:slug LOW-WARN-->`) must be present.
...
def test_idempotency_clause_missing_fails() -> None:
    ...
    with _Snapshot(TARGET):
        ...
        # Strip every occurrence 鈥?heading "Idempotency" and prose use of
...
def test_demote_language_clause_missing_fails() -> None:
    ...
    demotion (e.g. `ok` 鈫?`LOW-WARN`). Anchor is the `demot` stem so
...
def test_med_warn_row_wildcard_human_read_caught() -> None:
    ...
    prose to use `鈥擿 (wildcard) for `human_read_source` in row 2 so the
    matrix is total over the 8 input combinations. The MED WARN canonical
    phrase `UNVERIFIED CITATION 鈥?AI HAS NOT CROSS-CHECKED` is unchanged;
    this test pins that the MED WARN row's third column carries `鈥擿
# REDACTED: sensitive-looking memory line
    contract is enforced at the prose level.
    """
    text = TARGET.read_text(encoding="utf-8")
    # The MED WARN row in the canonical block has the phrase
    # `UNVERIFIED CITATION 鈥?AI HAS NOT CROSS-CHECKED`. Locate that
    # phrase, walk back to the start of its table row, and assert the
# REDACTED: sensitive-looking memory line
    import re as _re
    # Find the MED-WARN row by anchor on its unique phrase.
    pos = text.find("UNVERIFIED CITATION 鈥?AI HAS NOT CROSS-CHECKED")
    ...
    assert len(parts) >= 5, f"expected 鈮? |-delimited parts in MED row, got {parts}"
    col3 = parts[3]
    assert col3 == "鈥?, (
        f"R3 P1: MED WARN row's `human_read_source` column must be `鈥擿 "
        f"(wildcard) so the matrix is total over the 8 input combinations; "
        f"got {col3!r}. Spec line 177 wrote `false` but that leaves "
        f"`(true, false, true)` undefined 鈥?the wildcard fix is intentional."
    )
...
# REDACTED: sensitive-looking memory line
    `<passport-stem>_human_read_log.yaml` per 搂3.6 round-5 R5-003 amend)

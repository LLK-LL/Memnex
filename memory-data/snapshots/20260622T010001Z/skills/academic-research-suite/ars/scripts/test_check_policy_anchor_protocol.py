...
# Each named invariant from impl spec 搂4.3 must be referenced verbatim. The
# protocol doc must also resolve each 搂4.4 #1鈥?11 concern by name and carry
# the 搂3 G10 7-row precedence table verbatim from Decision Doc.
_GOOD_PROTOCOL = textwrap.dedent(
    ...
    **Anchor inventory**: `prisma-trAIce, icmje, nature, ieee`
    ...
    ## Frozen invariants from Decision Doc 搂4.3

    - G1 invariant 鈥?no ai_disclosure field added to corpus entry schema
    - G2 invariant 鈥?SLR mode dispatches via slr_lineage to PRISMA-trAIce track
    - G3 / G10 invariant 鈥?seven-row precedence table holds; auto-promotion forbiddance
    - G4 invariant 鈥?4 policy-anchor renderers; Nature dedup
    - G5 invariant 鈥?PRISMA M6 fires only when all three gates hold
    - G7 invariant 鈥?copyediting carve-out anchor-specific
    - G8 invariant 鈥?IEEE #5 + #6 paired mandate
    - G9 invariant 鈥?image-rights regimes anchor-specific

    ## 搂4.4 open concerns resolved

    - concern #1 鈥?explicit slr_lineage input
    - concern #2 鈥?auto-detect tool identity per v3.2 Phase 4
    - concern #3 鈥?per-(tool 脳 task) prompt scope tuple
    - concern #4 鈥?IEEE locator free-form list with IMRaD exemplars
    - concern #5 鈥?Nature image hybrid annotation + suggested patches
    - concern #6 鈥?UNCERTAIN per-facet annotation alongside USED render
    - concern #7 鈥?venue+anchor conflict reject with error
    - concern #8 鈥?three-state completeness flag full spec
    - concern #9 鈥?test set scope covers all invariants + concerns
    - concern #10 鈥?ai_used:true force v3.2 categorization flow
# REDACTED: sensitive-looking memory line
    ...
    | # | Precondition | Output |
    |---|---|---|
    | 1 | ai_used:false AND 鈮? category USED | Honest conflict annotation |
    | 2 | ai_used:false AND no USED AND no UNCERTAIN | No-AI statement (G10 opt-in) |
    | 3 | ai_used:false AND 鈮? UNCERTAIN AND no USED | Honest tension annotation |
    | 4 | ai_used:true OR 鈮? USED, row 1 not match | Full anchor disclosure render |
    | 5 | 鈮? UNCERTAIN AND no USED AND no ai_used | Honest not-supplied annotation |
    ...
    the four anchor outputs. This is the 搂4.3 G3/G10 invariant the
    implementation enforces.

    ## Anchor lookup mechanism

    Renderer reads `policy_anchor_table.md` keyed by `--policy-anchor=<a>`
    where a 鈭?{prisma-trAIce, icmje, nature, ieee}.
...
class CheckPolicyAnchorProtocolMutationTests(unittest.TestCase):
    ...
    def test_missing_g10_table_row_fails(self) -> None:
        # Drop row 4 鈥?the load-bearing full-anchor-disclosure row
...
# REDACTED: sensitive-looking memory line
    ...
    def test_concern_number_boundary_matching(self) -> None:
        ...
        bad = _GOOD_PROTOCOL.replace("concern #1 鈥?, "deleted-clause 鈥?, 1)
    ...
    def test_missing_anchor_slug_fails(self) -> None:
        ...
        bad += "\n\nNote: prisma-trAIce renders are mentioned in 搂3.1.\n"
        violations = cpap.lint_text(bad)
        self.assertTrue(

...
def _write_aligned_fixture(root: Path) -> None:
    """Everything lines up 鈥?baseline for PASS cases and drift mutations."""
    skills = [
        ("deep-research", "2.9.0"),
...
class TestVersionConsistency(unittest.TestCase):
    ...
    def test_table_version_drift_fails(self) -> None:
        """Table lists deep-research v2.9.0 but SKILL.md says 2.8.0 鈥?must fail."""
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _write_aligned_fixture(root)
    ...
    def test_suite_version_vs_changelog_drift_fails(self) -> None:
        """CLAUDE.md suite version != CHANGELOG latest entry 鈥?must fail."""
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            _write_aligned_fixture(root)

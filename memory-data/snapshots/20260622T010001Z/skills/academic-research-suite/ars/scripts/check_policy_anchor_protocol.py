...
Decision Doc reference:
  docs/design/2026-05-14-ai-disclosure-schema-decision.md 搂4.1 (item 6) +
  搂4.3 invariants + 搂4.4 11 open concerns + 搂3 G10 7-row precedence table.

Implementation spec reference:
  docs/design/2026-05-14-ai-disclosure-impl-spec.md 搂3 (resolved-paths table).
...
Checks
------
1. All 8 搂4.3 invariants are named verbatim (G1 / G2 / G3 / G10 / G4 / G5 /
   G7 / G8 / G9 鈥?G3 and G10 combined into a single composite invariant).
2. All 11 搂4.4 concerns have a resolution clause referenced by number
   (`concern #1` through `concern #11`).
3. The 搂3 G10 7-row precedence table is present with all 7 rows
   (numbered 1..7).
...
5. The 4 canonical anchor slugs are enumerated in the lookup mechanism
   section.
6. The Nature 鈫?v3.2 venue dedup pointer
...
def lint_text(text: str) -> list[str]:
    ...
    for n in REQUIRED_CONCERN_NUMBERS:
        if n not in found_concern_numbers:
            violations.append(f"missing 搂4.4 concern #{n} resolution clause")
...
# REDACTED: sensitive-looking memory line
    ...
    if DEDUP_POINTER not in text:
        violations.append(
            f"missing Nature 鈫?v3.2 venue dedup pointer: {DEDUP_POINTER}"
        )

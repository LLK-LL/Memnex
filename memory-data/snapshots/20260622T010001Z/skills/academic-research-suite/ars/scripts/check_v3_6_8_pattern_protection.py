...
Spec: docs/design/2026-04-30-ars-v3.6.8-trust-provenance-and-drift-transparency-spec.md
      ...
      搂 Step 3a 鈥?Two-Layer Citation Emission (uniform across all modes)
...
# explicitly requires the SHARED function (see spec 搂 Step 0 line ~389:
...
# Byte-order mark stripped per spec 搂 Step 0: "the file's BOM (if any) is
...
def _resolve_default_branch() -> tuple[str | None, str | None]:
    ...
    (1) `git symbolic-ref --quiet --short refs/remotes/origin/HEAD` 鈫?strip 'origin/'
    (2) `$GITHUB_DEFAULT_BRANCH` env (GitHub Actions fallback)
    (3) None 鈥?caller must hard-fail.
    """
...
def _ensure_full_clone() -> str | None:
    ...
    if out.strip().lower() != "true":
        return None  # full clone 鈥?proceed
...
def _v3_6_7_base_commit() -> tuple[str | None, str | None]:
    ...
    This is the single source of truth derivation per spec 搂 Step 0
...
def _detect_pr_base_ref() -> str | None:
    ...
    Order: $GITHUB_BASE_REF (CI fast path) 鈫?origin/<default-branch> (resolved
    ...
    derivation alone" 鈥?local-only attacks are out of scope (the user can see
    their own diff).
    """
...
def _v3_6_7_manifest_unchanged_in_pr() -> tuple[bool, str | None]:
    ...
    `git show B:<protected>` returns the modified content 鈥?self-baseline
    ...
    (no PR base detectable 鈥?local detached state). Returns (False, msg)
    when the manifest changed in the PR or was touched by any PR commit.
    """
    pr_base = _detect_pr_base_ref()
    if pr_base is None:
        # Local-only / detached state: treat as advisory 鈥?surface a note but
    ...
    if rc_mb != 0 or not mb:
        ...
        # warn but do not block 鈥?CI on the canonical repo will catch it.
        return True, None
    mb = mb.strip()
    ...
    if rc_log != 0:
        # Couldn't list touching commits 鈥?be loud, don't pass silently.
        return False, (
    ...
    if head_bytes != base_bytes:
        return False, (
            ...
            "history-shape anomaly 鈥?investigate before proceeding]"
        )
    return True, None
...
def _strip_file_bom(file_bytes: bytes) -> bytes:
    ...
    Per spec 搂 Step 0 SHA normalization: "the FILE's BOM (if any) is
    ...
    heading as a hidden mutation 鈥?round-8 codex P2 closure: spec
...
def _extract_block_bytes(file_bytes: bytes) -> bytes | None:
    ...
    Spec 搂 388 canonical range: "start at the line containing
    ...
    Spec 搂 Step 0 SHA normalization: "bytes are read raw; the FILE's
    ...
    heading) are NOT stripped 鈥?they're real content mutations the
    ...
    """
    # Strip file-level BOM (byte 0 only) per spec 搂 Step 0. This is the
    ...
    # block 鈥?so the hashed range was wrong.
    #
...
# =============================================================================
# Step 3a 鈥?Two-Layer Citation Emission invariants
# =============================================================================
#
...
# REDACTED: sensitive-looking memory line
    ...
    # "resolve" used negatively ("never resolve") is allowed 鈥?handled by
...
# corpus context already in its prompt 鈥?never from frontmatter.
#
...
# frontmatter" 鈥?those must NOT trigger this check. R1 P1-2 closure: bare
...
# Negation must scope to the matched read-verb's neighborhood 鈥?i.e. the
...
# REDACTED: sensitive-looking memory line
# explicitly to direct preposition of the read verb (鈮?0 chars upstream)
# rather than free-floating same-sentence presence 鈥?closes R1 P1-2.
# REDACTED: sensitive-looking memory line
...
# REDACTED: sensitive-looking memory line
    ...
    Algorithm: take the 鈮?0-char left window; find the LAST sentence
...
# non-whitespace (e.g., `### Two-Layer Citation Emission (v3.7.1) 鈥?extended`)
...
def _find_all_two_layer_block_positions(text: str) -> list[int]:
    ...
    R1 P2 + R2 P2 + R3 P1-A closure: counts H1鈥揌6 exact AND drift
    ...
    requires the FIRST canonical EXACT-title H2 鈥?see that function's
    docstring.
    """
    positions: list[int] = []
...
def check_step3a_invariants(verbose: bool = True) -> int:
    ...
    if not files:
        # PR-1 ships an empty list; Step 3a populates. Empty list is fine 鈥?
        # invariants vacuously hold (no agents to check).
        if verbose:
            print(
                "[v3.7.1 Step 3a invariants] manifest 'files' empty 鈥?Step 3a "
    ...
    for rel in files:
        ...
        for _, drift_line in drifts:
            failures.append(
                ...
                f"trailing text is a duplicate-by-drift 鈥?its body sits "
...
# REDACTED: sensitive-looking memory line
        ...
        # then for each verb-target match, check negation in the 鈮?0-char
    ...
    for rel in files_v367:
        ...
        if head_block is None:
            failures.append(
                ...
                "boundary rule violated 鈥?v3.7.1 must NOT mutate v3.6.7 blocks)"
            )
            continue
        if codex_baseline is not None:
            ...
            if base_block is None:
                failures.append(
                    f"  [{rel}] PATTERN PROTECTION (v3.6.7) marker missing at "
                    f"v3.6.7 base commit {base_commit[:12]} 鈥?manifest "

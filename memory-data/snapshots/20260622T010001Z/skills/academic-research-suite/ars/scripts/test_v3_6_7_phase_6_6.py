"""Static checks for ARS v3.6.7 Step 6 Phase 6.6 鈥?orchestrator prompt 搂3.5
Audit Artifact Gate subsection.

Spec: docs/design/2026-04-30-ars-v3.6.7-step-6-orchestrator-hooks-spec.md
Implementation plan: docs/design/2026-05-05-phase-6.6-scoping-note.md 搂5

Phase 6.6 ships a ~50-line decision-policy summary of the 搂5.6 audit gate
into pipeline_orchestrator_agent.md. The full Path A 鈫?Path B procedure
stays in spec 搂5.6 as the implementation contract; the prompt only carries
the policy summary plus P-PA-* / P-PB-* phase IDs as cross-references.

Verification gate (per spec 搂10 Phase 6.6, line 2387 of the spec): the
...
prompt as cross-references to spec 搂5.6.

This test file enforces that gate as four assertions:
    1. The 搂3.5 Audit Artifact Gate subsection exists.
    2. All 24 P-PA-* / P-PB-* phase IDs are present.
    3. The three hard rules from spec 搂5.6 are present.
    4. Prompt size is within the +60-line budget over the baseline.

Pre-Step-6 baseline is recorded as `BASELINE_LINE_COUNT` below 鈥?captured
...
# Per Phase 6.6 verification gate (spec 搂10): +60 lines over pre-Step-6
# baseline. The ~50-line decision-policy summary plus 5鈥?0 lines of headroom.
LINE_BUDGET_OVER_BASELINE = 60

# v3.7.1 Step 3b additionally ships the `## Cite-Time Provenance Finalizer
# (v3.7.1)` subsection per spec 搂 Step 3b (line 449). The subsection adds
# the 搂3.3 4-cell matrix + idempotency + revision-loop preservation +
...
# v3.7.3 additionally ships the `## Cite-Time Provenance Finalizer 鈥?
# v3.7.3 extension` subsection per spec
#   docs/design/2026-05-12-ars-v3.7.3-claim-faithfulness-and-contaminated-source-spec.md
# 搂3.1+搂3.2. The subsection adds the L3-1 precedence-zero NO-LOCATOR check
...
#   docs/design/2026-05-15-issue-103-claim-alignment-audit-spec.md 搂5
...
# explanation 鈥?orchestrator 搂3.6 only restates the bare table + mode
# flag + handoff bullets.
LINE_BUDGET_V3_8_AUDIT_GATE = 60

# v3.9.0 additionally ships the `## Cite-Time Provenance Finalizer 鈥?
...
def _read_prompt() -> str:
    ...
    /simplify advisory P2-1 closure 鈥?10 test methods each calling
    `read_text()` produced 10 disk reads of the same ~600-line file per
    `python -m unittest` invocation. The cache collapses to 1脳 IO with
...
class Phase66SubsectionPresenceTest(unittest.TestCase):
    """Test 1 鈥?搂3.5 Audit Artifact Gate subsection exists."""
    ...
    def test_subsection_heading_present(self) -> None:
        ...
        self.assertIn(
            ...
            text,
            "Phase 6.6 deliverable missing: 搂3.5 Audit Artifact Gate "
            "subsection heading not found in orchestrator prompt. Per spec "
            "搂10 Phase 6.6, the subsection must exist between current 搂3 "
            "Checkpoint Management and 搂4 Transition Management.",
        )


class Phase66PhaseIdReferencesTest(unittest.TestCase):
    """Test 2 鈥?All 24 P-PA-* / P-PB-* phase IDs are present.

    Per spec 搂10 Phase 6.6 verification gate: phase IDs appear "as
    cross-references to spec 搂5.6, not as inline procedural definitions"
    (the procedural definitions stay in 搂5.6). This test enforces presence
    only 鈥?whether each citation contextualises as a cross-reference is
    ...
    def test_all_24_phase_ids_present(self) -> None:
        ...
        self.assertFalse(
            ...
            f"prompt: {missing}. Per spec 搂10 Phase 6.6 verification gate, "
            f"the 搂5.6 inventory's phase IDs (P-PA-* / P-PB-*) appear at "
            f"least once each in the prompt as referenceable handles.",
        )


class Phase66HardRulesTest(unittest.TestCase):
    """Test 3 鈥?Three hard rules from spec 搂5.6 are present.

    Spec 搂5.6 Hard rules block:
    ...
    def test_hard_rule_no_skip(self) -> None:
        ...
        self.assertTrue(
            ...
            "Spec 搂5.6 declares this as the first hard rule.",
        )
    ...
    def test_hard_rule_runs_before_observers(self) -> None:
        ...
        self.assertIn(
            ...
            "collaboration_depth_agent. Spec 搂5.6 declares this as the "
        ...
        self.assertIn(
            ...
            text,
            "Phase 6.6 hard rule missing: audit gate runs BEFORE "
            "integrity_verification_agent. Spec 搂5.6 declares this as the "
            "second hard rule (audit is first transition-time check).",
        )
    ...
    def test_hard_rule_pass_does_not_skip_integrity(self) -> None:
        ...
        self.assertTrue(
            ...
            "check. Spec 搂5.6 declares this as the third hard rule "
...
def _measure_v3_8_audit_gate_block_lines(text: str) -> int:
    """Return the number of lines in the v3.8 搂3.6 Claim-Faithfulness Audit
    Gate subsection (`### 3.6 Claim-Faithfulness Audit Gate (v3.8)` H3 block).

    v3.8 adds 搂3.6 as the orchestrator handoff slot between the v3.7.x
    ...
    measurer reads exactly the 搂3.6 H3 block. v3.7.x extension measurers
    use `#{1,2}` because those blocks are H2 鈥?H3 internals are part of
    their scope. 搂3.6 is itself an H3, so the next H3 closes it.
    """
    import re as _re
    anchor = _re.compile(
...
def _measure_v3_7_3_extension_block_lines(text: str) -> int:
    ...
    subsection (`## Cite-Time Provenance Finalizer 鈥?v3.7.3 extension ...`
    H2 block).
    ...
    subtracted from the v3.6.7 Phase 6.6 +60 budget. Spec:
      docs/design/2026-05-12-ars-v3.7.3-claim-faithfulness-and-contaminated-source-spec.md 搂3.1+搂3.2

    Returns 0 if the v3.7.3 H2 heading is absent.

    The v3.7.3 section uses H3 (`### ...`) subsections internally, so the
    block-end anchor matches the next H1 (`# `) or H2 (`## `) only 鈥?NOT
    H3 鈥?distinct from `_measure_finalizer_block_lines()` which uses
    `#{1,3}` because the v3.7.1 Step 3b block has no internal H3.
    """
    import re as _re
    anchor = _re.compile(
        r"(?m)^[ \t]*##[ \t]+Cite-Time Provenance Finalizer "
        r"[鈥?]+[ \t]*v3\.7\.3 extension[^\n]*$"
    )
    m = anchor.search(text)
    if m is None:
        return 0
...
def _measure_v3_9_0_extension_block_lines(text: str) -> int:
    ...
    subsection (`## Cite-Time Provenance Finalizer 鈥?v3.9.0 extension ...`
    H2 block).
    ...
    v3.7.3), so the block-end anchor matches the next H1 or H2 only 鈥?
    NOT H3 鈥?to avoid prematurely closing on internal subheadings.
    """
    import re as _re
    anchor = _re.compile(
        r"(?m)^[ \t]*##[ \t]+Cite-Time Provenance Finalizer "
        r"[鈥?]+[ \t]*v3\.9\.0 extension[^\n]*$"
    )
    m = anchor.search(text)
    if m is None:
        return 0
...
class Phase66LineBudgetTest(unittest.TestCase):
    """Test 4 鈥?Prompt size within v3.6.7 Phase 6.6 +60 line budget,
    measured EXCLUDING any v3.7.1+ subsections.

    Per spec 搂10 Phase 6.6 verification gate: orchestrator prompt is no
    more than +60 lines vs pre-Step-6 baseline (the ~50-line decision-
    policy summary plus 5鈥?0 lines of headroom).
    ...
    def test_prompt_size_within_budget(self) -> None:
        ...
        # finalizer extension, v3.8 搂3.6 audit-gate, AND v3.9.0
        ...
        self.assertLessEqual(
            ...
            f"搂3.6 audit-gate subsection, and {v3_9_0_lines} are in "
            ...
            f"{LINE_BUDGET_OVER_BASELINE}). Tighten the 搂3.5 Audit "
            f"Artifact Gate subsection.",
        )


class V371Step3bLineBudgetTest(unittest.TestCase):
    """Test 5 鈥?v3.7.1 Step 3b finalizer block within +40 line budget.
    ...
    Phase 6.6 budget. Spec 搂 Step 3b (line 449) does not specify a
...
class V373ExtensionLineBudgetTest(unittest.TestCase):
    """Test 6 鈥?v3.7.3 finalizer extension block within
    `LINE_BUDGET_V3_7_3_EXTENSION` line budget.

    Dedicated budget test for the `## Cite-Time Provenance Finalizer 鈥?
    ...
    def test_v3_7_3_extension_block_within_budget(self) -> None:
        ...
        self.assertGreater(
            ...
            "pipeline_orchestrator_agent.md (expected H2 heading "
            "'## Cite-Time Provenance Finalizer 鈥?v3.7.3 extension ...').",
        )
        self.assertLessEqual(
            block_lines,
...
class V38AuditGateLineBudgetTest(unittest.TestCase):
    """Test 7 鈥?v3.8 搂3.6 Claim-Faithfulness Audit Gate block within
    ...
    def test_v3_8_audit_gate_block_within_budget(self) -> None:
        ...
        self.assertGreater(
            block_lines,
            0,
            "v3.8 搂3.6 audit-gate subsection missing from "
        ...
        self.assertLessEqual(
            ...
            f"v3.8 搂3.6 audit-gate block exceeds "
...
class V390ExtensionLineBudgetTest(unittest.TestCase):
    """Test 8 鈥?v3.9.0 finalizer extension block within
    ...
    al. arXiv:2605.07723 搂3 (cross-index triangulation). Spec:
      docs/design/2026-05-17-ars-v3.9.0-cross-index-triangulation-measurement-spec.md
      搂3.3
    ...
    def test_v3_9_0_extension_block_within_budget(self) -> None:
        ...
        Dedicated budget test for the `## Cite-Time Provenance Finalizer 鈥?
        v3.9.0 extension (triangulation tiers)` subsection per spec
        docs/design/2026-05-17-ars-v3.9.0-cross-index-triangulation-measurement-spec.md 搂3.3.
        """
        text = _read_prompt()
        ...
        self.assertGreater(
            ...
            "pipeline_orchestrator_agent.md (expected H2 heading "
            "'## Cite-Time Provenance Finalizer 鈥?v3.9.0 extension "
            "(triangulation tiers)').",
        )
        self.assertLessEqual(
            block_lines,

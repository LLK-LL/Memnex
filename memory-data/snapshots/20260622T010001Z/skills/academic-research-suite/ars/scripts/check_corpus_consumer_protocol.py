#!/usr/bin/env python3
"""Lint corpus consumer protocol per spec 搂5.2 (v3.6.5).

Enforces nine invariants L1鈥揕9. Manifest-driven for L3鈥揕6.
...
STUB_STATUS_LINE = "**Status:** Stub 鈥?implementation in PR-B (v3.6.5)"
...
# frozenset collapsing mask the closed-set match. The spec 搂5.2 L8
# language is "supported_consumers[] MUST exactly match" 鈥?we honour
...
PRE_SCREENED_LINE_MARKERS = (
    ...
    "citation_keys",
    # F3 conditional zero-hit note line (canonical per spec 搂3.2).
    "Zero-hit note (emit per F3 only when Included: 0)",
    # F4 inline-comment anchors (canonical per spec 搂3.2 + 搂4.2). These
...
def _extract_pre_screened_block(text: str) -> str | None:
    ...
    Marker checks must run against the template body, not the full file 鈥?
...
def check_l7() -> list[str]:
    ...
    for agent_path in _manifested_agent_paths():
        ...
        # Truncation prose check stays scoped to the full file: the spec 搂5.2
        # L7 places this prose in surrounding text, not inside the template.
        if "truncation rule" not in text.lower():
            failures.append(
                f"L7: {rel} missing 'truncation rule' prose mention (spec 搂5.2 L7)"
            )
    return failures

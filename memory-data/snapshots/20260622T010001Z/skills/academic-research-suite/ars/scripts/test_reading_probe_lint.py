...
Target spec: docs/design/2026-04-22-ars-v3.7.3-reading-check-probe-design.md 搂5.1.

These are file-content lints (grep / regex / structure assertions) against:
- deep-research/agents/socratic_mentor_agent.md 搂"Optional Reading Probe Layer"
- deep-research/references/socratic_mode_protocol.md 搂"Reading Probe"
- deep-research/WORKFLOW.md 搂"Opt-in Reading Probe (v3.5.1)"
- academic-pipeline/references/process_summary_protocol.md 搂"Reading Probe Outcomes"

Pattern matches test_check_compliance_report.py 鈥?no LLM runtime, no subprocess fork,
...
class ReadingProbeLintTests(unittest.TestCase):
    """Spec 搂5.1 鈥?7 file-content lints for the reading-check probe."""

    def test_mentor_file_has_probe_section(self) -> None:
        """Mentor file contains 搂'Optional Reading Probe Layer' with 6 subsections."""
        text = MENTOR_AGENT.read_text(encoding="utf-8")
        self.assertIn("## Optional Reading Probe Layer", text,
                      f"{MENTOR_AGENT} missing 搂'Optional Reading Probe Layer' heading")
        # Required subsections 鈥?match spec 搂3.1-3.7 ordering
    ...
    def test_env_var_name_consistent_across_repo(self) -> None:
        ...
        # Spec 搂5.1 item 2: "agent, protocol, SKILL, README".
        # process_summary_protocol is excluded 鈥?it carries the [READING-PROBE:]
    ...
    def test_probe_gated_by_goal_oriented(self) -> None:
        ...
        self.assertTrue(probe_section,
                        f"{MENTOR_AGENT.name}: 搂'{PROBE_HEADING}' section not found")
        # Required phrasing (spec 搂3.2 activation clause)
    ...
    def test_no_probe_in_scoring_files(self) -> None:
        ...
        # Mentor convergence-signal subsection 鈥?scan outside the probe section
        mentor_text = MENTOR_AGENT.read_text(encoding="utf-8")
        ...
        for ident in banned_identifiers:
            self.assertNotIn(
                ...
                f"{MENTOR_AGENT.name} has {ident!r} leaking OUTSIDE 搂'{PROBE_HEADING}'"
            )
        for f in scoring_files:
            ...
            for ident in banned_identifiers:
                self.assertNotIn(
                    ident, text,
                    f"{f.relative_to(REPO_ROOT)} contains {ident!r} 鈥?"
                    f"probe must not appear in scoring/rubric files"
                )

    def test_banned_praise_phrases(self) -> None:
        """Banned-phrases list contains the 8 exact quoted strings from spec 搂3.6; "check" absent."""
        text = MENTOR_AGENT.read_text(encoding="utf-8")
        ...
        # Exact strings, quoted as in spec 搂3.6
        expected_banned = [
            '"correct"',
            '"right"',
            '"wrong"',
        ...
        # (per spec 搂3.6 note 鈥?"check" has non-evaluative uses elsewhere in the section)
        self.assertNotRegex(
            banned_section,
            r'(?m)^\s*[-*]\s*["\'`]check["\'`]',
            "'check' must NOT appear as a list-item entry in banned-phrases list "
            "(per spec 搂3.6 explicit carve-out). Inline prose uses of `check` are fine."
        )

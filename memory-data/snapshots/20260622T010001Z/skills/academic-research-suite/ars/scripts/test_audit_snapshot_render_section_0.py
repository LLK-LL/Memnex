"""Render-side regression test for v3.7.1 Step 2 鈥?Section 0 Scope Report.
...
wrapper-dispatched audit prompt, defeating the spec 搂3.2 contract that the
...
def test_render_prompt_emits_section_0_scope_report_header() -> None:
    ...
    assert b"## Section 0 \xe2\x80\x94 Scope Report" in rendered, (
        "render_prompt does not emit Section 0 Scope Report header 鈥?"
        "spec 搂3.2 contract violated (codex round-1 PR #78 P1)."
    )

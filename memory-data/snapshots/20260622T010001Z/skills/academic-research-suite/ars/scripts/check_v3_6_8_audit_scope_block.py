#!/usr/bin/env python3
"""ARS v3.7.1 Step 2 鈥?D2 audit Scope Report block lint.
...
Spec: docs/design/2026-04-30-ars-v3.6.8-trust-provenance-and-drift-transparency-spec.md
      ...
      搂4 Step 2 鈥?Audit report Scope Report block (lines 412-425)
...
Enforces, on the codex audit prompt template:
  ...
  R1. A Section 0 Scope Report header appears strictly BEFORE the first
      ...
      summary"). The header is the literal "## Codex Audit Round N 鈥?Scope
...
# REDACTED: sensitive-looking memory line
  R5. Additive-prepend invariant per Q5 amend (spec 搂3.2 line 131,
      搂3.7 line 346): the Section 1 heading must appear with exact bytes
      "## Section 1 鈥?Round metadata" so prepending Section 0 has not
...
Attack-surface design notes (informed by issue #77 lessons):
  - Heading-anchored matching uses begin-of-line regex `^## ` to skip
    ...
    its own "## Codex Audit Round N 鈥?Scope Report" heading; our header
    ...
    OR at top-level 鈥?but require that AT LEAST ONE such header anchor
  ...
  - Multiple Section-0 H2 anchors at top level 鈫?reject (mirror of #77
...
# anchored so a suffixed form like "## Codex Audit Round N 鈥?Scope Report
...
# anything else after the canonical bytes 鈥?including a space + suffix 鈥?
...
# `## Overall Outcome`, etc. 鈥?H2 headings whose title carries any
# REDACTED: sensitive-looking memory line
# REDACTED: sensitive-looking memory line
...
# to catch BOTH PASS and FAIL 鈥?anything resembling a verdict line counts
...
def _find_section_1_position(text_no_fences: str) -> int:
    ...
    `## Section 1 鈥?Round metadata (renamed)` as if it were the canonical
...
def check(target: Path) -> tuple[int, list[str]]:
    ...
    if section_1_pos == -1:
        ...
        report.append(
            ...
            "Q5 additive-prepend invariant broken (spec 搂3.2 line 131)."
        )
    ...
    # N 鈥?Scope Report"`) inside Section 0 must NOT satisfy the contract.
    # Codex round-12 P2: require line equality (not prefix) so a suffixed
    # form like `... 鈥?Scope Report (draft)` cannot satisfy R1. Uses the
    ...
    # or documentation block must NOT satisfy the contract 鈥?the audit
...
# REDACTED: sensitive-looking memory line
    ...
    # text if Section 0 anchor missing 鈥?already failed by R1). The
    ...
    for match in bare_passed_re.finditer(cap_rule_surface):
        ...
        report.append(
# REDACTED: sensitive-looking memory line
            f"(spec line 152 cap rule). Context: 鈥excerpt}鈥?"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="ARS v3.7.1 Step 2 鈥?D2 audit Scope Report block lint."
    )
    parser.add_argument(
        "--target",
        type=Path,

...
1. **Bucket A coverage** 鈥?all 23 single-phase agents MUST carry a
   `## Phase Boundary (v3.9.2)` or `## Phase Boundary (v3.9.4)` H2 block.

2. **Bucket B/C/D exclusion** 鈥?all 16 multi-phase / phase-orthogonal /
...
3. **Block content shape** 鈥?each Bucket A block must contain the four
   ...
   These are scoped to the Phase Boundary block itself (via H2 鈫?next H2
...
# Bucket A 鈥?23 single-phase agents that MUST have Phase Boundary block.
...
# Buckets B/C/D 鈥?16 agents that MUST NOT have the block.
# B (4): multi-phase; C (8): phase-orthogonal; D (4): cross-phase-meta.
BUCKET_BCD_AGENTS = [
    # Bucket B 鈥?multi-phase (4)
...
def check_bucket_bcd(path: Path) -> list[str]:
    ...
    if PHASE_BOUNDARY_RE.search(text):
        errors.append(
            ...
            f"design 鈥?adding a single-phase fence would either falsely block "

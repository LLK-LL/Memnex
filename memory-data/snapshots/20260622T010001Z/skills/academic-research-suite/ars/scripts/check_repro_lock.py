...
def main() -> int:
    ...
    lock = doc["repro_lock"]
    if lock is None:
        print("WARN: repro_lock is null 鈥?honest opt-out. Reproducibility reduced.", file=sys.stderr)
        print("OK (with WARN): passport valid; repro_lock explicitly null 鈥?see stderr.", flush=True)
        return 0

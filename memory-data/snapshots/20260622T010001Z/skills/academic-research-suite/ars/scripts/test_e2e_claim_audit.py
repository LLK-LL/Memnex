"""End-to-end claim-faithfulness audit test (spec 搂7.6 + 搂13 step 9).

Wires `detect_uncited_assertions` 鈫?`run_audit_pipeline` 鈫?`apply_finalizer`
against a synthetic 5-citation paper per the spec 搂7.6 contract:
...
| c5  | violates declared negative constraint   | judge VIOLATED 鈫?row judgment=UNSUPPORTED, defect=negative_constraint_violation | HIGH-WARN refuse |
...
Acceptance:
  ...
  - Scenario B (c3 corrected to SUPPORTED + c5 rewritten with hedged
    ...
    remain. The fixture keeps c5 as a cited claim 鈥?Scenario B
...
`detect_uncited_assertions` with the new `adjacent_text` field 鈥?a sentence
...
Spec: docs/design/2026-05-15-issue-103-claim-alignment-audit-spec.md 搂7.6 + 搂13 step 9.
...
# Step 9 RED 鈫?GREEN transition is complete: all three modules below now
...
# so INV-6 never fires 鈥?anchor=none short-circuit is exercised in T-P1, not here.
...
def _run_e2e(
    *,
    ...
    """Drive detector 鈫?pipeline 鈫?finalizer; return (passport, finalizer_out).
    ...
    # Spec 搂13 step 9 contract: every e2e scenario exercises the full
    # detector 鈫?pipeline 鈫?finalizer chain. Always invoke the detector even
    # when there are no extra sentences 鈥?passing through `[]` keeps Scenario
...
# ---------------------------------------------------------------------------
# Scenario A 鈥?gate refuses with exactly the c3 + c5 reasons.
# ---------------------------------------------------------------------------
...
class E2EScenarioARefuses(_E2ETestBase):
    ...
    def test_c1_emits_no_annotation_c2_and_c4_pass_gate(self) -> None:
        ...
        # c1 SUPPORTED 鈫?no annotation row at all.
        # c2 AMBIGUOUS 鈫?LOW-WARN advisory present but not in gate reasons.
        ...
        # c4 paywall 鈫?LOW-WARN-UNVERIFIED present but not in gate reasons.
    ...
    def test_citation_row_contracts_match_spec_7_6_matrix(self) -> None:
        ...
        Annotation-string parity is necessary but not sufficient 鈥?`C-003`
        ...
        shape directly so the spec 搂7.6 (judgment, defect_stage,
...
# ---------------------------------------------------------------------------
# Scenario B 鈥?corrected fixture passes the gate.
# ---------------------------------------------------------------------------
...
class E2EScenarioBPasses(_E2ETestBase):
    ...
    def test_corrected_run_keeps_c2_advisory_and_c4_unverified(self) -> None:
        ...
        # c2 AMBIGUOUS + c4 paywall persist independently of c3/c5 fixes 鈥?
        ...
        # AMBIGUOUS + c4 UNVERIFIED. c1/c3/c5 SUPPORTED 鈫?zero annotations.
        self.assertEqual(
            len(annotations),
            2,
...
# REDACTED: sensitive-looking memory line
    ...
    def test_e2e_pipeline_runs_through_filtered_sentences_cleanly(self) -> None:
        # Smoke test the detector 鈫?pipeline 鈫?finalizer path with one
        ...
        # Exactly two HIGH-WARN reasons 鈥?uncited advisory does not promote
        # to gate-refuse.
        self.assertEqual(len(out["gate_refuse_reasons"]), 2)

        # Round-1 codex P2 closure: pin that the detector 鈫?pipeline wiring

#!/usr/bin/env python3
"""Tests for check_v3_9_0_triangulation.py lint per spec v3.9.0 搂3.8."""
...
def test_lint_detects_missing_allowlist_entry(tmp_path):
    """Remove CONTAMINATED-TRIANGULATION-UNMATCHED from formatter 鈥?lint fails."""
...
def test_rule1_missing_marker_fails(tmp_path):
    """Rule 1: remove CONTAMINATED-PARTIAL-UNMATCH from orchestrator subsection 鈥?lint fails."""
...
# ---------------------------------------------------------------------------
# Rule 2 鈥?preprint composition order
# ---------------------------------------------------------------------------
...
def test_rule2_preprint_order_violated_fails(tmp_path):
    """Rule 2: inject CONTAMINATED-COVERAGE-NOISE+PREPRINT into orchestrator 鈥?lint fails."""
    orch = REPO_ROOT / "academic-pipeline/agents/pipeline_orchestrator_agent.md"
    content = orch.read_text()
...
def test_rule3_legacy_compat_violated_fails(tmp_path):
    """Rule 3: change the legacy row's suffix to COVERAGE-NOISE 鈥?lint fails."""
    orch = REPO_ROOT / "academic-pipeline/agents/pipeline_orchestrator_agent.md"
    content = orch.read_text()
    broken = content.replace(
...
def test_rule3_preprint_legacy_drift_fails(tmp_path):
    """Rule 3: drift preprint legacy row's suffix to CONTAMINATED-PREPRINT+COVERAGE-NOISE 鈥?lint fails."""
    orch = REPO_ROOT / "academic-pipeline/agents/pipeline_orchestrator_agent.md"
    content = orch.read_text()
    broken = content.replace(
...
def test_rule3_missing_legacy_rows_fails(tmp_path):
    """Rule 3: delete BOTH legacy S2 rows from the table 鈥?lint must fail."""
    orch = REPO_ROOT / "academic-pipeline/agents/pipeline_orchestrator_agent.md"
    content = orch.read_text()
...
# ---------------------------------------------------------------------------
# Rule 4 鈥?no *-BLOCK in v3.9.0 subsection
# ---------------------------------------------------------------------------
...
def test_rule4_high_block_injection_fails(tmp_path):
    """Rule 4: inject `CONTAMINATED-HIGH-BLOCK` into v3.9.0 subsection 鈥?lint fails."""
    orch = REPO_ROOT / "academic-pipeline/agents/pipeline_orchestrator_agent.md"
    content = orch.read_text()
    broken = content.replace(
        "## Cite-Time Provenance Finalizer 鈥?v3.9.0 extension",

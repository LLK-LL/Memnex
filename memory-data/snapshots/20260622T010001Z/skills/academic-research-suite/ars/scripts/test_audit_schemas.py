...
Per spec 搂10 verification gate:
  1. All four schemas parse as valid JSON Schema 2020-12.
  2. Example payloads in 搂3.1 / 搂3.3 / 搂3.4 / 搂3.5 validate against their
...
(搂3.7 family A-F rows, --mode {proposal,persisted} arms, mirror rules,
...
# ---------------------------------------------------------------------------
# Positive examples 鈥?drawn directly from spec 搂3.1 / 搂3.3 / 搂3.4 / 搂3.5.
# ---------------------------------------------------------------------------

# 搂3.1 / 搂3.2 鈥?persisted entry (verified_at + verified_by present, MINOR)
...
# 搂3.2 鈥?proposal entry (verified_at + verified_by absent)
...
# 搂3.2 鈥?proposal AUDIT_FAILED (proposal-arm only, persisted excludes per 搂3.2)
...
# 搂3.3 鈥?canonical four-event JSONL run (one validate per event row)
...
# precedes the matching item.completed for command_execution etc. Spec 搂3.3
...
# 搂3.4 鈥?sidecar example (clean run)
...
# 搂3.4 鈥?AUDIT_FAILED sidecar (jsonl_thread_id may be empty string)
...
# 搂3.5 鈥?verdict file (clean MINOR)
...
# 搂3.5 鈥?verdict file (AUDIT_FAILED variant)
...
# ---------------------------------------------------------------------------
# Negative examples 鈥?schema MUST reject each.
# ---------------------------------------------------------------------------

# Persisted-shape entry whose verdict.status is not in the persisted enum.
# (Removing verified_at alone falls into the proposal arm 鈥?that's a lint-side
...
# 搂3.7 A2 鈥?AUDIT_FAILED proposal without failure_reason must reject so that
...
# 搂3.7 A2 inverse 鈥?non-AUDIT_FAILED proposal carrying failure_reason must reject.
...
# 搂3.7 A4 鈥?acknowledgement requires verdict.status == MATERIAL.
...
# 搂5.4 ship_with_known_residue shape 鈥?must validate.
...
# 36-char garbage (all dashes / all hex without separators) must NOT pass 鈥?
...
class TestEntrySchema(_SchemaTestBase):
    ...
    def test_rejects_persisted_minor_with_acknowledgement(self) -> None:
        ...
        # to claim residue acknowledgement that the 搂5.4 prompt never solicited.

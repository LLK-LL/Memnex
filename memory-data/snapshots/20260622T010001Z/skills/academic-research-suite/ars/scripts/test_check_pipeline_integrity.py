...
class CheckPipelineIntegrityTests(unittest.TestCase):
    ...
    def test_phase5_with_only_editorial_synthesizer_flags_missing_da(self) -> None:
        """editorial_synthesizer alone is NOT enough 鈥?DA category and
...
# REDACTED: sensitive-looking memory line
    ...
    def test_multiple_phase5_dirs_each_independently_checked(self) -> None:
        ...
        with TemporaryDirectory() as td:
            _build_workspace(Path(td), {
                ...
                "phase5_review_r2": [
                    "review.md",  # Incomplete 鈥?missing all 3 categories
                ],
            })
            result = _run(Path(td))
        self.assertEqual(result.returncode, 0)
    ...
    def test_unicode_filenames_with_canonical_stem_match(self) -> None:
        ...
        agent stem in ASCII should still match. E.g., devils_advocate_鍫卞憡.md"""
        with TemporaryDirectory() as td:
            _build_workspace(Path(td), {
                "phase5_review": [
                    "devils_advocate_鍫卞憡.md",
                    "editor_in_chief_姹哄畾.md",
                    "ethics_review_鍫卞憡.md",
                ],
            })
            result = _run(Path(td))
        self.assertEqual(result.returncode, 0)

#!/usr/bin/env python3
"""Tests for `scripts/_text_similarity.py` 鈥?shared title-similarity helpers
...
class NormalizeTitleTest(unittest.TestCase):
    """Per protocol 搂"Query Patterns" Pattern 1: case-insensitive, punctuation
...
class ConstantsTest(unittest.TestCase):
    """Lock the magic numbers 鈥?these are protocol-level invariants, not
    arbitrary tuning."""

    def test_title_similarity_threshold_is_protocol_value(self) -> None:
        """Per PaperOrchestra (Song et al. 2026 Appx D.3) + protocol 搂Query
        Patterns Pattern 1."""
        self.assertEqual(ts._TITLE_SIMILARITY_THRESHOLD, 0.70)

    def test_backoff_seconds(self) -> None:
        """Per protocol: 429 鈫?2s backoff 脳 3 retries."""

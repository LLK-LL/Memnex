...
429 鈫?2s backoff 脳 3 retries, 5xx 鈫?skip. Mirrors
...
class OpenAlexUnavailable(Exception):
    """OpenAlex API degraded 鈥?caller MUST omit `openalex_unmatched`."""
...
class OpenAlexClient:
    ...
    def _throttle(self) -> None:
        ...
        # elapsed and either huge sleep or zero sleep (#128 搂6). Aligns
        ...
        for attempt in range(_MAX_RETRIES + 1):
            try:
                with urllib.request.urlopen(req, timeout=30) as resp:
                    ...
                    # status surface as OpenAlexUnavailable 鈥?honoring the
                    # v3.9.0 spec 搂3.7 per-API degradation contract (one
                    ...
                    try:
                        ...
                        # (truncated body 鈥?canonical mid-stream socket drop)
                ...
                if e.code == 429 and attempt < _MAX_RETRIES:
                    ...
                    # already counts the 2s 脳 N backoff) and re-trigger 429.
                    self._last_request_at = time.monotonic()
                    continue

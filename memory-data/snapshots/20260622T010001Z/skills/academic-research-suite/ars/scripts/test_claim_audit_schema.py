...
The test plan section 搂7.1 lists T-S1..T-S8 keyed to claim_audit_result
(搂3.1) and claim_intent_manifest (搂3.2) and uncited_assertion (搂3.3).
搂6 lint coverage is broader: it includes claim_drift D-INV-1..4 (搂3.4),
constraint_violation CV-INV-1..4 (搂3.5) and audit_sampling_summary
S-INV-1..4 (搂4 step 3). This file follows 搂6 because that is the lint
contract under test 鈥?drift / constraint-violation / sampling
...
Spec 搂7 names the test file `tests/test_claim_audit_schema.py`. This
...
def supported_entry() -> dict[str, Any]:
    """Minimal SUPPORTED row 鈥?INV-1 positive baseline."""
    return {
        "claim_id": "C-001",
...
# ---------------------------------------------------------------------------
# T-S1: Schema-shape only 鈥?minimal SUPPORTED entry validates against the
...
class TS2ClaimAuditInvariants(_LintTestBase):
    ...
    def test_inv_6_anchor_none_with_non_empty_anchor_value(self) -> None:
        ...
        e = supported_entry()
        e["anchor_kind"] = "none"
        e["anchor_value"] = "123"  # stale residual 鈥?must be rejected
        e["judgment"] = "RETRIEVAL_FAILED"
        e["audit_status"] = "inconclusive"
        e["defect_stage"] = "not_applicable"
        e["ref_retrieval_method"] = "not_attempted"
...
# REDACTED: sensitive-looking memory line
    ...
    def test_inv_15_sentinel_manifest_permitted(self) -> None:
        # INV-15 sentinel positive case 鈥?MANIFEST-MISSING fallback row.
        e = supported_entry()
    ...
    def test_inv_16_url_encoded_whitespace_does_not_bypass(self) -> None:
        ...
        # rule 鈥?docstring + schema both require non-empty *after* URL-decode.
        # Step 13 R1 Gemini finding (a832d3f).
        e = supported_entry()
    ...
    def test_inv_17_constraint_id_inner_hyphen_form(self) -> None:
        # INV-17: NC-C{n}-{m} parse rule 鈥?NO inner hyphen between C and digits
        ...
        # Schema pattern itself rejects this 鈥?lint surfaces INV-17 explicitly.
...
class TS3AnchorNoneInv6(_LintTestBase):
    ...
    def test_anchor_none_wrong_retrieval_method(self) -> None:
        ...
        # INV-11 鈫?INV-6 mismatch 鈥?either fires; spec couples the firm rule via INV-6/11.
        self.assertLintFinds(build_passport(results=[e]), invariant="INV-6")
...
class TS6ManifestInv3(_LintTestBase):
    ...
    def test_claim_level_nc_collides_with_mnc_id(self) -> None:
        # M-INV-3 鈥?claim-level constraint reusing an MNC-* id is the override
...
# Spec 搂3.1 table: 9 positive rows, lint rejects every (j, a, d) triple
# outside the table; 鈮? disallowed combinations exercised explicitly.
# ---------------------------------------------------------------------------

class TS8AllowedMatrix(_LintTestBase):
    """T-S8: every allowed triple validates; 鈮? disallowed combinations rejected."""

    # Positive matrix rows 鈥?each is a self-contained passport fragment.
    ALLOWED_ROWS = [
    ...
    # Negative cases 鈥?鈮? representative disallowed combinations.
    DISALLOWED_ROWS = [
    ...
    def _build_row(self, j: str, a: str, d: Any, overrides: dict[str, Any]) -> dict[str, Any]:
        ...
        if d == "negative_constraint_violation":
            ...
            e["rationale"] = "Paywall 鈥?full text not retrievable."
        elif d is not None:
...
# ---------------------------------------------------------------------------
# Spec 搂6 lint 4a 鈥?D-INV-1..D-INV-4 claim_drift cross-array integrity.
# Not labelled T-Sx in spec 搂7.1 but mandatory per 搂6.4a. Paired pos/neg.
# ---------------------------------------------------------------------------

class TSDDriftInvariants(_LintTestBase):
    """Spec 搂6.4a: claim_drift D-INV-1..D-INV-4."""
...
# ---------------------------------------------------------------------------
# Spec 搂6.4b 鈥?CV-INV-1..CV-INV-4 constraint_violation cross-array integrity.
# ---------------------------------------------------------------------------

class TSCVConstraintViolationInvariants(_LintTestBase):
    """Spec 搂6.4b: constraint_violation CV-INV-1..CV-INV-4."""
...
# ---------------------------------------------------------------------------
# Step 13 R8 codex P2-1 鈥?CV-INV-4 dedupe key extension. Two manifests in
...
class TSCVDedupeManifestScope(_LintTestBase):
    ...
    def _two_manifests_with_colliding_mnc(self) -> list[dict[str, Any]]:
        ...
        # bound to different manifests (M-INV-4 permits 鈥?manifest_id is
    ...
    def test_same_constraint_id_across_manifests_not_deduped(self) -> None:
        ...
        # manifest_negative_constraints scoping 鈥?dedupe must keep both.
        ...
        # Pre-fix: CV-INV-4 false-positives 鈫?lint reports CV-INV-4 line.
...
# ---------------------------------------------------------------------------
# Spec 搂6.4c 鈥?S-INV-1..S-INV-4 audit_sampling_summary invariants.
# ---------------------------------------------------------------------------

class TSSamplingInvariants(_LintTestBase):
    """Spec 搂6.4c: audit_sampling_summary S-INV-1..S-INV-4."""
...
# ---------------------------------------------------------------------------
# Spec 搂6.4d (v3.8.2 / #118) 鈥?UAF-INV-1..UAF-INV-5 uncited_audit_failure
# cross-array integrity and dedup. Mirrors CV-INV pattern.
# ---------------------------------------------------------------------------


class TSUAFUncitedAuditFailureInvariants(_LintTestBase):
    """Spec 搂6.4d: uncited_audit_failure UAF-INV-1..UAF-INV-5 (v3.8.2 / #118)."""
    ...
    def test_uaf_inv_5_rationale_wrong_fault_prefix(self) -> None:
        # Rationale starts with "made_up_class:" 鈥?schema rejected via fault_class
        ...
        # tag 鈥?should still trip UAF-INV-5 (prefix must match the row's
...
# Step 13 R4 codex P2 #4 鈥?malformed aggregates (non-list / non-dict entries)
...
class TS9MalformedPassportGuard(_LintTestBase):
    ...
    def test_claim_audit_results_with_non_dict_entry(self) -> None:
        ...
        self.assertNotIn(
            "Traceback",
            err,
            msg=f"lint must not raise 鈥?got traceback:\n{err}",
        )
    ...
    # Step 13 R6 codex P2 + R8 P2-2 鈥?nested schema-invalid shapes must not
    ...
    def test_non_string_claim_text_does_not_crash_cv_inv_4_dedupe(self) -> None:
        ...
        # 鈥?if claim_text is a list / dict / int (schema-invalid), the `or ""`
...
# v3.9.1 / #130 鈥?manifest_id non-string guard in _build_manifest_index +

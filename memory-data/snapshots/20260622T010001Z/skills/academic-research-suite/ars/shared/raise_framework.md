...
RAISE's official scope is **evidence synthesis** (systematic review, meta-analysis, scoping review, rapid review, evidence mapping). When ARS applies RAISE **principles** to non-evidence-synthesis work 鈥?e.g., `academic-paper full` on primary research 鈥?this is a **principle extension**, not official RAISE compliance. ARS's `compliance_agent` will only claim RAISE compliance for outputs produced in `systematic_review` or `other_evidence_synthesis` modes.
...
**Warn:** Mismatch in 鈮? tool, or reporting lacks one of {stage, prompt, parameters, version}.
**Fail:** Multiple tools undeclared OR systematically missing prompts/parameters.

### Principle 3 鈥?Reproducibility
...
**Pass:** Per-tool justification + 鈮? validation reference.
...
Used when `raise.mode == "full"` (SR and other_evidence_synthesis). Each role carries 2鈥? responsibilities extracted from the RAISE consultation PDF. The agent maps ARS users and ARS itself onto the roles they occupy.
...
### Role 4 鈥?Publishers of evidence synthesis
...
### Role 5 鈥?Users of evidence synthesis
...
### Role 6 鈥?Trainers of evidence synthesis methods
...
### Role 7 鈥?Organisations producing evidence synthesis
...
### Role 8 鈥?Funders of evidence synthesis
...
Block decisions follow tier semantics: any principle at `fail` status + `systematic_review` mode 鈫?`block`. `primary_research` mode **never** blocks.

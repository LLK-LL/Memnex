...
**Status**: v3.3.2 鈥?narrative hub doc; for declarative annotations see
`shared/handoff_schemas.md`

---

## 搂 1 鈥?Why ground-truth isolation matters
...
held-out data 鈥?a textbook case of reward hacking. The failure is not about
...
AI research system 鈥?the first to pass blind peer review end-to-end 鈥?models
...
## 搂 2 鈥?The three-layer mental model
...
**Layer 1 鈥?raw inputs** covers user queries, primary sources retrieved from
...
**Layer 2 鈥?verified artifacts** are outputs that have cleared an integrity
...
**Layer 3 鈥?ground truth and evaluation rubrics** includes gold labels,
...
filter 鈥?it is an epistemological firewall.
...
| `redacted` | Boundary 1鈫? | Operates on sanitized material with no new raw ingestion |
...
## 搂 3 鈥?Rules for adding a skill or agent
...
requirements 鈥?not answer keys, not expected content. For calibration gold
# REDACTED: sensitive-looking memory line
...
privately.** The review workflow is: reviewer reads paper + rubric 鈫?reviewer
produces natural-language feedback 鈫?paper-writing agent reads paper +
...
## 搂 4 鈥?Today's implementation

The isolation pattern is already instantiated across the ARS codebase through
a set of narrowly scoped protocol files. This section is a navigational map 鈥?
...
in one of those files, the more specific file governs for that mechanism 鈥?
and that conflict should be surfaced as an issue so this document can be
updated.

---

## 搂 5 鈥?What this pattern is NOT
...
material into a raw-layer skill's context can do so 鈥?the pattern is a design
...
assurance, and policy analysis 鈥?work whose quality depends on domain judgment
...
## 搂 6 鈥?Future evolution (intentionally out of scope)
...
contributors do not propose them as overlooked features 鈥?they are deferred,

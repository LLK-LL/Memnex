...
**Spec:** v3.6.7 搂7.1 鈥?pattern protection reference for `research_architect_agent` (survey designer mode), Pattern B2.
...
- Positive: "My institution provides the resources I need to do my work well." (1=strongly disagree 鈫?5=strongly agree)
...
**Operational definition:** A statement that probes a **different construct** (or a different dimension of the same construct) and is included to measure something else 鈥?discriminant validity, related-but-distinct construct, social-desirability check.
...
- "I find my work intrinsically meaningful." 鈥?This probes intrinsic motivation, not perceived institutional support. It is contrast / divergent-validity, not a reverse item.
...
If that justification cannot be written without slipping into a different construct, the item is **not** reverse-coded 鈥?it is a contrast item, and it must either move to its own scale or be removed.
...
1. **Calendar drift:** Asking "in the last 12 months" without anchoring to a salient event produces noisy recall 鈥?respondents reconstruct an approximate window, not a true 12-month boundary.
...
**The standard mitigation:** Event-anchored phrasing 鈥?anchor the recall window to a salient, datable event the respondent's unit shares.
...
**Calendar-anchored phrasing 鈥?only acceptable when the entire sample shares a common event date:**
...
> "In the last 12 months, how often did X occur?" 鈥?This invites calendar drift and telescoping; the respondent has no shared anchor.
...
- DeVellis, R. F. (2017). *Scale Development: Theory and Applications* (4th ed.). Sage. 鈥?reverse-coding and acquiescence bias.
- Tourangeau, R., Rips, L. J., & Rasinski, K. (2000). *The Psychology of Survey Response*. Cambridge. 鈥?recall bias and event anchoring.
- Loftus, E. F., & Marburger, W. (1983). Since the eruption of Mt. St. Helens, has anyone beaten you up? Improving the accuracy of retrospective reports with landmark events. *Memory & Cognition*, 11(2), 114鈥?20. 鈥?primary empirical source for event-anchoring effect.

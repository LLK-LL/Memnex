...
**Status**: v3.2
**Source**: Lu et al. (2026, Nature 651:914-919) Figure 1c 鈥?template-based mode produces higher-quality, lower-variance papers; template-free mode produces more diverse outputs but lower mean quality and higher variance. ARS maps this finding onto its own mode taxonomy.

---

## Why this spectrum exists

Not all modes should treat templates and examples the same way. A `systematic-review` needs rigid structure (PRISMA checklist, predefined sections) 鈥?templates are load-bearing. A `socratic` dialogue needs room to wander 鈥?heavy templates kill exploration.
...
| deep-research | `fact-check` | Fidelity | Heavy | Claim 鈫?evidence 鈫?verdict pipeline, no room for drift |
...
| academic-paper | `disclosure` (v3.2) | Fidelity | Heavy | Venue policy database 鈫?templated output |
...
- User wants "predictable" / "consistent" 鈫?suggest **Fidelity** modes
- User wants to "explore" / "think through" 鈫?suggest **Originality** modes
- Unclear 鈫?suggest **Balanced** modes as default

...
| GPT-5.4 Pro | `gpt-5.4-pro` | OpenAI | Cross-verification 鈥?strongest reasoning |
| GPT-5.4 | `gpt-5.4` | OpenAI | Cross-verification 鈥?balanced cost/performance |
| Gemini 3.1 Pro | `gemini-3.1-pro-preview` | Google | Cross-verification 鈥?strong at factual verification |
...
- Cross-model receives only the reference text and paper context 鈥?not Claude's verification result (to prevent anchoring)
...
# REDACTED: sensitive-looking memory line
   ...
   Reference 1: [full reference text] 鈥?Context: [sentence where cited]
   Reference 2: [full reference text] 鈥?Context: [sentence where cited]
   ... (up to 5 per batch)
   ```
...
- This directly addresses frame-lock 鈥?a different model may attack from a different angle
...
3. Any cross-model finding not already covered 鈫?add to report as `[CROSS-MODEL-FINDING]`
4. Log: `[CROSS-MODEL: X findings received, Y novel (not in primary DA report)]`

### Peer Review (academic-paper-reviewer) 鈥?Future
...
| Peer review (planned, not yet implemented) | 鈥?| 鈥?|
...
3. **Response format differences.** Different models structure responses differently. The agent must parse varied formats 鈥?keep verification prompts simple and structured to minimize parsing issues.
...
- Continue with single-model verification 鈥?never block the pipeline on cross-model failure

...
**Default rule**: When intent is ambiguous between `plan` and `full`, **prefer `plan`** 鈥?it is safer to guide a user who needs help than to produce a paper they can't use. The user can always switch to `full` later.

**Example triggers** (illustrative, not exhaustive):
"guide my paper", "help me plan my paper", "I don't know how to start", 銆屽紩灏庢垜瀵珫鏂囥€嶃€屽公鎴戣鍔冭珫鏂囥€? or equivalent in any language

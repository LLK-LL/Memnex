...
# Visualization Agent 鈥?Publication-Quality Figure Generation
...
1. **Data-driven selection** 鈥?choose the chart type that best represents the data structure and research question
2. **APA 7.0 compliance** 鈥?all figures follow APA 7th edition formatting guidelines (Chapter 7)
3. **Accessibility first** 鈥?colorblind-safe palettes, sufficient contrast, readable font sizes
4. **Reproducibility** 鈥?generated code is self-contained, commented, and runnable without modification
5. **Integration-ready** 鈥?output includes LaTeX `\includegraphics` code for seamless inclusion in the paper
...
鈹?
鈹溾攢鈹€ Categorical comparison (groups vs. values)
鈹?  鈹溾攢鈹€ Few categories (鈮?7) 鈫?Bar chart
鈹?  鈹溾攢鈹€ Many categories (> 7) 鈫?Horizontal bar chart
鈹?  鈹斺攢鈹€ Proportions that must sum to 100% 鈫?Stacked bar chart (NOT pie chart)
鈹?
鈹溾攢鈹€ Distribution
鈹?  鈹溾攢鈹€ Single variable across groups 鈫?Boxplot
鈹?  鈹溾攢鈹€ Need to show distribution shape 鈫?Violin plot
鈹?  鈹斺攢鈹€ Single variable, one group 鈫?Histogram (with density curve)
鈹?
鈹溾攢鈹€ Trend over time
鈹?  鈹溾攢鈹€ Single series 鈫?Line chart
鈹?  鈹溾攢鈹€ Multiple series (鈮?5) 鈫?Multi-line chart
鈹?  鈹斺攢鈹€ Many series (> 5) 鈫?Small multiples / faceted line charts
鈹?
鈹溾攢鈹€ Correlation / Relationship
鈹?  鈹溾攢鈹€ Two variables 鈫?Scatter plot + regression line
鈹?  鈹溾攢鈹€ Many variables 鈫?Correlation heatmap
鈹?  鈹斺攢鈹€ Network / conceptual 鈫?Network graph or concept map
鈹?
鈹溾攢鈹€ Meta-analysis
鈹?  鈹溾攢鈹€ Effect sizes 鈫?Forest plot
鈹?  鈹斺攢鈹€ Bias check 鈫?Funnel plot
鈹?
鈹斺攢鈹€ Unsure 鈫?Default to the simplest chart that conveys the message
```
...
**Primary palette (viridis)** 鈥?perceptually uniform, colorblind-safe:
```
#440154, #46327E, #365C8D, #277F8E, #1FA187, #4AC16D, #9FDA3A, #FDE725
```

**Alternative palette (cividis)** 鈥?optimized for deuteranopia/protanopia:
```
...
| Dual y-axes | Misleading 鈥?implies correlation where none may exist | Use two separate panels |
...
Step 6.5: VLM Figure Verification (Optional) 鈥?NEW v3.3

# Adapter Contract 鈥?Producing `literature_corpus[]` Entries
...
An **adapter** is a program (in any language) that reads a user-owned corpus source 鈥?a Zotero export, an Obsidian vault, a folder of PDFs, a Notion database, a custom SQLite library 鈥?and produces:
...
| `adapter_version` | string | 鈥?|
| `contamination_signals` | object | v3.7.3 + v3.9.0 contaminated-source advisory field (spec v3.7.3 搂3.2 + v3.9.0 搂3.4鈥撀?.5). |
...
| `source_acquired` | boolean | v3.7.1 trust-chain field (spec 搂 3.1, D1). |
...
**Entry-level problems** 鈫?push to rejection log, continue.
**Adapter-level problems** (input file missing, unreadable, malformed at the root) 鈫?write nothing, print a clear error to stderr, exit with code 1.
...
Each `literature_corpus` entry SHOULD carry `obtained_via` and `obtained_at`. The `rejection_log` MUST carry `adapter_name`, `adapter_version`, and `generated_at`. These fields let downstream users trace which adapter version produced which entry 鈥?useful when a library's schema or a source KB layout changes.
...
The three reference adapters have pytest coverage under `scripts/adapters/tests/` 鈥?copying that pattern for your own adapter is a good starting point.

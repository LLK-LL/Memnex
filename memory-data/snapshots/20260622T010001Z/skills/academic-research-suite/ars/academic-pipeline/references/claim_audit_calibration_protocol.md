...
1. **Gold set**: 20 tuples (12 alignment + 8 constraint per the canonical shipped fixture at `scripts/fixtures/claim_audit_calibration/gold_set.json`). Per-tuple shape and the four ingestion-gate rules (alignment-judgment coverage, 鈮? NOT_VIOLATED floor, required constraint fields, tuple_kind validity) are spelled out in **Gold tuple schema** below + spec 搂7.7 rules (a)-(d). The shipped fixture is the canonical example.
...
3. **Thresholds** (optional): `{"FNR": 0.15, "FPR": 0.10}` per spec 搂7.7 + 搂9 acceptance. Tightening these is a spec bump; loosening them is not permitted without a corresponding judge-quality-improvement justification recorded in the calibration report's `thresholds` block.
...
1. Call `validate_gold_set(tuples)`. Any of the four spec 搂7.7 rules (a)/(b)/(c)/(d) failing raises `GoldSetValidationError` with the offending tuple index and the rule name. Do NOT catch the exception 鈥?fix the gold set.
...
- Alignment tuples (all four `expected_judgment` values including `RETRIEVAL_FAILED`) 鈫?`retrieved_excerpt = tuple.ref_text_excerpt` (may be `null` for `RETRIEVAL_FAILED` tuples), `active_constraints=[]`. The runner does NOT pre-filter `RETRIEVAL_FAILED` tuples 鈥?the judge stub is responsible for returning the matching label.
- Constraint tuples 鈫?`retrieved_excerpt = tuple.ref_text_excerpt` (typically `null`), `active_constraints=[{"constraint_id": ..., "rule": ..., "scope": "MNC" | "NC"}]`. Scope is derived from `constraint_under_test_id`: `MNC-N` 鈫?`"MNC"`, `NC-CN-M` 鈫?`"NC"`. Malformed ids are rejected by `_derive_constraint_scope` with a diagnostic naming the expected shape.

No ensembling at this layer. Spec 搂7.7 does not require N-run majority voting (reviewer mode does, but reviewer-paper unit-of-analysis is heavier and benefits more from variance reduction; per-tuple alignment is a simpler call). Re-running the same tuple is the operator's responsibility if a high-variance judge_model warrants it.

### Phase 2: Confusion-matrix accumulation

The runner accumulates two confusion matrices:

- **Aggregate** (the rates that T-C1 gates against): a tuple contributes a FN when `expected_judgment 鈮?actual judgment`. For alignment, every mismatched tuple counts as both FN (for the expected class) and FP (for the wrongly-picked class), so aggregate FNR and FPR are symmetric. For constraint tuples, VIOLATED is the positive class (gate-refuse signal); the matrix is the standard binary form.
...
- A regression (judge quality degraded 鈥?same gold set, higher FNR/FPR than the previous run).
- A spec bump (tighter thresholds 鈥?same judge, but T-C1 now demands more).
...
- Detect rubric-discrimination-power problems on the LLM-as-judge side 鈥?that's the `#89 / gold fixtures` work tracked separately (per spec 搂2 out-of-scope).
...
|---|---|
| Stage 4 鈫?5 audit dispatch | Calibration runs offline; results inform the `ARS_CLAIM_AUDIT` ramp-on decision. |
| Cite-Time Provenance Finalizer | No direct coupling 鈥?calibration evaluates the judge in isolation; the finalizer's matrix is downstream. |
...
## Resolved design decisions (2026-05-15, per spec 搂10 OQs)
...
- **Threshold values**: FNR < 0.15 + FPR < 0.10. Tightened from the reviewer-mode 0.17 / 0.50 Lu 2026 reference points because the audit unit (per-claim) is simpler than the reviewer unit (whole paper) 鈥?the gate should track the stricter end of plausible LLM-as-judge accuracy.
...
- **Cross-model verification**: out of scope for the calibration runner 鈥?`ARS_CROSS_MODEL` interacts with the audit agent dispatch path, not the calibration script. An operator wanting cross-model calibration runs the runner twice with different `judge_model` settings.
...
- Spec: `docs/design/2026-05-15-issue-103-claim-alignment-audit-spec.md` 搂7.7 (test contract) + 搂1 deliverable 7 (this doc) + 搂9 (acceptance criteria) + 搂13 step 10 (implementation order).
...
- Parent agent: `academic-pipeline/agents/claim_ref_alignment_audit_agent.md` 鈥?dispatches calibration mode and consumes the report's thresholds block. The agent prompt and this doc form a two-way pair following the v3.6.5 protocol-doc convention (`literature_corpus_consumers.md` 鈫?`bibliography_agent.md`).
...
- Lu, C. et al. (2026). *Nature* 651, 914-919 鈥?Table 1 reference rates for LLM-vs-human agreement on whole-paper decisions; this protocol uses tighter thresholds for the simpler per-claim unit.

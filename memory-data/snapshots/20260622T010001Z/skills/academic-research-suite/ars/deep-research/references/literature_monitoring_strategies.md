# Literature Monitoring Strategies 鈥?Reference Guide
...
3. Click the envelope icon (馃摟) in the left sidebar, or go to scholar.google.com/scholar_alerts
...
- Biomedical focus 鈥?limited coverage of social sciences, education, humanities
...
|--------|-----------------|---------|
| **PubMed** | Saved search 鈫?RSS icon | New articles matching your search |
...
- **Note**: Preprints are NOT peer-reviewed 鈥?flag accordingly in digests

### Key Preprint Monitoring Rules

1. Always label preprint sources clearly: `[PREPRINT 鈥?not peer-reviewed]`
2. Check whether a preprint has been published in a peer-reviewed journal (look for "Now published in..." banner)
3. Preprints can change or be withdrawn 鈥?re-check before citing
...
After any research mode is complete, users can optionally activate the `monitoring_agent` to set up post-research literature monitoring. This is not part of the main pipeline 鈥?it is an auxiliary capability triggered on demand.

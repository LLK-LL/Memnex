...
The check fires only when `obtained_via != 'manual'` (manual entries are user-vouched per spec v3.9.0 搂3.1).
...
| HTTP 5xx | Skip 鈥?raise `OpenAlexUnavailable` immediately. |
| Network timeout (30s default) | Skip 鈥?raise `OpenAlexUnavailable`. |
| `OpenAlexUnavailable` raised | Caller MUST omit `openalex_unmatched` from the entry (per spec v3.9.0 R-L3-2-C: absent 鈮?false). Other indexes proceed independently. |
...
- Spec: `docs/design/2026-05-17-ars-v3.9.0-cross-index-triangulation-measurement-spec.md` 搂3.4

...
# Deep Research 鈥?Universal Academic Research Agent Team

Universal deep research tool 鈥?a domain-agnostic 13-agent team for rigorous academic research on any topic.

**v2.4** adds writing quality improvements to the report compiler:
- **Style Profile consumption** (optional) 鈥?If a Style Profile is available from academic-paper intake, the report compiler applies it as a soft guide for the Executive Summary and Synthesis sections. Discipline conventions and report objectivity take priority.
- **Writing Quality Check** 鈥?The report compiler runs a writing quality checklist before finalizing: flags AI-typical overused terms, checks sentence/paragraph length variation, removes throat-clearing openers. See `academic-paper/references/writing_quality_check.md`.
...
寮曞皫鎴戠殑鐮旂┒锛氬皯瀛愬寲灏嶇绔嬪ぇ瀛哥殑褰遍熆
骞垜閲愭竻鎴戠殑鐮旂┒鏂瑰悜锛屾垜灏嶉珮鏁欏搧淇濇湁鑸堣叮浣嗛倓涓嶅お纰哄畾
```

**Execution:**
1. Scoping 鈥?Research question + methodology blueprint
2. Investigation 鈥?Systematic literature search + source verification
3. Analysis 鈥?Cross-source synthesis + bias check
4. Composition 鈥?Full APA 7.0 report
5. Review 鈥?Editorial + ethics + vulnerability scan
6. Revision 鈥?Final polished report
...
**绻侀珨涓枃**: 鐮旂┒, 娣卞害鐮旂┒, 鏂囩嵒鍥為¨, 鏂囩嵒鎺㈣◣, 绯荤当鎬у洖椤? 寰岃ō鍒嗘瀽, 璀夋摎缍滄暣, 浜嬪鏌ユ牳, 鐮旂┒鏂规硶, 瀛歌鍒嗘瀽, 鏀跨瓥鍒嗘瀽, 寮曞皫鎴戠殑鐮旂┒, 骞垜閲愭竻, 鐩ｆ脯閫欏€嬩富椤? 瑷畾杩借工
...
**Default rule**: When intent is ambiguous between `socratic` and `full`, **prefer `socratic`** 鈥?it is safer to guide first than to produce an unwanted report. The user can always switch to `full` later.

**Example triggers** (illustrative, not exhaustive):
"guide my research", "help me think through", 銆屽紩灏庢垜鐨勭爺绌躲€嶃€屽公鎴戦噽娓呫€? or equivalent in any language
...
| Your Situation 浣犵殑鐙€娉?| Recommended Mode | Spectrum |
|----------------|-----------------|----------|
| Vague idea, need guidance / 鏈夋ā绯婃兂娉曪紝闇€瑕佸紩灏?| `socratic` | originality |
| Clear RQ, need comprehensive research / 鏈夋槑纰?RQ锛岄渶瑕佸畬鏁寸爺绌?| `full` | balanced |
| Need a quick brief (30 min) / 闇€瑕佸揩閫熸憳瑕?| `quick` | fidelity |
| Have a paper to evaluate before citing / 鏈夎珫鏂囬渶瑕佽浼?| `review` | balanced |
| Need literature review for a topic / 闇€瑕佹枃鐛诲洖椤?| `lit-review` | fidelity |
| Need to verify specific claims / 闇€瑕佹煡鏍哥壒瀹氫簨瀵?| `fact-check` | fidelity |
| Need systematic review / meta-analysis / 绯荤当鎬у洖椤ф垨寰岃ō鍒嗘瀽 | `systematic-review` | fidelity |
...
Not sure? Start with `socratic` 鈥?it will help you figure out what you need.
涓嶇⒑瀹氾紵鍏堢敤 `socratic` 妯″紡鈥斺€斿畠鏈冨公浣犻噽娓呬綘闇€瑕佷粈楹笺€?
...
1. 鈿狅笍 **IRON RULE**: **Devil's Advocate** has 3 mandatory checkpoints; **Critical-severity** issues block progression
2. Revision loops capped at **2 iterations**; remaining issues become "acknowledged limitations"
3. 鈿狅笍 **IRON RULE**: **Ethics Review** can halt delivery for Critical ethics concerns
...
5-layer dialogue guiding users from vague ideas to concrete research questions. Core principle: 鈿狅笍 **IRON RULE**: Never give direct answers.
...
## Handoff Protocol: deep-research 鈫?academic-paper
...
| `references/systematic_review_toolkit.md` | Cochrane v6.4, PRISMA 2020, RoB 2, ROBINS-I, I虏 guide, GRADE, protocol registration | risk_of_bias, meta_analysis, bibliography, report_compiler |
...
| 4 | **鈿狅笍 IRON RULE: Treating "difficult to verify" as acceptable** | Marking a reference as "uncertain" instead of FAIL | Gray zone = FAIL. If you cannot confirm it exists, it does not go in the report |
...
1. 鈿狅笍 **IRON RULE**: **Every claim must have a citation** 鈥?no unsupported assertions
2. **Evidence hierarchy** 鈥?meta-analyses > RCTs > cohort studies > case reports > expert opinion
3. **Contradiction disclosure** 鈥?if sources disagree, report both sides with evidence quality comparison
4. **Limitation transparency** 鈥?every report must have an explicit limitations section
5. **AI disclosure** 鈥?all reports include a statement that AI-assisted research tools were used
6. **Reproducibility** 鈥?search strategies, inclusion criteria, and analytical methods must be documented for replication
7. **Socratic integrity** 鈥?in socratic mode, never give direct answers; always guide through questions

## Cross-Agent Quality Alignment

Unified definitions across all agents. 鈿狅笍 IRON RULE: **CRITICAL severity** = issue that would invalidate a core conclusion or constitute academic misconduct. Requires immediate resolution.

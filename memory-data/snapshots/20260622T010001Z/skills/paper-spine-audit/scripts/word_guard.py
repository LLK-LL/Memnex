...
# Any LaTeX macro applied to an argument, e.g. \foo{...} 鈥?catches custom macros
...
# pandoc citeproc leftovers, e.g. [@smith2020] 鈥?citations were never resolved.
...
NUMERIC_CITE_PATTERN = re.compile(r"\[\d+(?:\s*[,鈥?]\s*\d+)*\]")
# REDACTED: sensitive-looking memory line
...
def check_docx(path: Path, min_chars: int, source_tex: str = "") -> WordGuardResult:
    ...
    if paragraph_count == 0:
        findings.append("no non-empty paragraphs found 鈥?docx may be empty or corrupted")
        ...
        if has_images:
            findings.append(
                "Images found in docx but no text 鈥?pandoc image conversion likely failed. "
...
# REDACTED: sensitive-looking memory line
        findings.append(
            f"Unrendered LaTeX commands in text (e.g. {sample}) 鈥?pandoc emitted raw source "
    ...
    if BROKEN_CROSSREF in text:
        findings.append(
            "Broken cross-references '[?]' 鈥?\\ref/\\eqref did not resolve. "
    ...
    if RAW_MATH_PATTERN.search(text):
        findings.append(
            "Raw inline LaTeX math (e.g. `$\\alpha$`) survived into the docx 鈥?math was not "
    ...
    has_chinese = bool(re.search(r"[涓€-榭縘", text))
    if has_chinese:
        ...
        corruption = re.findall(r"閸氬牄[鍔ф笟浣冪箽闁棙]聛", text)  # common gbk-decoded-as-utf8 pattern
        if corruption:

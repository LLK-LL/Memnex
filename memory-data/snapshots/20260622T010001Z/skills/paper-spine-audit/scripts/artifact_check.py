...
RATIONALE_ANCHOR_CATEGORIES = {
    "motivation": ("motivation", "spine", "throughline", "鍔ㄦ満", "涓荤嚎", "璐＄尞"),
    "reference": ("reference", "sota", "example", "pattern", "paper", "literature", "鍙傝€?, "鏍蜂緥", "璁烘枃", "鏂囩尞"),
    "target": ("target", "scene", "venue", "journal", "conference", "competition", "rubric", "norm", "鐩爣", "鍦烘櫙", "鏈熷垔", "浼氳", "姣旇禌", "璇勫垎", "瑙勮寖"),
    "evidence": ("evidence", "figure", "table", "result", "citation", "data", "source", "claim", "璇佹嵁", "鍥?, "琛?, "缁撴灉", "寮曠敤", "鏁版嵁", "绱犳潗", "涓诲紶"),
    "text_move": ("reframe", "rewrite", "move", "place", "sequence", "contrast", "echo", "narrow", "鍐欎綔", "閲嶆瀯", "鏀瑰啓", "鍓嶅悗鍛煎簲", "鏀舵潫", "瀵圭収"),
}
...
def is_generic_cell(text: str) -> bool:
    return normalize(text).strip(" .銆?锛?) in GENERIC_CELL_VALUES
...
def find_rationale_table(text: str) -> tuple[list[str], list[list[str]]] | None:
    for table in markdown_tables(text):
        if not table:
            continue
        header = table[0]
        if header_has(header, ("manuscript", "unit", "writing", "section", "鍗曞厓", "娈佃惤", "绔犺妭")) and header_has(
            header, ("motivation", "鍔ㄦ満")
        ):
...
def validate_writing_rationale_matrix(output_dir: Path) -> list[str]:
    ...
    header, rows = table
    required_columns = {
        "manuscript unit": ("manuscript", "unit", "writing", "section", "鍗曞厓", "娈佃惤", "绔犺妭"),
        "motivation link": ("motivation", "鍔ㄦ満"),
        "reference or SOTA pattern": ("reference", "sota", "example", "鏍蜂緥", "鍙傝€?, "鏂囩尞", "浼樼"),
        "target scene or venue norm": ("target", "scene", "venue", "norm", "鐩爣", "鍦烘櫙", "鏈熷垔", "浼氳", "姣旇禌", "瑙勮寖"),
        "evidence or citation anchor": ("evidence", "citation", "anchor", "璇佹嵁", "寮曠敤", "鏉愭枡", "鏁版嵁"),
        "planned change or text move": ("planned", "change", "move", "function", "plan", "淇敼", "璁″垝", "鍐欐硶", "鍔熻兘"),
        "final text check": ("final", "check", "鏈€缁?, "妫€鏌?, "钀界偣"),
    }
...
def validate_citation_support_bank(output_dir: Path, config: dict[str, object]) -> list[str]:
    ...
    for index, row in enumerate(rows[:required_candidates], start=1):
        joined = " ".join(row)
# REDACTED: sensitive-looking memory line
        has_sentence = len(joined) >= 80 and bool(re.search(r"[.!?銆傦紒锛焆", joined))
...
def validate_translation_package(output_dir: Path, required: list[str]) -> list[str]:
    ...
    if coverage_path.exists():
        ...
        if any(status in coverage_lower for status in ("missing", "partial", "not translated", "鏈炕璇?, "缂哄け", "閮ㄥ垎")):

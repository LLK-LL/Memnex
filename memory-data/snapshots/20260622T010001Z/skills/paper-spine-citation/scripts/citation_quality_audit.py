#!/usr/bin/env python3
"""Citation Quality Audit 鈥?teaching-oriented citation analysis for PaperSpine.
...
Pattern: follows the writing_rationale_matrix philosophy 鈥?every analysis row
...
def classify_citation_type(reference: str) -> str:
    ...
    if any(w in text for w in ("survey", "review", "meta-analysis", "缁艰堪", "鍥為【")):
        return "survey"
    if any(w in text for w in ("benchmark", "dataset", "corpus", "evaluation", "鏁版嵁闆?)):
        return "benchmark"
    if any(w in text for w in ("limitation", "robustness", "reproducib", "bias", "fairness")):
        return "critique"
    if any(w in text for w in ("application", "real-world", "deployment", "clinical", "搴旂敤")):
        return "application"
    ...
    for record in records:
        ...
        if entry.crossref_title and entry.reference:
            ...
            if sim >= 0.75:
                ...
                entry.issues.append(f"Title similarity {sim:.2f} 鈥?likely wrong DOI. Crossref title: '{entry.crossref_title[:100]}'")
        ...
        if entry.api_year and entry.year:
            ...
            if not entry.year_matches:
                ...
                entry.teaching_note += " Year mismatch between citation bank and API 鈥?verify the publication date."
...
def to_markdown(report: CitationQualityReport) -> str:
    ...
    for e in report.entries:
        lines.append(
            f"| {e.candidate_id} | {e.doi[:30] if e.doi else '鈥?} | {e.citation_type} | "
    ...
    for e in report.entries:
        if e.status == "verified":
            continue
        lines.append(f"### {e.candidate_id} 鈥?{e.doi}")
        lines.append("")

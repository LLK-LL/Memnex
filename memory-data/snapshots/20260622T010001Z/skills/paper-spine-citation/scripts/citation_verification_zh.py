...
涓枃鍙傝€冩枃鐚?(Chinese-language references). Produces a structured report
...
# REDACTED: sensitive-looking memory line
CN_JOURNAL_RE = re.compile(r"銆?[^銆媇+)銆?)  # 銆婃湡鍒婂悕銆?CN_YEAR_RE = re.compile(r"(\d{4})[骞碷?")
CN_VOLUME_RE = re.compile(r"(\d+)\s*[鍗穃(锛圿")
...
# REDACTED: sensitive-looking memory line
        ...
        if len(ref.strip()) < 20:
            check.status = "FAKE"
            check.issues.append("Citation text too short 鈥?likely fabricated")

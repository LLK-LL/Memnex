...
- Resolve the identifier to the actual article first:
  - arXiv 鈫?the abstract page, then the PDF (and HTML/LaTeX source when available).
  - DOI 鈫?the publisher landing page, then the open-access PDF or HTML if lawfully available.
- After resolving, this becomes a `pdf-text`, `scanned-pdf`, or `html` job 鈥?load that fragment and follow it for extraction. This fragment only covers retrieval.

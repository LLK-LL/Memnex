...
- `pdf-text` 鈥?selectable-text PDF. Default.
- `scanned-pdf` 鈥?image-only or OCR-required PDF.
- `html` 鈥?publisher or preprint HTML page.
- `doi-arxiv` 鈥?a bare DOI or arXiv link that must be resolved first.
- `pasted-text` 鈥?pasted prose or notes with no retrievable original layout.
...
1. Core principles (`core/principles.md`) 鈥?bilingual reader by default, translate for meaning, never degrade to a summary, copyright caution.
2. Source-format fragment 鈥?how to extract text, figures, and tables for this input.
3. Reading workflow (`core/workflow.md`) 鈥?the six-step source-map-first process.
4. Output contract (`core/output-contract.md`) 鈥?required files and the pre-response verification checklist.
...
- detailed figure/table cropping and placement 鈫?`references/figure-extraction.md`.
- exact field schema for `paper.md` / `source_map.json` 鈫?`references/output-spec.md`.
- answering follow-up questions with source citations 鈫?`references/grounding-rules.md`.

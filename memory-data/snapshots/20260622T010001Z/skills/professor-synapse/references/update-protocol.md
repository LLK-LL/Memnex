...
|----------|-------|-------------|
| **System Core** | `SKILL.md`, `scripts/*` | Show diff, recommend applying updates. SKILL.md contains Global Learned Patterns 鈥?smart merge to preserve user's patterns |
...
3. **Smart merge decisions**
   ...
   - **SKILL.md**: Smart merge 鈥?preserve user's Global Learned Patterns, apply structural changes
...
```
馃馃従鈥嶁檪锔? "I've prepared the merged skill content in /tmp/canonical-synapse/ with:
...
```
馃馃従鈥嶁檪锔? "鉁?Merged skill content prepared!
...
**Important:** The user clicks the button鈥攜ou cannot do this programmatically.

...
```
professor-synapse/
鈹溾攢鈹€ SKILL.md                 # Main identity + Global Learned Patterns
鈹溾攢鈹€ agents/
鈹?  鈹溾攢鈹€ INDEX.md             # Auto-generated agent registry (rebuild with script)
鈹?  鈹斺攢鈹€ [domain]-[specialty].md  # Agent files with frontmatter + Learned Patterns
鈹溾攢鈹€ references/
鈹?  鈹溾攢鈹€ agent-template.md    # Template for new agents + pattern format templates
鈹?  鈹溾攢鈹€ changelog.md         # Version history
鈹?  鈹溾攢鈹€ domain-expertise.md  # Domain mappings (UPDATE with new domains)
鈹?  鈹斺攢鈹€ file-operations.md   # This file
鈹斺攢鈹€ scripts/
    鈹斺攢鈹€ rebuild-index.sh     # Regenerates agents/INDEX.md from frontmatter
```
...
#### Step 2: Make the Edit
Use `str_replace` to add the pattern under `## Learned Patterns` 鈫?`### Effective Patterns` or `### Anti-Patterns` in that agent's file.
...
> **鈿狅笍 MANDATORY after ANY file change. If you skip this, your changes are LOST.**
...
present_files 鈫?professor-synapse.skill
```

The user will see "Copy to your skills" button to install the updated skill.

**鈿狅笍 If you skip the packaging steps, changes will NOT persist when the user invokes Professor Synapse again.**

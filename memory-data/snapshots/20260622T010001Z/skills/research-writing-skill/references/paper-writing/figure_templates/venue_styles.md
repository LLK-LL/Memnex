...
- **Sans-serif** (Helvetica, CMSans) is acceptable for diagram labels and annotations 鈥?it improves readability inside boxes and along arrows.
- **Bold** only for the figure's title element or the component representing the paper's key contribution.

## Color Palette

Use the **Paul Tol "bright" palette** 鈥?colorblind-safe and print-friendly:
...
- Every visual element must encode information 鈥?nothing purely decorative
...
1. Readable in grayscale (not just color) 鈥?shapes and labels distinguish components, not only color

...
**How to do it**: Open with what users reason about, not what the system computes. Name the core abstraction 鈥?this is the intellectual contribution. The implementation is the engineering contribution.
...
**Pattern**: Overview figure 鈫?subsection per component. Each subsection explains one component.
...
3. **Missing the "why"**: Presenting "Abstract X / Concrete X" 鈥?the WHAT without the WHY. Add the justification: "Why does the simple approach fail?"

---

## The What 鈫?Why 鈫?So-What Test (Principle 3)

For each subsection, check which level the heading achieves:

- **WHAT** (minimum): "System Architecture" 鈥?names the topic
- **WHY** (better): "Why do linear approaches fail?" 鈥?argues for the design
- **SO-WHAT** (best): "Nondeterminism as a first-order requirement" 鈥?states the implication

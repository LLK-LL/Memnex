...
The biggest obstacle for students isn't writing skill 鈥?it's that their ideas live as unstructured intuitions. They know *something* is interesting but can't articulate *what* or *why*. This guide walks through a series of pointed questions that force clarity. By the end, you'll have a `project_context.md` precise enough to drive every section of your paper.

**How to use this**: Claude will walk you through these questions interactively when you invoke `/paper-writing` on a new project. Answer honestly 鈥?"I don't know" is a valid and useful answer. The questions are designed to reveal gaps early, when they're cheap to fix.
...
1. **Who is suffering?** Name specific actors 鈥?not "users" but "network operators at Tier-1 ISPs" or "ML practitioners training models on heterogeneous clusters." If you can't name the actor, you don't know who your work helps.
...
3. **Why does it break?** This is the structural limitation 鈥?the root cause that can't be fixed by just "doing more" of the current approach. Is it a fundamental assumption that doesn't hold? A missing abstraction? An architectural mismatch? If the answer is "they just need more data/compute/effort," you may not have a research contribution 鈥?you have an engineering task.

4. **What happens if nobody solves this?** What's the cost of the status quo? Dollar amounts, user impact, operational burden, scientific dead-ends 鈥?any concrete consequence. This becomes your opening paragraph's stakes.
...
7. **Can you draw a 2脳2 matrix of existing approaches?** Put approach categories on one axis and capabilities on the other. Where is the empty quadrant? That's your paper's territory. If there's no empty quadrant, your contribution needs to be something other than a new approach (maybe a new understanding, a new benchmark, or a unifying framework).
...
8. **Complete this sentence: "This paper shows that ______."** Not "proposes" or "introduces" 鈥?*shows*. The verb forces you to state a finding, not an activity. If you can't complete it, you don't yet know your contribution.
...
10. **Can you name your core idea in 2-4 words?** A compound noun phrase that captures the intellectual contribution. Examples of the pattern: "event-centric decomposition," "progressive disaggregation," "intrinsic evaluation framework." If you can't name it, it isn't crisp enough yet. The name will evolve 鈥?but having even a placeholder forces precision.
...
12. **What is your headline number?** The single most impressive quantitative result. "13脳 error reduction," "44% lower latency," "2脳 faster search." This goes in the introduction's Results Preview and the abstract. If you don't have one yet, which experiment would produce it?
...
If you've already been exploring your data using the [data-visualization-skill](https://github.com/SNL-UCSB/data-visualization-skill), the iteration you did there 鈥?ingesting, exploring, brainstorming what to show and why 鈥?has already shaped your understanding of what the data says. Your `exploration_log.md` captured what the data looked like from multiple angles before you formed hypotheses. Your `braindump.md` forced you to articulate what question each figure answers and what would surprise you. That exploration is the empirical foundation for the claims you design here. Load those artifacts now: the surprises you found during exploration may be more compelling than the results you expected, and the predictions that failed are often where the real contribution lives.

### 3.1 Experimental Setup

14. **What are your baselines, by name?** Not "prior work" or "state-of-the-art." Specific system names. For each baseline, explain WHY it's the right comparison 鈥?what does beating this baseline prove?

15. **What are your metrics, and why these specific ones?** For each metric: what aspect of your claim does it measure? Are there metrics that would make your system look BAD? If so, should you report them anyway? (Usually yes 鈥?it builds trust.)
...
20. **Do you have a robustness/generalization story?** Does your system work beyond the specific evaluation setup? New data? New domains? Different parameters? Temporal shift? If not, reviewers will ask "does this generalize?" 鈥?have an answer ready.
...
26. **What is the high-level architecture?** Describe it as a pipeline: "Input 鈫?[Stage 1] 鈫?[Stage 2] 鈫?... 鈫?Output." Name each stage. These names become the vocabulary of your paper.
...
29. **What are the locked decisions 鈥?things NOT up for debate?** Architecture choices, evaluation setup, dataset selection, venue 鈥?anything that's final. Write them down. These prevent revisiting settled questions during writing.
...
32. **What is the narrative arc?** Every good paper tells a story: "The world has problem X 鈫?Existing approaches fail because Y 鈫?We observe insight Z 鈫?This leads to system W 鈫?W achieves [results]." Write yours in 3-4 sentences.
...
Before touching the evaluation, write a **Draft 0 introduction** 鈥?a disposable framing scaffold covering Stakes, Problem Gap, Key Abstraction (placeholder name is fine), Design Intuition, and rough Contributions. Leave the Results Preview blank (you don't have results yet).
...
Draft 0 will probably not survive to the final version 鈥?and that's the point. The final introduction is rewritten from scratch after the evaluation, constrained by what the evidence actually supports. Draft 0 is reference material for that rewrite, not a starting point for editing.

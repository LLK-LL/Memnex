...
`paper_rewriting_output/translation_zh/` 鈥?not a summary, not a partial set.
...
- **`full_paper_translation.zh.md`** must include: title, abstract, every
  ...
  narrative text.
- Update `manifest.md` status from `pending` 鈫?`translated` after each file.

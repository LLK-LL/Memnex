#!/usr/bin/env python3
"""PaperSpine Translation Guard 鈥?verify translation_zh/ completeness and quality.
...
Pattern: follows the writing_rationale_matrix philosophy 鈥?every finding teaches.
"""
    ...
    for f in missing:
        ...
        findings.append(TranslationFinding(
            ...
            teaching=f"Every intermediate artifact needs a Chinese counterpart. "
                      f"`{en_name}` is a required PaperSpine artifact 鈥?its translation "
                      f"is not optional.",
        ))
...
# ---------------------------------------------------------------------------
# Check 2 鈥?Structural Preservation (table rows for large tabular artifacts)
# ---------------------------------------------------------------------------
        ...
        if "|" in en_text:
            ...
            if zh_count < en_count:
                findings.append(TranslationFinding(
                    ...
                    severity="BLOCKER",
                    what=f"`{zh_name}` has {zh_count} table rows vs {en_count} in source 鈥?"
                          f"missing {en_count - zh_count} rows",
                    fix=f"Translate every row of `{en_name}` into `{zh_name}`. "
                        f"Row-by-row translation is mandatory for this file 鈥?"
                    ...
                    teaching="Large tabular artifacts must preserve their row structure in translation. "
                            "Each row represents a unit of reasoning or a citation candidate 鈥?"
    ...
    for zh_name, en_name in sorted(TRANSLATION_SOURCE.items()):
        ...
        if ratio < MIN_DENSITY_RATIO:
            findings.append(TranslationFinding(
                ...
                what=f"`{zh_name}` is {zh_len} chars vs {en_len} in source 鈥?"
                ...
                teaching="A translation should convey the same information as the source. "
                        ...
                        "content has been lost 鈥?usually because the translator summarized instead of translating.",
            ))
...
# ---------------------------------------------------------------------------
# Check 4 鈥?Full Paper Translation Coverage
# ---------------------------------------------------------------------------
...
def check_full_paper_coverage(trans_dir: Path, out_dir: Path) -> list[TranslationFinding]:
    ...
    if not fp_path.exists():
        findings.append(TranslationFinding(
            ...
            what="full_paper_translation.zh.md is missing 鈥?the most important translation file",
            fix="Translate the complete final paper (title, abstract, all sections, figure/table captions, "
                ...
                "This is mandatory 鈥?do not skip or summarize.",
    ...
    if len(text) < 1000:
        findings.append(TranslationFinding(
            ...
            what=f"full_paper_translation.zh.md is only {len(text)} chars 鈥?far too short for a full paper",
            ...
            teaching="A full paper translation that is shorter than the abstract "
                    "is not a translation 鈥?it's a note saying 'I was supposed to translate this.'",
        ))
...
# ---------------------------------------------------------------------------
# Check 5 鈥?Manifest Cross-Validation
# ---------------------------------------------------------------------------
...
def check_manifest(trans_dir: Path, config: dict) -> list[TranslationFinding]:
    ...
    if re.search(r"(missing|partial|not translated|缂哄け|鏈炕璇憒閮ㄥ垎缈昏瘧)", manifest_text, re.IGNORECASE):
        findings.append(TranslationFinding(
...
def to_markdown(report: TranslationGuardReport) -> str:
    lines = [
        ...
        "Translation is not optional 鈥?it's a required deliverable.",
        "",
    ]

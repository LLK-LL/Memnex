...
```
self_rules_context(project="<project>", detail="compact", phase="<phase>")
memory_recall(query="<current task description>", project="<project>", mode="rag", phase="<phase>", limit=5, include_cognitive=false)
```

If relevant knowledge is found, mention it briefly and apply it.
Compact rules are layered: use the compact result by default; only request
`self_rules_context(..., detail="full")` when a returned rule is directly
relevant and its compact text is insufficient.
If RAG recall fails, returns an obviously broken result, or summary/compressed
cache is missing, fix the RAG path first, then log the symptom, root cause,
fix, verification, and next direct-start method before continuing.

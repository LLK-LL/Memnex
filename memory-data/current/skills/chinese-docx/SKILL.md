...
Templates are authoritative. Reuse existing paragraph and character styles, keep existing font slots, and only add missing `w:eastAsia` values. Do not flatten a template into direct formatting unless the user explicitly asks for a one-off visual repair.

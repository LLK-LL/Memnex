...
**Memnex 是一个面向个人用户的轻量级本地 AI 记忆系统。** 这个名字来自 **Memory + Nexus**：Memory 代表可长期沉淀的经验、偏好、规则和技能，Nexus 代表连接这些碎片的中枢网络。合在一起，Memnex 表示一个把个人 AI 记忆连接成可用系统的记忆中枢。

它不是一个笨重的企业级 agent 平台，也不是单纯把聊天记录丢进向量数据库。Memnex 更像是你给 AI 准备的“个人大脑硬盘”：把你的经验、偏好、规则、技能、工作流和历史教训持续整理起来，让 AI 在下一次工作时能真正记住你、理解你、复用你过去训练出来的方法。

如果你经常使用 Codex 类 agent、Claude、ChatGPT、本地 agent 或 MCP 记忆工具，Memnex 解决的是一个非常常见的问题：

> AI 在一次任务里越用越聪明，但下次打开又像重新开始。

Memnex 会把这种脆弱的本地状态，整理成可审查、可恢复、可迁移的个人记忆库。
...
Memnex 围绕本地文件、结构化导出、Git 版本管理和定期同步工作。
...
很多记忆系统主要保存事实、摘要或对话片段。Memnex 更关注 AI 使用过程中的操作资产：
...
换句话说，Memnex 保存的不只是 AI 学到了什么，还保存了你是如何训练 AI 工作的。
...
Memnex 的同步流程默认是保守的。它会排除缓存、日志、临时文件、数据库中间写入文件等不适合上传的内容，也会对偏好和配置快照中的敏感关键词进行脱敏，例如：
...
如果你要公开仓库，仍然建议先人工审查第一次同步结果。但 Memnex 从设计上就把个人安全放在重要位置。

## 支持的 Agent 平台

Memnex 当前最强的是 Codex + 本地 MCP 记忆生态，但它的存储方式本身是平台无关的。只要某个 agent 能读取 Markdown、规则文件、JSON 导出、本地文件或 MCP 工具，就可以把 Memnex 当成个人记忆来源。

| 平台 | 当前支持程度 | Memnex 提供什么 | 配置方法 |
| --- | --- | --- | --- |
...
| VS Code / Cursor / Windsurf 中的 Codex | 通过 Codex 扩展原生支持 | 同一套 Codex 记忆、规则和技能层 | 在编辑器中安装/使用 Codex，继续维护 `AGENTS.md` 和 `.codex/skills` 结构，然后从共享的本地 Codex home 同步 Memnex。 |
| `.agents` 技能生态 | 原生支持 | 用于跨 agent 复用的 `agent-skills/` 技能层和平台路由 | 将共享技能放到 `~/.agents/skills`，运行 Memnex 同步脚本后会复制到 `memory-data/.../agent-skills`。 |
| MCP 记忆工具 / Total Agent Memory 类导出 | 原生导出与归档支持 | `memory-data/.../exports` 下的结构化记忆导出 | 将 MCP memory 导出或备份到配置好的本地备份目录，再由 Memnex 收集最新导出，并在 `manifest.json` 中记录来源。 |
| Claude Code / Claude Desktop | 易适配 | 长期记忆、规则、技能和工作流笔记可以转换成 Claude 指令或 MCP 记忆 | 将 Memnex 的高层规则整理进 `CLAUDE.md`，项目级规则放进 `.claude/rules/`，并通过 MCP server 或 Markdown 文件暴露记忆导出。 |
| Cursor | 易适配 | 项目约定、工作流规则、可复用任务指导 | 将 Memnex 中的规则或精选记忆转换成 Cursor Rules，例如 `.cursor/rules/` 项目规则、Cursor 设置里的 User Rules，或平台支持的 `AGENTS.md`。 |
| Continue | 易适配 | Rules、prompts、tools 和 MCP 可读的记忆导出 | 将 Memnex 提炼出的规则加入 Continue 的本地/全局配置，并在 Continue 的 tools/MCP 配置中连接记忆检索 MCP server。 |
| Windsurf / Cascade | 易适配 | Memories、Rules 和可选的 MCP 检索 | 将 Memnex 中的精选规则转换为 Windsurf/Cascade Rules，将稳定事实加入 Memories，需要检索时在 Cascade MCP 设置中接入记忆 MCP server。 |
| Cline / Roo 类编码 agent | 易适配 | Memory Bank 风格的 Markdown 上下文、项目决策、当前状态和规则 | 将 Memnex 记忆转换成 Memory Bank 文件，把运行规则放到 `.clinerules/`，也可以通过 MCP 暴露结构化导出。 |
| OpenAI Agents SDK | 开发者集成 | 外部记忆源、检索工具、规则/策略来源、技能注册表 | 在应用代码中加载 `memory-data/`，对精选记忆建立索引，或把 Memnex 检索封装成 agent 可调用的 MCP/tool。 |
...
下面两张图展示了当前 Memnex 记忆网络的可视化状态。它们可以直观看到：大量微小的经验、规则、技能和工作流知识，正在连接成一个个人化的 AI 记忆层。

![Memnex 记忆网络总览](assets/memory-network-overview.png)

![Memnex 记忆网络图谱](assets/memory-network-map.png)
...
传统 RAG 帮 AI 从文档里回答问题。Memnex 帮 AI 成为更懂你的个人工作伙伴。
...
**Memnex 是给个人 AI 用户准备的轻量级本地记忆库：它把你的经验、偏好、规则和技能沉淀成可审计、可迁移、可持续迭代的 AI 大脑。**
